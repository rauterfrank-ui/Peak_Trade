"""COMPOSITION_PRIMARY_PAGE_SHARE_DOMINANCE_V1 — presentation contracts."""
...
PLAN = (
    ...
    / "product"
    / "evidence"
    / "composition_rebaseline_post_decision_compression_v1_20260717T004021Z"
...
def test_authorized_slice_title_points_to_primary_page_share_dominance() -> None:
    text = PLAN.read_text(encoding="utf-8")
    assert "COMPOSITION_PRIMARY_PAGE_SHARE_DOMINANCE_V1" in text


def test_primary_dominance_tokens_and_layout_owner() -> None:
    tokens = TOKENS.read_text(encoding="utf-8")
    layout = LAYOUT.read_text(encoding="utf-8")
    assert "--pt-primary-chart-min-height: 450px;" in tokens
    assert "--pt-decision-funnel-stages-max-height:" in tokens
    assert "--pt-decision-funnel-aux-max-height:" in tokens
    assert "COMPOSITION_PRIMARY_PAGE_SHARE_DOMINANCE_V1" in layout
    assert 'data-market-primary-page-share-dominance-v1="true"' in layout
    assert "max-height: var(--pt-decision-funnel-stages-max-height)" in layout


def test_market_template_primary_dominance_markers_and_landmark_order() -> None:
    html = MARKET_TMPL.read_text(encoding="utf-8")
    assert 'data-landmark="PRIMARY_MARKET_SURFACE"' in html
...
def test_ssr_primary_dominance_and_readonly_markers(
    client_primary_dominance: TestClient,
) -> None:
    resp = client_primary_dominance.get("/market?timeframe=1h")
