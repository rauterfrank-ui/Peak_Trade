...
def test_legacy_venue_metadata_coverage_panel_absent_when_gate_disabled(
...
def test_legacy_venue_metadata_coverage_panel_absent_when_enabled(
    client_on: TestClient,
) -> None:
    html = client_on.get("/observability").text
    assert 'data-workflow-panel-kraken-metadata-coverage-v1="true"' not in html
    assert "Kraken Futures Metadata Coverage" not in html


def test_legacy_venue_metadata_coverage_does_not_replace_missing_truth(
    client_on: TestClient,
) -> None:
    html = client_on.get("/observability").text
    section = _workflow_dashboard_section(html)
    ...
    assert "SELECTED_FUTURE_NOT_PERSISTED" in section
    assert "FUTURE_DETAIL_NOT_AVAILABLE" in section
    assert 'data-workflow-panel-kraken-metadata-coverage-v1="true"' not in section
