"""Active Browser Verification Policy: Chrome primary, Safari/WebKit secondary."""
...
def test_runbook_browser_policy_chrome_primary() -> None:
    ...
    assert "Canonical Composition + Technical Discovery Edition" in text
    assert "PART I — Product Runbook" in text
    assert "PART II — Technical Discovery" in text
    assert "## Browser" in text
    assert "Google Chrome über Playwright" in text
    assert "PRIMARY_PLAYWRIGHT_CHANNEL=chrome" in text
    assert 'playwright.chromium.launch(channel="chrome")' in text or (
        "playwright.chromium.launch(channel='chrome')" in text
    )
    assert "Playwright Chromium" in text
    assert "BROWSER_ACTUAL=PLAYWRIGHT_CHROMIUM" in text
    assert "nicht als real Chrome claimen" in text
    assert "WebKit" in text
    assert "Safari" in text
    assert "Sekundär" in text or "sekundär" in text
    assert "nicht als real Safari claimen" in text
...
def test_runbook_version_not_bumped_and_single_file() -> None:
    ...
    text = RUNBOOK.read_text(encoding="utf-8")
    assert "Runbook v1.3" in text
    assert "Runbook v1.4" not in text
