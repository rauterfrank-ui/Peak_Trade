...
def test_market_and_double_play_chartjs_vendor_primary_self_only_v1(
    client: TestClient,
) -> None:
    """Chart.js loads from local vendor only (Phase 1B self-only; read-only markers)."""
    for path, script_id, shell_id in (
        ("/market", "peak-trade-market-chartjs-vendor-v1", "market-v0-shell"),
        (
            "/market/double-play",
        ...
        assert "data-chartjs-vendor-load-error" in html
...
def test_market_remodel_ia_v2_primary_values_ssr_render_wiring(client: TestClient) -> None:
    ...
    assert 'data-market-futures-first-v1="true"' in html or "unavailable" in hero_window
