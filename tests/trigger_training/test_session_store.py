...
def test_load_with_session_ids_filter(tmp_path: Path) -> None:
    ...
    save_session_to_store(
        "SESSION_A", [_make_event("2025-01-01T10:00:00Z", TriggerOutcome.HIT)], store_path
    )
    save_session_to_store(
        "SESSION_B", [_make_event("2025-01-02T10:00:00Z", TriggerOutcome.MISSED)], store_path
    )
    save_session_to_store(
        "SESSION_C", [_make_event("2025-01-03T10:00:00Z", TriggerOutcome.LATE)], store_path
    )
...
def test_list_session_ids(tmp_path: Path) -> None:
    ...
    # Sessions hinzufügen
    save_session_to_store(
        "SESSION_X", [_make_event("2025-01-01T10:00:00Z", TriggerOutcome.HIT)], store_path
    )
    save_session_to_store(
        "SESSION_Y", [_make_event("2025-01-02T10:00:00Z", TriggerOutcome.MISSED)], store_path
    )
