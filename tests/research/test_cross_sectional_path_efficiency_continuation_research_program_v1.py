"""Definition-only contract tests for CS path-efficiency continuation program v1."""
...
# Fingerprints captured at GO base (f903e46) — CSRHR must remain byte-identical.
...
def test_repo_program_definition_only_evaluation_unauthorized() -> None:
    ...
    assert (
        report["program_id"] == "CROSS_SECTIONAL_PATH_EFFICIENCY_CONTINUATION_RESEARCH_PROGRAM_V1"
    )
...
def test_causal_independence_and_frozen_identities() -> None:
    ...
    assert payload["signal_family"] == "CROSS_SECTIONAL_PATH_EFFICIENCY"
    assert (
        payload["target_phenomenon"] == "CROSS_SECTIONAL_PATH_EFFICIENCY_DIRECTIONAL_CONTINUATION"
    )
    assert (
...
def test_owner_map_registers_program_surface() -> None:
    ...
    assert "CROSS_SECTIONAL_PATH_EFFICIENCY_CONTINUATION_RESEARCH_PROGRAM_V1" in surfaces
    paths = surfaces["CROSS_SECTIONAL_PATH_EFFICIENCY_CONTINUATION_RESEARCH_PROGRAM_V1"][
        "path_prefixes"
    ]
