...
def test_static_ids_and_bindings() -> None:
    assert STRATEGY_IDENTITY == "VOLATILITY_CONTRACTION_EXPANSION_BREAKOUT_V1"
...
def test_no_vcb_or_vep_slot_reuse_and_no_authorization_created() -> None:
    binding = load_and_validate_entry_point_binding(REPO)
    assert binding["development_evaluation_authorized"] is False
    assert binding["development_run_count"] == 0
    assert binding["runner_start_count"] == 0
    assert binding["strategy_identity"] == "VOLATILITY_CONTRACTION_EXPANSION_BREAKOUT_V1"
