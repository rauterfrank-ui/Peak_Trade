...
def fixture_scope_ratification(complete_binding: dict) -> dict:
    return materialize_open_interest_level_rank_offline_economic_evaluation_scope_ratification_v0(
        repo_root=REPO_ROOT,
...
def test_precheck_rejects_data_digest_mismatch(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (
            repo
            / "config/research/cross_sectional_open_interest_level_rank_v0_versioned_hypothesis_binding_v0.json"
        ).write_text(
...
def test_ops_config_loads(complete_binding: dict) -> None:
    cfg = load_ops_evaluation_config_v0(REPO_ROOT)
    assert cfg["strategy_id"] == "cross_sectional_open_interest_level_rank"
    assert cfg["binding_digest"] == complete_binding["binding_digest"]


def test_execution_path_has_no_runtime_imports() -> None:
    module_name = (
        "src.research.cross_sectional_open_interest_level_rank_v0_offline_economic_"
...
def test_research_scope_and_hypothesis_id(complete_binding: dict) -> None:
    assert complete_binding["research_scope"] == "cross_sectional_open_interest_level_rank/v0"
    assert (
        complete_binding["research_hypothesis_id"] == "cross_sectional_open_interest_level_rank_v0"
    )


def test_level_rank_uses_level_not_delta_lookback(complete_binding: dict) -> None:
    parameter_binding = complete_binding["parameter_binding"]
    assert "rank_lookback_k" not in parameter_binding
    assert parameter_binding["open_interest_level_definition"] == (
        "point_in_time_open_interest_level_at_lagged_observation"
    )
...
def test_delta_rank_entry_point_remains_distinct() -> None:
    ...
    from src.research.cross_sectional_open_interest_level_rank_v0_offline_economic_evaluation_execution_v0 import (
        CANONICAL_FULL_EVALUATION_CALLABLE as LEVEL_CALLABLE,
        STRATEGY_ID as LEVEL_STRATEGY_ID,
    )

    assert DELTA_CALLABLE == LEVEL_CALLABLE == "run_full_offline_economic_evaluation_v0"
    assert DELTA_STRATEGY_ID != LEVEL_STRATEGY_ID
    assert LEVEL_STRATEGY_ID == "cross_sectional_open_interest_level_rank"


def test_level_rank_orchestrator_rejects_delta_lookback_binding() -> None:
    ...
    binding = dict(default_open_interest_level_rank_operator_binding_v0())
    binding["numeric_bindings"] = dict(binding["numeric_bindings"])
    binding["numeric_bindings"]["rank_lookback_k"] = {"status": "BOUND", "value": 4}
    result = run_cross_sectional_open_interest_level_rank_orchestrator_v0(
        binding=binding,
