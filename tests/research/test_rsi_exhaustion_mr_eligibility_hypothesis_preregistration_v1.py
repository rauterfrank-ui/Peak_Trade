"""Contract tests for RSI-exhaustion MR eligibility hypothesis preregistration v1.

Definition-only. No backtest. No economic metrics. No holdout content inspection.
Orthogonal to PR #5361's failed ATR(14) percentile mid-band eligibility mechanism.
"""
...
EVIDENCE = REPO / "docs/evidence/preregister_rsi_exhaustion_mr_eligibility_hypothesis_v1"
GOVERNANCE = (
    REPO
...
def test_repo_contract_validates_against_seal_registry() -> None:
    ...
    assert report["filter_id"] == "canonical_rsi_exhaustion_entry_eligibility_v1"
...
def test_prior_failed_atr_and_regime_features_may_not_be_reused() -> None:
    contract = _load(CONTRACT_PATH)
...
def test_orthogonality_markers_filter_is_rsi_not_atr() -> None:
    contract = _load(CONTRACT_PATH)
    eligibility = contract["eligibility_filter"]
    assert eligibility["filter_id"] == "canonical_rsi_exhaustion_entry_eligibility_v1"
    assert "atr" not in eligibility["filter_id"].lower()
    assert "regime" not in eligibility["filter_id"].lower()
    assert eligibility["primitive"] == "rsi_exhaustion_filter"
    assert eligibility["calculator_ssot"] == "src/strategies/rsi.py::calculate_rsi"
    assert eligibility["wilder_smoothing_excluded"] is True
    frozen = eligibility["frozen_parameters"]
    assert frozen["rsi_period"] == 14
    assert frozen["oversold"] == 30
    assert frozen["overbought"] == 70
    assert frozen["use_wilder"] is False
    # No ATR/vol-regime parameter keys leaked into the frozen RSI params.
    for atr_key in ("vol_window", "vol_method", "vol_percentile_low", "vol_percentile_high"):
        assert atr_key not in frozen
    ...
    with pytest.raises(
        HypothesisPreregistrationError, match="PRIOR_FAILED_HYPOTHESES_MUST_LIST_BOTH_PRIORS"
    ):
        validate_preregistration_contract(bad)
...
def test_baseline_treatment_authority_and_promotion_forbidden() -> None:
    ...
    assert treatment["treatment_id"] == "rsi_exhaustion_mr_entry_eligibility_v1"
...
def test_evidence_and_governance_definition_only() -> None:
    ...
    assert summary["filter_id"] == "canonical_rsi_exhaustion_entry_eligibility_v1"
