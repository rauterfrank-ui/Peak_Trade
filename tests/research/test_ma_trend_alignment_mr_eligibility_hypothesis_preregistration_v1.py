"""Contract tests for MA trend-alignment MR eligibility hypothesis preregistration v1.
...
and regime-gated eligibility mechanisms.
"""
...
EVIDENCE = REPO / "docs/evidence/preregister_ma_trend_alignment_mr_eligibility_hypothesis_v1"
GOVERNANCE = (
    REPO
...
def test_repo_contract_validates_against_seal_registry() -> None:
    ...
    assert report["filter_id"] == "canonical_ma_trend_alignment_entry_eligibility_v1"
...
def test_prior_failed_atr_rsi_adx_and_regime_features_may_not_be_reused() -> None:
    ...
    assert "close_vs_sma_50h" in feature_ids
...
def test_orthogonality_markers_filter_is_ma_trend_not_atr_rsi_adx_or_regime() -> None:
    contract = _load(CONTRACT_PATH)
    eligibility = contract["eligibility_filter"]
    assert eligibility["filter_id"] == "canonical_ma_trend_alignment_entry_eligibility_v1"
    ...
    assert eligibility["primitive"] == "ma_trend_alignment_filter"
    assert eligibility["calculator_ssot"] == "src/strategies/rsi_reversion.py"
    assert eligibility["calculator_method"] == "SMA_ROLLING_MEAN"
    frozen = eligibility["frozen_parameters"]
    assert frozen["ma_period"] == 50
    assert frozen["ma_type"] == "SMA"
    assert frozen["side_aware"] is True
    assert frozen["warmup_bars"] == 50
    for banned_key in (
        "vol_window",
        "vol_method",
        "vol_percentile_low",
    ...
    assert len(contract["prior_failed_hypotheses"]) == 4
...
def test_baseline_treatment_authority_and_promotion_forbidden() -> None:
    ...
    assert treatment["treatment_id"] == "ma_trend_alignment_mr_entry_eligibility_v1"
...
def test_evidence_and_governance_definition_only() -> None:
    ...
    assert summary["filter_id"] == "canonical_ma_trend_alignment_entry_eligibility_v1"
    assert summary["base_sha"] == "284fb55a46bfeacdb3a13d9dc0af3320e25b69e7"
