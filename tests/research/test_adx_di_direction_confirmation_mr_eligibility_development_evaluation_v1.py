"""Boundary + freeze tests for ADX DI direction-confirmation MR eligibility DEVELOPMENT evaluation v1.
...
from src.research.adx_di_direction_confirmation_mr_eligibility_development_evaluation_v1.decision_v1 import (
...
def test_feature_formula_freeze_stable() -> None:
    ...
    assert payload["calculator_method"] == "WILDER_ADX_PLUS_DI_MINUS_DI"
...
def test_compute_plus_minus_di_matches_ssot() -> None:
    ...
    _adx, expected_plus, expected_minus = strategy._compute_adx(bars)
    pd.testing.assert_series_equal(plus_di, expected_plus.astype(float), check_names=False)
    pd.testing.assert_series_equal(minus_di, expected_minus.astype(float), check_names=False)


def test_long_eligible_when_minus_di_dominates() -> None:
    idx = pd.date_range("2023-01-01", periods=3, freq="h", tz="UTC")
    bars = pd.DataFrame(
        {"open": [10.0] * 3, "high": [10.5] * 3, "low": [9.5] * 3, "close": [10.0] * 3},
        index=idx,
    )

    import src.research.adx_di_direction_confirmation_mr_eligibility_development_evaluation_v1.adx_di_direction_confirmation_eligibility_filter_v1 as filt_mod

    orig = filt_mod.compute_plus_minus_di
    try:
        # bar0: minus>plus -> long; bar1: equal -> neither; bar2: plus>minus -> short
        filt_mod.compute_plus_minus_di = lambda _bars: (  # noqa: E731
            pd.Series([10.0, 15.0, 20.0], index=idx),
            pd.Series([20.0, 15.0, 10.0], index=idx),
        )
        long_mask = long_eligible_mask_from_bars(bars, warmup_bars=0)
        short_mask = short_eligible_mask_from_bars(bars, warmup_bars=0)
    finally:
        filt_mod.compute_plus_minus_di = orig
    assert list(long_mask) == [True, False, False]
    assert list(short_mask) == [False, False, True]


def test_short_eligible_when_plus_di_dominates() -> None:
    idx = pd.date_range("2023-01-01", periods=4, freq="h", tz="UTC")
    bars = pd.DataFrame(
        {"open": [10.0] * 4, "high": [10.5] * 4, "low": [9.5] * 4, "close": [10.0] * 4},
        index=idx,
    )
    ...
    try:
        filt_mod.compute_plus_minus_di = lambda _bars: (  # noqa: E731
            pd.Series([20.0, 10.0, 20.0, 10.0], index=idx),
            pd.Series([10.0, 20.0, 10.0, 20.0], index=idx),
        )
        short_mask = short_eligible_mask_from_bars(bars, warmup_bars=0)
...
def test_warmup_bars_always_ineligible() -> None:
    ...
    assert not long_mask.iloc[:28].any()
    assert not short_mask.iloc[:28].any()
...
def test_is_entry_eligible_side_aware_long() -> None:
    assert is_entry_eligible(signal=1, plus_di=10.0, minus_di=20.0, in_warmup=False) is True
    assert is_entry_eligible(signal=1, plus_di=20.0, minus_di=10.0, in_warmup=False) is False
    assert is_entry_eligible(signal=1, plus_di=15.0, minus_di=15.0, in_warmup=False) is False


def test_is_entry_eligible_side_aware_short() -> None:
    assert is_entry_eligible(signal=-1, plus_di=20.0, minus_di=10.0, in_warmup=False) is True
    assert is_entry_eligible(signal=-1, plus_di=10.0, minus_di=20.0, in_warmup=False) is False
    assert is_entry_eligible(signal=-1, plus_di=15.0, minus_di=15.0, in_warmup=False) is False


def test_is_entry_eligible_flat_signal_never_blocked() -> None:
    assert is_entry_eligible(signal=0, plus_di=10.0, minus_di=20.0, in_warmup=False) is True
    assert is_entry_eligible(signal=0, plus_di=10.0, minus_di=20.0, in_warmup=True) is False


def test_is_entry_eligible_warmup_and_nonfinite_fail_closed() -> None:
    assert is_entry_eligible(signal=1, plus_di=10.0, minus_di=20.0, in_warmup=True) is False
    assert (
        is_entry_eligible(signal=1, plus_di=float("nan"), minus_di=20.0, in_warmup=False) is False
    )
    assert is_entry_eligible(signal=-1, plus_di=math.inf, minus_di=10.0, in_warmup=False) is False
...
def test_treatment_gate_monkeypatch_side_aware_blocks_wrong_side_entries(
    ...
    import src.research.adx_di_direction_confirmation_mr_eligibility_development_evaluation_v1.panel_runner_v1 as panel_runner_mod
    ...
    def _fake_compute_di(_bars: pd.DataFrame):
        # bar0: minus>plus -> long eligible; bar1: plus>minus -> short eligible;
        # bar2: minus>plus -> short ineligible
        return (
            pd.Series([10.0, 20.0, 10.0], index=idx),
            pd.Series([20.0, 10.0, 20.0], index=idx),
        )
    ...
    monkeypatch.setattr(panel_runner_mod, "compute_plus_minus_di", _fake_compute_di)
...
def test_runner_script_is_adx_di_specific() -> None:
    runner = (
        REPO
        / "scripts/research/run_evaluate_adx_di_direction_confirmation_mr_eligibility_development_v1.py"
    )
    text = runner.read_text(encoding="utf-8")
    assert "adx_di_direction_confirmation_mr_eligibility_development_evaluation_v1" in text
    assert "ma_trend_alignment_mr_eligibility_development_evaluation_v1" not in text
    assert "macd_histogram_countertrend_mr_eligibility_development_evaluation_v1" not in text
    assert "evaluate_adx_di_direction_confirmation_mr_eligibility_development_v1" in text
