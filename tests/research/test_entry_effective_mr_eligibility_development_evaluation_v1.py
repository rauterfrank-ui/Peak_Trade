"""Boundary + freeze tests for entry-effective MR eligibility DEVELOPMENT evaluation v1.
...
from src.research.entry_effective_mr_eligibility_development_evaluation_v1.panel_runner_v1 import (
...
def test_strict_bounds_exclude_exact_25_and_75() -> None:
    # min_bars=0 so the boundary rule (not warmup) is what is under test.
    idx = pd.date_range("2023-01-01", periods=5, freq="h", tz="UTC")
    rank = pd.Series([10.0, 25.0, 50.0, 75.0, 90.0], index=idx)
    mask = eligibility_mask_from_rank(rank, min_bars=0)
    # rank==25 and rank==75 must NOT be eligible (STRICT, not inclusive).
    assert list(mask) == [False, False, True, False, False]


def test_strict_bounds_reject_nan_rank() -> None:
    idx = pd.date_range("2023-01-01", periods=3, freq="h", tz="UTC")
    rank = pd.Series([float("nan"), 50.0, float("nan")], index=idx)
    mask = eligibility_mask_from_rank(rank, min_bars=0)
    assert list(mask) == [False, True, False]


def test_min_bars_warmup_always_ineligible() -> None:
    idx = pd.date_range("2023-01-01", periods=3, freq="h", tz="UTC")
    rank = pd.Series([50.0, 50.0, 50.0], index=idx)
    mask = eligibility_mask_from_rank(rank, min_bars=2)
...
def test_eligibility_labels_are_eligible_or_stand_aside() -> None:
    ...
    for i in range(1, 200):
        vals.append(vals[-1] * (1.0 + (0.001 if i % 5 == 0 else 0.0)))
    ...
    assert labels.iloc[:30].eq(STAND_ASIDE).all()  # min_bars warmup => stand aside
...
def test_apply_eligibility_to_mapped_position_signal_blocks_new_entries() -> None:
    # Ineligible + new entry intent (+-1) => forced to stand-aside (0).
...
def test_apply_eligibility_to_mapped_position_signal_flat_always_passes() -> None:
    # 0 (flat/exit) must pass through unchanged regardless of eligibility.
...
def test_treatment_gate_monkeypatch_zeros_mapped_entry_on_ineligible_bar(
    ...
    """The gate must act on the MAPPED MV2 position signal, not the raw strategy series.

    This reproduces the original bug scenario: mock
    ``map_decision_evidence_to_position_signal_v1`` (the function the MV2
    replay bar loop actually calls to derive its traded signal) to always
    return ``+1``, and confirm the gate forces it to ``0`` when the current
    bar (tracked via the wrapped ``bind_bar_for_mv2_wiring_v1``) is
    ineligible, while leaving eligible bars unaffected and incrementing the
    gate-effective ``entries_blocked_by_gate`` counter exactly once.
    """
    import src.backtest.mv2_research_wiring_v1 as wiring_mod
    import src.research.entry_effective_mr_eligibility_development_evaluation_v1.panel_runner_v1 as panel_runner_mod
    ...
    # Monkeypatch must be reverted on context exit.
...
def test_decision_fail_on_no_divergence_even_with_good_economics() -> None:
    # Economics look strictly better for treatment, but no eligibility
    # divergence was observed => must FAIL, never PASS/INCONCLUSIVE.
    out = decide_development_evaluation(
...
def test_decision_profit_factor_none_is_fail_closed() -> None:
    out = decide_development_evaluation(
        baseline=_baseline_metrics(profit_factor=None),
        treatment=_treatment_metrics(profit_factor=1.5),
        entry_eligibility_divergence_observed=True,
        minimum_trade_count=50,
    )
    # None on either side must never be treated as "greater than".
    assert out["checks"]["profit_factor_treatment_gt_control"] is False
    assert out["result_class"] == RESULT_FAIL
