"""Contract tests for V8 Operator Clarification Authority (no evaluation / no panel)."""
...
def test_valid_authority_and_digests() -> None:
    report = load_and_validate_authority(
        REPO, require_registered=True, require_ready_status=True, require_authorized_status=False
    )
...
def test_missing_authority_fail_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "src.research.bollinger_mr_midband_exit_reentry_cooldown_operator_clarification_authority_v8.AUTHORITY_REL_PATH",
        "config/research/__missing_operator_clarification_authority_v8.json",
    )
...
def test_unregistered_authority_fail_closed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "src.research.bollinger_mr_midband_exit_reentry_cooldown_operator_clarification_authority_v8._assert_registered",
        _boom,
    )
...
def test_gate_failures_before_slot_and_artifacts(
    ...
    monkeypatch.setattr(runner, "assert_v8_authority_and_prereg_gates", _boom)
...
def test_runner_source_order_gates_before_claim_before_panel() -> None:
    ...
    gate_pos = evaluate_fn.find("assert_v8_authority_and_prereg_gates(")
    claim_pos = evaluate_fn.find("_claim_run_slot_atomic_v8(")
    ...
    assert preflight_fn.find("assert_v8_authority_and_prereg_gates(") < preflight_fn.find(
        "output_dir.mkdir("
    )
    assert preflight_fn.find("assert_v8_authority_and_prereg_gates(") < preflight_fn.find(
...
def test_cli_evaluate_fail_closed_without_hypothesis_flag(tmp_path: Path) -> None:
    ...
    assert "V8_EVALUATION_NOT_AUTHORIZED" in proc.stdout or "NOT_AUTHORIZED" in proc.stdout
