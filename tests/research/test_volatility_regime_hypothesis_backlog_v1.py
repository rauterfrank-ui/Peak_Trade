...
def test_repo_backlog_open_with_one_preregistration() -> None:
    ...
    assert report["hypothesis_id"] == "VOLATILITY_EXPANSION_PERSISTENCE_NON_BITCOIN_PERPETUALS_V1"
...
def test_sibling_closed_lanes_and_inventory() -> None:
    ...
    term = backlog["terminal_hypotheses"][0]
    assert term["strategy_identity"] == "VOLATILITY_COMPRESSION_BREAKOUT_V1"
    assert term["terminal_result"] == "FAIL_CLOSED_NO_RETRY"
    ...
    assert hyp["strategy_identity"] == "VOLATILITY_EXPANSION_PERSISTENCE_V1"
