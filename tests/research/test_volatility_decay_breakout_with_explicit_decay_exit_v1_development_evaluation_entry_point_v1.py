"""Contract/unit tests for VDBX v1 development-evaluation entry point."""
...
from src.research.volatility_decay_breakout_with_explicit_decay_exit_v1_development_evaluation_v1.authorization_v1 import (
...
def test_owner_registry_and_entry_point_files_bound() -> None:
    ...
    assert (
        "tests/research/"
        "test_volatility_decay_breakout_with_explicit_decay_exit_v1_development_evaluation_"
        "entry_point_v1.py"
    ) in prefixes
...
def test_static_ids_and_bindings() -> None:
    assert STRATEGY_IDENTITY == "VOLATILITY_DECAY_BREAKOUT_WITH_EXPLICIT_DECAY_EXIT_V1"
    assert PREDECESSOR_STRATEGY_ID == "VOLATILITY_DECAY_BREAKOUT_V1"
    ...
    assert binding["portfolio_aggregation_id"] == PORTFOLIO_AGGREGATION_ID
...
def test_cli_runtime_flags_rejected() -> None:
    proc = subprocess.run(
        [sys.executable, str(REPO / CLI_REL_PATH), "--mode", "preflight", "--live"],
        cwd=REPO,
        capture_output=True,
        text=True,
        check=False,
    ...
    assert proc.returncode == 2
    payload = json.loads(proc.stdout.strip().splitlines()[-1])
    assert payload["reason"] == "RUNTIME_FLAG_REJECTED"


def test_measurement_contract_authorized_slot_consumed() -> None:
    contract = _load(CONTRACT)
    ...
    exit_sem = contract["exit_semantics"]
    assert exit_sem["evaluator_side_reconstruction_of_missing_strategy_exits_forbidden"] is True
    assert exit_sem["exit_state_machine_implemented"] is True
    assert exit_sem["productive_exit_pnl_evaluator_ref"] == PRODUCTIVE_PNL_EVALUATOR_REL_PATH
    assert exit_sem["second_pnl_truth_forbidden"] is True
