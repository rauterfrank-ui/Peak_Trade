...
/ ``run_arm``, and never sets ``PEAK_TRADE_ADX_DI_HOLDOUT_EXECUTION_GO``. It only
...
import src.research.adx_di_direction_confirmation_mr_eligibility_holdout_evaluation_v1 as holdout_pkg
...
from src.research.adx_di_direction_confirmation_mr_eligibility_holdout_evaluation_v1.panel_runner_v1 import (
    CONFIG_ID,
    EVALUATION_RUN_ID,
)
...
def test_package_markers_present() -> None:
    assert holdout_pkg.ADX_DI_DIRECTION_CONFIRMATION_MR_ELIGIBILITY_HOLDOUT_EVALUATION_V1 is True
...
def test_preflight_execution_gates_block_on_consumed_contract() -> None:
    contract = _load(CONTRACT_PATH)
    with pytest.raises(HoldoutPreregistrationError, match="HOLDOUT_RUN_ALREADY_CONSUMED"):
...
def test_preflight_blocks_when_development_not_pass() -> None:
    tampered = _definition_preflight_view()
    tampered["development_binding"] = dict(tampered["development_binding"])
    tampered["development_binding"]["development_result_class"] = "FAIL"
    with pytest.raises(HoldoutPreregistrationError, match="DEVELOPMENT_MUST_BE_PASS"):
        preflight_holdout_execution_gates(tampered)


def test_preflight_blocks_when_development_run_count_not_one() -> None:
    tampered = _definition_preflight_view()
    tampered["development_binding"] = dict(tampered["development_binding"])
    tampered["development_binding"]["development_run_count"] = 0
    with pytest.raises(HoldoutPreregistrationError, match="DEVELOPMENT_RUN_COUNT_MUST_BE_1"):
        preflight_holdout_execution_gates(tampered)


# --- development contract remains holdout-forbidden -------------------------
...
def test_evaluation_run_id_and_config_id_constants() -> None:
    assert EVALUATION_RUN_ID == "evaluate_adx_di_direction_confirmation_mr_eligibility_holdout_v1"
...
def test_terminal_evaluation_evidence_complete() -> None:
    evidence = (
        REPO / "docs/evidence/evaluate_adx_di_direction_confirmation_mr_eligibility_holdout_v1"
    )
    required = [
        "summary.json",
        "run_manifest.json",
    ...
    assert summary["result_class"] == "ARTIFACT_OR_EXECUTION_FAILURE_NO_RERUN"
