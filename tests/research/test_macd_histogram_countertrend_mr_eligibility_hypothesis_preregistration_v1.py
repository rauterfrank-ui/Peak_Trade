"""Contract tests for MACD histogram-countertrend MR eligibility hypothesis preregistration v1.
...
MA trend-alignment, and regime-gated eligibility mechanisms.
"""
...
EVIDENCE = (
    REPO / "docs/evidence/preregister_macd_histogram_countertrend_mr_eligibility_hypothesis_v1"
)
GOVERNANCE = (
    REPO
...
def test_repo_contract_validates_against_seal_registry() -> None:
    ...
    assert report["filter_id"] == "canonical_macd_histogram_countertrend_entry_eligibility_v1"
...
def test_prior_failed_atr_rsi_adx_and_regime_features_may_not_be_reused() -> None:
    contract = _load(CONTRACT_PATH)
    ...
    assert "atr_14h_rolling_percentile_rank_100h" not in feature_ids
    ...
    assert "macd_histogram_12_26_9" in feature_ids
    ...
    with pytest.raises(
        HypothesisPreregistrationError, match="PRIOR_FAILED_FEATURE_REUSE|ATR_FEATURE_REUSE"
    ):
        validate_preregistration_contract(bad)
    ...
    with pytest.raises(
        HypothesisPreregistrationError, match="PRIOR_FAILED_FEATURE_REUSE|RSI_FEATURE_REUSE"
    ):
    ...
    with pytest.raises(
        HypothesisPreregistrationError, match="PRIOR_FAILED_FEATURE_REUSE|ADX_FEATURE_REUSE"
    ):
    ...
    with pytest.raises(
        HypothesisPreregistrationError, match="PRIOR_FAILED_FEATURE_REUSE|MA_TREND_FEATURE_REUSE"
    ):
        validate_preregistration_contract(bad_ma)

    bad2 = copy.deepcopy(contract)
    bad2["eligibility_filter"]["features"].append(
...
def test_orthogonality_markers_filter_is_macd_histogram_not_atr_rsi_adx_ma_or_regime() -> None:
    contract = _load(CONTRACT_PATH)
    eligibility = contract["eligibility_filter"]
    assert eligibility["filter_id"] == "canonical_macd_histogram_countertrend_entry_eligibility_v1"
    assert "atr" not in eligibility["filter_id"].lower()
    assert "rsi" not in eligibility["filter_id"].lower()
    assert "adx" not in eligibility["filter_id"].lower()
    assert "regime" not in eligibility["filter_id"].lower()
    assert "ma_trend" not in eligibility["filter_id"].lower()
    assert "macd" in eligibility["filter_id"].lower()
    assert eligibility["primitive"] == "macd_histogram_sign_countertrend_filter"
    assert eligibility["calculator_ssot"] == "src/strategies/macd.py"
    assert eligibility["calculator_method"] == "MACD_EMA_HISTOGRAM"
    frozen = eligibility["frozen_parameters"]
    assert frozen["fast_ema"] == 12
    assert frozen["slow_ema"] == 26
    assert frozen["signal_ema"] == 9
    assert frozen["eligibility_on"] == "histogram_sign_side_aware"
    assert frozen["warmup_bars"] == 35
    for banned_key in (
        "vol_window",
        "vol_method",
        "vol_percentile_low",
    ...
    assert len(contract["prior_failed_hypotheses"]) == 5
...
def test_baseline_treatment_authority_and_promotion_forbidden() -> None:
    ...
    assert treatment["treatment_id"] == "macd_histogram_countertrend_mr_entry_eligibility_v1"
...
def test_evidence_and_governance_definition_only() -> None:
    ...
    assert summary["filter_id"] == "canonical_macd_histogram_countertrend_entry_eligibility_v1"
    assert summary["base_sha"] == "4138ba5af3ebfd8d35fdff10986beec7bba88aae"
