"""Contract tests for CS funding-rate dispersion z-score reversion v0 research scope ratification."""

from __future__ import annotations

from pathlib import Path

from src.research.cross_sectional_funding_rate_dispersion_zscore_reversion_v0_research_scope_ratification_v0 import (
...
GOVERNANCE_DOC = (
    ...
    "CROSS_SECTIONAL_FUNDING_RATE_DISPERSION_ZSCORE_REVERSION_V0_RESEARCH_SCOPE_RATIFICATION.md"
)
SCOPE_CONFIG = (
...
class TestCrossSectionalFundingRateDispersionZscoreReversionV0ResearchScopeRatification:
    def test_ratification_materializes_with_required_fields(self) -> None:
        ...
        validation = validate_dispersion_zscore_reversion_research_scope_ratification_v0(
            ratification
        )
        ...
        assert (
            ratification["material_difference_vs_persistence_reversal_filter_v0_confirmed"] is True
        )
        ...
        assert (
            ratification["terminalized_parent_binding_digest"] == TERMINALIZED_PARENT_BINDING_DIGEST
        )
        assert ratification["terminalized_parent_binding_digest_unchanged_retry_forbidden"] is True
    ...
    def test_governance_doc_exists_and_states_no_eval(self) -> None:
        ...
        assert "z-score" in text.lower() or "z_score" in text.lower()
        assert "persistence_reversal_filter" in text.lower()
