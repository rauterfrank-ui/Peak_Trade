"""Contract/unit tests for VEP v1 development-evaluation entry point."""
...
def test_owner_registry_and_entry_point_files_bound() -> None:
    ...
    assert "src/research/volatility_expansion_persistence_v1_development_evaluation_v1/" in prefixes
...
def test_static_ids_and_bindings() -> None:
    assert STRATEGY_IDENTITY == "VOLATILITY_EXPANSION_PERSISTENCE_V1"
...
def test_missing_authorization_blocks_evaluate() -> None:
    ...
    assert "ENTRY_POINT_BINDING_DEVELOPMENT_EVALUATION_AUTHORIZED_FALSE" in decision.reason_codes
...
def test_measurement_contract_unchanged_and_guards() -> None:
    ...
    # Preregistration reserves slot authorization on the measurement contract;
    # entry-point binding remains the execution gate and stays false.
    ...
    assert guards["entry_point_binding_authorized"] is False
