"""Contract tests for ADX range-admission MR eligibility hypothesis preregistration v1.

Definition-only. No backtest. No economic metrics. No holdout content inspection.
Orthogonal to prior FAIL ATR-percentile, RSI-exhaustion, and regime-gated eligibility mechanisms.
"""
...
EVIDENCE = REPO / "docs/evidence/preregister_adx_range_admission_mr_eligibility_hypothesis_v1"
GOVERNANCE = (
    REPO
...
def test_repo_contract_validates_against_seal_registry() -> None:
    ...
    assert report["filter_id"] == "canonical_adx_range_admission_entry_eligibility_v1"
...
def test_prior_failed_atr_rsi_and_regime_features_may_not_be_reused() -> None:
    contract = _load(CONTRACT_PATH)
...
def test_orthogonality_markers_filter_is_adx_range_not_atr_rsi_or_regime() -> None:
    contract = _load(CONTRACT_PATH)
    eligibility = contract["eligibility_filter"]
    assert eligibility["filter_id"] == "canonical_adx_range_admission_entry_eligibility_v1"
    ...
    assert eligibility["primitive"] == "adx_range_admission_filter"
    assert (
        eligibility["calculator_ssot"]
    ...
    assert frozen["adx_threshold"] == 25.0
    assert frozen["eligibility_comparator"] == "lt"
    assert frozen["warmup_bars"] == 28
    assert frozen["calculator"] == "wilder_ewm_alpha_1_over_period"
    for banned_key in (
        "vol_window",
        "vol_method",
        "vol_percentile_low",
    ...
    assert len(contract["prior_failed_hypotheses"]) == 3
...
def test_baseline_treatment_authority_and_promotion_forbidden() -> None:
    ...
    assert treatment["treatment_id"] == "adx_range_admission_mr_entry_eligibility_v1"
...
def test_evidence_and_governance_definition_only() -> None:
    ...
    assert summary["filter_id"] == "canonical_adx_range_admission_entry_eligibility_v1"
