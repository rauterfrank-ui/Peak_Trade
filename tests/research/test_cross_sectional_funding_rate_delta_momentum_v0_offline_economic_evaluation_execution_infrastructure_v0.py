"""Contract tests for funding-rate delta momentum execution infrastructure v0."""
...
def test_ops_config_loads(complete_binding: dict) -> None:
    cfg = load_ops_evaluation_config_v0(REPO_ROOT)
    assert cfg["strategy_id"] == "cross_sectional_funding_rate_delta_momentum"
    assert cfg["binding_digest"] == complete_binding["binding_digest"]


def test_execution_path_has_no_runtime_imports() -> None:
    module_name = "src.research.cross_sectional_funding_rate_delta_momentum_v0_offline_economic_evaluation_execution_v0"
