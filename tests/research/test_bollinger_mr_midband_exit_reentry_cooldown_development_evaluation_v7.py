"""Contract/unit tests for V7 wiring evaluation surfaces (clarification-bound).
...
from src.research.bollinger_mr_midband_exit_reentry_cooldown_development_evaluation_v7.hypothesis_dispatch_v7 import (
...
def test_package_constants_match_frozen_prereg_digest() -> None:
    assert PACKAGE_MARKER.endswith("V7=true")
    assert HYPOTHESIS_ID.endswith("_V7")
    assert COOLDOWN_BARS == 24
    assert MECHANISM_ID.endswith("reentry_cooldown_v1")
    assert OWNER_SURFACE.endswith("_EVALUATION_V7")
    assert EVIDENCE_REL_PATH.endswith("_v7/")
...
def test_decision_b1_identical_exits_and_reentry_divergence() -> None:
    ...
    out = decide_development_evaluation_v7(
        control=control,
        treatment=treatment,
    ...
    missing = decide_development_evaluation_v7(
        control=control,
        treatment=treatment,
...
def test_decision_b2_maxdd_and_b7_pf() -> None:
    ...
    bad = decide_development_evaluation_v7(
        control=control2,
...
def test_decision_b3_infra() -> None:
    infra = decide_development_evaluation_v7(
        control={},
        treatment={},
        reentry_divergence_observed=True,
        exit_fills_identical=True,
...
def test_dispatch_and_unauthorized_runner(tmp_path: Path) -> None:
    entry = resolve_v7_dispatch(HYPOTHESIS_ID)
    assert entry["cli"].endswith("_v7.py")
    with pytest.raises(HypothesisDispatchError):
        resolve_v7_dispatch("UNKNOWN_HYPOTHESIS")
    with pytest.raises(RuntimeError, match="V7_EVALUATION_NOT_AUTHORIZED"):
        run_development_evaluation(
            output_dir=tmp_path / "evaluate_v7_must_not_create_canon_path",
...
def test_panel_runner_auth_and_falsy_zero() -> None:
    ...
    assert "V7_EVALUATION_NOT_AUTHORIZED" in text
    assert "INCONCLUSIVE_INFRASTRUCTURE_FAILURE" in text
    assert "assert_v7_authority_and_prereg_gates" in text
    claim_pos = text.find("_claim_run_slot_atomic_v7(")
