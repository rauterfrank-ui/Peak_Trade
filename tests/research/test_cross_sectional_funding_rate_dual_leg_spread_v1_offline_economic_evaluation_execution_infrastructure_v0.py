"""Contract tests for funding-rate dual-leg spread v1 execution infrastructure v0."""
...
def fixture_scope_ratification(complete_binding: dict) -> dict:
    return materialize_dual_leg_spread_offline_economic_evaluation_scope_ratification_v1(
        repo_root=REPO_ROOT,
...
def fixture_bound_staging() -> Path:
    tmp = Path(tempfile.mkdtemp(prefix="cs_dual_leg_spread_exec_v1_"))
...
def test_bound_funding_materialization_is_complete(
    ...
    result = materialize_bound_funding_panel_dataset_v1(
        bound_staging,
...
def test_ops_config_loads(complete_binding: dict) -> None:
    cfg = load_ops_evaluation_config_v0(REPO_ROOT)
    assert cfg["strategy_id"] == "cross_sectional_funding_rate_dual_leg_spread"
...
def test_execution_path_has_no_runtime_imports() -> None:
    module_name = "src.research.cross_sectional_funding_rate_dual_leg_spread_v1_offline_economic_evaluation_execution_v0"
