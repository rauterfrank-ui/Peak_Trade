...
CONTRACT_PATH = Path(
    "docs/research/final_research_fleet_versioned_binding_contracts_no_eval_v0.json"
)
...
def test_final_research_fleet_binding_contracts_are_complete_no_eval_v0():
    ...
    for item in bindings:
        ...
        assert missing == []
        assert (
            item["canonical_decision_chain_digest"]
            == payload["source_evidence"]["parity_closeout_manifest_sha256"]
        )
        assert (
            item["backtest_runtime_parity_digest"]
            == payload["source_evidence"]["parity_closeout_manifest_sha256"]
        )
