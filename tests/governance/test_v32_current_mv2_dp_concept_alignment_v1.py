...
def test_alignment_consistent_with_v32_lifecycle_adjudication() -> None:
    ...
    assert binding["d26_implemented"] is False
...
def test_normative_spec_lists_d26_partial() -> None:
    text = (REPO_ROOT / NORMATIVE_SPEC).read_text(encoding="utf-8")
    assert "D26_STATUS=PARTIAL_CURRENT" in text
    assert "D26_IMPLEMENTED=false" in text
