"""Seventh Economic Guard class: SideState ARMED identity-split authorization."""
...
from src.governance.explicit_owner_adjudicated_sidestate_armed_identity_split_authorization_v1 import (
    ...
    ARMED_IDENTITY_SPLIT_MUTATION_PURPOSE,
    ARMED_IDENTITY_SPLIT_SCOPE_CLASS,
    ...
    compute_armed_identity_split_evidence_digest,
...
FOREIGN_MASTER_V2_PATH = "src/trading/master_v2/double_play_entry_exit_policy_v0.py"
EXCLUDED_PATH = "src/trading/master_v2/double_play_entry_exit_policy_v0.py"
EXECUTION_PREFIX_PATH = "src/execution/armed_identity_split_probe_v1.py"
...
def _active_grant(
    ...
    *,
    ...
    auth["allowed_surface_classes"] = [ARMED_IDENTITY_SPLIT_SCOPE_CLASS]
    auth["bound_diff_base_sha"] = diff_base_sha
    auth["slice_grant_id"] = FIXTURE_SLICE_GRANT_ID
    auth["authorized_evidence_digest"] = compute_armed_identity_split_evidence_digest(
        file_diffs=diffs,
...
class TestArmedIdentitySplitCommittedGrantV1:
    def test_committed_artifact_is_valid_active_slice_grant(self) -> None:
        ...
        assert reasons == (REASON_ARMED_IDENTITY_SPLIT_AUTH_VALID,)
        assert auth["contract_version"] == ARMED_IDENTITY_SPLIT_AUTH_VERSION
        ...
        assert auth["class_attestation"] == ARMED_IDENTITY_SPLIT_CLASS_ATTESTATION_RELATIVE
        assert auth["bound_authority_spec"] == ARMED_IDENTITY_SPLIT_BOUND_AUTHORITY_SPEC
        assert (REPO_ROOT / ARMED_IDENTITY_SPLIT_CLASS_ATTESTATION_RELATIVE).is_file()
        ...
        assert auth["required_semantic_invariants"]["SIXTH_CLASS_GRANT_REOPENED"] is False
        claims = auth["human_adjudicated_slice_claims"]
        assert claims["NEUTRAL_START_AND_SWITCH_TERMINAL_ARE_DISTINCT"] is True
        assert claims["PENDING_SEMANTICS_UNCHANGED"] is True
        assert claims["ENTRY_EXIT_SEMANTICS_UNCHANGED"] is True
        assert claims["LAST_ACTIVE_SIDE_BINDING_AUTHORIZED"] is False
...
class TestArmedIdentitySplitAdmissionPositiveV1:
    def test_exact_authorized_fixture_and_digest_pass(self) -> None:
        changed = [RUNTIME_PATH]
        diffs = {RUNTIME_PATH: _identity_split_diff(RUNTIME_PATH)}
        auth = _active_grant(changed, diffs)
        report = _report(changed, auth=auth, diffs=diffs)
        assert report.admissible is True
        assert report.fail_closed is False
        assert report.sidestate_armed_identity_split_authorization_applied is True
        assert REASON_ARMED_IDENTITY_SPLIT_AUTHORIZED in report.reason_codes
        ...
        assert report.to_dict()["new_sidestate_armed_identity_split_authorization_applied"] is True
...
class TestArmedIdentitySplitAdmissionNegativeV1:
    def test_without_class_master_v2_diff_blocks(self) -> None:
        ...
        assert REASON_ARMED_IDENTITY_SPLIT_AUTHORIZED not in report.reason_codes

    def test_skip_class_blocks_authorized_runtime_path(self) -> None:
        changed = [RUNTIME_PATH]
        diffs = {RUNTIME_PATH: _identity_split_diff(RUNTIME_PATH)}
        report = _report(changed, diffs=diffs, skip_armed=True)
        assert report.admissible is False
        assert report.sidestate_armed_identity_split_authorization_applied is False
    ...
    def test_foreign_master_v2_file_blocks(self) -> None:
        ...
        diffs = {path: _identity_split_diff(path) for path in changed}
        auth = _active_grant(allowed, diffs)
        report = _report(changed, auth=auth, diffs=diffs)
        assert report.admissible is False
        assert REASON_ARMED_IDENTITY_SPLIT_PATH_UNAUTHORIZED in report.reason_codes
    ...
    def test_additional_hunk_digest_mismatch_blocks(self) -> None:
        changed = [RUNTIME_PATH]
        original = {RUNTIME_PATH: _identity_split_diff(RUNTIME_PATH)}
        extra = {
        ...
        assert report.admissible is False
        assert REASON_ARMED_IDENTITY_SPLIT_DIGEST_MISMATCH in report.reason_codes
    ...
    def test_wrong_base_sha_blocks(self) -> None:
        ...
        assert report.admissible is False
        assert REASON_ARMED_IDENTITY_SPLIT_BASE_MISMATCH in report.reason_codes
    ...
    def test_excluded_path_in_diff_blocks(self) -> None:
        ...
        report = _report(changed, auth=auth, diffs=diffs)
        assert report.admissible is False
        assert REASON_ARMED_IDENTITY_SPLIT_EXCLUDED_PATH in report.reason_codes
    ...
    def test_required_runtime_path_missing_from_diff_blocks(self) -> None:
        ...
        changed = [other]
        diffs = {other: _identity_split_diff(other)}
        auth = _active_grant(allowed, diffs, required_runtime_paths=[RUNTIME_PATH])
        report = _report(changed, auth=auth, diffs=diffs)
        assert report.admissible is False
        assert REASON_ARMED_IDENTITY_SPLIT_REQUIRED_RUNTIME_MISSING in report.reason_codes
    ...
    def test_glob_in_path_blocks(self) -> None:
        ...
        assert "ARMED_IDENTITY_SPLIT_ALLOWED_PATHS_NOT_EXACT_FILES" in reasons
    ...
    def test_unknown_json_key_blocks(self) -> None:
        ...
        valid, reasons = validate_armed_identity_split_authorization(auth, repo_root=REPO_ROOT)
        assert valid is False
        assert REASON_ARMED_IDENTITY_SPLIT_UNKNOWN_FIELD in reasons
        report = _report([RUNTIME_PATH], auth=auth, diffs=diffs)
        assert report.admissible is False

    def test_fifth_class_purpose_in_seventh_class_blocks(self) -> None:
        diffs = {RUNTIME_PATH: _identity_split_diff(RUNTIME_PATH)}
        auth = _active_grant([RUNTIME_PATH], diffs)
        auth["mutation_purpose_class"] = "PRODUCTIVE_CANONICAL_MAPPING_CONTRACT_RUNTIME_BIND"
        valid, reasons = validate_armed_identity_split_authorization(auth, repo_root=REPO_ROOT)
        assert valid is False
        assert "ARMED_IDENTITY_SPLIT_FIFTH_CLASS_PURPOSE_FORBIDDEN" in reasons

    def test_sixth_class_purpose_in_seventh_class_blocks(self) -> None:
        diffs = {RUNTIME_PATH: _identity_split_diff(RUNTIME_PATH)}
        auth = _active_grant([RUNTIME_PATH], diffs)
        auth["mutation_purpose_class"] = "SCOPE_DIRECTION_GENERATOR_FALLBACK_REPAIR"
        valid, reasons = validate_armed_identity_split_authorization(auth, repo_root=REPO_ROOT)
        assert valid is False
        assert "ARMED_IDENTITY_SPLIT_SIXTH_CLASS_PURPOSE_FORBIDDEN" in reasons
    ...
    def test_forbidden_execution_prefix_blocks_even_if_listed(self) -> None:
        ...
        report = _report(changed, auth=auth, diffs=diffs)
        assert report.admissible is False
        assert REASON_ARMED_IDENTITY_SPLIT_FORBIDDEN_PREFIX in report.reason_codes
        assert report.sidestate_armed_identity_split_authorization_applied is False
