...
from src.ops.section_11_14_live_order_and_economic_evidence_ladder_v1.constants_v1 import (
    ...
    EXACT_SINGLE_LIVE_FILL_EVIDENCE_RUN_ID,
    ...
    HISTORICAL_EXACT_FEE_RESTART_DURABILITY_CLOSURE_SHA,
    ...
    HISTORICAL_GET_ONLY_TRADE_FEE_REFRESH_SHA,
    HISTORICAL_GET_ONLY_TRADE_FEE_REFRESH_SLICE,
    HISTORICAL_GET_ONLY_EARLIEST_UNRESOLVED_DEPENDENCY,
    HISTORICAL_GET_ONLY_NEXT_OWNER_GO_REQUIRED,
)
...
EXACT_SINGLE_BOUND_FILL_THEN_PRE_RESTART_CAPTURE_SPEC = (
    REPO_ROOT
...
GET_ONLY_TRADE_FEE_REFRESH_SPEC = (
    REPO_ROOT
    / "docs/ops/specs/SECTION_11_14_CURRENT_ORIGIN_MAIN_8605BF_GET_ONLY_TRADE_FEE_AND_STALE_PRETRADE_PREDICATE_REFRESH_V1.md"
)
EXACT_SINGLE_LIVE_FILL_SPEC = (
    REPO_ROOT
    / "docs/ops/specs/SECTION_11_14_LIVE_HANDOFF_EXACT_SINGLE_LIVE_FILL_REQUIRES_SEPARATE_OWNER_EXECUTION_GO_V1.md"
)
HISTORICAL_SPEC = (
    REPO_ROOT
...
GET_ONLY_PRETRADE_PACK = (
    REPO_ROOT
    / "evidence/ops"
    / ("section_11_14_live_handoff_current_origin_main_bound_get_only_pretrade_readiness_v1")
    / GET_REFRESH_EVIDENCE_RUN_ID
)
EXACT_SINGLE_LIVE_FILL_EVIDENCE = (
    REPO_ROOT
    / "evidence/ops"
...
HISTORICAL_LIVE_IDENTITY_BOUND_VENUE_FILL_READINESS_EVIDENCE = (
    REPO_ROOT
    / "evidence/ops"
    / "section_11_14_live_order_and_economic_evidence_ladder_v1"
    / HISTORICAL_LIVE_IDENTITY_BOUND_VENUE_FILL_READINESS_RUN_ID
...
def test_current_slice_constants_target_flatten_authority_pre_execution_repair() -> None:
    ...
    assert GET_REFRESH_EVIDENCE_RUN_ID == "20260907T203636Z"
    assert EXACT_SINGLE_LIVE_FILL_EVIDENCE_RUN_ID == "20260907T211230Z"
    ...
    assert GET_REFRESH_EVIDENCE.name == GET_REFRESH_EVIDENCE_RUN_ID
    assert EXACT_SINGLE_LIVE_FILL_EVIDENCE.name == EXACT_SINGLE_LIVE_FILL_EVIDENCE_RUN_ID
...
def test_runbook_live_identity_bound_venue_fill_readiness_and_exact_execution_contract_slice() -> (
    ...
    end = text.find(CANONICAL_EXACT_SINGLE_BOUND_FILL_THEN_PRE_RESTART_CAPTURE_SLICE_HEADING, start)
    assert start >= 0
    assert end > start
    section = text[start:end]
    assert HISTORICAL_LIVE_IDENTITY_BOUND_VENUE_FILL_READINESS_OWNER_GO in section
    assert (
        "THIS_SLICE=11.14.LIVE_HANDOFF_LIVE_IDENTITY_BOUND_VENUE_FILL_READINESS_AND_EXACT_EXECUTION_CONTRACT"
        in section
    )
    assert (
        "PREDECESSOR_SLICE=11.14.LIVE_HANDOFF_CONTEMPORANEOUS_PRE_RESTART_CAPTURE_OBSERVATION_AND_NON_EXECUTION_PROOF"
        in section
    )
    assert (
        f"EXPECTED_ORIGIN_MAIN_SHA={HISTORICAL_LIVE_IDENTITY_BOUND_VENUE_FILL_READINESS_SHA}"
        in section
    )
    assert "SECTION_11_14_AUTHORIZED=false" in section
    assert "SECTION_11_14_COMPLETE=false" in section
...
def test_runbook_exact_single_live_identity_bound_venue_fill_then_contemporaneous_pre_restart_capture_slice() -> (
    None
):
    text = MASTER_RUNBOOK.read_text(encoding="utf-8")
    ...
    assert "LIVE_FILL_READINESS=false" in section
    assert "MINIMAL_ECONOMIC_ACTION_CONTRACT_STATUS=BLOCKED" in section
    assert "EXACT_ECONOMIC_ACTION_CONTRACT_STATUS=BLOCKED" in section
    assert "LIVE_FILL_EXECUTION_AUTHORIZED=false" in section
    assert "LIVE_FILL_EXECUTED=false" in section
    assert "CODE_CHANGE_REQUIRED=true" in section
    assert "TERMINAL_STATE=CODE_GAP_FOUND" in section
    assert "CONTEMPORANEOUS_PRE_RESTART_CAPTURE_OBSERVATION=NOT_EXECUTED" in section
    assert "CONTEMPORANEOUS_PRODUCTIVE_CAPTURE_EXECUTED=false" in section
    assert "CURRENT_RUNTIME_EXECUTION_AUTHORIZED=false" in section
    assert "AUTHORIZED_RUNTIME_SURFACE=NONE" in section
...
def test_runbook_standing_fee_slippage_and_complete_non_executing_exact_execution_envelope_slice() -> (
    ...
    assert "FUTURE_EXECUTION_OWNER_GO_REQUIRED=true" in section
    assert "NEXT_SLICE_AUTHORIZED=false" in section
    assert HISTORICAL_GET_ONLY_NEXT_OWNER_GO_REQUIRED in section
    assert HISTORICAL_GET_ONLY_EARLIEST_UNRESOLVED_DEPENDENCY in section
...
def test_runbook_exact_fee_restart_durability_and_pre_execution_readiness_closure_slice() -> None:
    ...
    end = text.find(CANONICAL_GET_ONLY_TRADE_FEE_REFRESH_SLICE_HEADING, start)
    assert start >= 0
    assert end > start
    section = text[start:end]
    assert HISTORICAL_EXACT_FEE_RESTART_DURABILITY_CLOSURE_OWNER_GO in section
    assert HISTORICAL_EXACT_FEE_RESTART_DURABILITY_CLOSURE_SLICE in section
    assert (
        f"EXPECTED_ORIGIN_MAIN_SHA={HISTORICAL_EXACT_FEE_RESTART_DURABILITY_CLOSURE_SHA}" in section
    )
    assert "SECTION_11_14_AUTHORIZED=false" in section
    assert "SECTION_11_14_COMPLETE=false" in section
    assert "LIVE_ACCOUNTING_RECONSTRUCTED=true" in section
...
def test_runbook_get_only_trade_fee_and_stale_pretrade_refresh_slice() -> None:
    text = MASTER_RUNBOOK.read_text(encoding="utf-8")
    start = text.find(CANONICAL_GET_ONLY_TRADE_FEE_REFRESH_SLICE_HEADING)
    end = text.find(CANONICAL_EXACT_SINGLE_LIVE_FILL_SLICE_HEADING, start)
    assert start >= 0
    assert end > start
    section = text[start:end]
    assert HISTORICAL_GET_ONLY_TRADE_FEE_REFRESH_OWNER_GO in section
    assert HISTORICAL_GET_ONLY_TRADE_FEE_REFRESH_SLICE in section
    assert f"EXPECTED_ORIGIN_MAIN_SHA={HISTORICAL_GET_ONLY_TRADE_FEE_REFRESH_SHA}" in section
    assert "SECTION_11_14_AUTHORIZED=false" in section
    assert "SECTION_11_14_COMPLETE=false" in section
    assert "EXACT_OKX_FEE_FORMULA_UNPROVEN=true" in section
    assert "BOUNDED_FEE_ENVELOPE_CURRENT_NUMERIC_PROVEN=true" in section
    assert "TECHNICAL_PRE_EXECUTION_READINESS=true" in section
    assert "TECHNICAL_EXECUTION_READINESS=true" in section
    assert "OWNER_EXECUTION_AUTHORIZED=false" in section
    assert "GET_PERFORMED=true" in section
    assert "POST_PERFORMED=false" in section
    assert "LIVE_SUBMIT_EXECUTED=false" in section
    ...
    assert HISTORICAL_GET_ONLY_NEXT_OWNER_GO_REQUIRED in section
    assert HISTORICAL_GET_ONLY_EARLIEST_UNRESOLVED_DEPENDENCY in section
    assert GET_REFRESH_EVIDENCE_RUN_ID in section
    for field_name in LADDER_FIELDS:
        assert field_name in section
...
def test_runbook_exact_single_live_fill_consumed_no_submit_slice() -> None:
    ...
    assert "POSITION_NET_QTY=1" in section
    assert "OPEN_POSITION_PRESENT=true" in section
    assert "WOULD_INCREASE_EXISTING_POSITION=true" in section
    assert "MAX_POSITIONS_GATE_PASS=false" in section
    assert "EXECUTION_DECISION=NO_EXECUTION" in section
    assert "SUBMIT_ATTEMPT_COUNT=0" in section
    assert "GET_PERFORMED=true" in section
    assert "POST_PERFORMED=false" in section
    assert "LIVE_SUBMIT_EXECUTED=false" in section
    assert "WIRE_SEND_EXECUTED=false" in section
    assert "RESTART_EXECUTED=false" in section
    assert "CRASH_TEST_EXECUTED=false" in section
    assert "AUTOMATIC_RESUBMIT=false" in section
...
def test_spec_mot_atlas_and_evidence_exist() -> None:
    ...
    assert (
        "SECTION_11_14_CURRENT_ORIGIN_MAIN_8605BF_GET_ONLY_TRADE_FEE_AND_STALE_PRETRADE_PREDICATE_REFRESH_V1.md"
        in mot
    )
    assert (
    ...
    exact_spec = EXACT_SINGLE_BOUND_FILL_THEN_PRE_RESTART_CAPTURE_SPEC.read_text(encoding="utf-8")
    assert (
        "DOCS_TOKEN_SECTION_11_14_LIVE_HANDOFF_EXACT_SINGLE_LIVE_IDENTITY_BOUND_VENUE_FILL_THEN_CONTEMPORANEOUS_PRE_RESTART_CAPTURE_V1"
        in exact_spec
    )
    assert "LIVE_IDENTITY_BOUND_VENUE_FILL_REQUIRED=true" in exact_spec
    assert "LIVE_FILL_READINESS_MATRIX_STATUS=COMPLETE" in exact_spec
    assert "LIVE_FILL_READINESS=false" in exact_spec
    assert "MINIMAL_ECONOMIC_ACTION_CONTRACT_STATUS=BLOCKED" in exact_spec
    assert "EXACT_ECONOMIC_ACTION_CONTRACT_STATUS=BLOCKED" in exact_spec
    assert "CODE_CHANGE_REQUIRED=true" in exact_spec
    assert "TERMINAL_STATE=CODE_GAP_FOUND" in exact_spec
    assert "LIVE_FILL_EXECUTION_AUTHORIZED=false" in exact_spec
    assert "LIVE_FILL_EXECUTED=false" in exact_spec
    assert "CONTEMPORANEOUS_PRE_RESTART_CAPTURE_OBSERVATION=NOT_EXECUTED" in exact_spec
    assert "CONTEMPORANEOUS_PRODUCTIVE_CAPTURE_EXECUTED=false" in exact_spec
    assert "CURRENT_RUNTIME_EXECUTION_AUTHORIZED=false" in exact_spec
    assert "AUTHORIZED_RUNTIME_SURFACE=NONE" in exact_spec
    assert "IMPLEMENTATION_AUTHORIZED=true" in exact_spec
    ...
    refresh_spec = GET_ONLY_TRADE_FEE_REFRESH_SPEC.read_text(encoding="utf-8")
    assert (
        "DOCS_TOKEN_SECTION_11_14_CURRENT_ORIGIN_MAIN_8605BF_GET_ONLY_TRADE_FEE_AND_STALE_PRETRADE_PREDICATE_REFRESH_V1"
        in refresh_spec
    )
    assert "EXACT_OKX_FEE_FORMULA_STATUS=UNPROVEN" in refresh_spec
    assert "TECHNICAL_PRE_EXECUTION_READINESS=true" in refresh_spec
    assert "OWNER_EXECUTION_AUTHORIZED=false" in refresh_spec
    assert "GET_PERFORMED=true" in refresh_spec
    assert "POST_PERFORMED=false" in refresh_spec
    fill_spec = EXACT_SINGLE_LIVE_FILL_SPEC.read_text(encoding="utf-8")
    assert (
        "DOCS_TOKEN_SECTION_11_14_LIVE_HANDOFF_EXACT_SINGLE_LIVE_FILL_REQUIRES_SEPARATE_OWNER_EXECUTION_GO_V1"
        in fill_spec
    )
    assert "OWNER_EXECUTION_GO_STATUS=CONSUMED_NO_SUBMIT" in fill_spec
    assert "EXECUTION_DECISION=NO_EXECUTION" in fill_spec
    ...
    reprice_spec = (
        REPO_ROOT / "docs/ops/specs/SECTION_11_14_ENVELOPE_REPRICE_OR_FRESHNESS_AT_SEND_V1.md"
    ).read_text(encoding="utf-8")
    assert "DOCS_TOKEN_SECTION_11_14_ENVELOPE_REPRICE_OR_FRESHNESS_AT_SEND_V1" in reprice_spec
    assert "REPRICE_EXECUTED=true" in reprice_spec
    assert "ENVELOPE_REBUILD_EXECUTED=true" in reprice_spec
    assert "GET_CALL_COUNT=7" in reprice_spec
    assert "NEW_LIMIT_PRICE=0.8185" in reprice_spec
    assert "RECEIPT_ATTACHED=false" in reprice_spec
    assert "POST_PERFORMED=false" in reprice_spec
    assert "HTTP_POST_EXECUTED=false" in reprice_spec
    ...
    hmac_spec = (REPO_ROOT / "docs/ops/specs/SECTION_11_14_HMAC_GENERATION_V1.md").read_text(
        encoding="utf-8"
    )
    assert "DOCS_TOKEN_SECTION_11_14_HMAC_GENERATION_V1" in hmac_spec
    assert "HMAC_GENERATION=PROVEN" in hmac_spec
    assert "HMAC_GENERATION_WIRED=true" in hmac_spec
    assert "HMAC_GENERATION_ON_SEND_PATH=false" in hmac_spec
    assert "HMAC_GENERATION_REQUIRES_WIRE_SEND_AUTHORITY_VERIFY_ACCEPT=true" in hmac_spec
    assert "POST_PERFORMED=false" in hmac_spec
    assert "WIRE_SEND_EXECUTED=false" in hmac_spec
    assert "REAL_POST_COUNT=0" in hmac_spec
    ...
    assert (
        "id: PHASE:section_11_14_current_origin_main_8605bf_get_only_trade_fee_and_stale_pretrade_predicate_refresh"
        in catalog
    )
    assert (
    ...
    assert HISTORICAL_LIVE_IDENTITY_BOUND_VENUE_FILL_READINESS_EVIDENCE.is_dir()
    historical_readiness_verified = verify_manifest_v1(
        HISTORICAL_LIVE_IDENTITY_BOUND_VENUE_FILL_READINESS_EVIDENCE
    )
    assert int(historical_readiness_verified.get("MANIFEST_VERIFY_RC", 1)) == 0
    historical_readiness_summary = (
        HISTORICAL_LIVE_IDENTITY_BOUND_VENUE_FILL_READINESS_EVIDENCE / "SUMMARY.json"
    ).read_text(encoding="utf-8")
    assert '"LIVE_FILL_READINESS": false' in historical_readiness_summary
    assert '"MINIMAL_ECONOMIC_ACTION_CONTRACT_STATUS": "BLOCKED"' in historical_readiness_summary
    assert '"LIVE_FILL_EXECUTION_AUTHORIZED": false' in historical_readiness_summary
    assert (
        HISTORICAL_LIVE_IDENTITY_BOUND_VENUE_FILL_READINESS_EVIDENCE
        / "LIVE_FILL_READINESS_MATRIX.json"
    ).is_file()
    ...
    assert '"EXACT_ECONOMIC_ACTION_CONTRACT_STATUS": "BLOCKED"' in current_summary
    assert '"CODE_CHANGE_REQUIRED": true' in current_summary
    assert '"TERMINAL_STATE": "CODE_GAP_FOUND"' in current_summary
    ...
    assert '"REPAIR_MERGE_AUTHORIZED": false' in current_summary
    ...
    assert (EVIDENCE / "CONTEMPORANEOUS_GATE_MATRIX.json").is_file()
    assert (EVIDENCE / "MINIMAL_ECONOMIC_ACTION_CONTRACT.json").is_file()
    assert (EVIDENCE / "CODE_GAP.json").is_file()
    assert (EVIDENCE / "CAPTURE_SEAM_REPAIR.json").is_file()
    ...
    assert GET_REFRESH_EVIDENCE.is_dir()
    refresh_verified = verify_manifest_v1(GET_REFRESH_EVIDENCE)
    assert int(refresh_verified.get("MANIFEST_VERIFY_RC", 1)) == 0
    assert GET_ONLY_PRETRADE_PACK.is_dir()
    assert int(verify_manifest_v1(GET_ONLY_PRETRADE_PACK).get("MANIFEST_VERIFY_RC", 1)) == 0
    refresh_summary = (GET_REFRESH_EVIDENCE / "SUMMARY.json").read_text(encoding="utf-8")
    assert '"EXACT_OKX_FEE_FORMULA_STATUS": "UNPROVEN"' in refresh_summary
    assert '"BOUNDED_FEE_ENVELOPE_CURRENT_NUMERIC_PROVEN": true' in refresh_summary
    assert '"TECHNICAL_PRE_EXECUTION_READINESS": true' in refresh_summary
    assert '"OWNER_EXECUTION_AUTHORIZED": false' in refresh_summary
    assert '"GET_PERFORMED": true' in refresh_summary
    assert '"POST_PERFORMED": false' in refresh_summary
    assert '"RESTART_EXECUTED": false' in refresh_summary
    assert '"CRASH_TEST_EXECUTED": false' in refresh_summary
