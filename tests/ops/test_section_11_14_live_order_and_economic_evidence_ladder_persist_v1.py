...
def test_current_slice_constants_target_fee_observed_adjudication() -> None:
    assert THIS_SLICE == "11.14.LIVE_FEE_OBSERVED_ADJUDICATION"
    assert PREDECESSOR_SLICE == "11.14.LIVE_FILL_OBSERVED_ADJUDICATION"
    assert OWNER_GO.endswith("LIVE_FEE_OBSERVED_MAXIMUM_SAFE_LEVERAGE_V1")
    assert EXPECTED_ORIGIN_MAIN_SHA == "de053526a5066526a1fd1ebaf5f5c4045a3ad4d5"
    assert EARLIEST_UNRESOLVED_DEPENDENCY == "LIVE_POSITION_RECONCILED"
    assert NEXT_OWNER_GO_REQUIRED == "OWNER_GO_FOR_LIVE_POSITION_RECONCILED"
    assert LAST_CANONICALLY_CLOSED_STEP == "SECTION_11_14_LIVE_FEE_OBSERVED"
    assert CANONICAL_EVIDENCE_RUN_ID == "20260904T173813Z"
...
def test_spec_mot_atlas_and_evidence_exist() -> None:
    ...
    assert '"LIVE_POSITION_RECONCILED": false' in current_summary
    assert '"POST_USED": false' in current_summary
    assert '"GET_PERFORMED": true' in current_summary
    assert '"CASE_ADJUDICATION": "CASE_LIVE_FEE_OBSERVED_POSITION_INELIGIBLE"' in current_summary
    assert (EVIDENCE / "FEE_OBSERVED_ADJUDICATION.json").is_file()
