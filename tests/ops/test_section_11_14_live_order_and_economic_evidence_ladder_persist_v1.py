...
def test_current_slice_constants_target_fill_observed_adjudication() -> None:
    assert THIS_SLICE == "11.14.LIVE_FILL_OBSERVED_ADJUDICATION"
    assert PREDECESSOR_SLICE == "11.14.LIVE_SUBMIT_ACK_OBSERVED_ADJUDICATION"
    assert OWNER_GO.endswith("LIVE_FILL_OBSERVED_MAXIMUM_SAFE_LEVERAGE_V1")
    assert EXPECTED_ORIGIN_MAIN_SHA == "fead386cc6746524301a01b7b7489bea0621e4f3"
    assert EARLIEST_UNRESOLVED_DEPENDENCY == "LIVE_FEE_OBSERVED"
    assert NEXT_OWNER_GO_REQUIRED == "OWNER_GO_FOR_LIVE_FEE_OBSERVED"
    assert LAST_CANONICALLY_CLOSED_STEP == "SECTION_11_14_LIVE_FILL_OBSERVED"
    assert CANONICAL_EVIDENCE_RUN_ID == "20260904T165859Z"
...
def test_spec_mot_atlas_and_evidence_exist() -> None:
    ...
    assert '"LIVE_SUBMIT_ACK_OBSERVED": true' in current_summary
    assert '"LIVE_FILL_OBSERVED": true' in current_summary
    assert '"LIVE_FEE_OBSERVED": false' in current_summary
    assert '"FULL_FILL_OBSERVED": true' in current_summary
    assert '"PARTIAL_FILL_OBSERVED": false' in current_summary
    assert '"POST_USED": false' in current_summary
    assert '"GET_PERFORMED": true' in current_summary
    assert '"CASE_ADJUDICATION": "CASE_LIVE_FILL_OBSERVED_FEE_INELIGIBLE"' in current_summary
    assert (EVIDENCE / "FILL_OBSERVED_ADJUDICATION.json").is_file()
    assert (EVIDENCE / "GET_FILLS.raw.json").is_file()
    assert (EVIDENCE / "GET_ORDER_STATUS.raw.json").is_file()
