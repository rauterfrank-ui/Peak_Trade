"""CURRENT_PRODUCTIVE LIVE_AUTHORIZED send-capable adapter tests. Offline. No POST."""

from __future__ import annotations

import json
from pathlib import Path

import pytest

from src.ops.capability_11_1_execution_domain_and_order_lifecycle_contracts_v1.execution_ports_v1 import (
    ExecutionPortConstructionForbiddenError,
    construct_live_execution_port_v1,
)
from src.ops.full_core_live_path_composition_root_v1.constants_v1 import (
    CAP_11_1_SEND_CAPABLE_ADAPTER_CONSTRUCTED,
    EXTERNAL_EFFECT_AUTHORIZED,
    EXTERNAL_EFFECT_GATE_IMPLEMENTED,
    LIVE_ARMED,
    LIVE_AUTHORIZED,
    LIVE_AUTHORIZED_DOES_NOT_IMPLY_EXTERNAL_EFFECT,
    LIVE_AUTHORIZED_DOES_NOT_IMPLY_POST,
    LIVE_AUTHORIZED_DOES_NOT_IMPLY_STEP_29Q,
    LIVE_AUTHORIZED_STANDING_ADMISSION_SEAM_IMPLEMENTED,
    LIVE_AUTHORIZED_TRUE_IS_NOT_AUTOMATIC_SEND,
    LIVE_ENABLED,
    PRODUCTIVE_WIRE_SEND_REACHABLE,
    PRODUCTIVE_WIRE_SEND_REACHABLE_DOES_NOT_IMPLY_EXTERNAL_EFFECT,
    PRODUCTIVE_WIRE_SEND_REACHABLE_DOES_NOT_IMPLY_POST,
    SUBMISSION_AUTHORIZED,
    WIRE_SEND_PERMITTED,
    current_productive_first_real_blocker_v1,
)
from src.ops.full_core_live_path_composition_root_v1.external_effect_gate_v1 import (
    FullCoreExternalEffectNotAuthorizedError,
    evaluate_external_effect_v1,
)
from src.ops.full_core_live_path_composition_root_v1.gated_productive_wire_transport_v1 import (
    FullCoreGatedProductiveWireTransportV1,
    FullCoreSendCredentialHandleV1,
)
from src.ops.full_core_live_path_composition_root_v1.live_admission_gap_dag_v1 import (
    gap_node_v1,
)
from src.ops.full_core_live_path_composition_root_v1.live_authorized_v1 import (
    evaluate_live_authorized_v1,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.constants_v1 import (
    CURRENT_PRODUCTIVE_LIVE_AUTHORIZED_AND_CAP_11_1_SEND_CAPABLE_ADAPTER_CREATED,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.current_productive_live_authorized_and_cap_11_1_send_capable_adapter_v1 import (
    CANONICAL_PACK_RELPATH,
    EXPECTED_ORIGIN_MAIN_SHA,
    OWNER_GO,
    THIS_SLICE,
    CurrentProductiveLiveAuthorizedSendCapableError,
    execute_current_productive_live_authorized_and_cap_11_1_send_capable_adapter_v1,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.package_1_s6_mapping_classification_v1 import (
    verify_manifest_sha256_v1,
)
from src.ops.section_11_13_5_live_canary_minimum_exposure_v1.constants_v1 import (
    LIVE_ARMED as CANARY_LIVE_ARMED,
    LIVE_ORDER_AUTHORIZED as CANARY_LIVE_ORDER_AUTHORIZED,
)
from src.ops.section_11_14_live_order_and_economic_evidence_ladder_v1.constants_v1 import (
    LIVE_ARMED as SECTION_11_14_LIVE_ARMED,
)

REPO_ROOT = Path(__file__).resolve().parents[2]
RUNBOOK = REPO_ROOT / "docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md"
MOT_PATH = REPO_ROOT / "docs/governance/PEAK_TRADE_MAP_OF_TRUTH.md"
SPEC_PATH = (
    REPO_ROOT
    / "docs/ops/specs/FULL_CORE_CURRENT_PRODUCTIVE_LIVE_AUTHORIZED_AND_CAP_11_1_SEND_CAPABLE_ADAPTER_V1.md"
)
ATLAS_PATH = REPO_ROOT / "docs/system_atlas/entities/catalog.yaml"
DH_HEADING = (
    "### 11.2.1.DH FULL_CORE_CURRENT_PRODUCTIVE_LIVE_AUTHORIZED_AND_CAP_11_1_SEND_CAPABLE_ADAPTER"
)
PROTECTED_ALGORITHM_FILES = (
    "src/ops/governed_futures_universe_producer_v1/eligibility_v1.py",
    "src/ops/productive_futures_ranking_producer_v1/ranking_v1.py",
    "src/ops/productive_futures_ranking_producer_v1/policy_v1.py",
    "src/ops/single_selected_future_policy_v1/selection_v1.py",
    "src/ops/single_selected_future_policy_v1/policy_v1.py",
)


def test_standing_flags_close_without_external_effect() -> None:
    assert CURRENT_PRODUCTIVE_LIVE_AUTHORIZED_AND_CAP_11_1_SEND_CAPABLE_ADAPTER_CREATED is True
    assert LIVE_ENABLED is True
    assert LIVE_ARMED is True
    assert WIRE_SEND_PERMITTED is True
    assert SUBMISSION_AUTHORIZED is True
    assert LIVE_AUTHORIZED is True
    assert LIVE_AUTHORIZED_STANDING_ADMISSION_SEAM_IMPLEMENTED is True
    assert LIVE_AUTHORIZED_TRUE_IS_NOT_AUTOMATIC_SEND is True
    assert LIVE_AUTHORIZED_DOES_NOT_IMPLY_EXTERNAL_EFFECT is True
    assert LIVE_AUTHORIZED_DOES_NOT_IMPLY_POST is True
    assert LIVE_AUTHORIZED_DOES_NOT_IMPLY_STEP_29Q is True
    assert CAP_11_1_SEND_CAPABLE_ADAPTER_CONSTRUCTED is True
    assert PRODUCTIVE_WIRE_SEND_REACHABLE is True
    assert PRODUCTIVE_WIRE_SEND_REACHABLE_DOES_NOT_IMPLY_EXTERNAL_EFFECT is True
    assert PRODUCTIVE_WIRE_SEND_REACHABLE_DOES_NOT_IMPLY_POST is True
    assert EXTERNAL_EFFECT_AUTHORIZED is False
    assert EXTERNAL_EFFECT_GATE_IMPLEMENTED is True
    assert CANARY_LIVE_ARMED is False
    assert CANARY_LIVE_ORDER_AUTHORIZED is False
    assert SECTION_11_14_LIVE_ARMED is False
    assert EXPECTED_ORIGIN_MAIN_SHA == "d3e0e6b35b1893d005fa015fb4c7f50de5a877ec"
    assert current_productive_first_real_blocker_v1() == "EXTERNAL_EFFECT_NOT_AUTHORIZED"
    assert gap_node_v1("LIVE_AUTHORIZED").implementation_status == (
        "STANDING_TRUE_NOT_AUTOMATIC_SEND"
    )
    assert gap_node_v1("LiveExecutionPort").implementation_status == (
        "SEND_CAPABLE_NOT_EXTERNAL_EFFECT"
    )
    assert gap_node_v1("EXTERNAL_EFFECT").implementation_status == "IMPLEMENTED_FAIL_CLOSED"


def test_default_and_missing_predicates_deny() -> None:
    denied = evaluate_live_authorized_v1(standing_live_authorized=False)
    assert denied.live_authorized is False
    assert denied.fail_closed is True
    assert "LIVE_AUTHORIZED_STANDING_FALSE" in denied.reason_codes
    missing = evaluate_live_authorized_v1(submission_authorized=False)
    assert missing.live_authorized is False
    assert "SUBMISSION_AUTHORIZED_FALSE" in missing.reason_codes


def test_malformed_and_post_attempt_deny() -> None:
    malformed = evaluate_live_authorized_v1(
        standing_live_authorized="UNKNOWN",  # type: ignore[arg-type]
    )
    assert malformed.live_authorized is False
    assert "LIVE_AUTHORIZED_STANDING_MALFORMED" in malformed.reason_codes
    post = evaluate_live_authorized_v1(attempt_post=True)
    assert post.live_authorized is False
    assert "POST_FROM_LIVE_AUTHORIZED_FORBIDDEN" in post.reason_codes
    effect = evaluate_external_effect_v1(attempt_post=True)
    assert effect.external_effect_authorized is False
    assert "POST_FROM_EXTERNAL_EFFECT_GATE_FORBIDDEN" in effect.reason_codes


def test_send_capable_construction_does_not_post() -> None:
    with pytest.raises(ExecutionPortConstructionForbiddenError):
        construct_live_execution_port_v1()
    handle = FullCoreSendCredentialHandleV1(handle_id="full-core-gated-handle", bound=True)
    transport = FullCoreGatedProductiveWireTransportV1(handle=handle)
    with pytest.raises(FullCoreExternalEffectNotAuthorizedError):
        transport.attempt_trade_order_post(payload={"instId": "MUST_NOT_POST"})
    assert transport.post_count == 0
    assert transport.wire_send_occurred is False
    with pytest.raises(ValueError, match="CREDENTIAL_MATERIAL_LOADED_FORBIDDEN"):
        FullCoreSendCredentialHandleV1(
            handle_id="full-core-gated-handle",
            bound=True,
            material_loaded=True,
        )


def test_owner_go_and_sha_fail_closed(tmp_path: Path) -> None:
    with pytest.raises(CurrentProductiveLiveAuthorizedSendCapableError, match="OWNER_GO_MISMATCH"):
        execute_current_productive_live_authorized_and_cap_11_1_send_capable_adapter_v1(
            owner_go="WRONG",
            origin_main_sha=EXPECTED_ORIGIN_MAIN_SHA,
            evidence_root=tmp_path / "a",
        )
    with pytest.raises(
        CurrentProductiveLiveAuthorizedSendCapableError,
        match="ORIGIN_MAIN_SHA_MISMATCH",
    ):
        execute_current_productive_live_authorized_and_cap_11_1_send_capable_adapter_v1(
            owner_go=OWNER_GO,
            origin_main_sha="0" * 40,
            evidence_root=tmp_path / "b",
        )


def test_evaluate_closes_live_authorized_without_external_effect(tmp_path: Path) -> None:
    result = execute_current_productive_live_authorized_and_cap_11_1_send_capable_adapter_v1(
        owner_go=OWNER_GO,
        origin_main_sha=EXPECTED_ORIGIN_MAIN_SHA,
        evidence_root=tmp_path / "store",
    )
    claims = json.loads((Path(result.store_root) / "claims.json").read_text(encoding="utf-8"))
    assert result.live_enabled == "true"
    assert result.live_armed == "true"
    assert result.wire_send_permitted == "true"
    assert result.live_authorized == "true"
    assert result.admitted == "true"
    assert result.host_joined == "true"
    assert result.port_constructed == "true"
    assert result.send_capable == "true"
    assert result.submission_authorized == "true"
    assert result.productive_wire_send_reachable == "true"
    assert result.external_effect_authorized == "false"
    assert result.step_29q_status == "PLAN_ONLY"
    assert result.step_29p_risk_admissible == "true"
    assert result.cap24_bound_instrument_id
    assert result.first_real_blocker == "EXTERNAL_EFFECT_NOT_AUTHORIZED"
    assert result.blocker_class == "E"
    assert result.post_count == "0"
    assert result.manifest_verify_rc == 0
    assert claims["LIVE_AUTHORIZED"] == "true"
    assert claims["CAP_11_1_SEND_CAPABLE_ADAPTER_CONSTRUCTED"] == "true"
    assert claims["PRODUCTIVE_WIRE_SEND_REACHABLE"] == "true"
    assert claims["EXTERNAL_EFFECT_AUTHORIZED"] == "false"
    assert claims["STEP_29Q_STATUS"] == "PLAN_ONLY"
    assert claims["POST_COUNT"] == "0"
    assert claims["ACTUAL_ORDER_SUBMIT_PERFORMED"] == "false"
    assert claims["VENUE_MUTATION_PERFORMED"] == "false"
    assert claims["PROTECTED_SURFACES_CHANGED"] == "false"
    assert claims["SAFETY_AUTHORITY_WEAKENED"] == "false"
    assert claims["CANARY_FULL_CORE_BOUNDARY_CHANGED"] == "false"
    with pytest.raises(ExecutionPortConstructionForbiddenError):
        construct_live_execution_port_v1()
    assert verify_manifest_sha256_v1(store_root=Path(result.store_root)) == 0


def test_protected_algorithm_files_unchanged_vs_origin_main() -> None:
    import subprocess

    diff = subprocess.run(
        ["git", "diff", "--name-only", "origin/main", "--", *PROTECTED_ALGORITHM_FILES],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
        text=True,
    )
    assert diff.stdout.strip() == ""


def test_ssot_docs_once_present() -> None:
    runbook = RUNBOOK.read_text(encoding="utf-8")
    mot = MOT_PATH.read_text(encoding="utf-8")
    spec = SPEC_PATH.read_text(encoding="utf-8")
    atlas = ATLAS_PATH.read_text(encoding="utf-8")
    assert DH_HEADING in runbook
    assert THIS_SLICE in runbook
    dh_section = runbook[
        runbook.index(
            "11.2.1.DH FULL_CORE_CURRENT_PRODUCTIVE_LIVE_AUTHORIZED_AND_CAP_11_1_SEND_CAPABLE_ADAPTER"
        ) : runbook.index("## 11.3 Autonomy state model")
    ]
    assert "LIVE_AUTHORIZED=true" in dh_section
    assert "CAP_11_1_SEND_CAPABLE_ADAPTER_CONSTRUCTED=true" in dh_section
    assert "PRODUCTIVE_WIRE_SEND_REACHABLE=true" in dh_section
    assert "EXTERNAL_EFFECT_AUTHORIZED=false" in dh_section
    assert "STEP_29Q_STATUS=PLAN_ONLY" in dh_section
    assert "POST_COUNT=0" in dh_section
    assert "FIRST_DEFINITIVE_BLOCK=EXTERNAL_EFFECT_NOT_AUTHORIZED" in dh_section
    dg_section = runbook[
        runbook.index(
            "11.2.1.DG FULL_CORE_CURRENT_PRODUCTIVE_SUBMISSION_AUTHORIZED"
        ) : runbook.index(
            "11.2.1.DH FULL_CORE_CURRENT_PRODUCTIVE_LIVE_AUTHORIZED_AND_CAP_11_1_SEND_CAPABLE_ADAPTER"
        )
    ]
    assert "SUBMISSION_AUTHORIZED=true" in dg_section
    assert "LIVE_AUTHORIZED=false" in dg_section
    assert SPEC_PATH.name in mot
    assert (
        "DOCS_TOKEN_FULL_CORE_CURRENT_PRODUCTIVE_LIVE_AUTHORIZED_AND_CAP_11_1_SEND_CAPABLE_ADAPTER_V1"
        in spec
    )
    assert "11.2.1.DH" in atlas
    assert "current_productive_live_authorized_and_cap_11_1_send_capable_adapter_v1.py" in atlas
    pack = REPO_ROOT / CANONICAL_PACK_RELPATH
    claims = json.loads((pack / "claims.json").read_text(encoding="utf-8"))
    assert claims["LIVE_ENABLED"] == "true"
    assert claims["LIVE_ARMED"] == "true"
    assert claims["WIRE_SEND_PERMITTED"] == "true"
    assert claims["LIVE_AUTHORIZED"] == "true"
    assert claims["ADMITTED"] == "true"
    assert claims["HOST_JOINED"] == "true"
    assert claims["SUBMISSION_AUTHORIZED"] == "true"
    assert claims["CAP_11_1_SEND_CAPABLE_ADAPTER_CONSTRUCTED"] == "true"
    assert claims["PRODUCTIVE_WIRE_SEND_REACHABLE"] == "true"
    assert claims["EXTERNAL_EFFECT_AUTHORIZED"] == "false"
    assert claims["FIRST_REAL_BLOCKER"] == "EXTERNAL_EFFECT_NOT_AUTHORIZED"
    assert claims["POST_COUNT"] == "0"
    assert claims["STEP_29P_RISK_ADMISSIBLE"] == "true"
    assert claims["STEP_29Q_STATUS"] == "PLAN_ONLY"
    assert verify_manifest_sha256_v1(store_root=pack) == 0
