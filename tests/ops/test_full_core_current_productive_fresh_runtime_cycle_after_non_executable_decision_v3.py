"""CURRENT_PRODUCTIVE fresh runtime cycle after non-executable DP decision V3.
...
from src.ops.governed_productive_account_equity_authority_producer_v1.current_productive_fresh_runtime_cycle_after_non_executable_decision_v3 import (
    ...
    THIS_SLICE,
    CurrentProductiveFreshRuntimeCycleAfterNonExecutableV3Error,
    execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v3,
)
...
SPEC_PATH = (
    ...
    "AFTER_NON_EXECUTABLE_DECISION_V3.md"
)
...
def _run(tmp_path: Path, **overrides):
    ...
    return execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v3(
        **payload
    )


def test_standing_flags_keep_real_post_fail_closed() -> None:
    assert CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_V3_CREATED is True
    ...
    assert EXPECTED_ORIGIN_MAIN_SHA == "04a3f1e9b36f9d2f4c5fd9b544e5c523140a32b8"
...
def test_owner_go_and_sha_fail_closed(tmp_path: Path) -> None:
    with pytest.raises(
        CurrentProductiveFreshRuntimeCycleAfterNonExecutableV3Error, match="OWNER_GO_MISMATCH"
    ):
...
def test_ssot_docs_once_present() -> None:
    ...
    assert DQ_HEADING in runbook
    assert THIS_SLICE in runbook
    dp_section = runbook[runbook.index(DQ_HEADING) : runbook.index("## 11.3 Autonomy state model")]
    ...
    assert (
        ...
        "NON_EXECUTABLE_DECISION_V3"
    ) in spec
    assert "11.2.1.DQ" in atlas
    assert "current_productive_fresh_runtime_cycle_after_non_executable_decision_v3.py" in atlas
    ...
    assert claims["VENUE_MUTATION_PERFORMED"] == "false"
    assert claims["PRE_EXTERNAL_EFFECT_BOUNDARY_REACHED"] == "true"
    assert claims["MASTER_V2_RUNTIME_CYCLE_ID"] == "dq-0G-USDT-SWAP-2026-09-15T23:47:40Z"
    ...
    assert lineage["DP_PACK"].endswith("non_executable_decision_v2/20260916T011500Z")
