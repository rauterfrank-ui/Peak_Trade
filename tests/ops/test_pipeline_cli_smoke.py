...
def test_pipeline_cli_risk_smoke(tmp_path):
    cmd = [
        sys.executable, str(ROOT / "scripts" / "pipeline_cli.py"),
        "--run-id", "smoke_pipeline",
        "--artifacts-dir", str(tmp_path),
        "--sandbox",
        "risk", "var",
        "--returns-file", str(ROOT / "tests" / "fixtures" / "returns_normal_5k.txt"),
        "--alpha", "0.99",
        "--method", "historical",
    ]
    p = subprocess.run(cmd, cwd=str(ROOT), capture_output=True, text=True, env={**__import__("os").environ, "PEAKTRADE_SANDBOX": "1"})
