...
from src.ops.governed_productive_account_equity_authority_producer_v1.current_productive_fresh_runtime_cycle_after_non_executable_decision_v1 import (
    ...
    THIS_SLICE,
    CurrentProductiveFreshRuntimeCycleAfterNonExecutableError,
    execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v1,
)
...
SPEC_PATH = (
    ...
    "AFTER_NON_EXECUTABLE_DECISION_V1.md"
)
...
def _run(tmp_path: Path, **overrides):
    ...
    return execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v1(
        **payload
    )


def test_standing_flags_keep_real_post_fail_closed() -> None:
    assert CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_CREATED is True
    ...
    assert EXPECTED_ORIGIN_MAIN_SHA == "aadd3c9acb8e34fab98bfda245525ef3926d7412"
...
def test_owner_go_and_sha_fail_closed(tmp_path: Path) -> None:
    with pytest.raises(
        CurrentProductiveFreshRuntimeCycleAfterNonExecutableError, match="OWNER_GO_MISMATCH"
    ):
...
def test_ssot_docs_once_present() -> None:
    ...
    assert DO_HEADING in runbook
    assert THIS_SLICE in runbook
    do_section = runbook[runbook.index(DO_HEADING) : runbook.index("## 11.3 Autonomy state model")]
    assert "ONE_SHOT_REAL_POST_SEAM_IMPLEMENTED=true" in do_section
    assert "EXTERNAL_EFFECT_AUTHORIZED=false" in do_section
    assert "REAL_EXTERNAL_EFFECT_AUTHORIZED=false" in do_section
    assert "STEP_29Q_STATUS=PLAN_ONLY" in do_section
    assert "POST_COUNT=0" in do_section
    assert "PERMIT_CREATED=false" in do_section
    assert "EXTERNAL_EFFECT_PERMIT_CREATED=false" in do_section
    assert SPEC_PATH.name in mot
    assert (
        "DOCS_TOKEN_FULL_CORE_CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_"
        "NON_EXECUTABLE_DECISION_V1"
    ) in spec
    assert "11.2.1.DO" in atlas
    assert "current_productive_fresh_runtime_cycle_after_non_executable_decision_v1.py" in atlas
