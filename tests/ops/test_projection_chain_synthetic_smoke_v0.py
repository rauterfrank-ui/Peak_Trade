...
Chains: closeout + registry fixtures -> payload builder -> Notion dry-run report.
...
FORBIDDEN_LEAK_SUBSTRINGS = ("/Users/", "AKIA", "sk-", "Bearer ")
...
def test_happy_path_chain_smoke(builder, dry_run, tmp_path: Path):
    ...
    _assert_no_leaks(json.dumps(payload))
...
def test_manifest_verify_failed_blocks_chain(builder, dry_run, tmp_path: Path):
...
def test_notion_projection_not_allowed_blocks_notion_only(builder, dry_run, tmp_path: Path):
...
def test_market_dashboard_projection_allowed_can_be_false_without_ui_consumer(
    builder,
    tmp_path: Path,
):
    ...
    assert payload["consumers"]["market_dashboard_projection_allowed"] is False
    assert payload["consumers"]["notion_projection_allowed"] is True


def test_authority_violation_blocks_chain(builder, dry_run, tmp_path: Path):
