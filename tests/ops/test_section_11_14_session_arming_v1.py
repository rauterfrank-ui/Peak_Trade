"""Offline bound session-arming tests. No live GET. No live POST.

No send_permitted promotion. No durable consume. No inner.send. No urllib.
"""
...
ARMING_SRC = PACKAGE / "session_arming_v1.py"
...
def _session_artifact() -> dict:
    return issue_owner_network_session_authority_v1(
        explicit=current_section_11_14_network_session_explicit_v1(issued_at="2026-09-08T06:40:00Z")
    )["artifact"]
...
def _wire_send_artifact() -> dict:
    payload: dict = {
        ...
        "issued": True,
        "issued_at": "2026-09-08T06:40:00Z",
        "section": "11.14",
...
def test_valid_prerequisites_arm_bound_adapter(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result["ARMING_BIND_VALIDATION"] == "PASS"
    assert result["SESSION_ARMED_BEFORE"] is False
    assert result["SESSION_ARMED"] is True
    assert result["SESSION_ARMING_EXECUTED"] is True
    ...
    assert result["SESSION_ARMING_STANDING"] is False
...
def test_missing_network_session_predecessor_does_not_arm(
...
def test_unauthorized_inner_does_not_arm(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(urllib.request, "urlopen", _boom)
    monkeypatch.setattr(socket, "create_connection", _boom)
    session = _session_artifact()
    wire = _wire_send_artifact()
...
def test_binding_mismatch_does_not_arm(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(urllib.request, "urlopen", _boom)
    monkeypatch.setattr(socket, "create_connection", _boom)
    session = _session_artifact()
    wire = _wire_send_artifact()
    ...
    assert sha["armed"] is False
    assert "NETWORK_SESSION_SHA_MISMATCH" in sha["reasons"]
    assert bind.adapter.session_armed is False
    envelope = arm_productive_session_v1(
        bind=bind,
        network_session=session,
        wire_send=wire,
        origin_main_sha=BOUND_ORIGIN_MAIN_SHA,
    ...
    assert envelope["armed"] is False
    assert "NETWORK_SESSION_ENVELOPE_MISMATCH" in envelope["reasons"]
    instrument = arm_productive_session_v1(
        bind=bind,
        network_session=session,
        wire_send=wire,
        origin_main_sha=BOUND_ORIGIN_MAIN_SHA,
    ...
    assert instrument["armed"] is False
    assert "NETWORK_SESSION_INSTRUMENT_MISMATCH" in instrument["reasons"]
    missing = arm_productive_session_v1(
        bind=None,
        network_session=session,
        wire_send=wire,
        origin_main_sha=BOUND_ORIGIN_MAIN_SHA,
    ...
    assert missing["armed"] is False
    assert "PRODUCTIVE_TRANSPORT_BIND_SEND_CAPABLE_MISSING" in missing["reasons"]
    assert bind.adapter.session_armed is False


def test_invalid_unaccepted_consumed_wire_send_does_not_arm(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(urllib.request, "urlopen", _boom)
    monkeypatch.setattr(socket, "create_connection", _boom)
    ...
    invalid = arm_productive_session_v1(
        bind=bind,
        network_session=session,
        wire_send=bad_id,
        origin_main_sha=BOUND_ORIGIN_MAIN_SHA,
    ...
    assert invalid["armed"] is False
    assert "WIRE_SEND_AUTHORITY_ID_MISMATCH" in invalid["reasons"]
    consumed = dict(wire)
    consumed["consumed"] = True
    reused = arm_productive_session_v1(
        bind=bind,
        network_session=session,
        wire_send=consumed,
        origin_main_sha=BOUND_ORIGIN_MAIN_SHA,
    ...
    assert reused["armed"] is False
    assert "WIRE_SEND_CONSUMED_CANNOT_BE_REUSED" in reused["reasons"]
    assert bind.adapter.session_armed is False
    assert wire["consumed"] is False


def test_orchestrator_after_arming_stops_at_send_permitted(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(urllib.request, "urlopen", _boom)
    monkeypatch.setattr(socket, "create_connection", _boom)
    ...
    assert "SESSION_NOT_ARMED" not in orch["reasons"]
...
def test_arming_module_has_no_urllib_or_issuer() -> None:
    text = ARMING_SRC.read_text(encoding="utf-8")
