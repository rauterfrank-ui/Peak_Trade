"""Registry contract for STEP 29M ehlers_cycle_filter v1 economic evaluation config."""
...
EHLERS_CONFIG = REPO_ROOT / contract.DEFAULT_EVALUATION_CONFIG_PATH


def _load_config() -> dict:
    return json.loads(EHLERS_CONFIG.read_text(encoding="utf-8"))


def test_ehlers_config_registered_in_canonical_registry() -> None:
    ...
    assert EHLERS_CONFIG.is_file()


def test_ehlers_config_digest_stable() -> None:
    cfg = _load_config()
    digest = compute_evaluation_config_digest_v1(cfg)
    assert len(digest) == 64
    assert digest == compute_evaluation_config_digest_v1(
        json.loads(EHLERS_CONFIG.read_text(encoding="utf-8"))
    )


def test_ehlers_sizing_config_digest_matches_sizing_contract() -> None:
    cfg = _load_config()
...
def test_ehlers_evaluation_config_digest_rejected_in_sizing_binding() -> None:
    cfg = _load_config()
    bad = deepcopy(cfg)
