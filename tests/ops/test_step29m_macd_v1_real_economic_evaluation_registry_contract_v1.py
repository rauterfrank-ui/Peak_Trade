"""Registry contract for completed STEP 29M macd v1 real economic evaluation v1."""
...
def test_macd_v1_real_evaluation_registry_truth_global_and_section() -> None:
    text = _read_registry()
    ...
    for scope in (text, section):
        ...
        assert _field_value(scope, "REAL_EVALUATION_INPUT_STATUS") == "MACD_V1_EVALUATION_COMPLETE"
...
def test_macd_v1_real_evaluation_evidence_manifest_and_provenance() -> None:
    ...
    assert summary["STRATEGY_PARAMS_DIGEST"] == (
        "4742e4dbed66dd6a8078d09803234a468261ed444f16c533b8cd70f78f869c00"
    )
    assert summary["MANIFEST_VERIFY_RC"] == 0
    run_output = EVIDENCE_DIR / "run_output" / "economic_viability_evidence_v1.json"
    evidence = json.loads(run_output.read_text(encoding="utf-8"))
    binding = evidence["strategy_signal_binding"]
    assert binding["configured_strategy_id"] == binding["executed_strategy_id"] == "macd"
    assert binding["strategy_signal_digest"] == binding["engine_signal_digest"]
    assert binding["engine_signal_source"] == "configured_strategy_signal"
