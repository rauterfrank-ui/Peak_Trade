"""Static + offline bounded Futures Testnet pilot-envelope lifecycle integration (v0).

Docs/tests-only. No runtime, network, credentials, or Testnet start.
PE-24 static PE-12 readiness_decision pilot-envelope proof composition only.
"""
...
TEST_PACKAGE_MARKER = "BOUNDED_FUTURES_TESTNET_PILOT_ENVELOPE_LIFECYCLE_INTEGRATION_GUARD_V0=true"
_CLASS4_SCOPED_EXCEPTION_MARKER = "BOUNDED_FUTURES_TESTNET_PILOT_ENVELOPE_LIFECYCLE_INTEGRATION_GUARD_CLASS4_SCOPED_EXCEPTION_V0=true"
...
def test_package_marker_present() -> None:
    ...
    assert "BOUNDED_FUTURES_TESTNET_PILOT_ENVELOPE_LIFECYCLE_INTEGRATION_CONTRACT_V0=true" in (
        INTEGRATION_MODULE.read_text(encoding="utf-8")
    )


def test_pe12_pe19_pe20_pe22_pe23_owners_referenced_not_duplicated() -> None:
...
def test_global_safety_flags_remain_blocked() -> None:
    ...
    assert GLOBAL_PILOT_READINESS is False
...
def test_deterministic_identical_inputs_same_payload_and_digest() -> None:
    ...
    left_result = evaluate_pilot_envelope_lifecycle_integration(left)
    right_result = evaluate_pilot_envelope_lifecycle_integration(right)
    assert left_result["integration_proof_digest"] == right_result["integration_proof_digest"]


def test_valid_pe12_pe19_pe20_pe22_pe23_composition_passes() -> None:
    integration_input = default_minimal_integration_input(source_revision=VALID_COMMIT_SHA)
    result = evaluate_pilot_envelope_lifecycle_integration(integration_input)
    assert result["integration_pass"] is True
    assert result["pilot_envelope_static_ready"] is True
    assert result["pe12_pilot_envelope_static_integration_proven"] is True
...
def test_missing_pe12_readiness_decision_proof_fails() -> None:
    ...
    result = evaluate_pilot_envelope_lifecycle_integration(bad)
...
def test_source_revision_mismatch_fails() -> None:
    integration_input = default_minimal_integration_input(source_revision=VALID_COMMIT_SHA)
    result = evaluate_pilot_envelope_lifecycle_integration(
        integration_input,
...
def test_pilot_envelope_static_ready_true_without_chain_fails() -> None:
    integration_input = default_minimal_integration_input()
    result = evaluate_pilot_envelope_lifecycle_integration(
        integration_input,
        pilot_envelope_static_ready_without_proof_chain=True,
    )
    assert result["integration_pass"] is False
...
def test_positive_network_flag_fails() -> None:
    integration_input = default_minimal_integration_input()
    result = evaluate_pilot_envelope_lifecycle_integration(integration_input, network_allowed=True)
    assert result["integration_pass"] is False
...
def test_positive_orders_flag_fails() -> None:
    integration_input = default_minimal_integration_input()
    result = evaluate_pilot_envelope_lifecycle_integration(integration_input, orders_allowed=True)
    assert result["integration_pass"] is False
...
def test_safety_snapshot_drift_fails() -> None:
    ...
    bad_snapshot = replace(snapshot, network_allowed=True)
    integration_input = default_minimal_integration_input()
    bad = replace(integration_input, safety_snapshot=bad_snapshot)
    result = evaluate_pilot_envelope_lifecycle_integration(bad)
    assert result["integration_pass"] is False
    assert any(
        "safety_snapshot: network_allowed must be False" in r for r in result["fail_reasons"]
    )


def test_valid_static_proof_remains_non_authorizing() -> None:
    result = evaluate_pilot_envelope_lifecycle_integration(default_minimal_integration_input())
    assert result["integration_pass"] is True
...
def test_contract_version_constants() -> None:
    assert CONTRACT_VERSION == "bounded_futures_testnet_pilot_envelope_lifecycle_integration.v0"
