...
def test_real_productive_transport_send_is_never_invoked(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(urllib.request, "urlopen", _boom)
    monkeypatch.setattr(socket, "create_connection", _boom)
...
def test_adapter_refuses_real_class_before_send_call() -> None:
    text = ADAPTER_SRC.read_text(encoding="utf-8")
    assert "isinstance(self.inner, AuthenticatedGatedProductiveFlattenTransportV1)" in text
    assert "PRODUCTIVE_INNER_SEND_EXECUTION_NOT_AUTHORIZED" in text
