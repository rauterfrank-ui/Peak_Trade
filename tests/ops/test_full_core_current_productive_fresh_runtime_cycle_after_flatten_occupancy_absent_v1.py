...
from src.ops.governed_productive_account_equity_authority_producer_v1.current_productive_fresh_runtime_cycle_after_flatten_occupancy_absent_v1 import (
    ...
    THIS_SLICE,
    CurrentProductiveFreshRuntimeCycleAfterFlattenError,
    execute_current_productive_fresh_runtime_cycle_after_flatten_occupancy_absent_v1,
)
...
SPEC_PATH = (
    ...
    "AFTER_FLATTEN_OCCUPANCY_ABSENT_V1.md"
)
...
def _run(tmp_path: Path, **overrides):
    ...
    return execute_current_productive_fresh_runtime_cycle_after_flatten_occupancy_absent_v1(
        **payload
    )


def test_standing_flags_keep_real_post_fail_closed() -> None:
    assert CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_FLATTEN_OCCUPANCY_ABSENT_CREATED is True
    ...
    assert EXPECTED_ORIGIN_MAIN_SHA == "5b58f5432fed3ade2b51fff52a52028d5bde5178"
...
def test_owner_go_and_sha_fail_closed(tmp_path: Path) -> None:
    with pytest.raises(
        CurrentProductiveFreshRuntimeCycleAfterFlattenError, match="OWNER_GO_MISMATCH"
    ):
...
def test_ssot_docs_once_present() -> None:
    ...
    assert DN_HEADING in runbook
    assert THIS_SLICE in runbook
    dn_section = runbook[runbook.index(DN_HEADING) : runbook.index("## 11.3 Autonomy state model")]
    assert "ONE_SHOT_REAL_POST_SEAM_IMPLEMENTED=true" in dn_section
    assert "EXTERNAL_EFFECT_AUTHORIZED=false" in dn_section
    assert "REAL_EXTERNAL_EFFECT_AUTHORIZED=false" in dn_section
    assert "STEP_29Q_STATUS=PLAN_ONLY" in dn_section
    assert "POST_COUNT=0" in dn_section
    assert "PERMIT_CREATED=false" in dn_section
    assert "EXTERNAL_EFFECT_PERMIT_CREATED=false" in dn_section
    assert SPEC_PATH.name in mot
    assert (
        "DOCS_TOKEN_FULL_CORE_CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_"
        "FLATTEN_OCCUPANCY_ABSENT_V1"
    ) in spec
    assert "11.2.1.DN" in atlas
    assert "current_productive_fresh_runtime_cycle_after_flatten_occupancy_absent_v1.py" in atlas
