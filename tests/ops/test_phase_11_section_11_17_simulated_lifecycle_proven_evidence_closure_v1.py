"""Tests for Phase 11 §11.17 SIMULATED_LIFECYCLE_PROVEN evidence closure."""
...
from src.ops.phase_11_section_11_17_simulated_lifecycle_proven_evidence_closure_v1.binding_v1 import (
    SimulatedLifecycleProvenBindingError,
...
def test_cap71_binding_establishes_simulated_lifecycle_proven() -> None:
    binding = bind_simulated_lifecycle_proven_from_cap71_v1()
    assert binding["ok"] is True
    assert binding["SECTION_11_17_FIELD"] == SECTION_11_17_FIELD
    assert binding["SIMULATED_LIFECYCLE_PROVEN"] is True
...
def test_binding_rejects_silent_ready_or_later_field_overclaim() -> None:
    binding = bind_simulated_lifecycle_proven_from_cap71_v1()
    assert binding["TESTNET_LIFECYCLE_PROVEN"] is False
    ...
    assert binding["FULLY_AUTONOMOUS_LIVE_TRADING_READY"] is False
    assert binding["FULLY_AUTONOMOUS_LIVE_TRADING_ACTIVE"] is False
    with pytest.raises(SimulatedLifecycleProvenBindingError):
        raise SimulatedLifecycleProvenBindingError("PROBE")


def test_verifier_pass_preserves_later_section_11_17_residuals() -> None:
    result = verify_phase_11_section_11_17_simulated_lifecycle_proven_v1()
    ...
    assert claims["SIMULATED_LIFECYCLE_PROVEN"] is True
...
def test_cap_11_12_still_has_simulated_lifecycle_unconsumed_until_separate_update() -> None:
    from src.ops.capability_11_12_fully_autonomous_live_readiness_ratification_v1 import (
        constants_v1 as cap1112,
    )

    # Cap 11.12 consumer update is out of scope for this binding package.
