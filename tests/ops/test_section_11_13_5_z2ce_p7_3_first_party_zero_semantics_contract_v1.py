"""§11.13.5.Z2CE P7.3 first-party zero-semantics contract.

Locks that proven venue semantics do not change consumer behavior,
do not authorize flatten/live, and do not promote empty data to zero.
Offline only. No venue access.
"""
...
def _block_network(monkeypatch: pytest.MonkeyPatch) -> None:
    def _blocked(*_a: Any, **_k: Any) -> Any:
        raise AssertionError("NETWORK_FORBIDDEN_IN_Z2CE_CONTRACT_TESTS")
...
def test_explicit_zero_row_is_already_evaluable_without_consumer_change() -> None:
    try:
        observe_target_position_flatten_candidate_v1(
            positions_payload={
                "code": "0",
...
def test_z2ce_persist_go_is_not_flatten_execute_go() -> None:
    accepted, reasons = evaluate_flatten_execute_authority_v1(
        ...
        owner_go=Z2CE_PERSIST_OWNER_GO,
    )
    assert accepted is False
    assert "FLATTEN_EXECUTE_OWNER_GO_MISMATCH" in reasons
    assert Z2CE_PERSIST_OWNER_GO != FLATTEN_EXECUTE_OWNER_GO_CANONICAL
