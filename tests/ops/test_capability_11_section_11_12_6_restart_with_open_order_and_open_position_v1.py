"""Tests for Cap 11 §11.12.6 restart with open order and open position."""
...
from src.ops.capability_11_section_11_12_6_restart_with_open_order_and_open_position_v1.section_11_12_6_v1 import (
    Section11126RestartWithOpenOrderAndOpenPositionError,
    ...
    refuse_section_11_12_7_v1,
    refuse_silent_reinitialization_v1,
    reuse_cap_11_5_section_11_12_6_restart_path_v1,
)
from src.ops.capability_11_section_11_12_6_restart_with_open_order_and_open_position_v1.verifier_v1 import (
...
def _complete_kwargs(**overrides):
    bound, pred_digest = mark_section_11_12_5_predecessor_bound_v1(
    ...
    base = {
        ...
        "expected_venue": "OKX",
        "section_11_12_5_predecessor_bound": bound,
        "section_11_12_5_execution_binding_digest": pred_digest,
        "client_order_id_prefix": "pt-coid-section-11-12-6-test",
    }
    base.update(overrides)
    return base


def test_productive_restart_binds_predecessor() -> None:
    record = execute_section_11_12_6_restart_with_open_order_and_open_position_v1(
        **_complete_kwargs()
    )
    assert record.restart_with_open_order_and_open_position_performed is True
    assert record.cap_11_5_restart_with_open_order_position_contract_reused is True
    ...
    assert record.paths_completed == ALLOWED_SECTION_11_12_6_PATHS
    ...
    assert all(r.pre_restart_state == "OPEN" for r in record.path_results)
    assert all(r.post_restart_state == "OPEN" for r in record.path_results)
    assert all(r.reconciliation_before_alpha is True for r in record.path_results)
    assert all("OPEN" in r.history for r in record.path_results)
    ...
    assert record.section_11_12_7_started is False
    ...
    assert bool(record.section_11_12_5_execution_binding_digest)
...
def test_incomplete_preconditions_fail_closed() -> None:
    with pytest.raises(
        Section11126RestartWithOpenOrderAndOpenPositionError,
        match="SECTION_11_12_6_NOT_ADMISSIBLE",
    ):
        execute_section_11_12_6_restart_with_open_order_and_open_position_v1(
            **_complete_kwargs(section_11_12_5_predecessor_bound=False)
        )
...
def test_cap_11_5_reuse_negatives_and_path_refusal() -> None:
    with pytest.raises(
        Section11126RestartWithOpenOrderAndOpenPositionError,
        match="SECTION_11_12_6_PATH_FORBIDDEN",
    ):
    ...
    for path_name in ALLOWED_SECTION_11_12_6_PATHS:
        ...
        assert life.pre_restart_state == "OPEN"
        assert life.post_restart_state == "OPEN"
        assert life.reconciliation_before_alpha is True
        assert "OPEN" in life.history


def test_downstream_and_activation_refusals() -> None:
    with pytest.raises(
        Section11126RestartWithOpenOrderAndOpenPositionError, match="ORDER_SEND_FORBIDDEN"
    ):
        refuse_order_send_v1()
    with pytest.raises(
        Section11126RestartWithOpenOrderAndOpenPositionError, match="NETWORK_WRITE_FORBIDDEN"
    ):
        refuse_network_write_v1(method="POST")
    with pytest.raises(
        Section11126RestartWithOpenOrderAndOpenPositionError, match="NETWORK_SUBMIT_FORBIDDEN"
    ):
        refuse_network_submit_v1()
    with pytest.raises(
        Section11126RestartWithOpenOrderAndOpenPositionError,
        match="SILENT_REINITIALIZATION_FORBIDDEN",
    ):
        refuse_silent_reinitialization_v1(claimed_action="reset_to_zero")
    with pytest.raises(
        Section11126RestartWithOpenOrderAndOpenPositionError,
        match="RESTART_NETWORK_SESSION_ACTIVATION_FORBIDDEN",
    ):
        refuse_restart_network_session_activation_v1(session_id="session-restart")
    with pytest.raises(
        Section11126RestartWithOpenOrderAndOpenPositionError, match="SECTION_11_12_7"
    ):
    ...
    assert SECTION_11_12_7_STARTED is False
...
def test_prove_and_verifier_pass() -> None:
    ...
    assert proof["restart_with_open_order_and_open_position_performed"] is True
    assert proof["cap_11_5_restart_with_open_order_position_contract_reused"] is True
    assert proof["network_effect"] == "NONE"
    assert proof["exchange_submit_performed"] is False
    assert proof["section_11_12_7_started"] is False
    ...
    assert proof["paths_completed"] == list(ALLOWED_SECTION_11_12_6_PATHS)
    verification = (
        verify_capability_11_section_11_12_6_restart_with_open_order_and_open_position_v1()
    )
    ...
    assert verification["claims"]["RESTART_WITH_OPEN_ORDER_AND_OPEN_POSITION_PERFORMED"] is True
    assert verification["claims"]["SECTION_11_12_7_STARTED"] is False
    ...
    assert verification["claims"]["SILENT_REINITIALIZATION_BLOCKED"] is True
    assert verification["claims"]["RESTART_NETWORK_SESSION_ACTIVATION_BLOCKED"] is True
