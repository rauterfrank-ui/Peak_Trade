"""CURRENT_PRODUCTIVE LIVE_ENABLED standing-gate tests. Offline. No wire. No POST."""
...
from src.ops.governed_productive_account_equity_authority_producer_v1.current_productive_live_enabled_standing_gate_v1 import (
    ...
    OWNER_GO,
    THIS_SLICE,
    CurrentProductiveLiveEnabledStandingGateError,
    execute_current_productive_live_enabled_standing_gate_v1,
)
...
from src.ops.section_11_14_live_order_and_economic_evidence_ladder_v1.constants_v1 import (
    LIVE_ENABLED as SECTION_11_14_LIVE_ENABLED,
)
...
SPEC_PATH = (
    REPO_ROOT / "docs/ops/specs/FULL_CORE_CURRENT_PRODUCTIVE_LIVE_ENABLED_STANDING_GATE_V1.md"
)
ATLAS_PATH = REPO_ROOT / "docs/system_atlas/entities/catalog.yaml"
DA_HEADING = "### 11.2.1.DA FULL_CORE_CURRENT_PRODUCTIVE_LIVE_ENABLED_STANDING_GATE"
...
def test_standing_flags_close_live_enabled_without_activation() -> None:
    assert CURRENT_PRODUCTIVE_LIVE_ENABLED_STANDING_GATE_ADAPTER_CREATED is True
    assert LIVE_ENABLED is True
    assert LIVE_ENABLED_STANDING_GATE_CLOSED is True
...
def test_owner_go_and_sha_fail_closed(tmp_path: Path) -> None:
    with pytest.raises(CurrentProductiveLiveEnabledStandingGateError, match="OWNER_GO_MISMATCH"):
        execute_current_productive_live_enabled_standing_gate_v1(
            owner_go="WRONG",
    ...
    with pytest.raises(
        CurrentProductiveLiveEnabledStandingGateError, match="ORIGIN_MAIN_SHA_MISMATCH"
    ):
        execute_current_productive_live_enabled_standing_gate_v1(
            owner_go=OWNER_GO,
...
def test_evaluate_closes_live_enabled_and_halts_on_live_armed(tmp_path: Path) -> None:
    result = execute_current_productive_live_enabled_standing_gate_v1(
        owner_go=OWNER_GO,
        origin_main_sha=EXPECTED_ORIGIN_MAIN_SHA,
...
def test_ssot_docs_once_present() -> None:
    ...
    assert "current_productive_live_enabled_standing_gate_v1.py" in atlas
    ...
    assert claims["FIRST_REAL_BLOCKER"] == "LIVE_ARMED_STANDING_GATE_REMAINS_FALSE"
    assert claims["POST_COUNT"] == "0"
