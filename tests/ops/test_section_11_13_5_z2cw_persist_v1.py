"""§11.13.5.Z2CW FX offline reproof persist invariants.
...
def _z2cv_section(text: str) -> str:
    ...
    end = text.find(Z2CW_HEADING, start)
    assert end > start, "missing §11.13.5.Z2CW boundary after Z2CV"
    return text[start:end]


def test_z2cw_heading_is_unique_and_follows_z2cv() -> None:
...
def test_z2cv_historical_cover_usdc_slice_was_not_rewritten() -> None:
    ...
    assert "EXACT_Z2AR_CLASS=COVER_USDC" in section
    assert (
        "REMAINING_UNRANKED_AFTER_THIS_CLASS=FX;ROUNDING;FINISHED_RISK_ENVELOPE_NUMERIC;USD_USDC_ACCOUNT_SETTLEMENT"
        in section
    )
    assert "FX_ADJUDICATED_THIS_PERSIST=false" in section
    assert "GET_EXECUTED_THIS_PERSIST=false" in section
    assert "Z2CW" not in section


def test_z2cw_docs_bind_fail_closed_fx_without_get() -> None:
    section = _z2cw_section(_read(MASTER_RUNBOOK))
    required = (
        "AUTHORIZED_SCOPE=A_Z2CW_Z2AR_FX_OFFLINE_REPROOF_DOCS_ONLY",
        f"OWNER_GO={OWNER_GO}",
        ...
        "FORBIDDEN_COLLAPSE_FX_WITH_ACCOUNT_SETTLEMENT=true",
        "FORBIDDEN_COLLAPSE_FX_WITH_COVER_USDC=true",
        ...
        "FINISHED_RISK_ENVELOPE_NUMERIC_ADJUDICATED_THIS_PERSIST=false",
    )
    for token in required:
        assert token in section, token


def test_z2cw_docs_forbid_activation_and_class_collapse() -> None:
    section = _z2cw_section(_read(MASTER_RUNBOOK))
    forbidden = (
...
def test_map_of_truth_remains_navigation_only() -> None:
    ...
    assert "Z2CW" not in text
    assert "11.13.5.Z2CW" not in text
...
def test_python_adjudication_matches_persist() -> None:
    ...
    assert "IDXPX_1_IS_NOT_USD_USDC_OPERATOR" in BLOCKING_EVIDENCE_GAPS
