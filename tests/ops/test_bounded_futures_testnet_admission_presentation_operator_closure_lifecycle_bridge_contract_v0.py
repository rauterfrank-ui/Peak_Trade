"""Static + offline bounded Futures Testnet admission presentation ↔ PE-25 closure bridge (v0).

Docs/tests-only. No runtime, network, credentials, operator review, admission, or Testnet start.
PE-34/35/36/37 admission presentation lifecycle ↔ PE-25 operator closure static bridge only.
"""
...
TEST_PACKAGE_MARKER = (
    "BOUNDED_FUTURES_TESTNET_ADMISSION_PRESENTATION_OPERATOR_CLOSURE_LIFECYCLE_BRIDGE_GUARD_V0=true"
)
_CLASS4_SCOPED_EXCEPTION_MARKER = "BOUNDED_FUTURES_TESTNET_ADMISSION_PRESENTATION_OPERATOR_CLOSURE_LIFECYCLE_BRIDGE_GUARD_CLASS4_SCOPED_EXCEPTION_V0=true"
...
def test_package_marker_present() -> None:
    ...
    assert (
        "BOUNDED_FUTURES_TESTNET_ADMISSION_PRESENTATION_OPERATOR_CLOSURE_LIFECYCLE_BRIDGE_CONTRACT_V0=true"
...
def test_upstream_owners_referenced_not_duplicated() -> None:
    ...
    assert (
        "bounded_futures_testnet_operator_closure_lifecycle_integration_contract_v0" in bridge_text
    )
...
def test_happy_path_coherent_admission_and_pe25_composition_passes() -> None:
    ...
    assert result["admission_presentation_operator_closure_lifecycle_bridge_eligibility"] is True
    assert result["admission_presentation_lifecycle_bound"] is True
    assert result["operator_closure_lifecycle_bound"] is True
...
def test_valid_static_proof_remains_non_authorizing() -> None:
    result = evaluate_admission_presentation_operator_closure_lifecycle_bridge(
        _valid_bridge_input()
    )
    assert result["bridge_pass"] is True
...
def test_deterministic_identical_inputs_same_payload_and_digest() -> None:
    ...
    left_result = evaluate_admission_presentation_operator_closure_lifecycle_bridge(left)
    right_result = evaluate_admission_presentation_operator_closure_lifecycle_bridge(right)
    ...
    assert left_result["bridge_proof_digest"] == compute_bridge_proof_digest(
        left,
        admission_presentation_operator_closure_lifecycle_bridge_eligibility=True,
    )
...
def test_missing_admission_presentation_lifecycle_fails() -> None:
    ...
    result = evaluate_admission_presentation_operator_closure_lifecycle_bridge(broken)
    assert result["bridge_pass"] is False
    assert any("admission_presentation_integration_input" in r for r in result["fail_reasons"])


def test_missing_pe25_closure_lifecycle_fails() -> None:
    bridge_input = _valid_bridge_input()
    broken_pe25 = replace(
        bridge_input.pe25_closure_integration_input,
        closure_id="",
    )
    broken = replace(bridge_input, pe25_closure_integration_input=broken_pe25)
    result = evaluate_admission_presentation_operator_closure_lifecycle_bridge(broken)
    assert result["bridge_pass"] is False
    assert any("pe25_closure_integration_input" in r for r in result["fail_reasons"])
...
def test_missing_pe34_pe35_pe36_pe37_binding_fails() -> None:
    ...
    assert result["bridge_pass"] is False
    assert any(
        "pe35_staleness_revocation_recovery_bound must be true" in r for r in result["fail_reasons"]
    )
...
def test_owner_identity_drift_fails() -> None:
    bridge_input = _valid_bridge_input()
    broken_proof = replace(
        bridge_input.pe25_operator_closure_proof,
        closure_owner="wrong.owner.v0",
    )
    broken = replace(bridge_input, pe25_operator_closure_proof=broken_proof)
    result = evaluate_admission_presentation_operator_closure_lifecycle_bridge(broken)
    assert result["bridge_pass"] is False
    assert any("closure_owner must be" in r for r in result["fail_reasons"])
...
def test_proof_digest_drift_fails() -> None:
    ...
    result = evaluate_admission_presentation_operator_closure_lifecycle_bridge(
        bridge_input,
        expected_admission_integration_proof_digest="0" * 64,
    )
    assert result["bridge_pass"] is False
    assert any("admission_integration_proof_digest mismatch" in r for r in result["fail_reasons"])
...
def test_closure_references_wrong_admission_proof_fails() -> None:
    bridge_input = _valid_bridge_input()
    broken_proof = replace(
        bridge_input.pe25_operator_closure_proof,
...
def test_stale_handoff_fails() -> None:
    bridge_input = _valid_bridge_input()
    broken_pe35 = replace(
        bridge_input.admission_presentation_integration_input.pe35_boundary_input,
        lifecycle_metadata=replace(
            bridge_input.admission_presentation_integration_input.pe35_boundary_input.lifecycle_metadata,
...
def test_cross_lifecycle_source_revision_mismatch_fails() -> None:
    bridge_input = _valid_bridge_input()
    broken_pe25 = default_minimal_pe25_input(
...
def test_closure_proof_chain_admission_reference_mismatch_fails() -> None:
    bridge_input = _valid_bridge_input()
    broken_chain = replace(
        bridge_input.proof_chain,
        closure_referenced_admission_proof_digest="0" * 64,
    )
    broken = replace(bridge_input, proof_chain=broken_chain)
    result = evaluate_admission_presentation_operator_closure_lifecycle_bridge(broken)
    assert result["bridge_pass"] is False
    assert any("closure_referenced_admission_proof_digest" in r for r in result["fail_reasons"])
...
def test_pe25_closure_proof_digest_mismatch_fails() -> None:
    bridge_input = _valid_bridge_input()
    result = evaluate_admission_presentation_operator_closure_lifecycle_bridge(
        bridge_input,
        expected_pe25_closure_proof_digest="0" * 64,
    )
    assert result["bridge_pass"] is False
    assert any("pe25_closure_proof_digest mismatch" in r for r in result["fail_reasons"])
...
def test_canonical_owners_match_expected() -> None:
    ...
    assert PE25_CONTRACT_VERSION == (
        "bounded_futures_testnet_operator_closure_lifecycle_integration.v0"
    )
