...
from src.ops.governed_productive_account_equity_authority_producer_v1.current_productive_fresh_runtime_to_pre_external_effect_applicability_v1 import (
    ...
    THIS_SLICE,
    CurrentProductiveFreshRuntimeToPreExternalEffectError,
    execute_current_productive_fresh_runtime_to_pre_external_effect_applicability_v1,
)
...
SPEC_PATH = (
    REPO_ROOT / "docs/ops/specs/FULL_CORE_CURRENT_PRODUCTIVE_FRESH_RUNTIME_TO_"
    "PRE_EXTERNAL_EFFECT_APPLICABILITY_V1.md"
)
...
def _run(tmp_path: Path, **overrides):
    ...
    return execute_current_productive_fresh_runtime_to_pre_external_effect_applicability_v1(
        **payload
    )


def test_standing_flags_keep_real_post_fail_closed() -> None:
    assert CURRENT_PRODUCTIVE_FRESH_RUNTIME_TO_PRE_EXTERNAL_EFFECT_APPLICABILITY_CREATED is True
...
def test_owner_go_and_sha_fail_closed(tmp_path: Path) -> None:
    with pytest.raises(
        CurrentProductiveFreshRuntimeToPreExternalEffectError, match="OWNER_GO_MISMATCH"
    ):
...
def test_injected_path_runs_cycle_without_fabricating_enter(tmp_path: Path) -> None:
    ...
    assert claims["HOLD_CLASS"] in {
        "A_HOLD_MISSING_CURRENT_TRADING_CONDITION",
        "B_CONFIRMATION_STATE_ADVANCED_AND_PERSISTED",
    }
...
def test_ssot_docs_once_present() -> None:
    ...
    assert DT_HEADING in runbook
    assert THIS_SLICE in runbook
    dt_section = runbook[runbook.index(DT_HEADING) : runbook.index("## 11.3 Autonomy state model")]
    assert "ONE_SHOT_REAL_POST_SEAM_IMPLEMENTED=true" in dt_section
    assert "EXTERNAL_EFFECT_AUTHORIZED=false" in dt_section
    assert "REAL_EXTERNAL_EFFECT_AUTHORIZED=false" in dt_section
    assert "STEP_29Q_STATUS=PLAN_ONLY" in dt_section
    assert "POST_COUNT=0" in dt_section
    assert "PERMIT_CREATED=false" in dt_section
    assert "EXTERNAL_EFFECT_PERMIT_CREATED=false" in dt_section
    assert SPEC_PATH.name in mot
    assert (
        "DOCS_TOKEN_FULL_CORE_CURRENT_PRODUCTIVE_FRESH_RUNTIME_TO_"
        "PRE_EXTERNAL_EFFECT_APPLICABILITY_V1"
    ) in spec
    assert "11.2.1.DT" in atlas
    assert "current_productive_fresh_runtime_to_pre_external_effect_applicability_v1.py" in atlas
