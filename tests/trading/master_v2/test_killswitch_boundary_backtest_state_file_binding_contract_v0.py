...
def test_no_automatic_resume_represented_in_backtest_v0() -> None:
    evidence = evaluate_backtest_state_file_boundary_only_v0(
...
def test_semantic_flags_represented_for_all_boundary_modes_v0() -> None:
    for mode in (
        KillSwitchBoundaryMode.BLOCK_NEW,
        KillSwitchBoundaryMode.CANCEL_PENDING,
        KillSwitchBoundaryMode.REDUCE_TO_FLAT,
    ):
        evidence = evaluate_backtest_state_file_boundary_only_v0(_record(mode=mode.value))
        assert evidence.killswitch_boundary_represented_in_backtest is True
        assert evidence.no_order_without_safety_and_killswitch_pass_represented_in_backtest
