...
def test_default_deny():
    ...
    assert al.is_allowed(adapter="mock", market="BTC-USD") is False


def test_allow_when_both_match():
    al = NetworkedAllowlistV1.from_iterables(adapters=["mock"], markets=["BTC-USD"])
    assert al.is_allowed(adapter="mock", market="BTC-USD") is True


def test_deny_if_only_adapter_matches():
    al = NetworkedAllowlistV1.from_iterables(adapters=["mock"], markets=["ETH-USD"])
    assert al.is_allowed(adapter="mock", market="BTC-USD") is False
...
def test_guard_raises():
    ...
    with pytest.raises(ExecutionEntryGuardError) as exc_info:
        guard_allowlist_v1(allowlist=al, adapter="mock", market="BTC-USD")
