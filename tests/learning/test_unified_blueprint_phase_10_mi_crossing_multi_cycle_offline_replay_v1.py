"""Phase 10 — MI orchestrator → M5→M8 multi-cycle offline replay integration."""
...
def _failure_request() -> CanonicalFailureMemoryRecordRequestV1:
    ...
    return CanonicalFailureMemoryRecordRequestV1(
        ...
        hypothesis_id="hyp.mi.phase10.v1",
...
def test_phase_10_mi_crossing_multi_cycle_m5_through_m8(tmp_path: Path) -> None:
            ...
            cycles=(
                MiCrossingOfflineReplayCycleInputV1(
                    ...
                    orchestrator_input=_orchestrator_input(
                        tmp_path,
                        info_ref="phase10.integration.c0",
                        failure_store=store_root,
                ...
                MiCrossingOfflineReplayCycleInputV1(
                    cycle_index=1,
                    orchestrator_input=_orchestrator_input(
                        tmp_path, info_ref="phase10.integration.c1"
                    ),
                    replay_seed=7,
                ),
            )
    ...
    assert result["multi_cycle_loop_closed"] is True
    ...
    assert cycle0["return_input_ack"]["status"] == "ACCEPTED_OFFLINE_EVIDENCE_INPUT"
    assert cycle0["meta_learning_ingest"]["status"] == INGEST_STATUS_COMPLETE
    assert cycle0["bounded_research_feedback_decision"]["feedback_decision_identity"]
    assert cycle0["failure_memory_replay"]["deterministic_replay_equal"] is True
