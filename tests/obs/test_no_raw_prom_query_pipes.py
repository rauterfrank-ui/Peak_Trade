...
def test_no_raw_curl_python_pipe_for_prom_query_in_obs_scripts() -> None:
    ...
    # Detect direct curl-to-python pipes on /api/v1/query (most brittle case).
    pipe_re = re.compile(r"curl[^\n]*api/v1/query[^\n]*\|[^\n]*python", re.IGNORECASE)
    ...
    assert offenders == [], "raw prom query curl|python pipe found in: " + ", ".join(offenders)
