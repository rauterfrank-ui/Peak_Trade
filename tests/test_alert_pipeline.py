...
class TestPhase84RunbookIntegration:
    ...
    def test_manager_attaches_runbooks_registry_error_does_not_break_alert(self):
        """
        Testet dass Fehler in der Runbook-Registry Alerts nicht blockieren.
        
        Dieser Test validiert das defensive Verhalten von _attach_runbooks():
        - Wenn resolve_runbooks_for_alert() eine Exception wirft
        - Wird der Fehler geloggt
        - Der Alert wird trotzdem verarbeitet (Channels + Persistierung)
        - Runbooks sind entweder leer oder fehlen im Context
        """
        sent_alerts: List[AlertMessage] = []
        ...
        # Monkeypatch: resolve_runbooks_for_alert wirft Exception
        # Der Import passiert lazy via "from src.infra.runbooks import resolve_runbooks_for_alert"
        # Wir müssen das Package patchen, nicht das Registry-Modul
        with patch("src.infra.runbooks.resolve_runbooks_for_alert") as mock_resolve:
            mock_resolve.side_effect = Exception("Simulated registry failure")

            # Alert erstellen der normalerweise Runbooks hätte
            alert = AlertMessage(
                title="Test Alert with Registry Error",
                body="Body",
                severity=AlertSeverity.WARN,
                category=AlertCategory.RISK,
                source="live_risk_severity",
            ...
            # Act: Alert senden - sollte NICHT crashen
            manager.send(alert)

        # Assert: Kein Crash, Alert wurde trotzdem an Channel gesendet
        assert len(sent_alerts) == 1, "Alert muss trotz Registry-Fehler gesendet werden"
        
        # Assert: Alert ist intakt
        sent_alert = sent_alerts[0]
        assert sent_alert.title == "Test Alert with Registry Error"
        assert sent_alert.severity == AlertSeverity.WARN
        assert sent_alert.category == AlertCategory.RISK
        assert sent_alert.session_id == "test_session"
        
        # Assert: Ursprünglicher Context erhalten
        assert sent_alert.context.get("metric") == 100.0
        
        # Assert: Runbooks entweder leer oder nicht vorhanden
        runbooks = sent_alert.context.get("runbooks", [])
        assert runbooks == [] or runbooks is None, (
            "Runbooks sollten leer sein wenn Registry fehlschlägt"
        )

    def test_manager_persists_alert_even_when_runbook_resolution_fails(self, tmp_path: Path, monkeypatch):
        """
        Testet dass Alerts auch bei Runbook-Registry-Fehler persistiert werden.
        
        Safety Property: "Runbook failures must never prevent alerts from being 
        delivered or stored."
        """
        from src.live.alert_storage import reset_default_storage, list_recent_alerts
        import src.live.alert_storage as storage_module

        # Setup: Temporäres Storage-Verzeichnis
        reset_default_storage()
        alerts_dir = tmp_path / "alerts"
        alerts_dir.mkdir(parents=True)
        monkeypatch.setattr(storage_module, "_DEFAULT_ALERTS_DIR", alerts_dir)
        ...
        # Manager MIT Persistierung
        manager = AlertPipelineManager([CaptureChannel()], persist_alerts=True)

        # Monkeypatch: resolve_runbooks_for_alert wirft Exception
        # Der Import passiert lazy via "from src.infra.runbooks import resolve_runbooks_for_alert"
        with patch("src.infra.runbooks.resolve_runbooks_for_alert") as mock_resolve:
            mock_resolve.side_effect = RuntimeError("Registry unavailable")

            alert = AlertMessage(
                title="Critical Alert - Registry Down",
                body="This alert must be persisted despite registry failure",
                severity=AlertSeverity.CRITICAL,
                category=AlertCategory.RISK,
                source="live_risk_limits",
                context={"breach_type": "max_daily_loss"},
            )

            # Act: Alert senden
            manager.send(alert)

        # Assert: Alert wurde gesendet
        assert len(sent_alerts) == 1, "Alert muss an Channel gesendet werden"

        # Assert: Alert wurde persistiert
        stored = list_recent_alerts(limit=10, hours=24)
        assert len(stored) == 1, "Alert muss trotz Registry-Fehler persistiert werden"
        assert stored[0].title == "Critical Alert - Registry Down"
...
# =============================================================================
# PHASE 85 - ESCALATION INTEGRATION TESTS
# =============================================================================


class TestPhase85EscalationIntegration:
    """
    Tests für Phase 85: Escalation-Integration in Alerts-Pipeline.

    Testet:
    - Kritische Alerts triggern Eskalation
    - Eskalation-Fehler blockieren Alerts nicht
    - Config-Gating funktioniert
    - Integration mit EscalationManager
    """

    def test_critical_alert_triggers_escalation(self):
        """
        Testet dass CRITICAL Alerts Eskalation triggern.

        Create a CRITICAL alert via the normal pipeline.
        Configure escalation to be enabled + environment allowed.
        Assert that escalation is triggered with a correctly populated event.
        """
        ...
        # EscalationManager konfigurieren
        escalation_manager = EscalationManager(
            provider=provider,
            targets=[EscalationTarget(name="Primary", routing_key="test-key")],
            enabled=True,
        ...
        # AlertPipelineManager mit Escalation
        manager = AlertPipelineManager(
            [CaptureChannel()],
        ...
        alert = AlertMessage(
            title="Risk Severity RED",
            body="Critical risk event detected",
            severity=AlertSeverity.CRITICAL,
            category=AlertCategory.RISK,
            source="live_risk_severity",
            session_id="session_123",
            context={"old_status": "yellow", "new_status": "red"},
        )
        manager.send(alert)

        # Assert: Alert wurde an Channel gesendet
        assert len(sent_alerts) == 1

        # Assert: Eskalation wurde getriggert
        assert len(provider.sent_payloads) == 1
        payload = provider.sent_payloads[0]

        # Assert: Payload enthält korrektes Event
        assert payload["payload"]["severity"] == "critical"
        assert "Risk Severity RED" in payload["payload"]["summary"]
        assert payload["payload"]["custom_details"]["alert_type"] == "RISK"

    def test_warn_alert_does_not_trigger_escalation(self):
        """Testet dass WARN Alerts keine Eskalation triggern."""
        from src.infra.escalation import (
            EscalationManager,
            EscalationTarget,
            PagerDutyLikeProviderStub,
        ...
        escalation_manager = EscalationManager(
            provider=provider,
            targets=[EscalationTarget(name="Primary")],
            enabled=True,
            enabled_environments={"live"},
        ...
        alert = AlertMessage(
            title="Risk Warning",
            body="Warning event",
            severity=AlertSeverity.WARN,  # Nicht CRITICAL
            category=AlertCategory.RISK,
            source="live_risk_severity",
        )
        manager.send(alert)
        ...
        # Assert: Keine Eskalation
        assert len(provider.sent_payloads) == 0

    def test_escalation_failure_does_not_block_alert(self):
        """
        Testet dass Eskalations-Fehler Alerts nicht blockieren.

        Make the escalation provider raise an exception.
        Run the alert pipeline to create a CRITICAL alert.
        Assert:
        - The alert is still sent to channels
        ...
        # Mock-EscalationManager der Exception wirft
        failing_escalation_manager = MagicMock()
        failing_escalation_manager.maybe_escalate.side_effect = RuntimeError(
        ...
        manager = AlertPipelineManager(
            [CaptureChannel()],
            persist_alerts=False,
            escalation_manager=failing_escalation_manager,
        )

        # Act: CRITICAL Alert senden - darf NICHT crashen
        alert = AlertMessage(
            title="Critical Alert with Escalation Failure",
        ...
        # Assert: Alert wurde trotzdem an Channel gesendet
        assert len(sent_alerts) == 1
        assert sent_alerts[0].title == "Critical Alert with Escalation Failure"

    def test_escalation_disabled_in_non_live_environment(self):
        """Testet dass Eskalation in non-live Umgebungen deaktiviert ist."""
        from src.infra.escalation import (
            EscalationManager,
            EscalationTarget,
            PagerDutyLikeProviderStub,
        ...
        # CRITICAL Alert in paper environment
        alert = AlertMessage(
            title="Critical in Paper",
            body="Should not escalate",
            severity=AlertSeverity.CRITICAL,
            category=AlertCategory.RISK,
            source="test",
        )
        ...
        # Assert: Alert gesendet
        assert len(sent_alerts) == 1

        # Assert: Keine Eskalation (falsches Environment)
        assert len(provider.sent_payloads) == 0

    def test_no_escalation_manager_works(self):
        """Testet dass Pipeline ohne EscalationManager funktioniert."""
        sent_alerts: List[AlertMessage] = []
        ...
        # Manager OHNE escalation_manager
        manager = AlertPipelineManager(
            [CaptureChannel()],
            persist_alerts=False,
        ...
        alert = AlertMessage(
            title="Alert without Escalation",
            body="Body",
            severity=AlertSeverity.CRITICAL,
            category=AlertCategory.RISK,
            source="test",
        ...
        # Alert wurde trotzdem gesendet
        assert len(sent_alerts) == 1

    def test_build_pipeline_with_escalation_from_config(self):
        """Testet build_alert_pipeline_from_config mit Escalation."""
        config = {
            "alerts": {
                "enabled": True,
                "slack": {
        ...
        manager = build_alert_pipeline_from_config(config, environment="live")

        # Manager sollte EscalationManager haben
        assert manager.escalation_manager is not None
        assert manager.escalation_manager.enabled is True
        assert manager.escalation_manager.current_environment == "live"

    def test_build_pipeline_escalation_disabled_in_config(self):
        """Testet dass Escalation disabled sein kann in Config."""
        config = {
            "alerts": {
                "enabled": True,
            },
        ...
        # Kein EscalationManager wenn deaktiviert
        assert manager.escalation_manager is None

    def test_escalation_with_alert_storage(self, tmp_path, monkeypatch):
        """Testet dass Eskalation mit Alert-Storage zusammenarbeitet."""
        from src.live.alert_storage import reset_default_storage, list_recent_alerts
        import src.live.alert_storage as storage_module
        from src.infra.escalation import (
            EscalationManager,
        ...
        # CRITICAL Alert
        alert = AlertMessage(
            title="Critical with Storage and Escalation",
            body="Body",
            severity=AlertSeverity.CRITICAL,
            category=AlertCategory.RISK,
            source="test",
        ...
        # Assert: Alert persistiert
        stored = list_recent_alerts(limit=10, hours=24)
        assert len(stored) == 1
        assert stored[0].title == "Critical with Storage and Escalation"

        # Assert: Eskalation erfolgt
        assert len(provider.sent_payloads) == 1
