"""Contract tests for comparison_ssot.v1 result durable evidence binding."""
...
def _produce_result_manifest(tmp_path, durable_root) -> tuple[Path, dict, str]:
    ...
    return result_path, manifest, comparison_definition_id


def test_happy_path_result_binding(tmp_path, ssot_durable_output_dir) -> None:
    result_path, manifest, comparison_definition_id = _produce_result_manifest(
...
def test_byte_identical_manifest_copy(tmp_path, ssot_durable_output_dir) -> None:
    result_path, _, _ = _produce_result_manifest(tmp_path, ssot_durable_output_dir)
...
def test_input_snapshots_preserved_verbatim(tmp_path, ssot_durable_output_dir) -> None:
    result_path, manifest, _ = _produce_result_manifest(tmp_path, ssot_durable_output_dir)
    ...
    assert index["result_input_snapshots"] == manifest["input_snapshots"]


def test_input_snapshot_ordering_preserved(tmp_path, ssot_durable_output_dir) -> None:
    result_path, manifest, _ = _produce_result_manifest(tmp_path, ssot_durable_output_dir)
    ...
    index = json.loads((out / INDEX_ARTIFACT_REL).read_text(encoding="utf-8"))
    assert [snap["comparison_metric_input_id"] for snap in index["result_input_snapshots"]] == [
        snap["comparison_metric_input_id"] for snap in manifest["input_snapshots"]
    ]


def test_result_summary_preserved_verbatim(tmp_path, ssot_durable_output_dir) -> None:
    result_path, manifest, _ = _produce_result_manifest(tmp_path, ssot_durable_output_dir)
    ...
    index = json.loads((out / INDEX_ARTIFACT_REL).read_text(encoding="utf-8"))
    assert (
        index["result_summary_ref"]["ranking_result"]["ranking_status"]
        == manifest["ranking_result"]["ranking_status"]
    )
    assert index["result_summary_ref"]["overall_comparable"] == manifest["overall_comparable"]
...
def test_reverify_without_raw_data(tmp_path, ssot_durable_output_dir) -> None:
    ...
    reverify_comparison_ssot_result_durable_evidence_bundle_v1(output_dir=out)
...
def test_missing_manifest_fail_closed(tmp_path) -> None:
    ...
    with pytest.raises(ComparisonSsotResultDurableEvidenceBindingError, match="not found"):
        produce_comparison_ssot_result_durable_evidence_bundle_v1(
...
def test_wrong_manifest_filename_fail_closed(tmp_path, ssot_durable_output_dir) -> None:
    result_path, _, _ = _produce_result_manifest(tmp_path, ssot_durable_output_dir)
    bad_path = result_path.parent / "wrong_name.json"
    bad_path.write_text(result_path.read_text(encoding="utf-8"), encoding="utf-8")
    out = _durable_output(tmp_path, "badname")
    with pytest.raises(ComparisonSsotResultDurableEvidenceBindingError, match="filename"):
        produce_comparison_ssot_result_durable_evidence_bundle_v1(
...
def test_existing_output_dir_fail_closed(tmp_path, ssot_durable_output_dir) -> None:
    ...
    with pytest.raises(ComparisonSsotResultDurableEvidenceBindingError, match="already exists"):
        produce_comparison_ssot_result_durable_evidence_bundle_v1(
            manifest_path=result_path,
            output_dir=out,
        )
...
def test_symlink_manifest_rejected(tmp_path, ssot_durable_output_dir) -> None:
    ...
    with pytest.raises(ComparisonSsotResultDurableEvidenceBindingError, match="symlink"):
        produce_comparison_ssot_result_durable_evidence_bundle_v1(
            manifest_path=link,
            output_dir=out,
        )
...
def test_id_mismatch_fail_closed(tmp_path, ssot_durable_output_dir) -> None:
    ...
    tampered = result_path.parent / "tampered.json"
    tampered.write_text(json.dumps(payload), encoding="utf-8")
    out = _durable_output(tmp_path, "idmismatch")
    with pytest.raises(ComparisonSsotResultDurableEvidenceBindingError):
        produce_comparison_ssot_result_durable_evidence_bundle_v1(
...
def test_digest_mismatch_fail_closed(tmp_path, ssot_durable_output_dir) -> None:
    ...
    tampered = result_path.parent / "digest_tampered.json"
    tampered.write_text(json.dumps(payload), encoding="utf-8")
...
def test_copy_failure_cleans_up(
    ...
    monkeypatch.setattr(
        "src.meta.learning_loop.comparison_ssot_result_durable_evidence_binding_v1._copy_byte_identical",
        _boom,
    )
...
def test_manifest_verify_failure_cleans_up(
    ...
    monkeypatch.setattr(
        "src.meta.learning_loop.comparison_ssot_result_durable_evidence_binding_v1.verify_manifest_sha256",
        lambda _root: (False, "checksum mismatch"),
    )
    with pytest.raises(
        ComparisonSsotResultDurableEvidenceBindingError, match="verification failed"
    ):
        produce_comparison_ssot_result_durable_evidence_bundle_v1(
            manifest_path=result_path,
            output_dir=out,
...
def test_build_binding_index_rejects_forbidden_key(tmp_path, ssot_durable_output_dir) -> None:
    _, manifest, _ = _produce_result_manifest(tmp_path, ssot_durable_output_dir)
    ...
    with pytest.raises(ComparisonSsotResultDurableEvidenceBindingError, match="forbidden key"):
...
def test_output_under_tmp_rejected(
    ...
    result_path, _, _ = _produce_result_manifest(tmp_path, ssot_durable_output_dir)
    out = Path("/tmp/peak_trade_cmp_result_binding_reject_test")
    try:
        with pytest.raises(ComparisonSsotResultDurableEvidenceBindingError, match="outside /tmp"):
            produce_comparison_ssot_result_durable_evidence_bundle_v1(
                manifest_path=result_path,
                output_dir=out,
            )
...
def test_no_runtime_imports_in_binding_module() -> None:
    path = Path("src/meta/learning_loop/comparison_ssot_result_durable_evidence_binding_v1.py")
...
def test_reverify_detects_tampered_index(tmp_path, ssot_durable_output_dir) -> None:
    ...
    payload["result_input_snapshots"][0]["comparison_metric_input_id"] = "tampered"
    index_path.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises(ComparisonSsotResultDurableEvidenceBindingError, match="MANIFEST.sha256"):
        reverify_comparison_ssot_result_durable_evidence_bundle_v1(output_dir=out)
...
def test_check_reference_consistency_requires_valid_id(tmp_path, ssot_durable_output_dir) -> None:
    ...
    with pytest.raises(
        ComparisonSsotResultDurableEvidenceBindingError, match="comparison_definition_id"
    ):
...
def test_binding_does_not_read_metric_inputs_or_raw_sources(
    ...
    def _tracked_read_bytes(self: Path) -> bytes:
        if self.name == "comparison_metric_input_manifest_v1.json":
            raise AssertionError("metric input read forbidden during result binding")
        if self.suffix in {".csv", ".parquet"} or self.name == "run_summary.json":
            raise AssertionError("raw source read forbidden during result binding")
...
def test_result_manifest_filename_constant() -> None:
    assert MANIFEST_ARTIFACT_REL == RESULT_ARTIFACT_FILENAME


def test_check_reference_consistency_cross_fields(tmp_path, ssot_durable_output_dir) -> None:
    _, manifest, comparison_definition_id = _produce_result_manifest(
        tmp_path, ssot_durable_output_dir
    )
    cross = check_reference_consistency(manifest)
    assert cross.comparison_definition_id == comparison_definition_id
...
def test_build_binding_index_type(tmp_path, ssot_durable_output_dir) -> None:
    _, manifest, _ = _produce_result_manifest(tmp_path, ssot_durable_output_dir)
    cross = ComparisonResultBindingCrossReferences(
    ...
    assert index["schema_version"] == "comparison_ssot_result_durable_evidence_binding_v1"
