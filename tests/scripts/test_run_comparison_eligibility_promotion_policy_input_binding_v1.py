"""CLI tests for run_comparison_eligibility_promotion_policy_input_binding_v1."""
...
from scripts.run_comparison_eligibility_promotion_policy_input_binding_v1 import (
    EXIT_EVIDENCE_ERROR,
    EXIT_OK,
    EXIT_USAGE_ERROR,
    main,
...
def test_cli_happy_path(tmp_path, ssot_durable_output_dir, capsys) -> None:
    ...
    rc = main(
        [
            ...
            str(candidate_input.candidate_input_bundle_dir),
            "--output-dir",
            str(out),
        ]
    )
...
def test_cli_missing_step3(tmp_path, capsys) -> None:
    out = _durable_output(tmp_path)
    rc = main(
        [
            "--candidate-input-bundle-dir",
