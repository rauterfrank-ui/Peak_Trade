"""CLI tests for run_secure_handoff_envelope_v1."""
...
from scripts.run_secure_handoff_envelope_v1 import (
    EXIT_ENVELOPE_ERROR,
    EXIT_OK,
    EXIT_USAGE_ERROR,
    main,
)
from src.meta.learning_loop.secure_handoff_envelope_v1 import ARTIFACT_REL
...
def test_cli_happy_path(tmp_path, ssot_durable_output_dir, capsys) -> None:
    ...
    rc = main(
        [
            ...
            str(lease.authority_lease_bundle_dir),
            "--output-dir",
            str(out),
        ]
    )
    ...
    assert "envelope_status=ENVELOPE_VALID_FOR_OFFLINE_HANDOFF" in captured.out
    assert (out / ARTIFACT_REL).is_file()


def test_cli_missing_authority_lease_bundle(tmp_path, ssot_durable_output_dir, capsys) -> None:
    out = _durable_output(tmp_path)
    rc = main(
        [
            "--authority-lease-bundle-dir",
            "/nonexistent/authority_lease",
            "--output-dir",
            str(out),
        ]
    )
...
def test_cli_invalid_authority_lease_manifest(tmp_path, ssot_durable_output_dir, capsys) -> None:
    lease = produce_authority_lease_fixture(tmp_path, ssot_durable_output_dir)
