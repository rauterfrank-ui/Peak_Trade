"""CLI tests for run_order_intent_idempotency_v1."""
...
from scripts.run_order_intent_idempotency_v1 import EXIT_OK, main
from src.meta.learning_loop.order_intent_idempotency_v1 import ARTIFACT_REL, SELF_VERIFICATION_REL
...
def _durable_output(tmp_path: Path, name: str = "cli_idempotency_out") -> Path:
...
def test_cli_happy_path(tmp_path, ssot_durable_output_dir, capsys) -> None:
    fixture = produce_order_intent_idempotency_fixture(
        ...
        trading_session_name="script_trading_session_idempotency",
        lifecycle_name="script_lifecycle_idempotency",
    )
    ...
    assert "ORDER_INTENT_IDEMPOTENCY_EXACT_REPLAY" in captured.out
