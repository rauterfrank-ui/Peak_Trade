"""CLI tests for run_comparison_promotion_candidate_identity_binding_v1."""
...
from scripts.run_comparison_promotion_candidate_identity_binding_v1 import (
    EXIT_BINDING_ERROR,
    EXIT_OK,
    EXIT_USAGE_ERROR,
    main,
)
from src.meta.learning_loop.comparison_promotion_candidate_identity_binding_v1 import ARTIFACT_REL
...
def test_cli_happy_path(tmp_path, ssot_durable_output_dir, capsys) -> None:
    ...
    rc = main(
        [
            ...
            str(promotion_input.promotion_input_binding_bundle_dir),
            "--candidate-identity-bundle-dir",
            str(candidate.candidate_identity_bundle_dir),
            "--output-dir",
            str(out),
        ]
    )
...
def test_cli_missing_promotion_input(tmp_path, capsys) -> None:
    out = _durable_output(tmp_path)
    rc = main(
        [
            "--promotion-input-binding-bundle-dir",
