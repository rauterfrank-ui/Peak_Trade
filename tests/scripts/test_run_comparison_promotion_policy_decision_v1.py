"""CLI tests for run_comparison_promotion_policy_decision_v1."""
...
from scripts.run_comparison_promotion_policy_decision_v1 import (
    EXIT_DECISION_ERROR,
    EXIT_OK,
    EXIT_USAGE_ERROR,
    main,
...
def test_cli_happy_path(tmp_path, ssot_durable_output_dir, capsys) -> None:
    ...
    rc = main(
        [
            ...
            str(evidence.promotion_policy_input_evidence_bundle_dir),
            "--output-dir",
            str(out),
        ]
    )
    ...
    assert "decision_outcome=APPROVE" in captured.out
    assert (out / ARTIFACT_REL).is_file()


def test_cli_missing_step5(tmp_path, capsys) -> None:
    out = _durable_output(tmp_path)
    rc = main(
        [
            "--policy-input-evidence-bundle-dir",
...
def test_cli_invalid_step5_manifest(tmp_path, ssot_durable_output_dir, capsys) -> None:
    evidence = produce_policy_input_evidence_fixture(tmp_path, ssot_durable_output_dir)
    assert evidence.promotion_policy_input_evidence_bundle_dir is not None
    (evidence.promotion_policy_input_evidence_bundle_dir / "MANIFEST.sha256").write_text(
