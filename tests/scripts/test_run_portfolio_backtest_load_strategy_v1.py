"""
run_portfolio_backtest: canonical load_strategy() migration (offline, fail-closed).
"""
...
import run_portfolio_backtest as portfolio_script

TARGET_SCRIPT = project_root / "scripts/run_portfolio_backtest.py"
...
def test_load_strategy_momentum_matches_create_strategy_from_config_signals() -> None:
    ...
    params = portfolio_script._build_strategy_params_from_config(cfg, MOMENTUM_KEY)
    canonical = load_strategy(MOMENTUM_KEY)(df, params).replace(-1, 0)

    pd.testing.assert_series_equal(canonical, legacy)


def test_run_single_asset_backtest_calls_load_strategy_and_passes_full_params() -> None:
    from src.core.peak_config import PeakConfig
    ...
    with (
        ...
        patch.object(
            portfolio_script, "load_strategy", return_value=fake_signal_fn
        ) as load_strategy_mock,
        patch.object(
            portfolio_script.BacktestEngine, "run_realistic", side_effect=fake_run_realistic
        ),
    ):
...
def test_isolated_strategy_bindings_per_symbol() -> None:
    ...
    with (
        patch.object(portfolio_script, "load_data_for_symbol") as load_data,
        patch.object(portfolio_script, "load_strategy", side_effect=fake_load_strategy),
        patch.object(
            portfolio_script.BacktestEngine,
            "run_realistic",
            side_effect=fake_run_realistic,
        ),
    ):
        load_data.return_value = _sample_ohlcv()
        portfolio_script.run_single_asset_backtest("BTC/EUR", MA_CROSSOVER_KEY, cfg, n_bars=80)
        portfolio_script.run_single_asset_backtest("ETH/EUR", MOMENTUM_KEY, cfg, n_bars=80)
...
def test_no_silent_strategy_fallback_on_unknown_name() -> None:
    ...
    with pytest.raises(ValueError, match="Unbekannte Strategie"):
        load_strategy("bad_portfolio_strategy_name")
...
def test_import_smoke_no_main_execution() -> None:
    result = subprocess.run(
        [sys.executable, "-c", "import run_portfolio_backtest"],
...
def test_cli_help_smoke_no_run() -> None:
    ...
    assert result.returncode == 0, result.stderr
    assert "allocation" in result.stdout
