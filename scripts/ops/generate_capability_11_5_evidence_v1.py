#!/usr/bin/env python3
"""Generate durable evidence for Cap 11.5 restart/recovery/kill-switch closure."""
...
from src.ops.capability_11_5_testnet_restart_recovery_and_kill_switch_closure_v1.dependency_and_ownership_v1 import (  # noqa: E402
...
def main() -> int:
    ...
    test_node = (
        "tests/ops/test_capability_11_5_testnet_restart_recovery_and_kill_switch_closure_v1.py"
    )
    proc = subprocess.run(
    ...
    verification = verify_capability_11_5_v1()
    ...
    _write_json(
        productive / "unknown_submit_reconnect_recovery_proof.json",
        verification["proofs"]["unknown_submit_reconnect_recovery"],
    )
    _write_json(
        productive / "restart_with_open_order_position_proof.json",
        verification["proofs"]["restart_with_open_order_position"],
    )
    _write_json(
        productive / "kill_switch_and_emergency_control_proof.json",
        verification["proofs"]["kill_switch_and_emergency_control"],
    )
    _write_json(
        productive / "autonomous_recovery_degradation_proof.json",
        verification["proofs"]["autonomous_recovery_degradation"],
    )
    _write_json(
