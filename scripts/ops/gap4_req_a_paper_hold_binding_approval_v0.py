"""Gap-4 REQ-A bounded paper HOLD-binding approval contract v0.
...
BINDING_SCOPE = "gap4_req_a_bounded_paper_evidence_only"

GAP4_MIN_DURATION_SECONDS = 300
GAP4_MAX_DURATION_SECONDS = 900
GAP4_DEFAULT_DURATION_SECONDS = 300
...
def validate_paper_duration_seconds(duration_seconds: int) -> list[str]:
    if duration_seconds < GAP4_MIN_DURATION_SECONDS or duration_seconds > GAP4_MAX_DURATION_SECONDS:
        return [
            "duration-seconds must be between "
            f"{GAP4_MIN_DURATION_SECONDS} and {GAP4_MAX_DURATION_SECONDS} for {CONTRACT_PROFILE}"
        ]
    return []
