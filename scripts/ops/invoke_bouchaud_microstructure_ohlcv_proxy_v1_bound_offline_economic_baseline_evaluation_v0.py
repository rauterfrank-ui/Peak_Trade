#!/usr/bin/env python3
"""Canonical invocation adapter for bouchaud OHLCV proxy v1 bound offline baseline evaluation.

Offline-only adapter. Resolves repo-local ``.venv/bin/python``, reads ``GO_TOKEN`` from
the environment, and forwards ``--confirm-go-token`` to the canonical evaluation runner.
