#!/usr/bin/env python3
"""Generate durable evidence for Cap 11.4 testnet execution adapter and lifecycle closure."""
...
from src.ops.capability_11_4_testnet_execution_adapter_and_lifecycle_closure_v1.dependency_and_ownership_v1 import (  # noqa: E402
...
def main() -> int:
    ...
    test_node = (
        "tests/ops/test_capability_11_4_testnet_execution_adapter_and_lifecycle_closure_v1.py"
    )
    proc = subprocess.run(
    ...
    verification = verify_capability_11_4_v1()
    ...
    _write_json(
        productive / "testnet_execution_adapter_proof.json",
        verification["proofs"]["testnet_execution_adapter"],
    )
    _write_json(
        productive / "order_serialization_dry_run_proof.json",
        verification["proofs"]["order_serialization_dry_run"],
    )
    _write_json(
        productive / "testnet_lifecycle_closure_proof.json",
        verification["proofs"]["testnet_lifecycle_closure"],
    )
    _write_json(
