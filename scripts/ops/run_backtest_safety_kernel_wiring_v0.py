#!/usr/bin/env python3
"""Collect durable evidence for backtest Safety Kernel state-file wiring v0."""
...
DEFAULT_BASE_HEAD = "04cc599a37fa1483d03cb0d65d04af96775063b1"
DEFAULT_SOURCE_CLOSEOUT = (
    DEFAULT_ARCHIVE_ROOT
    / "research/pr4960_backtest_canonical_order_intent_wiring_v0_merge_closeout_20260707T123042Z"
)
DEFAULT_RUNBOOK = (
...
VERDICT = "BACKTEST_SAFETY_KERNEL_WIRING_V0_PASS"
PROCESS_CLASSIFICATION = "FULL_CANONICAL_BACKTEST_PARITY_NARROW_REWIRE"
SCOPE_CLASSIFICATION = (
    "BACKTEST_SAFETY_KERNEL_WIRING_V0_NO_RUNTIME_NO_ORDERS_NO_ECONOMIC_EVALUATION_V0"
)
TARGETED_TESTS = (
...
def collect_evidence(config: RunConfig, out_dir: Path | None = None) -> dict[str, object]:
    ...
    evidence_dir = out_dir or (
        config.durable_root / f"research/backtest_safety_kernel_wiring_v0_{stamp}"
    )
    ...
    source_manifest_log = "SOURCE_CLOSEOUT_ABSENT=true\n"
    if config.source_closeout.is_dir():
        source_manifest = _run(
            ["shasum", "-a", "256", "-c", "MANIFEST.sha256"],
            cwd=config.source_closeout,
        )
        ...
        source_manifest_log = "SOURCE_CLOSEOUT_PATH_MISSING=true\nSOURCE_MANIFEST_VERIFY_RC=1\n"
    ...
    (evidence_dir / "implementation_summary.txt").write_text(
        "\n".join(
            [
                ...
                "SAFETY_POLICY_DECISION_REPRESENTED_IN_BACKTEST=true",
                "SAFETY_BLOCK_REASONS_REPRESENTED_IN_BACKTEST=true",
                "NO_ORDER_WITHOUT_SAFETY_PASS_REPRESENTED_IN_BACKTEST=true",
    ...
    verdict = (
        ...
        else "BACKTEST_SAFETY_KERNEL_WIRING_V0_BLOCKED"
    )
...
def main() -> int:
    ...
    parser.add_argument("--scope", type=str, default="BACKTEST_SAFETY_KERNEL_WIRING_V0")
