#!/usr/bin/env python3
"""Bounded §11.13.5.Z2DP fresh create-readiness GET package. No POST."""
...
from src.ops.section_11_13_5_z2dp_post_z2do_fresh_create_readiness_evidence_v1.constants_v1 import (  # noqa: E402
    DEFAULT_VAULT_RELATIVE,
    EVIDENCE_DIRNAME,
    EXPECTED_ORIGIN_MAIN_SHA,
    OWNER_GO,
)
from src.ops.section_11_13_5_z2dp_post_z2do_fresh_create_readiness_evidence_v1.execute_v1 import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    except Z2DPCreateReadinessGetError as exc:
        packs = sorted(path for path in evidence_root.glob("*") if path.is_dir())
        if not packs:
            print(f"Z2DP_GET_FAILED_WITHOUT_EVIDENCE:{exc}", file=sys.stderr)
            return 2
        pack = packs[-1]
        print(f"Z2DP_GET_PERFORMED_FAIL_CLOSED:{exc}")
        print(f"EVIDENCE_PACK={pack}")
        return 0
    print("Z2DP_GET_RESULT=")
