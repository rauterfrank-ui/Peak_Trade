...
hint_file="$evi/P93_POST_STOP_PRIMARY_EVIDENCE_OPERATOR_HINTS.txt"
{
  echo "P93_POST_STOP_PRIMARY_EVIDENCE_OPERATOR_HINTS_v0"
  echo "HINT_ONLY=true"
  echo "PACK_NOT_EXECUTED_BY_P93=true"
  echo "P79_ARCHIVE_VERIFY_NOT_EXECUTED_BY_P93=true"
  ...
  echo ""
  echo "# Copy-paste: pack supervisor session evidence after STOP (operator-run explicitly; non-authorizing)"
  echo "python3 scripts/ops/pack_online_readiness_supervisor_evidence_v0.py \\"
  echo "  --out-dir \"${pack_out_dir}\" \\"
  echo "  --archive-root \"${pack_archive_root}\" \\"
  echo "  --primary-evidence-enforce"
  echo ""
  echo "# Copy-paste: P79 ARCHIVE_ROOT offline manifest verify against packed archive (operator-run explicitly; non-authorizing)"
  echo "ARCHIVE_ROOT=\"${pack_archive_root}\" bash scripts/ops/p79_supervisor_health_gate_v1.sh"
} > "$hint_file"
cat "$hint_file"

echo "P93_OK pin=$pin evi=$evi hint=$hint_file"
