#!/usr/bin/env python3
"""Generate durable evidence for Cap 11.7 Live private read-only and shadow reconciliation."""
...
from src.ops.capability_11_7_live_private_readonly_and_shadow_reconciliation_v1.dependency_and_ownership_v1 import (  # noqa: E402
...
def main() -> int:
    ...
    test_node = (
        "tests/ops/test_capability_11_7_live_private_readonly_and_shadow_reconciliation_v1.py"
    )
    proc = subprocess.run(
    ...
    verification = verify_capability_11_7_v1()
    ...
    _write_json(
        productive / "live_private_readonly_port_proof.json",
        verification["proofs"]["live_private_readonly_port"],
    )
    _write_json(
        productive / "live_shadow_reconciliation_proof.json",
        verification["proofs"]["live_shadow_reconciliation"],
    )
    _write_json(
        productive / "live_evidence_ladder_proof.json",
        verification["proofs"]["live_evidence_ladder"],
    )
    _write_json(
