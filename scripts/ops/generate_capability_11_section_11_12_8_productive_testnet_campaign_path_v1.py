#!/usr/bin/env python3
"""Generate durable evidence for Cap 11 §11.12.8 productive Testnet campaign path."""
...
def main() -> int:
    ...
    test_node = (
        "tests/ops/test_capability_11_section_11_12_8_productive_testnet_campaign_path_v1.py"
    )
    proc = subprocess.run(
    ...
    proof = verification["proofs"]["productive_path"]
    ...
    _write_json(productive / "productive_path_proof.json", proof)
    ...
    # Manifest over durable evidence files (relative paths).
    files: list[Path] = []
