"""One-shot runner for PEAK_TRADE_OWNER_GO_SECTION_11_14_LIVE_FEE_OBSERVED_MAXIMUM_SAFE_LEVERAGE_V1."""
...
from src.ops.section_11_14_live_order_and_economic_evidence_ladder_v1.fee_observed_execute_v1 import (  # noqa: E402
    execute_live_fee_observed_v1,
)
from src.ops.section_11_14_live_order_and_economic_evidence_ladder_v1.fee_observed_gets_v1 import (  # noqa: E402
...
def main() -> int:
    ...
    if origin_main_sha != EXPECTED_ORIGIN_MAIN_SHA:
        ...
        return 2
    result = execute_live_fee_observed_v1(
    ...
    write_json_v1(pack / "FEE_OBSERVED_ADJUDICATION.json", adjudication)
    names = ["SUMMARY.json", "FEE_OBSERVED_ADJUDICATION.json"]
    ...
    print(f"LIVE_FEE_OBSERVED={adjudication.get('LIVE_FEE_OBSERVED')}")
    ...
    print(f"LIVE_FEE_ADJUDICATION_REASON={adjudication.get('LIVE_FEE_ADJUDICATION_REASON')}")
    print(f"UNRESOLVED_REASON={adjudication.get('UNRESOLVED_REASON')}")
    print(f"RAW_FEE_IF_OBSERVED={adjudication.get('RAW_FEE_IF_OBSERVED')}")
    print(f"RAW_FEE_CCY_IF_OBSERVED={adjudication.get('RAW_FEE_CCY_IF_OBSERVED')}")
