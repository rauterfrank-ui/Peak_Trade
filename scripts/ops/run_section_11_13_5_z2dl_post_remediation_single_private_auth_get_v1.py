#!/usr/bin/env python3
"""One-shot §11.13.5.Z2DL authenticated account/config GET. No POST."""
...
from src.ops.section_11_13_5_z2dl_post_remediation_single_private_auth_get_v1.constants_v1 import (  # noqa: E402
    DEFAULT_VAULT_RELATIVE,
    EVIDENCE_DIRNAME,
    EXPECTED_ORIGIN_MAIN_SHA,
    OWNER_GO,
)
from src.ops.section_11_13_5_z2dl_post_remediation_single_private_auth_get_v1.execute_v1 import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    except Z2DLPrivateAuthGetError as exc:
        packs = sorted(path for path in evidence_root.glob("*") if path.is_dir())
        if not packs:
            print(f"Z2DL_GET_FAILED_WITHOUT_EVIDENCE:{exc}", file=sys.stderr)
            return 2
        pack = packs[-1]
        print(f"Z2DL_GET_PERFORMED_FAIL_CLOSED:{exc}")
        print(f"EVIDENCE_PACK={pack}")
        return 0
    print("Z2DL_GET_RESULT=")
