#!/usr/bin/env python3
"""Generate durable evidence for Cap 11.8 Live dry-run order-plan parity."""
...
from src.ops.capability_11_8_live_dry_run_order_plan_parity_v1.dependency_and_ownership_v1 import (  # noqa: E402
...
def main() -> int:
    ...
    test_node = "tests/ops/test_capability_11_8_live_dry_run_order_plan_parity_v1.py"
    proc = subprocess.run(
    ...
    verification = verify_capability_11_8_v1()
    ...
    _write_json(
        productive / "live_dry_run_order_plan_proof.json",
        verification["proofs"]["live_dry_run_order_plan"],
    )
    _write_json(
        productive / "live_order_plan_parity_proof.json",
        verification["proofs"]["live_order_plan_parity"],
    )
    _write_json(
        productive / "live_order_plan_evidence_ladder_proof.json",
        verification["proofs"]["live_order_plan_evidence_ladder"],
    )
    _write_json(
