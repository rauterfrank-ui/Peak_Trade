#!/usr/bin/env python3
"""
Bounded Pilot Entry Gate Wrapper — Pre-Entry-Checks only.

Read-only gate for the first strictly bounded real-money pilot.
Runs Pre-Entry-Checks (go/no-go, cockpit, config). Does NOT start a live session.

Exit codes:
  0 — All gates GREEN; bounded pilot entry ready (live session start not yet implemented)
  1 — One or more gates RED; entry not permitted
  2 — Script error (e.g. cockpit build failed)

Usage:
  python3 scripts/ops/run_bounded_pilot_session.py
  python3 scripts/ops/run_bounded_pilot_session.py --json

Reference: BOUNDED_REAL_MONEY_PILOT_ENTRY_CONTRACT, bounded_pilot_entry_point_slice_review
"""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

# Add repo root for imports
_REPO_ROOT = Path(__file__).resolve().parents[2]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

NOT_IMPLEMENTED_MSG = (
    "Bounded pilot entry ready; live session start not yet implemented. "
    "Operator must invoke manually."
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Bounded Pilot Entry Gate — Pre-Entry-Checks only (no live start)"
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Output result as JSON",
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
        default=None,
        help="Repo root (default: parent of scripts/ops)",
    )
    args = parser.parse_args()
    repo_root = args.repo_root or (_REPO_ROOT if _REPO_ROOT.exists() else Path.cwd())

    try:
        from scripts.ops.pilot_go_no_go_eval_v1 import evaluate
        from src.webui.ops_cockpit import build_ops_cockpit_payload

        payload = build_ops_cockpit_payload(repo_root=repo_root)
    except Exception as e:
        print(f"ERR: failed to build cockpit payload: {e}", file=sys.stderr)
        return 2

    result = evaluate(payload)
    verdict = result["verdict"]

    if verdict != "GO_FOR_NEXT_PHASE_ONLY":
        if args.json:
            out = {
                "contract": "run_bounded_pilot_session",
                "verdict": verdict,
                "entry_permitted": False,
                "message": f"Gates not satisfied: verdict={verdict}",
                "go_no_go": result,
            }
            print(json.dumps(out, indent=2))
        else:
            print(f"GATES_RED: verdict={verdict}", file=sys.stderr)
            for r in result["rows"]:
                if r["status"] != "PASS":
                    print(f"  Row {r['row']} {r['area']}: {r['status']}", file=sys.stderr)
            print("Entry not permitted. Fix blockers before retrying.", file=sys.stderr)
        return 1

    # All gates GREEN
    if args.json:
        out = {
            "contract": "run_bounded_pilot_session",
            "verdict": verdict,
            "entry_permitted": True,
            "message": NOT_IMPLEMENTED_MSG,
            "go_no_go": result,
        }
        print(json.dumps(out, indent=2))
    else:
        print(NOT_IMPLEMENTED_MSG)

    return 0


if __name__ == "__main__":
    sys.exit(main())
