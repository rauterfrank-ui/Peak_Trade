#!/usr/bin/env python3
"""Generate durable evidence for Cap 11.2 credential/authorization boundary."""
...
from src.ops.capability_11_2_credential_authorization_and_account_identity_boundary_v1.dependency_and_ownership_v1 import (  # noqa: E402
...
def main() -> int:
    ...
    test_node = "tests/ops/test_capability_11_2_credential_authorization_and_account_identity_boundary_v1.py"
    proc = subprocess.run(
    ...
    verification = verify_capability_11_2_v1()
    ...
    _write_json(
        productive / "credential_contract_proof.json",
        verification["proofs"]["credential_contract"],
    )
    _write_json(
        productive / "authorization_binding_proof.json",
        verification["proofs"]["authorization_binding"],
    )
    _write_json(
        productive / "account_identity_proof.json",
        verification["proofs"]["account_identity"],
    )
    _write_json(
        productive / "credential_load_gate_proof.json",
        verification["proofs"]["credential_load_gate"],
    )
    _write_json(
        productive / "autonomy_scope_limits_proof.json",
        verification["proofs"]["autonomy_scope_limits"],
    )
    _write_json(
