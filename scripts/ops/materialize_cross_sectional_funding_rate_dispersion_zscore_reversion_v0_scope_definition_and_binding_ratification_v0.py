#!/usr/bin/env python3
"""Materialize cross-sectional funding-rate dispersion-zscore-reversion v0 scope binding ratification v0.
...
Operator GO: GO_CROSS_SECTIONAL_FUNDING_RATE_DISPERSION_ZSCORE_REVERSION_V0_BINDING_RATIFICATION_NO_EVAL_NO_RUNTIME_AUTHORITY_V0
"""
...
CONFIRM_GO = (
    "GO_CROSS_SECTIONAL_FUNDING_RATE_DISPERSION_ZSCORE_REVERSION_V0_BINDING_RATIFICATION_"
...
ALLOWED_CONFIRM_GOS = frozenset({CONFIRM_GO, COMBINED_RATIFY_AND_EXECUTE_GO_TOKEN})

from src.research.cross_sectional_funding_rate_dispersion_zscore_reversion_v0_offline_economic_evaluation_scope_ratification_v0 import (  # noqa: E402
    CONFIG_REL_PATH,
    SCHEMA_VERSION,
    ValidationVerdictEnum,
    materialize_dispersion_zscore_reversion_offline_economic_evaluation_scope_ratification_v0,
...
from src.research.cross_sectional_funding_rate_dispersion_zscore_reversion_v0_versioned_research_binding_v0 import (  # noqa: E402
...
def run_materialization(
    *,
    ...
    if confirm not in ALLOWED_CONFIRM_GOS:
        _die(f"ERR: confirm_go_token_required:{'|'.join(sorted(ALLOWED_CONFIRM_GOS))}")
    ...
    evidence_dir = (
        ...
        / "research"
        / f"cross_sectional_funding_rate_dispersion_zscore_reversion_v0_binding_ratification_no_eval_{ts_slug}"
    )
    ...
    (evidence_dir / "SCOPE.md").write_text(
        "\n".join(
            [
                "# Cross-Sectional Funding Rate Dispersion-Zscore-Reversion v0 Scope Ratification",
                "",
                ...
                "",
                "Hypothesis: Cross-sectional dispersion-gated z-score mean reversion of funding rates identifies",
                "instruments whose relative funding attractiveness is shifting.",
                "",
                "Material difference: dispersion_zscore_reversion vs dual_leg_spread/v1 level spread",
    ...
    (evidence_dir / "MATERIAL_DIFFERENCE_ASSERTION.md").write_text(
        "\n".join(
            [
                ...
                "",
                "| Terminal Surface | Dispersion-Zscore-Reversion v0 Difference |",
                "|---|---|",
                "| dual_leg_spread/v1 level spread | dispersion-gated z-score mean reversion, single-slot, no dual-leg |",
    ...
    payload: dict[str, Any] = {
        "verdict": "CS_FUNDING_RATE_DISPERSION_ZSCORE_REVERSION_V0_SCOPE_BINDING_RATIFICATION_PASS",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize cross-sectional funding-rate dispersion-zscore-reversion v0 scope definition "
            "and binding ratification v0."
        )
    )
    parser.add_argument("--confirm-go-token", required=True, choices=sorted(ALLOWED_CONFIRM_GOS))
