#!/usr/bin/env python3
"""Generate durable evidence for Cap 11 productive private-readonly fetch reference-only."""
...
def main() -> int:
    ...
    test_node = (
        "tests/ops/test_capability_11_productive_private_readonly_fetch_reference_only_v1.py"
    )
    proc = subprocess.run(
    ...
    verification = verify_capability_11_productive_private_readonly_fetch_reference_only_v1()
    ...
    _write_json(
        productive / "reference_only_fetch_proof.json",
        verification["proofs"]["reference_only_fetch"],
    )
