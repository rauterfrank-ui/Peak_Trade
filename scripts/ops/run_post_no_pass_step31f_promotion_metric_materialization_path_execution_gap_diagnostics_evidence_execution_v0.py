#!/usr/bin/env python3
"""Run post-no-pass STEP31F promotion metric materialization path execution gap diagnostics v0."""
...
from scripts.research.post_no_pass_step31f_promotion_metric_materialization_path_execution_gap_diagnostics_evidence_execution_v0 import (  # noqa: E402
    CONFIRM_GO,
    CURRENT_STATE,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Execute post-no-pass STEP31F promotion metric materialization path execution gap "
            "diagnostics v0 over manifest-verified PR4888 source evidence."
        )
    )
    ...
    command_log = [
        " ".join(
            [
                ...
                str(
                    _REPO_ROOT
                    / "scripts/ops/run_post_no_pass_step31f_promotion_metric_materialization_path_execution_gap_diagnostics_evidence_execution_v0.py"
                ),
                "--confirm-go-token",
                CONFIRM_GO,
            ]
