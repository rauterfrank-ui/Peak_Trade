#!/usr/bin/env python3
"""Bounded §11.13.5.Z2DS post-Z2DR read-only runtime evidence GET package. No POST."""
...
from src.ops.section_11_13_5_z2ds_post_z2dr_runtime_read_only_evidence_max_leverage_v1.constants_v1 import (  # noqa: E402
    DEFAULT_VAULT_RELATIVE,
    EVIDENCE_DIRNAME,
    EXPECTED_ORIGIN_MAIN_SHA,
    OWNER_GO,
)
from src.ops.section_11_13_5_z2ds_post_z2dr_runtime_read_only_evidence_max_leverage_v1.execute_v1 import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--execute-get-package", action="store_true")
    ...
    if args.bound_origin_main_sha != EXPECTED_ORIGIN_MAIN_SHA:
        ...
        return 2
    if not args.execute_get_package:
        print("EXECUTE_GET_PACKAGE_FLAG_REQUIRED", file=sys.stderr)
        return 2
    ...
    except Z2DSRuntimeReadOnlyEvidenceGetError as exc:
        packs = sorted(path for path in evidence_root.glob("*") if path.is_dir())
        if not packs:
            print(f"Z2DS_GET_FAILED_WITHOUT_EVIDENCE:{exc}", file=sys.stderr)
            return 2
        pack = packs[-1]
        print(f"Z2DS_GET_PERFORMED_FAIL_CLOSED:{exc}")
        print(f"EVIDENCE_PACK={pack}")
        return 0
    print("Z2DS_GET_RESULT=")
