#!/usr/bin/env python3
"""Generate Peak_Trade System Atlas markdown views. ATLAS_AUTHORITY=NONE. Offline."""

from __future__ import annotations

import sys
from pathlib import Path

_REPO_ROOT = Path(__file__).resolve().parents[2]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from scripts.ops.system_atlas_v1.generate_v1 import write_generated_v1  # noqa: E402
from scripts.ops.system_atlas_v1.load_v1 import load_atlas_v1  # noqa: E402
from scripts.ops.system_atlas_v1.validate_v1 import AtlasValidationError, validate_atlas_v1  # noqa: E402


def main() -> int:
    atlas = load_atlas_v1(repo_root=_REPO_ROOT)
    try:
        validate_atlas_v1(atlas)
    except AtlasValidationError as exc:
        print(f"ATLAS_VALIDATION_FAIL:{exc}", file=sys.stderr)
        return 2
    written = write_generated_v1(atlas=atlas, repo_root=_REPO_ROOT)
    for path in written:
        print(path.relative_to(_REPO_ROOT))
    print("ATLAS_GENERATE_OK")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
