#!/usr/bin/env python3
"""Generate durable evidence for Cap 11.3 private read-only venue integration."""
...
from src.ops.capability_11_3_private_readonly_venue_integration_and_reconciliation_v1.constants_v1 import (  # noqa: E402
    CAPABILITY_ID,
    CLAIMS_FILENAME,
    EVIDENCE_DIRNAME,
    MANIFEST_FILENAME,
...
from src.ops.capability_11_3_private_readonly_venue_integration_and_reconciliation_v1.dependency_and_ownership_v1 import (  # noqa: E402
    STATE_OWNERSHIP_MATRIX_V1,
)
from src.ops.capability_11_3_private_readonly_venue_integration_and_reconciliation_v1.verifier_v1 import (  # noqa: E402
...
def main() -> int:
    ...
    test_node = (
        "tests/ops/test_capability_11_3_private_readonly_venue_integration_and_reconciliation_v1.py"
    )
    proc = subprocess.run(
    ...
    verification = verify_capability_11_3_v1()
    ...
    _write_json(
        productive / "private_readonly_venue_port_proof.json",
        verification["proofs"]["private_readonly_venue_port"],
    )
    _write_json(
        productive / "venue_session_and_connectivity_proof.json",
        verification["proofs"]["venue_session_and_connectivity"],
    )
    _write_json(
        productive / "exchange_clock_sync_proof.json",
        verification["proofs"]["exchange_clock_sync"],
    )
    _write_json(
        productive / "private_account_state_ingestion_proof.json",
        verification["proofs"]["private_account_state_ingestion"],
    )
    _write_json(
        productive / "reconciliation_hierarchy_proof.json",
        verification["proofs"]["reconciliation_hierarchy"],
    )
    _write_json(
        productive / "venue_adapter_anti_corruption_proof.json",
        verification["proofs"]["venue_adapter_anti_corruption"],
    )
    _write_json(
