#!/usr/bin/env python3
"""Collect durable evidence for backtest Reconciliation / Unknown Outcome wiring v0."""
...
VERDICT = "BACKTEST_RECONCILIATION_UNKNOWN_OUTCOME_WIRING_V0_PASS"
PROCESS_CLASSIFICATION = "FULL_CANONICAL_BACKTEST_PARITY_NARROW_REWIRE"
SCOPE_CLASSIFICATION = "BACKTEST_RECONCILIATION_UNKNOWN_OUTCOME_WIRING_V0_NO_RUNTIME_NO_ORDERS_V0"
TARGETED_TESTS = (
...
def collect_evidence(config: RunConfig, out_dir: Path | None = None) -> dict[str, object]:
    ...
    evidence_dir = out_dir or (
        config.durable_root / f"research/backtest_reconciliation_unknown_outcome_wiring_v0_{stamp}"
    )
    ...
    pr4961_rc, pr4961_log = _verify_source_manifest(
        config.source_evidence_pr4961,
        "SOURCE_EVIDENCE_PR4961",
    )
    source_manifest_rc = max(closeout_rc, pr4961_rc)
    (evidence_dir / "source_closeout_manifest_verify_pre_merge.log").write_text(
        closeout_log + pr4961_log,
        encoding="utf-8",
    )
    ...
    (evidence_dir / "git_context.txt").write_text(
        "\n".join(
            [
                ...
                f"SOURCE_EVIDENCE_PR4961={config.source_evidence_pr4961}",
    ...
    semantic_flags = [
        ...
        "NO_AUTO_RESUBMIT_AFTER_UNKNOWN_OUTCOME_REPRESENTED_IN_BACKTEST=true",
        "QUERY_BY_CLIENT_ORDER_ID_REPRESENTED_IN_BACKTEST=true",
        "OPEN_ORDERS_RECENT_ORDERS_FILLS_POSITION_RECONCILIATION_REPRESENTED_IN_BACKTEST=true",
        "RECONCILIATION_FAILURE_BLOCKS_NEW_EXPOSURE_REPRESENTED_IN_BACKTEST=true",
        "OPPOSITE_SIDE_REQUIRES_RECONCILED_FLAT_REPRESENTED_IN_BACKTEST=true",
        "NO_POSITION_INCREASE_DURING_UNRESOLVED_RECONCILIATION_REPRESENTED_IN_BACKTEST=true",
    ...
    (evidence_dir / "known_external_failures_policy.txt").write_text(
        "\n".join(
            [
                "KNOWN_EXTERNAL_NON_SLICE_FAILURES_FROM_PR4962_CLOSEOUT=true",
                "BROAD_KEYWORD_PYTEST_NOT_REQUIRED_FOR_SLICE_PASS=true",
                "NO_XFAIL_NO_TEST_WEAKENING=true",
                "STALE_NEXT_RECOMMENDED_SLICE_FIX_SCOPE_BOUND=true",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    verdict = (
        ...
        else "BACKTEST_RECONCILIATION_UNKNOWN_OUTCOME_WIRING_V0_BLOCKED"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--scope",
        type=str,
        default="BACKTEST_RECONCILIATION_UNKNOWN_OUTCOME_WIRING_V0",
    )
    ...
    parser.add_argument(
        "--source-evidence-pr4961",
        type=Path,
        default=DEFAULT_SOURCE_EVIDENCE_PR4961,
    )
    ...
    config = RunConfig(
        ...
        source_evidence_pr4961=args.source_evidence_pr4961,
    )
