#!/usr/bin/env python3
"""Minimum-necessary authenticated account/positions GETs for P08 CASE_C. No POST."""
...
from src.ops.section_11_13_5_p08_empty_data_not_zero_v1.constants_v1 import (  # noqa: E402
    DEFAULT_VAULT_RELATIVE,
    EVIDENCE_DIRNAME,
    EXPECTED_ORIGIN_MAIN_SHA,
    OWNER_GO,
)
from src.ops.section_11_13_5_p08_empty_data_not_zero_v1.execute_v1 import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = execute_p08_empty_data_not_zero_gets_v1(
            owner_go=args.owner_go,
            origin_main_sha=args.bound_origin_main_sha,
            evidence_root=evidence_root,
            vault_file=Path(args.vault_file),
        )
    except P08EmptyDataNotZeroError as exc:
        packs = sorted(path for path in evidence_root.glob("*") if path.is_dir())
        if not packs:
            print(f"P08_EMPTY_DATA_GETS_FAILED_WITHOUT_EVIDENCE:{exc}", file=sys.stderr)
            return 2
        pack = packs[-1]
        print(f"P08_EMPTY_DATA_GETS_PERFORMED_FAIL_CLOSED:{exc}")
        print(f"EVIDENCE_PACK={pack}")
        return 0
    print("P08_EMPTY_DATA_GETS_RESULT=")
