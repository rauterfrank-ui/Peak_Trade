#!/usr/bin/env python3
"""One-shot post-whitelist authenticated account/config GET. No POST. No retry."""
...
from src.ops.section_11_13_5_post_z2ds_post_whitelist_private_auth_attestation_v1.constants_v1 import (  # noqa: E402
    DEFAULT_VAULT_RELATIVE,
    EVIDENCE_DIRNAME,
    EXPECTED_ORIGIN_MAIN_SHA,
    OWNER_GO,
)
from src.ops.section_11_13_5_post_z2ds_post_whitelist_private_auth_attestation_v1.execute_v1 import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    except PostWhitelistPrivateAuthAttestationError as exc:
        packs = sorted(path for path in evidence_root.glob("*") if path.is_dir())
        if not packs:
            print(f"ATTESTATION_GET_FAILED_WITHOUT_EVIDENCE:{exc}", file=sys.stderr)
            return 2
        pack = packs[-1]
        print(f"ATTESTATION_GET_PERFORMED_FAIL_CLOSED:{exc}")
        print(f"EVIDENCE_PACK={pack}")
        return 0
    print("ATTESTATION_GET_RESULT=")
