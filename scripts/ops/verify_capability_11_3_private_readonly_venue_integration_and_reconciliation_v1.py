#!/usr/bin/env python3
"""Fail-closed verifier for Cap 11.3 durable evidence package."""
...
def main() -> int:
    ...
    required = [
        ...
        "private_readonly_venue_port_proof.json",
        "venue_session_and_connectivity_proof.json",
        "exchange_clock_sync_proof.json",
        "private_account_state_ingestion_proof.json",
        "reconciliation_hierarchy_proof.json",
    ...
    live_verify = verify_capability_11_3_v1()
    ...
    checks = {
        ...
        "no_private_readonly_network": reach.get("PRIVATE_READONLY_NETWORK_REACHABLE") is False,
        "no_private_readonly_activation": claims.get("PRIVATE_READONLY_VENUE_INTEGRATION_ACTIVATED")
        is False,
        "no_private_readonly_fetch": claims.get(
            "PRIVATE_READONLY_FETCH_PERFORMED_IN_CAPABILITY_11_3"
        )
        is False,
        "no_credential_load": claims.get("CREDENTIAL_LOAD_PERFORMED_IN_CAPABILITY_11_3") is False,
        "no_auth_consumption": claims.get("AUTHORIZATION_CONSUMPTION_ALLOWED") is False,
        "reconciliation_before_alpha": claims.get("RECONCILIATION_BEFORE_ALPHA") is True,
    }
