#!/usr/bin/env python3
"""CLI for Phase 9.2 Step-7 productive campaign execution path implementation.

Commands:
  preflight / prove-path           — prove productive campaign path (no session)
  prove-contrast                   — BINDING vs PRODUCTIVE campaign executor
  materialize-evidence             — offline implementation evidence
  prove-failure-injection          — offline gate/fault matrix
  execute-governed-campaign        — offline fail-closed in this capability

This capability never starts a real Public-MD network session and never mints
or consumes confirm tokens. Binding-only campaign CLI remains fail-closed and
unchanged.

Later separate Owner-GO Real-TTY campaign (after merge; not this capability):
...
from src.ops.phase_9_2_step_7_productive_campaign_execution_path_v1.confirm_token_path_v1 import (  # noqa: E402
    reject_confirm_token_argv_v1,
    reject_confirm_token_env_fallback_v1,
)
from src.ops.phase_9_2_step_7_productive_campaign_execution_path_v1.constants_v1 import (  # noqa: E402
    CAPABILITY_ID,
    NETWORK_SESSION_ALLOWED,
)
from src.ops.phase_9_2_step_7_productive_campaign_execution_path_v1.evidence_v1 import (  # noqa: E402
    materialize_productive_campaign_execution_path_evidence_v1,
)
from src.ops.phase_9_2_step_7_productive_campaign_execution_path_v1.executor_contrast_v1 import (  # noqa: E402
    prove_binding_vs_productive_campaign_executor_contrast_v1,
)
from src.ops.phase_9_2_step_7_productive_campaign_execution_path_v1.failure_injection_v1 import (  # noqa: E402
    run_step7_productive_campaign_execution_path_failure_injection_v1,
)
from src.ops.phase_9_2_step_7_productive_campaign_execution_path_v1.productive_campaign_executor_v1 import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "prove-contrast":
        payload = prove_binding_vs_productive_campaign_executor_contrast_v1(
            expected_repository_sha=sha,
            expected_config_digest=cfg,
            repo_root=_REPO_ROOT,
        )
    ...
    if args.command == "materialize-evidence":
        payload = materialize_productive_campaign_execution_path_evidence_v1(
            repository_sha=sha,
            config_digest=cfg,
    ...
    if args.command == "prove-failure-injection":
        payload = run_step7_productive_campaign_execution_path_failure_injection_v1(
            expected_repository_sha=sha,
            expected_config_digest=cfg,
            repo_root=_REPO_ROOT,
        )
