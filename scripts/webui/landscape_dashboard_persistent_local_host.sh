#!/usr/bin/env bash
# Peak Trade Landscape Dashboard V2 — persistent loopback LaunchAgent host.
#
# CAPABILITY_ID=LANDSCAPE_DASHBOARD_PERSISTENT_LOCAL_HOST_V1
# CANONICAL_BOOKMARK_URL=http://127.0.0.1:8765/market
#
...
#     install|enable|start|stop|disable|restart|status|open|logs
#
# Env overrides:
#   PEAK_TRADE_WEBUI_HOST (default 127.0.0.1)
#   PEAK_TRADE_WEBUI_PORT (default 8765 — fixed operator bookmark port)
...
#   PEAK_TRADE_MARKET_DASHBOARD_LABEL (default com.peaktrade.landscape-dashboard-persistent-v1)
...
SCRIPT_ABS="${REPO_ROOT}/scripts/webui/landscape_dashboard_persistent_local_host.sh"

HOST="${PEAK_TRADE_WEBUI_HOST:-127.0.0.1}"
PORT="${PEAK_TRADE_WEBUI_PORT:-8765}"
...
LABEL="${PEAK_TRADE_MARKET_DASHBOARD_LABEL:-com.peaktrade.landscape-dashboard-persistent-v1}"

ASGI_TARGET="src.webui.app:app"
IDENTITY_MARKER="peak_trade_landscape_dashboard_persistent_local_host_v1"
...
STATE_DIR="${REPO_ROOT}/.run/landscape_dashboard_persistent_local_host"
...
usage() {
  cat <<'EOF'
Usage: ./scripts/webui/landscape_dashboard_persistent_local_host.sh \\
  <install|enable|start|stop|disable|restart|status|open|logs>

  install   Render LaunchAgent plist only (no load)
  enable    Load LaunchAgent + kickstart (no health wait)
  start     enable + wait until health is OK (idempotent)
...
Bookmark: http://127.0.0.1:8765/market (loopback-only, no trading authority).
EOF
}
...
cmd_install() {
  ...
  render_plist
  echo "PLIST_PATH=${PLIST_PATH}"
  echo "CANONICAL_BOOKMARK_URL=http://127.0.0.1:${PORT}/market"
  echo "ACTION=INSTALLED"
  echo "LAUNCHD_LOADED=false"
}
