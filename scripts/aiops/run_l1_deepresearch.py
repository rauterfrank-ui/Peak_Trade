#!/usr/bin/env python3
"""
CLI Script: L1 DeepResearch Runner (Offline/Replay Mode)

Runs L1 DeepResearch with Proposer + Critic orchestration for research outputs.
...
from ai_orchestration.l1_runner import (
    L1Runner,
    L1RunnerError,
    SoDViolation,
...
def main():
    parser = argparse.ArgumentParser(
        description="L1 DeepResearch Runner (Offline/Replay Mode)",
...
Exit Codes:
  ...
  3: Runtime error
    ...
    parser.add_argument(
        "--question",
        required=True,
        help="Research question or topic (required)",
    )
    parser.add_argument(
        "--mode",
        default="replay",
    ...
    parser.add_argument(
        "--fixture",
        help="Fixture ID (e.g., 'l1_deepresearch_sample'). Resolves to tests/fixtures/transcripts/<id>.json",
    )
    parser.add_argument(
        "--out",
        type=Path,
    ...
    # Run L1 DeepResearch
    try:
        print(f"🔄 Starting L1 DeepResearch run...")
        ...
        print("✅ L1 DeepResearch run completed successfully!")
        print()
        print("=" * 70)
    ...
    except L1RunnerError as e:
        print(f"❌ L1 RUNNER ERROR", file=sys.stderr)
        print(f"   {e}", file=sys.stderr)
        return 2
