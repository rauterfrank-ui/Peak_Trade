...
def _load_patches_for_promotion() -> List[ConfigPatch]:
    ...
    if not demo_path.exists():
        ...
        print("[promotion_loop] Generate demo patches with: python scripts/generate_demo_patches_for_promotion.py")
        return []
    ...
    for patch_dict in patches_data:
        ...
        if patch_dict.get("applied_at"):
            patch_dict["applied_at"] = datetime.fromisoformat(patch_dict["applied_at"]) if patch_dict["applied_at"] else None
        if patch_dict.get("promoted_at"):
            patch_dict["promoted_at"] = datetime.fromisoformat(patch_dict["promoted_at"]) if patch_dict["promoted_at"] else None
...
def main() -> None:
    ...
    for candidate in candidates:
        ...
        if confidence and confidence >= 0.75:
            ...
            print(f"[promotion_loop] Marked {candidate.patch.id} as eligible_for_live (confidence: {confidence:.2f})")
        else:
            conf_str = f"{confidence:.2f}" if confidence else "N/A"
            print(f"[promotion_loop] Rejected {candidate.patch.id} due to low confidence ({conf_str})")
    ...
    p0_violations = [d for d in decisions if any(f.startswith("P0_") for f in d.candidate.safety_flags)]
    if p0_violations:
        print(f"[promotion_loop] WARNING: {len(p0_violations)} candidates have P0 safety violations")
    ...
    if live_path is None:
        print(
            f"[promotion_loop] Auto-apply: no changes applied (mode={args.auto_apply_mode!r})."
        )
    else:
