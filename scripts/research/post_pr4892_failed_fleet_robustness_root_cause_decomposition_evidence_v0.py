#!/usr/bin/env python3
"""Read-only post-PR4892 failed fleet robustness root-cause decomposition evidence v0.

Offline-only Class-E decomposition over PR4892/4893 source evidence and scope definition.
...
CONFIRM_GO = "GO_POST_PR4892_FAILED_FLEET_ROBUSTNESS_ROOT_CAUSE_DECOMPOSITION_EVIDENCE_EXECUTION_V0"
...
NEXT_CANONICAL_STEP = (
    ...
    "AFTER_POST_PR4892_ROOT_CAUSE_DECOMPOSITION_V0"
)
NEXT_ADMISSIBLE_GO = (
...
def _candidate_summary(
    ...
    *,
    ...
    return {
        ...
        "candidate_id": evidence.get("canonical_candidate_identifier") or f"{candidate}/v3",
        "strategy_version": evidence.get("strategy_version") or "v3",
        "evidence_source_bundle": parent_execution_ref,
        "evidence_artifact": f"candidate_evidence_{candidate}.json",
        "verdict": evidence.get("status") or evidence.get("evidence_status"),
        ...
        "retry_allowed": False,
        "terminal_negative": evidence.get("status") == "ROBUSTNESS_FAILED",
...
def _collect_decomposition(
    *,
    ...
    fleet_summary_path = parent_execution_ref / "fleet_evaluation_summary_v0.json"
    ...
    for candidate in failed_candidates:
        evidence_path = parent_execution_ref / f"candidate_evidence_{candidate}.json"
        ...
        for axis in decomposition_axes:
            ...
            total_count += 1
            if result["classification"] in {"TERMINAL_NEGATIVE", "INCONCLUSIVE_EVIDENCE"}:
                mapped_count += 1
        ...
        per_candidate[candidate] = _candidate_summary(
            ...
            evidence,
            axis_results,
            parent_execution_ref=str(parent_execution_ref),
        )
    ...
    execution_status = (
        ...
        else "ROOT_CAUSE_DECOMPOSITION_EXECUTION_COMPLETE_WITH_SOURCE_GAPS_V0"
    )

    input_evidence_index = [
        {
            "bundle_id": "parent_execution_pr4892",
            "bundle_path": str(parent_execution_ref),
            "role": "PR4892 owner-fix offline economic evaluation execution",
        ...
        {
            "bundle_id": "scope_definition_pr4893",
            "bundle_path": str(scope_definition_ref),
            "role": "PR4893 failed fleet robustness root-cause scope definition",
            "manifest_verify_rc": scope_manifest_rc,
            "artifacts": [
                "scope_definition_v0.json",
                "SCOPE_DEFINITION_REPORT.md",
    ...
    return {
        ...
        "source_evidence_refs": [str(parent_execution_ref), str(scope_definition_ref)],
        ...
        "economic_validity_offline_gate_pass": fleet_verdict.get(
            "economic_validity_offline_gate_pass"
        ),
...
def _write_reports(output_dir: Path, report: dict[str, Any]) -> None:
    ...
    report_lines = [
        "# Root Cause Decomposition Report",
        "",
    ...
    (output_dir / "ROOT_CAUSE_DECOMPOSITION_REPORT.md").write_text(
        "\n".join(report_lines) + "\n",
        encoding="utf-8",
    )
    ...
    (output_dir / "ROOT_CAUSE_DECOMPOSITION_SUMMARY.json").write_text(
...
def run_root_cause_decomposition_evidence_v0(
    *,
    ...
    parent_execution_ref = Path(config["parent_execution_evidence_ref"])
    scope_definition_ref = Path(config["parent_scope_definition_evidence_ref"])
    for label, ref in (
        ("parent_execution", parent_execution_ref),
        ("scope_definition", scope_definition_ref),
    ):
        if not ref.is_dir():
    ...
    if parent_manifest_rc != 0:
        _die(f"ERR:parent execution manifest invalid: {parent_execution_ref}")
    ...
    (output_dir / "root_cause_decomposition_full.json").write_text(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Execute read-only post-PR4892 failed fleet robustness root-cause "
            "decomposition evidence v0."
        )
    )
    ...
    report = run_root_cause_decomposition_evidence_v0(
