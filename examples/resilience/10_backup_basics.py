...
""")
        ...
        backup_id = recovery.create_backup(
            include_config=True,
            description="Initial configuration backup"
        )
...
def example_2_state_snapshots():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        backup_id = recovery.create_backup(
            include_state=True,
            description="State snapshot backup",
            tags=["state", "metrics"]
        )
        print(f"   ✅ Backup created: {backup_id}")
...
""".strip())
        ...
        if success:
            ...
            print(f"   - reports/daily_report.json exists: {(reports_dir / 'daily_report.json').exists()}")
...
def example_4_full_backup():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        recovery.state_snapshot.register_provider(
            "app_state",
            lambda: {"version": "1.0", "uptime_seconds": 3600}
        )
