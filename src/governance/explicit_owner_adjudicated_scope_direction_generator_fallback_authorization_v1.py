"""Sixth Economic Guard admission class: ScopeDirectionState generator fallback.
...
change, and the fifth productive mapping-contract runtime bind. Owner
adjudication is necessary and not sufficient. Exact files, reused SHA-256
evidence digest, bound diff-base SHA, required runtime paths, and
excluded-path / forbidden-prefix checks must validate jointly. Does not
waive MASTER_V2_MUTATION_ALLOWED=false. Does not reopen the fifth-class
grant.
"""
...
GENERATOR_FALLBACK_AUTH_VERSION = (
    "explicit_owner_adjudicated_scope_direction_generator_fallback_authorization_v1"
)
GENERATOR_FALLBACK_SCOPE_CLASS = "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_V1"
GENERATOR_FALLBACK_AUTHORIZATION_ID = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_AUTHORIZATION_V1"
)
GENERATOR_FALLBACK_MUTATION_PURPOSE = "SCOPE_DIRECTION_GENERATOR_FALLBACK_REPAIR"
GENERATOR_FALLBACK_SCOPE_ID = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_AUTHORIZATION_V1"
)
GENERATOR_FALLBACK_CLASS_ATTESTATION_RELATIVE = (
    "docs/ops/specs/"
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_AUTHORIZATION_V1.md"
)
DEFAULT_GENERATOR_FALLBACK_AUTH_PATH = (
    "config/governance/"
    "explicit_owner_adjudicated_scope_direction_generator_fallback_authorization_v1.json"
)
GENERATOR_FALLBACK_BOUND_AUTHORITY_SPEC = "docs/ops/specs/DIRECTIONAL_MAPPING_CONTRACT_REPAIR_V1.md"
...
REASON_GENERATOR_FALLBACK_AUTHORIZED = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_AUTHORIZED"
)
REASON_GENERATOR_FALLBACK_AUTH_VALID = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_AUTHORIZATION_VALID"
)
REASON_GENERATOR_FALLBACK_AUTH_INVALID = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_AUTHORIZATION_INVALID"
)
REASON_GENERATOR_FALLBACK_AUTH_MISSING = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_AUTHORIZATION_MISSING"
)
REASON_GENERATOR_FALLBACK_PATH_UNAUTHORIZED = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_PATH_UNAUTHORIZED"
)
REASON_GENERATOR_FALLBACK_DIGEST_MISSING = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_DIGEST_MISSING"
)
REASON_GENERATOR_FALLBACK_DIGEST_MALFORMED = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_DIGEST_MALFORMED"
)
REASON_GENERATOR_FALLBACK_DIGEST_MISMATCH = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_DIGEST_MISMATCH"
)
REASON_GENERATOR_FALLBACK_BASE_MISMATCH = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_DIFF_BASE_MISMATCH"
)
REASON_GENERATOR_FALLBACK_EXCLUDED_PATH = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_EXCLUDED_PATH_PRESENT"
)
REASON_GENERATOR_FALLBACK_REQUIRED_RUNTIME_MISSING = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_REQUIRED_RUNTIME_MISSING"
)
REASON_GENERATOR_FALLBACK_FORBIDDEN_PREFIX = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_FORBIDDEN_PREFIX"
)
REASON_GENERATOR_FALLBACK_UNKNOWN_FIELD = (
    "EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_UNKNOWN_FIELD"
)
...
_REQUIRED_EPISTEMICS = {
    ...
    "GENERATOR_FALLBACK_PROJECTS_FROM_SIDESTATE": "HUMAN_ADJUDICATED",
    "COMPOSITION_SELECTED_SIDE_MAY_WRITE_SCOPE_DIRECTION_STATE": "HUMAN_ADJUDICATED",
    "PENDING_SEMANTICS_UNCHANGED": "HUMAN_ADJUDICATED",
    "ENTRY_EXIT_SEMANTICS_UNCHANGED": "HUMAN_ADJUDICATED",
    "NEUTRAL_CHOP_KILL_FALLBACK_REMAINS_LONG": "HUMAN_ADJUDICATED",
    "CONTRACT_RUNTIME_BINDING_PROVEN_SCOPE": "DECLARED_OWNER_POLICY",
}


@dataclass(frozen=True)
class GeneratorFallbackAuthorizationDecision:
    applied: bool
    valid: bool
    version: str | None
...
def compute_generator_fallback_evidence_digest(
    *,
...
def validate_generator_fallback_authorization(
    ...
    *,
    ...
    reasons: list[str] = []
    if auth is None:
        return False, (REASON_GENERATOR_FALLBACK_AUTH_MISSING,)

    extra_keys = sorted(set(auth.keys()) - _ALLOWED_KEYS)
    if extra_keys:
        reasons.append(REASON_GENERATOR_FALLBACK_UNKNOWN_FIELD)
        reasons.extend(f"GENERATOR_FALLBACK_UNKNOWN_FIELD:{key}" for key in extra_keys)

    for flag in _FORBIDDEN_IMMUTABLE_TRUE:
        if auth.get(flag) is True:
            reasons.append(f"GENERATOR_FALLBACK_IMMUTABLE_FLAG_TRUE:{flag}")

    if auth.get("contract_version") != GENERATOR_FALLBACK_AUTH_VERSION:
        reasons.append("GENERATOR_FALLBACK_AUTH_VERSION_MISMATCH")
    if auth.get("authorized_scope_class") != GENERATOR_FALLBACK_SCOPE_CLASS:
        reasons.append("GENERATOR_FALLBACK_SCOPE_CLASS_MISMATCH")
    if auth.get("authorization_token") != GENERATOR_FALLBACK_AUTHORIZATION_ID:
        reasons.append("GENERATOR_FALLBACK_TOKEN_MISMATCH")
    purpose = auth.get("mutation_purpose_class")
    if purpose != GENERATOR_FALLBACK_MUTATION_PURPOSE:
        reasons.append("GENERATOR_FALLBACK_MUTATION_PURPOSE_MISMATCH")
    if purpose == "SEMANTICS_NEUTRAL_TECHNICAL_CANONICAL_WIRING":
        reasons.append("GENERATOR_FALLBACK_WIRING_PURPOSE_FORBIDDEN")
    if purpose == "PRODUCTIVE_CANONICAL_MAPPING_CONTRACT_RUNTIME_BIND":
        reasons.append("GENERATOR_FALLBACK_FIFTH_CLASS_PURPOSE_FORBIDDEN")
    if auth.get("scope_id") != GENERATOR_FALLBACK_SCOPE_ID:
        reasons.append("GENERATOR_FALLBACK_SCOPE_ID_MISMATCH")
    if auth.get("TOKEN_ALONE_IS_INSUFFICIENT") is not True:
        reasons.append("GENERATOR_FALLBACK_TOKEN_ALONE_NOT_MARKED_INSUFFICIENT")
    if auth.get("OWNER_APPROVED_ALONE_IS_INSUFFICIENT") is not True:
        reasons.append("GENERATOR_FALLBACK_OWNER_APPROVED_ALONE_NOT_MARKED_INSUFFICIENT")

    attestation = auth.get("class_attestation")
    if attestation != GENERATOR_FALLBACK_CLASS_ATTESTATION_RELATIVE:
        reasons.append("GENERATOR_FALLBACK_ATTESTATION_BINDING_INVALID")
    elif (
        repo_root is not None
        and not (repo_root / GENERATOR_FALLBACK_CLASS_ATTESTATION_RELATIVE).is_file()
    ):
        reasons.append("GENERATOR_FALLBACK_ATTESTATION_BINDING_INVALID")

    spec = auth.get("bound_authority_spec")
    if spec != GENERATOR_FALLBACK_BOUND_AUTHORITY_SPEC:
        reasons.append("GENERATOR_FALLBACK_BOUND_AUTHORITY_SPEC_MISMATCH")
    elif (
        repo_root is not None
        and not (repo_root / GENERATOR_FALLBACK_BOUND_AUTHORITY_SPEC).is_file()
    ):
        reasons.append("GENERATOR_FALLBACK_BOUND_AUTHORITY_SPEC_MISSING")

    grant_active = auth.get("grant_active")
    if grant_active is not False and grant_active is not True:
        reasons.append("GENERATOR_FALLBACK_GRANT_ACTIVE_INVALID")
        grant_active = False

    for flag in _REQUIRED_FALSE_FLAGS:
        if auth.get(flag) is not False:
            reasons.append(f"GENERATOR_FALLBACK_FLAG_NOT_FALSE:{flag}")

    if "pr_number" in auth or "branch_name" in auth or auth.get("temporary_bypass") is True:
        reasons.append("GENERATOR_FALLBACK_FORBIDDEN_EXCEPTION_FIELD")
    ...
    if not isinstance(forbidden_effects, dict) or not isinstance(invariants, dict):
        reasons.append("GENERATOR_FALLBACK_INVARIANTS_MISSING")
    else:
        for key in _REQUIRED_EFFECT_NONE:
            if forbidden_effects.get(key) != "NONE":
                reasons.append("GENERATOR_FALLBACK_EFFECT_FORBIDDEN")
                break
        ...
        for key in _REQUIRED_SEMANTIC_FALSE:
            if invariants.get(key) is not False:
                reasons.append("GENERATOR_FALLBACK_SEMANTIC_INVARIANT_NOT_FALSE")
                break
        for key in _REQUIRED_SEMANTIC_TRUE:
            if invariants.get(key) is not True:
                reasons.append("GENERATOR_FALLBACK_TRADING_SEMANTICS_CHANGED_NOT_TRUE")
                break
    if not isinstance(capabilities, dict):
        reasons.append("GENERATOR_FALLBACK_CAPABILITY_INVARIANTS_MISSING")
    else:
        for key in _REQUIRED_CAPABILITY_FALSE:
            if capabilities.get(key) is not False:
                reasons.append("GENERATOR_FALLBACK_CAPABILITY_INVARIANT_NOT_FALSE")
                break

    epistemics = auth.get("claim_epistemics")
    if not isinstance(epistemics, dict):
        reasons.append("GENERATOR_FALLBACK_EPISTEMICS_MISSING")
    else:
        for key, expected in _REQUIRED_EPISTEMICS.items():
            value = epistemics.get(key)
            if value not in _CLAIM_EPISTEMIC_ALLOWED:
                reasons.append("GENERATOR_FALLBACK_EPISTEMIC_INVALID")
                break
            if value != expected:
                reasons.append("GENERATOR_FALLBACK_EPISTEMIC_MISMATCH")
                break

    human_claims = auth.get("human_adjudicated_slice_claims")
    if not isinstance(human_claims, dict):
        reasons.append("GENERATOR_FALLBACK_HUMAN_CLAIMS_MISSING")
    else:
        if human_claims.get("GENERATOR_FALLBACK_PROJECTS_FROM_SIDESTATE") is not True:
            reasons.append("GENERATOR_FALLBACK_HUMAN_CLAIM_MISSING")
        if (
            human_claims.get("COMPOSITION_SELECTED_SIDE_MAY_WRITE_SCOPE_DIRECTION_STATE")
            is not False
        ):
            reasons.append("GENERATOR_FALLBACK_HUMAN_CLAIM_MISSING")
        ...
        if human_claims.get("ENTRY_EXIT_SEMANTICS_UNCHANGED") is not True:
            reasons.append("GENERATOR_FALLBACK_HUMAN_CLAIM_MISSING")
        if human_claims.get("NEUTRAL_CHOP_KILL_FALLBACK_REMAINS_LONG") is not True:
            reasons.append("GENERATOR_FALLBACK_HUMAN_CLAIM_MISSING")
        if (
            human_claims.get("CONTRACT_RUNTIME_BINDING_PROVEN_SCOPE")
            != "OFFLINE_FIXTURE_PROOF_ONLY_NOT_LIVE"
        ):
            reasons.append("GENERATOR_FALLBACK_OFFLINE_SCOPE_CLAIM_INVALID")

    allowed_paths = _as_str_list(auth.get("allowed_paths"))
    if allowed_paths is None:
        reasons.append("GENERATOR_FALLBACK_ALLOWED_PATHS_MISSING")
        allowed_paths = []
    else:
        if grant_active is True and not allowed_paths:
            reasons.append("GENERATOR_FALLBACK_ALLOWED_PATHS_EMPTY_WHILE_ACTIVE")
        if grant_active is False and allowed_paths:
            reasons.append("GENERATOR_FALLBACK_ALLOWED_PATHS_NONEMPTY_WHILE_INACTIVE")
        if allowed_paths:
            if not all(_is_exact_file_path(item) for item in allowed_paths):
                reasons.append("GENERATOR_FALLBACK_ALLOWED_PATHS_NOT_EXACT_FILES")
            if _detect_broad_master_v2_grant(allowed_paths):
                reasons.append("GENERATOR_FALLBACK_BROAD_MASTER_V2_GRANT")

    required_runtime = _as_str_list(auth.get("required_runtime_paths"))
    if required_runtime is None:
        reasons.append("GENERATOR_FALLBACK_REQUIRED_RUNTIME_PATHS_MISSING")
        required_runtime = []
    else:
        if grant_active is True and not required_runtime:
            reasons.append("GENERATOR_FALLBACK_REQUIRED_RUNTIME_PATHS_EMPTY_WHILE_ACTIVE")
        if grant_active is False and required_runtime:
            reasons.append("GENERATOR_FALLBACK_REQUIRED_RUNTIME_PATHS_NONEMPTY_WHILE_INACTIVE")
        if required_runtime:
            if not all(_is_exact_file_path(item) for item in required_runtime):
                reasons.append("GENERATOR_FALLBACK_REQUIRED_RUNTIME_PATHS_NOT_EXACT_FILES")
            ...
            if any(_normalize_path(item) not in allowed_set for item in required_runtime):
                reasons.append("GENERATOR_FALLBACK_REQUIRED_RUNTIME_NOT_IN_ALLOWED_PATHS")

    excluded = _as_str_list(auth.get("excluded_paths"))
    if excluded is None:
        reasons.append("GENERATOR_FALLBACK_EXCLUDED_PATHS_MISSING")
    elif excluded and not all(_is_exact_file_path(item) for item in excluded):
        reasons.append("GENERATOR_FALLBACK_EXCLUDED_PATHS_NOT_EXACT_FILES")

    prefixes = auth.get("authorized_path_prefixes")
    if prefixes not in ([], None):
        reasons.append("GENERATOR_FALLBACK_PATH_PREFIX_GRANT")

    deny_prefixes = _as_str_list(auth.get("forbidden_diff_prefixes"))
    if deny_prefixes is None:
        reasons.append("GENERATOR_FALLBACK_FORBIDDEN_DIFF_PREFIXES_MISSING")
    else:
        have = {_normalize_path(item) for item in deny_prefixes}
        required = set(REQUIRED_FORBIDDEN_DIFF_PREFIXES)
        if not required.issubset(have):
            reasons.append("GENERATOR_FALLBACK_FORBIDDEN_DIFF_PREFIXES_INCOMPLETE")
        if any("*" in item or item.endswith("**") for item in deny_prefixes):
            reasons.append("GENERATOR_FALLBACK_FORBIDDEN_DIFF_PREFIXES_GLOB")
        if not all(item.endswith("/") for item in deny_prefixes):
            reasons.append("GENERATOR_FALLBACK_FORBIDDEN_DIFF_PREFIXES_NOT_DIRECTORY")

    surface_classes = _as_str_list(auth.get("allowed_surface_classes"))
    if surface_classes is None:
        reasons.append("GENERATOR_FALLBACK_SURFACE_CLASSES_MISSING")
    elif grant_active is False and surface_classes:
        reasons.append("GENERATOR_FALLBACK_SURFACE_CLASSES_NONEMPTY_WHILE_INACTIVE")
    elif grant_active is True and surface_classes != [GENERATOR_FALLBACK_SCOPE_CLASS]:
        reasons.append("GENERATOR_FALLBACK_SURFACE_CLASS_MISMATCH")

    if auth.get("evidence_digest_algorithm") != EVIDENCE_DIGEST_ALGORITHM:
        reasons.append("GENERATOR_FALLBACK_DIGEST_ALGORITHM_INVALID")
    if auth.get("evidence_digest_canonicalization") != EVIDENCE_DIGEST_CANONICALIZATION:
        reasons.append("GENERATOR_FALLBACK_DIGEST_CANONICALIZATION_INVALID")
    ...
    if grant_active is True:
        if digest in (None, ""):
            reasons.append(REASON_GENERATOR_FALLBACK_DIGEST_MISSING)
        elif not isinstance(digest, str) or _SHA256_HEX_RE.fullmatch(digest) is None:
            reasons.append(REASON_GENERATOR_FALLBACK_DIGEST_MALFORMED)
        if not isinstance(bound_sha, str) or _GIT_OBJECT_SHA_RE.fullmatch(bound_sha) is None:
            reasons.append("GENERATOR_FALLBACK_BOUND_DIFF_BASE_SHA_INVALID")
        if not isinstance(slice_id, str) or not slice_id.strip():
            reasons.append("GENERATOR_FALLBACK_SLICE_GRANT_ID_MISSING")
    else:
        if digest not in ("", None):
            reasons.append("GENERATOR_FALLBACK_DIGEST_NONEMPTY_WHILE_INACTIVE")
        if bound_sha not in ("", None):
            reasons.append("GENERATOR_FALLBACK_BOUND_DIFF_BASE_SHA_NONEMPTY_WHILE_INACTIVE")
        if slice_id not in ("", None):
            reasons.append("GENERATOR_FALLBACK_SLICE_GRANT_ID_NONEMPTY_WHILE_INACTIVE")
    ...
    if not isinstance(rules, list) or not _REQUIRED_FAIL_CLOSED_RULES.issubset(set(rules)):
        reasons.append("GENERATOR_FALLBACK_FAIL_CLOSED_RULES_INCOMPLETE")

    if _detect_pr_or_branch_hardcode(auth):
        reasons.append("GENERATOR_FALLBACK_PR_OR_BRANCH_HARDCODE")

    if reasons:
        reasons.insert(0, REASON_GENERATOR_FALLBACK_AUTH_INVALID)
        return False, tuple(dict.fromkeys(reasons))
    return True, (REASON_GENERATOR_FALLBACK_AUTH_VALID,)


def evaluate_generator_fallback_authorization(
    ...
    *,
    ...
    valid, validation_reasons = validate_generator_fallback_authorization(auth, repo_root=repo_root)
    ...
    if not valid:
        return GeneratorFallbackAuthorizationDecision(
            applied=False,
            valid=False,
    ...
    if not grant_active or not forbidden_paths:
        return GeneratorFallbackAuthorizationDecision(
            applied=False,
            valid=True,
            version=GENERATOR_FALLBACK_AUTH_VERSION,
            reason_codes=(),
            authorized_paths=(),
            unauthorized_forbidden_paths=forbidden_paths,
            grant_active=grant_active,
    ...
    actual = compute_generator_fallback_evidence_digest(
        file_diffs=file_diffs,
    ...
    return GeneratorFallbackAuthorizationDecision(
        ...
        valid=True,
        version=GENERATOR_FALLBACK_AUTH_VERSION,
        reason_codes=(REASON_GENERATOR_FALLBACK_AUTHORIZED,),
        authorized_paths=allowed,
