"""
STEP 29M armstrong_cycle v1 economic evaluation admissibility contract v1.
...
CONTRACT_OWNER = "backtest.step29m_armstrong_cycle_v1_economic_evaluation_admissibility_contract_v1"

ARMSTRONG_CYCLE_V1_STRATEGY_ID = "armstrong_cycle"
ARMSTRONG_CYCLE_V1_STRATEGY_VERSION = "v1"
ARMSTRONG_CYCLE_V1_STRATEGY_OWNER = (
    "src.strategies.armstrong.armstrong_cycle_strategy.ArmstrongCycleStrategy"
)
...
ARMSTRONG_CYCLE_V1_REQUIRED_WARMUP_ROWS = 0

CYCLE_LENGTH_DAYS_MIN = 30
CYCLE_LENGTH_DAYS_MAX = 10000
EVENT_WINDOW_DAYS_MIN = 1
EVENT_WINDOW_DAYS_MAX = 1570
...
OPERATOR_POLICY_DERIVATION_REF = "operator_policy_decision:STEP29M_ARMSTRONG_CYCLE_V1"
...
CONFIG_SCHEMA_VERSION = "step29m_armstrong_cycle_v1_economic_evaluation_admissibility_v1"
...
class ArmstrongCycleV1AdmissibilityContractResultV1:
...
def load_armstrong_cycle_v1_evaluation_config_v1(
    repo_root: Path,
...
def verify_armstrong_cycle_v1_strategy_identity_v1() -> tuple[str, ...]:
    reasons: list[str] = []
    resolution = resolve_strategy_id(ARMSTRONG_CYCLE_V1_STRATEGY_ID)
    if resolution.canonical_strategy_id != ARMSTRONG_CYCLE_V1_STRATEGY_ID:
        reasons.append("strategy_id_not_canonical")
    entry = get_strategy_registry_entry(ARMSTRONG_CYCLE_V1_STRATEGY_ID)
    if entry.strategy_version != ARMSTRONG_CYCLE_V1_STRATEGY_VERSION:
        reasons.append("strategy_version_mismatch")
    if entry.implementation_ref != ARMSTRONG_CYCLE_V1_STRATEGY_OWNER:
...
def verify_armstrong_cycle_v1_strategy_params_v1(
    configured: Mapping[str, Any],
    effective: Mapping[str, Any],
) -> tuple[str, ...]:
    reasons: list[str] = []
    if set(configured.keys()) - set(ARMSTRONG_CYCLE_V1_CANONICAL_PARAMS):
    ...
    if set(effective.keys()) != set(ARMSTRONG_CYCLE_V1_CANONICAL_PARAMS):
        reasons.append("unknown_effective_strategy_param")
    for key, expected in ARMSTRONG_CYCLE_V1_CANONICAL_PARAMS.items():
        if effective.get(key) != expected:
            reasons.append(f"{key}_not_canonical")

    cycle_length, cycle_reasons = _validate_integer_window_v1(
        "cycle_length_days",
        effective.get("cycle_length_days"),
        minimum=CYCLE_LENGTH_DAYS_MIN,
        maximum=CYCLE_LENGTH_DAYS_MAX,
    )
    reasons.extend(cycle_reasons)
    _, event_reasons = _validate_integer_window_v1(
        "event_window_days",
        effective.get("event_window_days"),
        minimum=EVENT_WINDOW_DAYS_MIN,
        maximum=EVENT_WINDOW_DAYS_MAX,
    )
    reasons.extend(event_reasons)
    if cycle_length is not None:
        event_window = effective.get("event_window_days")
        if isinstance(event_window, int) and event_window >= cycle_length // 2:
            reasons.append("event_window_days_not_lt_half_cycle")

    reference_date = effective.get("reference_date")
    if not isinstance(reference_date, str) or not reference_date:
        reasons.append("reference_date_missing")
    elif reference_date != ARMSTRONG_CYCLE_V1_CANONICAL_PARAMS["reference_date"]:
        reasons.append("reference_date_not_canonical")

    phase_map = effective.get("phase_position_map")
    if phase_map != "default":
        reasons.append("phase_position_map_not_default")
    return tuple(reasons)


def verify_armstrong_cycle_v1_sizing_policy_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
...
def verify_armstrong_cycle_v1_instrument_binding_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
...
def verify_armstrong_cycle_v1_ratification_authority_v1(
    cfg: Mapping[str, Any],
) -> tuple[str, ...]:
    reasons: list[str] = []
    ...
    if ratification.get("strategy_id") != ARMSTRONG_CYCLE_V1_STRATEGY_ID:
        reasons.append("ratification_strategy_id_mismatch")
    if ratification.get("strategy_version") != ARMSTRONG_CYCLE_V1_STRATEGY_VERSION:
...
def verify_armstrong_cycle_v1_config_schema_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    ...
    if eval_section.get("strategy_id") != ARMSTRONG_CYCLE_V1_STRATEGY_ID:
    ...
    if not isinstance(strategy_params, Mapping):
        ...
        if set(strategy_params.keys()) != set(ARMSTRONG_CYCLE_V1_CANONICAL_PARAMS):
            reasons.append("unknown_strategy_params_in_config")

    return tuple(reasons)


def verify_armstrong_cycle_v1_signal_binding_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    configured = collect_configured_strategy_params_v1(cfg, ARMSTRONG_CYCLE_V1_STRATEGY_ID)
    try:
        ...
        warmup = compute_required_warmup_rows_v1(ARMSTRONG_CYCLE_V1_STRATEGY_ID, effective)
        if warmup != ARMSTRONG_CYCLE_V1_REQUIRED_WARMUP_ROWS:
...
def evaluate_armstrong_cycle_v1_admissibility_contract_v1(
    *,
    repo_root: Path,
    config_path: Optional[str] = None,
) -> ArmstrongCycleV1AdmissibilityContractResultV1:
    blocking: list[str] = []
    rel_path = config_path or DEFAULT_EVALUATION_CONFIG_PATH
    cfg = load_armstrong_cycle_v1_evaluation_config_v1(repo_root, rel_path)
    config_digest = compute_evaluation_config_digest_v1(cfg)

    blocking.extend(verify_armstrong_cycle_v1_strategy_identity_v1())
    blocking.extend(verify_armstrong_cycle_v1_config_schema_v1(cfg))
    blocking.extend(verify_armstrong_cycle_v1_sizing_policy_v1(cfg))
    blocking.extend(verify_armstrong_cycle_v1_instrument_binding_v1(cfg))
    blocking.extend(verify_armstrong_cycle_v1_ratification_authority_v1(cfg))
    blocking.extend(verify_armstrong_cycle_v1_signal_binding_v1(cfg))
    ...
    try:
        ...
        effective = dict(ARMSTRONG_CYCLE_V1_CANONICAL_PARAMS)
        params_digest = ""
        warmup = ARMSTRONG_CYCLE_V1_REQUIRED_WARMUP_ROWS

    blocking.extend(verify_armstrong_cycle_v1_strategy_params_v1(configured, effective))

    admissibility = AdmissibilityResult.PASS if not blocking else AdmissibilityResult.BLOCKED
    return ArmstrongCycleV1AdmissibilityContractResultV1(
        ...
        strategy_owner=ARMSTRONG_CYCLE_V1_STRATEGY_OWNER,
