...
def run_portfolio_from_registry(
    ...
    logger.info(
        f"Running portfolio backtest with {len(selected_strategies)} strategies: {selected_strategies}"
    )
...
def _combine_portfolio_results(
    ...
    if method == "equal_weight":
        ...
        sharpes = {
            name: result.stats.get("sharpe", 0.0) for name, result in strategy_results.items()
        }
