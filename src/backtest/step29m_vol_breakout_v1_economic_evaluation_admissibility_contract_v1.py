"""
STEP 29M vol_breakout v1 economic evaluation admissibility contract v1.
...
CONTRACT_OWNER = "backtest.step29m_vol_breakout_v1_economic_evaluation_admissibility_contract_v1"

VOL_BREAKOUT_V1_STRATEGY_ID = "vol_breakout"
VOL_BREAKOUT_V1_STRATEGY_VERSION = "v1"
VOL_BREAKOUT_V1_STRATEGY_OWNER = "src.strategies.vol_breakout"
VOL_BREAKOUT_V1_CANONICAL_PARAMS = {
    "lookback_breakout": 20,
    "vol_window": 14,
    "vol_percentile": 50.0,
    "side": "both",
}
VOL_BREAKOUT_V1_REQUIRED_WARMUP_ROWS = 40

LOOKBACK_BREAKOUT_MIN = 2
LOOKBACK_BREAKOUT_MAX = 500
VOL_WINDOW_MIN = 2
VOL_WINDOW_MAX = 500
VOL_PERCENTILE_MIN = 0.0
VOL_PERCENTILE_MAX = 100.0
ALLOWED_SIDE_VALUES = frozenset({"both"})
EXCLUDED_BINDING_PARAMS = frozenset({"atr_multiple"})
...
OPERATOR_POLICY_DERIVATION_REF = "operator_policy_decision:STEP29M_VOL_BREAKOUT_V1"
...
CONFIG_SCHEMA_VERSION = "step29m_vol_breakout_v1_economic_evaluation_admissibility_v1"
...
class VolBreakoutV1AdmissibilityContractResultV1:
...
def load_vol_breakout_v1_evaluation_config_v1(
    repo_root: Path,
...
def verify_vol_breakout_v1_strategy_identity_v1() -> tuple[str, ...]:
    reasons: list[str] = []
    resolution = resolve_strategy_id(VOL_BREAKOUT_V1_STRATEGY_ID)
    if resolution.canonical_strategy_id != VOL_BREAKOUT_V1_STRATEGY_ID:
        reasons.append("strategy_id_not_canonical")
    entry = get_strategy_registry_entry(VOL_BREAKOUT_V1_STRATEGY_ID)
    if entry.strategy_version != VOL_BREAKOUT_V1_STRATEGY_VERSION:
        reasons.append("strategy_version_mismatch")
    if entry.implementation_ref != VOL_BREAKOUT_V1_STRATEGY_OWNER:
...
def _validate_vol_percentile_v1(value: Any) -> tuple[str, ...]:
    if value is None:
        return ("vol_percentile_missing",)
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        return ("vol_percentile_not_numeric",)
    if float(value) < VOL_PERCENTILE_MIN or float(value) > VOL_PERCENTILE_MAX:
        return ("vol_percentile_out_of_range",)
    return ()


def _validate_side_v1(value: Any) -> tuple[str, ...]:
    if not isinstance(value, str):
        return ("side_not_string",)
    if value not in ALLOWED_SIDE_VALUES:
        return ("side_not_allowed",)
    return ()


def verify_vol_breakout_v1_strategy_params_v1(
    configured: Mapping[str, Any],
    effective: Mapping[str, Any],
) -> tuple[str, ...]:
    reasons: list[str] = []
    if set(configured.keys()) - set(VOL_BREAKOUT_V1_CANONICAL_PARAMS):
    ...
    if set(effective.keys()) != set(VOL_BREAKOUT_V1_CANONICAL_PARAMS):
        reasons.append("unknown_effective_strategy_param")
    for key, expected in VOL_BREAKOUT_V1_CANONICAL_PARAMS.items():
        if effective.get(key) != expected:
            reasons.append(f"{key}_not_canonical")

    _, lookback_reasons = _validate_integer_window_v1(
        "lookback_breakout",
        effective.get("lookback_breakout"),
        minimum=LOOKBACK_BREAKOUT_MIN,
        maximum=LOOKBACK_BREAKOUT_MAX,
    )
    reasons.extend(lookback_reasons)
    _, vol_window_reasons = _validate_integer_window_v1(
        "vol_window",
        effective.get("vol_window"),
        minimum=VOL_WINDOW_MIN,
        maximum=VOL_WINDOW_MAX,
    )
    reasons.extend(vol_window_reasons)
    reasons.extend(_validate_vol_percentile_v1(effective.get("vol_percentile")))
    reasons.extend(_validate_side_v1(effective.get("side")))
    return tuple(reasons)


def verify_vol_breakout_v1_sizing_policy_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
...
def verify_vol_breakout_v1_instrument_binding_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
...
def verify_vol_breakout_v1_ratification_authority_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    ...
    if ratification.get("strategy_id") != VOL_BREAKOUT_V1_STRATEGY_ID:
        reasons.append("ratification_strategy_id_mismatch")
    if ratification.get("strategy_version") != VOL_BREAKOUT_V1_STRATEGY_VERSION:
...
def verify_vol_breakout_v1_config_schema_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    ...
    if eval_section.get("strategy_id") != VOL_BREAKOUT_V1_STRATEGY_ID:
    ...
    if not isinstance(strategy_params, Mapping):
        ...
        _, lookback_reasons = _validate_integer_window_v1(
            "lookback_breakout",
            strategy_params.get("lookback_breakout"),
            minimum=LOOKBACK_BREAKOUT_MIN,
            maximum=LOOKBACK_BREAKOUT_MAX,
        )
        reasons.extend(lookback_reasons)
        _, vol_window_reasons = _validate_integer_window_v1(
            "vol_window",
            strategy_params.get("vol_window"),
            minimum=VOL_WINDOW_MIN,
            maximum=VOL_WINDOW_MAX,
        )
        reasons.extend(vol_window_reasons)
        reasons.extend(_validate_vol_percentile_v1(strategy_params.get("vol_percentile")))
        reasons.extend(_validate_side_v1(strategy_params.get("side")))
        if set(strategy_params.keys()) != set(VOL_BREAKOUT_V1_CANONICAL_PARAMS):
            reasons.append("unknown_strategy_params_in_config")

    return tuple(reasons)


def verify_vol_breakout_v1_signal_binding_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    configured = collect_configured_strategy_params_v1(cfg, VOL_BREAKOUT_V1_STRATEGY_ID)
    try:
        effective, _ = resolve_effective_strategy_params_v1(
            VOL_BREAKOUT_V1_STRATEGY_ID,
            project_strategy_params_for_binding_v1(
                VOL_BREAKOUT_V1_STRATEGY_ID,
                configured,
            ),
        )
        warmup = compute_required_warmup_rows_v1(VOL_BREAKOUT_V1_STRATEGY_ID, effective)
        if warmup != VOL_BREAKOUT_V1_REQUIRED_WARMUP_ROWS:
...
def evaluate_vol_breakout_v1_admissibility_contract_v1(
    *,
    repo_root: Path,
    config_path: Optional[str] = None,
) -> VolBreakoutV1AdmissibilityContractResultV1:
    blocking: list[str] = []
    rel_path = config_path or DEFAULT_EVALUATION_CONFIG_PATH
    cfg = load_vol_breakout_v1_evaluation_config_v1(repo_root, rel_path)
    config_digest = compute_evaluation_config_digest_v1(cfg)

    blocking.extend(verify_vol_breakout_v1_strategy_identity_v1())
    blocking.extend(verify_vol_breakout_v1_config_schema_v1(cfg))
    blocking.extend(verify_vol_breakout_v1_sizing_policy_v1(cfg))
    blocking.extend(verify_vol_breakout_v1_instrument_binding_v1(cfg))
    blocking.extend(verify_vol_breakout_v1_ratification_authority_v1(cfg))
    blocking.extend(verify_vol_breakout_v1_signal_binding_v1(cfg))
    ...
    try:
        ...
        effective = dict(VOL_BREAKOUT_V1_CANONICAL_PARAMS)
        params_digest = ""
        warmup = VOL_BREAKOUT_V1_REQUIRED_WARMUP_ROWS

    blocking.extend(verify_vol_breakout_v1_strategy_params_v1(configured, effective))

    admissibility = AdmissibilityResult.PASS if not blocking else AdmissibilityResult.BLOCKED
    return VolBreakoutV1AdmissibilityContractResultV1(
        admissibility_result=admissibility,
        blocking_reasons=tuple(sorted(set(blocking))),
        strategy_id=VOL_BREAKOUT_V1_STRATEGY_ID,
        strategy_version=VOL_BREAKOUT_V1_STRATEGY_VERSION,
        strategy_owner=VOL_BREAKOUT_V1_STRATEGY_OWNER,
        ...
        signal_semantics="LONG_SHORT_FLAT_NEG1_0_1",
