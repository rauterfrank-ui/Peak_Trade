"""
STEP 29M el_karoui_vol_model v1 economic evaluation admissibility contract v1.
...
CONTRACT_OWNER = (
    "backtest.step29m_el_karoui_vol_model_v1_economic_evaluation_admissibility_contract_v1"
)

EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID = "el_karoui_vol_model"
EL_KAROUI_VOL_MODEL_V1_STRATEGY_VERSION = "v1"
EL_KAROUI_VOL_MODEL_V1_STRATEGY_OWNER = (
    "src.strategies.el_karoui.el_karoui_vol_model_strategy.ElKarouiVolatilityStrategy"
)
EL_KAROUI_VOL_MODEL_V1_CANONICAL_PARAMS = {
    "vol_window": 20,
    "lookback_window": 252,
    "low_threshold": 0.30,
    "high_threshold": 0.70,
}
EL_KAROUI_VOL_MODEL_V1_REQUIRED_WARMUP_ROWS = 252

VOL_WINDOW_MIN = 2
VOL_WINDOW_MAX = 120
LOOKBACK_WINDOW_MIN = 20
LOOKBACK_WINDOW_MAX = 1000
LOW_THRESHOLD_MIN = 0.01
LOW_THRESHOLD_MAX = 0.99
HIGH_THRESHOLD_MIN = 0.01
HIGH_THRESHOLD_MAX = 0.99
...
OPERATOR_POLICY_DERIVATION_REF = "operator_policy_decision:STEP29M_EL_KAROUI_VOL_MODEL_V1"
...
CONFIG_SCHEMA_VERSION = "step29m_el_karoui_vol_model_v1_economic_evaluation_admissibility_v1"
...
class ElKarouiVolModelV1AdmissibilityContractResultV1:
...
def load_el_karoui_vol_model_v1_evaluation_config_v1(
    repo_root: Path,
...
def verify_el_karoui_vol_model_v1_strategy_identity_v1() -> tuple[str, ...]:
    reasons: list[str] = []
    resolution = resolve_strategy_id(EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID)
    if resolution.canonical_strategy_id != EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID:
        reasons.append("strategy_id_not_canonical")
    entry = get_strategy_registry_entry(EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID)
    if entry.strategy_version != EL_KAROUI_VOL_MODEL_V1_STRATEGY_VERSION:
        reasons.append("strategy_version_mismatch")
    if entry.implementation_ref != EL_KAROUI_VOL_MODEL_V1_STRATEGY_OWNER:
...
def verify_el_karoui_vol_model_v1_strategy_params_v1(
    configured: Mapping[str, Any],
    effective: Mapping[str, Any],
) -> tuple[str, ...]:
    reasons: list[str] = []
    if set(configured.keys()) - set(EL_KAROUI_VOL_MODEL_V1_CANONICAL_PARAMS):
    ...
    if set(effective.keys()) != set(EL_KAROUI_VOL_MODEL_V1_CANONICAL_PARAMS):
        reasons.append("unknown_effective_strategy_param")
    for key, expected in EL_KAROUI_VOL_MODEL_V1_CANONICAL_PARAMS.items():
        if effective.get(key) != expected:
            reasons.append(f"{key}_not_canonical")

    _, vol_reasons = _validate_integer_window_v1(
        "vol_window",
        effective.get("vol_window"),
        minimum=VOL_WINDOW_MIN,
        maximum=VOL_WINDOW_MAX,
    )
    reasons.extend(vol_reasons)
    _, lookback_reasons = _validate_integer_window_v1(
        "lookback_window",
        effective.get("lookback_window"),
        minimum=LOOKBACK_WINDOW_MIN,
        maximum=LOOKBACK_WINDOW_MAX,
    )
    reasons.extend(lookback_reasons)
    _, low_reasons = _validate_threshold_v1(
        "low_threshold",
        effective.get("low_threshold"),
        minimum=LOW_THRESHOLD_MIN,
        maximum=LOW_THRESHOLD_MAX,
    )
    reasons.extend(low_reasons)
    _, high_reasons = _validate_threshold_v1(
        "high_threshold",
        effective.get("high_threshold"),
        minimum=HIGH_THRESHOLD_MIN,
        maximum=HIGH_THRESHOLD_MAX,
    )
    reasons.extend(high_reasons)
    low = effective.get("low_threshold")
    high = effective.get("high_threshold")
    if isinstance(low, (int, float)) and isinstance(high, (int, float)):
        if float(low) >= float(high):
            reasons.append("low_threshold_not_lt_high_threshold")
    return tuple(reasons)


def verify_el_karoui_vol_model_v1_sizing_policy_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
...
def verify_el_karoui_vol_model_v1_instrument_binding_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
...
def verify_el_karoui_vol_model_v1_ratification_authority_v1(
    cfg: Mapping[str, Any],
) -> tuple[str, ...]:
    reasons: list[str] = []
    ...
    if ratification.get("strategy_id") != EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID:
        reasons.append("ratification_strategy_id_mismatch")
    if ratification.get("strategy_version") != EL_KAROUI_VOL_MODEL_V1_STRATEGY_VERSION:
...
def verify_el_karoui_vol_model_v1_config_schema_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    ...
    if eval_section.get("strategy_id") != EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID:
    ...
    if not isinstance(strategy_params, Mapping):
        ...
        if set(strategy_params.keys()) != set(EL_KAROUI_VOL_MODEL_V1_CANONICAL_PARAMS):
            reasons.append("unknown_strategy_params_in_config")

    return tuple(reasons)


def verify_el_karoui_vol_model_v1_signal_binding_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    configured = collect_configured_strategy_params_v1(cfg, EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID)
    try:
        effective, _ = resolve_effective_strategy_params_v1(
            EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID,
            project_strategy_params_for_binding_v1(
                EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID,
                configured,
            ),
        )
        warmup = compute_required_warmup_rows_v1(EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID, effective)
        if warmup != EL_KAROUI_VOL_MODEL_V1_REQUIRED_WARMUP_ROWS:
...
def evaluate_el_karoui_vol_model_v1_admissibility_contract_v1(
    *,
    repo_root: Path,
    config_path: Optional[str] = None,
) -> ElKarouiVolModelV1AdmissibilityContractResultV1:
    blocking: list[str] = []
    rel_path = config_path or DEFAULT_EVALUATION_CONFIG_PATH
    cfg = load_el_karoui_vol_model_v1_evaluation_config_v1(repo_root, rel_path)
    config_digest = compute_evaluation_config_digest_v1(cfg)

    blocking.extend(verify_el_karoui_vol_model_v1_strategy_identity_v1())
    blocking.extend(verify_el_karoui_vol_model_v1_config_schema_v1(cfg))
    blocking.extend(verify_el_karoui_vol_model_v1_sizing_policy_v1(cfg))
    blocking.extend(verify_el_karoui_vol_model_v1_instrument_binding_v1(cfg))
    blocking.extend(verify_el_karoui_vol_model_v1_ratification_authority_v1(cfg))
    blocking.extend(verify_el_karoui_vol_model_v1_signal_binding_v1(cfg))
    ...
    try:
        ...
        effective = dict(EL_KAROUI_VOL_MODEL_V1_CANONICAL_PARAMS)
        params_digest = ""
        warmup = EL_KAROUI_VOL_MODEL_V1_REQUIRED_WARMUP_ROWS

    blocking.extend(verify_el_karoui_vol_model_v1_strategy_params_v1(configured, effective))

    admissibility = AdmissibilityResult.PASS if not blocking else AdmissibilityResult.BLOCKED
    return ElKarouiVolModelV1AdmissibilityContractResultV1(
        admissibility_result=admissibility,
        blocking_reasons=tuple(sorted(set(blocking))),
        strategy_id=EL_KAROUI_VOL_MODEL_V1_STRATEGY_ID,
        strategy_version=EL_KAROUI_VOL_MODEL_V1_STRATEGY_VERSION,
        strategy_owner=EL_KAROUI_VOL_MODEL_V1_STRATEGY_OWNER,
