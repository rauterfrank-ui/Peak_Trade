"""
STEP 29M ehlers_cycle_filter v1 economic evaluation admissibility contract v1.
...
CONTRACT_OWNER = (
    "backtest.step29m_ehlers_cycle_filter_v1_economic_evaluation_admissibility_contract_v1"
)

EHLERS_CYCLE_FILTER_V1_STRATEGY_ID = "ehlers_cycle_filter"
EHLERS_CYCLE_FILTER_V1_STRATEGY_VERSION = "v1"
EHLERS_CYCLE_FILTER_V1_STRATEGY_OWNER = (
    "src.strategies.ehlers.ehlers_cycle_filter_strategy.EhlersCycleFilterStrategy"
)
EHLERS_CYCLE_FILTER_V1_CANONICAL_PARAMS = {
    "min_cycle_length": 6,
    "lookback": 100,
}
EHLERS_CYCLE_FILTER_V1_REQUIRED_WARMUP_ROWS = 100

MIN_CYCLE_LENGTH_MIN = 2
MIN_CYCLE_LENGTH_MAX = 100
LOOKBACK_MIN = 20
LOOKBACK_MAX = 500
...
OPERATOR_POLICY_DERIVATION_REF = "operator_policy_decision:STEP29M_EHLERS_CYCLE_FILTER_V1"
...
class EhlersCycleFilterV1AdmissibilityContractResultV1:
...
def load_ehlers_cycle_filter_v1_evaluation_config_v1(
    repo_root: Path,
...
def _join_ehlers_v1_dataset_path(root: Path, relpath: str) -> Path:
    return join_runtime_evidence_relpath(
        root, relpath, error_code="EHLERS_V1_DATASET_RELPATH_NOT_RELATIVE"
    )
...
def verify_ehlers_cycle_filter_v1_strategy_identity_v1() -> tuple[str, ...]:
    reasons: list[str] = []
    resolution = resolve_strategy_id(EHLERS_CYCLE_FILTER_V1_STRATEGY_ID)
    if resolution.canonical_strategy_id != EHLERS_CYCLE_FILTER_V1_STRATEGY_ID:
        reasons.append("strategy_id_not_canonical")
    entry = get_strategy_registry_entry(EHLERS_CYCLE_FILTER_V1_STRATEGY_ID)
    if entry.strategy_version != EHLERS_CYCLE_FILTER_V1_STRATEGY_VERSION:
        reasons.append("strategy_version_mismatch")
    if entry.implementation_ref != EHLERS_CYCLE_FILTER_V1_STRATEGY_OWNER:
...
def verify_ehlers_cycle_filter_v1_strategy_params_v1(
    configured: Mapping[str, Any],
    effective: Mapping[str, Any],
) -> tuple[str, ...]:
    reasons: list[str] = []
    if set(configured.keys()) - set(EHLERS_CYCLE_FILTER_V1_CANONICAL_PARAMS):
    ...
    if set(effective.keys()) != set(EHLERS_CYCLE_FILTER_V1_CANONICAL_PARAMS):
        reasons.append("unknown_effective_strategy_param")
    for key, expected in EHLERS_CYCLE_FILTER_V1_CANONICAL_PARAMS.items():
        if effective.get(key) != expected:
            reasons.append(f"{key}_not_canonical")

    _, min_cycle_reasons = _validate_integer_window_v1(
        "min_cycle_length",
        effective.get("min_cycle_length"),
        minimum=MIN_CYCLE_LENGTH_MIN,
        maximum=MIN_CYCLE_LENGTH_MAX,
    )
    reasons.extend(min_cycle_reasons)
    _, lookback_reasons = _validate_integer_window_v1(
        "lookback",
        effective.get("lookback"),
        minimum=LOOKBACK_MIN,
        maximum=LOOKBACK_MAX,
    )
    reasons.extend(lookback_reasons)
    return tuple(reasons)


def verify_ehlers_cycle_filter_v1_sizing_policy_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
...
def verify_ehlers_cycle_filter_v1_instrument_binding_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    binding = cfg.get("real_admissible_futures_evaluation_binding_v1")
    if not isinstance(binding, Mapping):
        return ("real_admissible_futures_evaluation_binding_v1_missing",)
...
def verify_ehlers_cycle_filter_v1_ratification_authority_v1(
    cfg: Mapping[str, Any],
) -> tuple[str, ...]:
    reasons: list[str] = []
    ...
    if ratification.get("strategy_id") != EHLERS_CYCLE_FILTER_V1_STRATEGY_ID:
        reasons.append("ratification_strategy_id_mismatch")
    if ratification.get("strategy_version") != EHLERS_CYCLE_FILTER_V1_STRATEGY_VERSION:
...
def verify_ehlers_cycle_filter_v1_config_schema_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    ...
    if eval_section.get("strategy_id") != EHLERS_CYCLE_FILTER_V1_STRATEGY_ID:
    ...
    if not isinstance(strategy_params, Mapping):
        ...
        _, min_cycle_reasons = _validate_integer_window_v1(
            "min_cycle_length",
            strategy_params.get("min_cycle_length"),
            minimum=MIN_CYCLE_LENGTH_MIN,
            maximum=MIN_CYCLE_LENGTH_MAX,
        )
        reasons.extend(min_cycle_reasons)
        _, lookback_reasons = _validate_integer_window_v1(
            "lookback",
            strategy_params.get("lookback"),
            minimum=LOOKBACK_MIN,
            maximum=LOOKBACK_MAX,
        )
        reasons.extend(lookback_reasons)
        if set(strategy_params.keys()) != set(EHLERS_CYCLE_FILTER_V1_CANONICAL_PARAMS):
            reasons.append("unknown_strategy_params_in_config")

    return tuple(reasons)


def verify_ehlers_cycle_filter_v1_signal_binding_v1(cfg: Mapping[str, Any]) -> tuple[str, ...]:
    reasons: list[str] = []
    configured = collect_configured_strategy_params_v1(cfg, EHLERS_CYCLE_FILTER_V1_STRATEGY_ID)
    try:
        effective, _ = resolve_effective_strategy_params_v1(
            EHLERS_CYCLE_FILTER_V1_STRATEGY_ID,
            project_strategy_params_for_binding_v1(
                EHLERS_CYCLE_FILTER_V1_STRATEGY_ID,
                configured,
            ),
        )
        warmup = compute_required_warmup_rows_v1(EHLERS_CYCLE_FILTER_V1_STRATEGY_ID, effective)
        if warmup != EHLERS_CYCLE_FILTER_V1_REQUIRED_WARMUP_ROWS:
...
def evaluate_ehlers_cycle_filter_v1_admissibility_contract_v1(
    *,
    repo_root: Path,
    config_path: Optional[str] = None,
) -> EhlersCycleFilterV1AdmissibilityContractResultV1:
    blocking: list[str] = []
    rel_path = config_path or DEFAULT_EVALUATION_CONFIG_PATH
    cfg = load_ehlers_cycle_filter_v1_evaluation_config_v1(repo_root, rel_path)
    config_digest = compute_evaluation_config_digest_v1(cfg)

    blocking.extend(verify_ehlers_cycle_filter_v1_strategy_identity_v1())
    ...
    try:
        ...
        effective = dict(EHLERS_CYCLE_FILTER_V1_CANONICAL_PARAMS)
        params_digest = ""
        warmup = EHLERS_CYCLE_FILTER_V1_REQUIRED_WARMUP_ROWS

    blocking.extend(verify_ehlers_cycle_filter_v1_strategy_params_v1(configured, effective))

    admissibility = AdmissibilityResult.PASS if not blocking else AdmissibilityResult.BLOCKED
    return EhlersCycleFilterV1AdmissibilityContractResultV1(
        admissibility_result=admissibility,
        blocking_reasons=tuple(sorted(set(blocking))),
        strategy_id=EHLERS_CYCLE_FILTER_V1_STRATEGY_ID,
        strategy_version=EHLERS_CYCLE_FILTER_V1_STRATEGY_VERSION,
        strategy_owner=EHLERS_CYCLE_FILTER_V1_STRATEGY_OWNER,
