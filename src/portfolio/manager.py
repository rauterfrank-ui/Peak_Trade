...
class PortfolioManager:
    ...
    def add_strategy(
        ...
        allocation = StrategyAllocation(
            name=name, signal_fn=signal_fn, params=params, weight=weight or 0.0, capital=0.0
        )
    ...
    def run_backtest(self, df: pd.DataFrame, allocation_method: str = "equal") -> PortfolioResult:
        ...
        for strat in self.strategies:
            ...
            result = engine.run_realistic(
                df=df, strategy_signal_fn=strat.signal_fn, strategy_params=strat.params
            )
        ...
        portfolio_stats = self._compute_portfolio_stats(portfolio_equity, strategy_results)
    ...
    def _combine_equity_curves(self, strategy_results: Dict[str, BacktestResult]) -> pd.Series:
        """
    ...
    def _compute_portfolio_stats(
        self, portfolio_equity: pd.Series, strategy_results: Dict[str, BacktestResult]
    ) -> Dict[str, float]:
