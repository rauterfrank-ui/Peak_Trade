"""Single-run DEVELOPMENT panel evaluation: baseline vs MACD histogram-countertrend eligibility gate."""
...
EVALUATION_RUN_ID = "evaluate_macd_histogram_countertrend_mr_eligibility_development_v1"
...
def _optional_treatment_entry_eligibility_gate(
    *, enabled: bool, bars: pd.DataFrame | None
    ...
    configured-strategy signal series. This gate wraps
    ``map_decision_evidence_to_position_signal_v1`` itself: it tracks the
    current bar timestamp (via wrapping ``bind_bar_for_mv2_wiring_v1`` and the
    warmup-observation bind path), looks up MACD histogram at that
    timestamp (via ``.asof``), determines warmup by timestamp membership in
    the frozen first-``warmup_bars`` window, and applies the SIDE-AWARE
    countertrend admission rule (``is_entry_eligible``): a mapped ``+1`` (long
    entry-candidate) requires ``histogram < 0``; a mapped ``-1`` (short
    entry-candidate) requires ``histogram > 0``. Mapped ``0`` (flat/exit)
    always passes through unchanged. Returns a mutable counters dict with
    ...
    hist_s = compute_macd_histogram(bars)
    ...
    def _map_gated(evidence):  # type: ignore[no-untyped-def]
        ...
        ts = state["current_ts"]
        if ts is None or len(hist_s) == 0:
            eligible = False
        else:
            hist_val = hist_s.asof(ts)
            in_warmup = bool(warmup_mask.asof(ts)) if len(warmup_mask) else True
            hist_f = float(hist_val) if pd.notna(hist_val) else float("nan")
            eligible = is_entry_eligible(signal=raw_signal, histogram=hist_f, in_warmup=in_warmup)
...
def _extract_member_economics(
    ...
    *,
    ...
    # MACD histogram-countertrend eligibility attribution on the decision segment.
    # NOTE: this is INFORMATIONAL ONLY — it reflects the (post-gate) resulting
    # trade entry times against the combined (long OR short) eligibility mask
    # for bar-share purposes, not proof that the gate blocked anything. Entry
    # eligibility counting below is SIDE-AWARE: for each trade the side is
    # classified and checked against the corresponding long/short mask at
    # entry_ts, never the OR-combined mask. For the baseline (ungated) arm a
    # non-zero `entries_on_ineligible_mask_bars` means the *unfiltered*
    # strategy would have entered on a bar ineligible for that trade's side;
    # it is NOT the same as `entries_blocked_by_gate`, which is the
    # gate-effective, monkeypatch-based counter of mapped +-1 -> 0 signal
    # overrides (the primary divergence proof).
...
def run_arm(
    *,
    ...
    for i, member in enumerate(members, start=1):
        ...
        # run_arm loops members and histogram series must match the bars
        # currently being replayed by run_mv2_research_backtest_wiring_v1 below.
...
def run_development_evaluation(
    *,
    ...
    # (MACD histogram-countertrend side-aware eligibility gated) — two arms, one run.
    baseline = run_arm(
        arm="baseline",
        cfg=cfg,
        archive_root=root,
    ...
    # a bare mask-based count, it is only non-zero if the gate genuinely
    # altered MV2 replay behavior.
    ...
    # original bug in a prior evaluation). Ship no economics in that state.
    if (
    ...
    config_snapshot = {
        "schema_version": "evaluate_macd_histogram_countertrend_mr_eligibility_development_config_snapshot.v1",
    ...
    code_hashes = {
        "macd_histogram_countertrend_eligibility_filter_v1.py": _sha256_file(
            repo
            / "src/research/macd_histogram_countertrend_mr_eligibility_development_evaluation_v1"
            / "macd_histogram_countertrend_eligibility_filter_v1.py"
        ),
    ...
    command = (
        ...
        "run_evaluate_macd_histogram_countertrend_mr_eligibility_development_v1.py "
        f"--output-dir {output_dir}"
    )
    base_sha = _git_base_sha(repo)
    summary = {
        "schema_version": "evaluate_macd_histogram_countertrend_mr_eligibility_development_summary.v1",
