"""Funding-rate extreme carry/reversion single-slot research orchestrator v0."""
...
PACKAGE_MARKER = (
    "CROSS_SECTIONAL_FUNDING_RATE_EXTREME_CARRY_REVERSION_SINGLE_SLOT_RESEARCH_ORCHESTRATOR_V0=true"
)
ORCHESTRATOR_VERSION = (
    "cross_sectional_funding_rate_extreme_carry_reversion_single_slot_research_orchestrator.v0"
)

FORBIDDEN_INSTRUMENT_TOKENS = frozenset({"btc", "xbt", "bitcoin"})


class FundingExtremeCarryOrchestratorErrorCode(str, Enum):
...
def run_cross_sectional_funding_rate_extreme_carry_reversion_orchestrator_v0(
    *,
    ...
    if not series_by_id:
        raise ValueError(FundingExtremeCarryOrchestratorErrorCode.PANEL_VALIDATION_FAILED.value)
    ...
    for epoch_index in range(bar_count):
        ...
        if len(epoch_scores) < min_eligible or selection_extreme.leg is FundingCarryLeg.FLAT:
            ...
            top_score = None
            error_codes.append(
                FundingExtremeCarryOrchestratorErrorCode.INSUFFICIENT_ELIGIBLE_MEMBERS.value
            )
        else:
...
def default_funding_extreme_carry_reversion_operator_binding_v0() -> Mapping[str, Any]:
