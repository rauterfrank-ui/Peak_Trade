...
Registers cross_sectional_open_interest_zscore_reversion/v0 as terminal for the exact unchanged
binding after offline economic evaluation with INSUFFICIENT_TRADE_SAMPLE, blocks same-binding
retry, and ratifies cross_sectional_futures_lead_lag_information_diffusion/v0 as the materially
distinct successor scope. Offline-only; no economic evaluation execution in this slice.
"""
...
VERSIONED_BINDING_CONFIG_REL_PATH = "config/research/cross_sectional_open_interest_zscore_reversion_v0_versioned_hypothesis_binding_v0.json"
LEAD_LAG_SCOPE_RATIFICATION_CONFIG_REL_PATH = (
    "config/research/"
...
CANONICAL_EVALUATION_DIR = (
    ...
    "evaluation_execution_v0_20260715T020351Z"
)
OPERATOR_RATIFICATION_DIR = (
    DURABLE_ARCHIVE_ROOT
    / "research/operator_ratification_distinct_robustness_or_new_research_scope_decision_"
    "post_pr5194_v0_20260715T015805Z"
)
PR5194_CLOSEOUT_DIR = (
    DURABLE_ARCHIVE_ROOT
    / "research/pr5194_merge_closeout_bouchaud_microstructure_ohlcv_proxy_v1_offline_"
    "productive_linear_diagnostics_and_promotion_binding_completion_reconciliation_v0_"
    "20260715T015047Z"
)

CANONICAL_EVALUATION_TIMESTAMP = "20260715T020351Z"
PRE_MERGE_ORIGIN_MAIN = "bb2e7ed2d389ca5f2648a032385eb914cf28e8f8"
...
NET_RETURN = -0.004188946023513163
TRADE_COUNT = 1
...
SELECTED_DISTINCT_STRATEGY_ID = "cross_sectional_futures_lead_lag_information_diffusion"
SELECTED_DISTINCT_STRATEGY_VERSION = "v0"
...
SELECTED_DISTINCT_CANONICAL_OWNER = (
    "src/research/cross_sectional_futures_lead_lag_information_diffusion_v0_offline_economic_"
    "evaluation_execution_v0.py"
)
SELECTED_DISTINCT_BINDING_DIGEST = (
    "9e9ab5676d8859d819dad1aed1eaa78163529682492fcc333ead001841e414c1"
)
SELECTED_DISTINCT_MATERIAL_DIFFERENCE_DIGEST = (
    "96de4479dca452f40ad4292370801e3386169cf846b9f7b5320884c24a9163df"
)
...
REQUIRED_CANONICAL_EVIDENCE_FILES = (
    "final_report.txt",
    "immutable_evaluation_contract.json",
    "economic_metrics.json",
    "sample_sufficiency_adjudication.json",
    "trade_or_rotation_ledger.jsonl",
)
...
def validate_registration_preconditions(
    *,
    ...
    operator_ratification_dir: Path = OPERATOR_RATIFICATION_DIR,
    pr5194_closeout_dir: Path = PR5194_CLOSEOUT_DIR,
) -> EvidenceBundleValidation:
    for bundle_dir in (operator_ratification_dir, pr5194_closeout_dir):
        if verify_manifest_sha256(bundle_dir) != 0:
            raise ValueError(f"manifest_verify_failed:{bundle_dir}")
    canonical = validate_evidence_bundle(canonical_dir)
    contract = _load_json(canonical_dir / "immutable_evaluation_contract.json")
    if contract.get("binding_digest") != BINDING_DIGEST:
        raise ValueError("binding_digest_mismatch")
    if contract.get("implementation_digest") != IMPLEMENTATION_DIGEST:
        raise ValueError("implementation_digest_mismatch")
    sample = _load_json(canonical_dir / "sample_sufficiency_adjudication.json")
...
def _metrics_from_canonical(canonical_dir: Path) -> dict[str, Any]:
    performance = _load_json(canonical_dir / "economic_metrics.json")
    sample = _load_json(canonical_dir / "sample_sufficiency_adjudication.json")
    return {
        ...
        "walk_forward_status": "NOT_EXECUTED_BASELINE_INSUFFICIENT_SAMPLE",
        "monte_carlo_status": "NOT_EXECUTED_BASELINE_INSUFFICIENT_SAMPLE",
        "stress_status": "NOT_EXECUTED_BASELINE_INSUFFICIENT_SAMPLE",
    }
    ...
    return {
        ...
        "candidates": [
            ...
            },
            {
                ...
                "status": "REJECTED",
                "rejected": True,
                "rejection_reason": "OI_RANKING_FAMILY_TERMINAL_OR_INCONCLUSIVE",
            },
            {
            ...
            {
                "candidate_type": "NEW_EVIDENCE_CLASS_DEFINITION",
                "evidence_class_id": (
                ...
                "rejected": True,
                "rejection_reason": (
                    "PREFLIGHT_EXECUTION_ADMISSIBLE_SIGNAL_SPARSITY_NOT_DATA_DEPTH_PRIMARY_CAUSE"
                ),
            },
        ],
    }
...
def build_material_difference_matrix() -> dict[str, Any]:
    return {
        ...
        "material_difference_proven": True,
        "material_difference_basis": "DISTINCT_SIGNAL_FAMILY_AND_DISTINCT_DATASET_UNIVERSE",
        "rows": [
            {
                "axis": "signal_mechanism",
                "baseline": "Cross-sectional open-interest z-score reversion extremes",
                "selected": "Panel-median-benchmark lagged return diffusion score",
                "material_difference": True,
            },
            {
                "axis": "dataset_identity",
                "baseline": "Self-accumulated 5-instrument OI panel (60 bars)",
                "selected": "399-instrument PIT OHLCV cross-sectional panel",
                "material_difference": True,
            },
            {
                "axis": "instrument_universe",
                "baseline": "5 fixed OI-eligible linear perpetuals",
                "selected": "Full non-Bitcoin OKX linear USDT perpetual cross-section",
                "material_difference": True,
            },
            {
                "axis": "temporal_structure",
                "baseline": "OI level z-score at lagged observation",
                "selected": "Multi-lag log-return diffusion vs panel median",
                "material_difference": True,
            },
            {
                "axis": "expected_trade_frequency",
                "baseline": "Sparse (observed trade_count=1 under unchanged binding)",
                "selected": "Higher cross-sectional rebalance surface on 399-instrument panel",
                "material_difference": True,
            },
            {
...
def build_exact_binding_retry_guard_report() -> dict[str, Any]:
    return {
        ...
        "blocked_retry_axes": [
            ...
            "OI_PANEL_DEPTH_ONLY_WITHOUT_NEW_HYPOTHESIS",
        ],
    }
...
def build_retry_non_equivalence_proof() -> dict[str, Any]:
    return {
        ...
        "dataset_digest_differs": True,
        "signal_family_differs": True,
...
def materialize_registration_config(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "lead_lag_scope_ratification_config_ref": LEAD_LAG_SCOPE_RATIFICATION_CONFIG_REL_PATH,
        "research_scope": RESEARCH_SCOPE,
        ...
        "operator_ratification_evidence_dir": str(OPERATOR_RATIFICATION_DIR),
        "pr5194_closeout_dir": str(PR5194_CLOSEOUT_DIR),
        ...
        "selected_distinct_binding_digest": SELECTED_DISTINCT_BINDING_DIGEST,
        ...
        "durable_evidence_refs": "; ".join(
            [
                ...
                f"{OPERATOR_RATIFICATION_DIR} (MANIFEST_VERIFY_RC=0)",
                f"{PR5194_CLOSEOUT_DIR} (MANIFEST_VERIFY_RC=0)",
            ]
        ),
        "metrics": metrics,
