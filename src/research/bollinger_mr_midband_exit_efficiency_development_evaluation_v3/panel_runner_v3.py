"""Single-run DEVELOPMENT panel evaluation v3: baseline vs midband exit-efficiency gate.

New independently preregistered DEVELOPMENT_ONLY measurement
(HYPOTHESIS_ID ...DEVELOPMENT_V3). Not a rerun of V1 or V2. Reuses the frozen V1
mechanism/gate/decision modules by import only (definition identical); no V1/V2
partial result, checkpoint, or economic claim is transferred. Binds the already-
merged PANEL_RUNNER_FALSY_ZERO_PREMEASUREMENT_HYGIENE pattern.
"""
...
def _assert_no_predecessor_partial_result_reuse(contract: Mapping[str, Any]) -> None:
    ...
    predecessor = contract.get("predecessor_development_v2") or {}
    if predecessor.get("hypothesis_id") != REQUIRED_PREDECESSOR_HYPOTHESIS_ID:
        raise RuntimeError("PREDECESSOR_V2_HYPOTHESIS_ID_MISMATCH")
    if predecessor.get("partial_results_reused") is not False:
        raise RuntimeError("PREDECESSOR_V2_PARTIAL_RESULTS_REUSED")
...
def run_development_evaluation(
    *,
    ...
    # Validate against the V3 preregistration validator BEFORE any measurement /
    # contract mutation. The V3 validator accepts DEFINITION_ONLY_PREREGISTERED
    ...
    if report.get("hypothesis_id") != HYPOTHESIS_ID:
        raise RuntimeError("CONTRACT_HYPOTHESIS_ID_NOT_V3")
    ...
    (output_dir / "trade_ledger_baseline.json").write_text(
        json.dumps(
            {
                "schema_version": "bollinger_mr_midband_exit_efficiency_trade_ledger.v3",
                "arm": "baseline",
    ...
    config_snapshot = {
        "schema_version": "evaluate_bollinger_mr_midband_exit_efficiency_development_config_snapshot.v3",
    ...
    v3_pkg = repo / "src/research/bollinger_mr_midband_exit_efficiency_development_evaluation_v3"
    code_hashes = {
        ...
        "panel_runner_v3.py": _sha256_file(v3_pkg / "panel_runner_v3.py"),
    ...
    command = (
        ...
        "run_evaluate_bollinger_mr_midband_exit_efficiency_development_v3.py "
        f"--output-dir {output_dir}"
    )
    summary = {
        "schema_version": "evaluate_bollinger_mr_midband_exit_efficiency_development_summary.v3",
    ...
    (output_dir / "safety_attestation.md").write_text(
        "\n".join(
            [
                "# Safety attestation — midband exit-efficiency DEVELOPMENT evaluation v3",
                "",
