"""Single-run DEVELOPMENT panel evaluation: baseline vs entry-effective eligibility gate."""
...
EVALUATION_RUN_ID = "evaluate_entry_effective_mr_eligibility_development_v1"
...
def _extract_member_economics(
    ...
    *,
    ...
    # Entry-effective eligibility attribution on the decision segment.
...
def run_development_evaluation(
    *,
    ...
    # (entry-effective eligibility gated) — two arms, one run.
    baseline = run_arm(
        arm="baseline",
        cfg=cfg,
        archive_root=root,
    ...
    config_snapshot = {
        "schema_version": "evaluate_entry_effective_mr_eligibility_development_config_snapshot.v1",
    ...
    code_hashes = {
        "atr_percentile_eligibility_filter_v1.py": _sha256_file(
            repo
            / "src/research/entry_effective_mr_eligibility_development_evaluation_v1"
            / "atr_percentile_eligibility_filter_v1.py"
        ),
    ...
    command = (
        ...
        "run_evaluate_entry_effective_mr_eligibility_development_v1.py "
        f"--output-dir {output_dir}"
    )
    base_sha = _git_base_sha(repo)
    summary = {
        "schema_version": "evaluate_entry_effective_mr_eligibility_development_summary.v1",
