"""Definition-only preregistration validator for CS short-horizon return reversal v1."""
...
PACKAGE_MARKER = "CROSS_SECTIONAL_SHORT_HORIZON_RETURN_REVERSAL_V1_HYPOTHESIS_PREREGISTRATION=true"
CONTRACT_REL_PATH = (
    "config/research/"
...
GOVERNANCE_REL_PATH = (
    "docs/governance/"
    "CROSS_SECTIONAL_SHORT_HORIZON_RETURN_REVERSAL_V1_PREREGISTERED_HYPOTHESIS_MEASUREMENT_V1.md"
)
EVIDENCE_REL_PATH = (
    "docs/evidence/preregister_cross_sectional_short_horizon_return_reversal_hypothesis_v1/"
)
...
def validate_measurement_contract(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(
        payload.get("strategy_identity") == "CROSS_SECTIONAL_SHORT_HORIZON_RETURN_REVERSAL_V1",
        "STRATEGY_IDENTITY",
    )
    _require(payload.get("signal_family") == "CROSS_SECTIONAL_RETURN_REVERSAL", "SIGNAL_FAMILY")
    _require(
        payload.get("target_phenomenon")
        == "SHORT_HORIZON_CROSS_SECTIONAL_RELATIVE_RETURN_REVERSAL",
        "TARGET_PHENOMENON",
    )
    ...
    _require(
        score.get("score_family_policy") == "negated_raw_trailing_log_return_fixed_lookback_v1",
        "SCORE_FAMILY",
    )
    _require(score.get("not_a_cs_momentum_parameter_retune") is True, "CS_MOMENTUM_RETUNE")
    _require(score.get("polarity") == "REVERSAL_NEGATED_TRAILING_LOG_RETURN", "POLARITY")
    frozen = (payload.get("parameter_governance") or {}).get("frozen_non_grid_parameters") or {}
    _require(frozen.get("lookback_N") == 24, "LOOKBACK_N")
    _require(frozen.get("rebalance_interval_bars") == 4, "REBALANCE_INTERVAL")
