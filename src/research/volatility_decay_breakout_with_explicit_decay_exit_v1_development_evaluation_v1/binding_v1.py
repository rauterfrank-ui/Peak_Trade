"""Deterministic binding/digest resolution for VDBX v1 eval entry point.

Cloned from the VEP development-evaluation binding (NOT from VDB corrective paths),
adapted for strategy-emitted exits: requires the exit state machine, forbids
evaluator-side reconstruction of missing strategy exits, and binds the frozen
productive exit/PnL evaluator reference.
"""
...
def resolve_measurement_contract(repo_root: Path) -> dict[str, Any]:
    ...
    _require(
        contract.get("predecessor_strategy_id") == PREDECESSOR_STRATEGY_ID,
        "PREDECESSOR_STRATEGY_ID_MISMATCH",
    )
    ...
    _require(
        portfolio.get("portfolio_aggregation_id") == PORTFOLIO_AGGREGATION_ID,
    ...
    _require(exit_sem.get("exit_state_machine_implemented") is True, "EXIT_SM_NOT_IMPLEMENTED")
    _require(
        exit_sem.get("evaluator_side_reconstruction_of_missing_strategy_exits_forbidden") is True,
    ...
    _require(
        exit_sem.get("synthetic_fills_solely_to_pair_trades_forbidden") is True,
        "SYNTHETIC_FILLS_ALLOWED",
    )
    return contract
...
def resolve_strategy_params(contract: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not frozen:
        frozen = {
            "atr_normalization": "ATR_DIV_CLOSE",
            "atr_period": 14,
            "channel_lookback_bars": 20,
            "cooldown_bars_after_exit": 0,
            "decay_confirmation_threshold_exclusive_max": 0.4,
            "decay_event_consumption": "SINGLE_USE",
            "decay_rearm_requires_percentile_at_or_above_threshold": True,
            "decay_window_end_offset": 8,
            "decay_window_start_offset": 1,
            "earliest_entry_offset_after_confirmation": 1,
            "exit_precedence": [
                "INITIAL_STOP",
                "TRAILING_STOP",
                "SIGNAL_EXIT",
            ...
            "high_vol_prior_threshold_inclusive_min": 0.7,
            "initial_stop_atr_multiple": 1.5,
            "max_entries_per_decay_event": 1,
            ...
            "regime_invalidation_percentile_lt": 0.5,
            "signal_exit_percentile_inclusive_min": 0.7,
            "signal_lag_bars": 1,
    ...
    return {
        ...
        "predecessor_strategy_id": PREDECESSOR_STRATEGY_ID,
...
def assert_shared_channel_core_bound() -> None:
    """VDBX admits via VDB reuse; both predecessor admission and baseline must share core."""
    ...
    from src.research import volatility_decay_breakout_v1_strategy_v1 as predecessor_strategy

    _require(
        predecessor_strategy.compute_prior_high_low_channel_bounds_v1
    ...
    _require(
        predecessor_strategy.classify_price_channel_break_v1
        is core.classify_price_channel_break_v1,
...
def assert_exit_state_machine_bound() -> None:
    """The strategy producer must import the frozen VDBX exit state machine (no local reimpl)."""
    from src.research import (
        volatility_decay_breakout_with_explicit_decay_exit_v1_exit_state_machine_v1 as exit_sm,
    )
    from src.research import (
        volatility_decay_breakout_with_explicit_decay_exit_v1_strategy_v1 as strategy,
    )
...
def materialize_entry_point_binding_payload(repo_root: Path) -> dict[str, Any]:
    ...
    impl = load_json(repo_root, IMPLEMENTATION_BINDING_REL_PATH)
    _require(
        impl.get("exit_state_machine_implemented") is True,
        "IMPL_EXIT_SM_NOT_IMPLEMENTED",
    )
    _require(
    ...
    _require(
        impl.get("strategy_identity") == STRATEGY_IDENTITY,
        "IMPL_IDENTITY_DRIFT",
    )
    ...
    return {
        ...
        "evidence_ref": (
            "docs/evidence/evaluate_volatility_decay_breakout_with_explicit_decay_exit_"
            "development_v1/"
        ),
        "exit_semantics": {
            "evaluator_side_reconstruction_of_missing_strategy_exits_forbidden": True,
            "exit_state_machine_implemented": True,
            "exit_state_machine_ref": EXIT_STATE_MACHINE_REL_PATH,
            "productive_pnl_evaluator_ref": PRODUCTIVE_PNL_EVALUATOR_REL_PATH,
        },
        ...
        "reused_canonical_owners": {
            ...
            "exit_state_machine": EXIT_STATE_MACHINE_REL_PATH.replace("/", ".").removesuffix(".py"),
        ...
        "signal_family": SIGNAL_FAMILY,
        "slice_class": "DEVELOPMENT_EVALUATION_EXECUTED_TERMINAL",
        ...
        "strategy_version": STRATEGY_VERSION,
        "time_segment_definition_id": TIME_SEGMENT_DEFINITION_ID,
        "verdict": "DEVELOPMENT_EVALUATION_EXECUTED_TERMINAL/FAIL_CLOSED_UNPAIRABLE_ENTRY_NO_EXIT",
    }
