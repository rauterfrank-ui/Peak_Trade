"""Panel wiring: bind VDBX treatment (strategy-emitted exits) + unconditional baseline.

Does not invent metrics or exit PnL. Treatment roundtrips are produced entirely by the
strategy's own exit state machine (`generate_vdbx_events_and_roundtrips_v1`); the
...
from src.research.volatility_decay_breakout_v1_vol_state_v1 import compute_atr14_v1
...
@dataclass(frozen=True)
class VdbxTreatmentBaselineWiringHandoffV1:
...
def wire_treatment_and_baseline_panel_events_v1(
    ...
    *,
    ...
    for series in panel_series:
        ...
        bar_results, roundtrips = generate_vdbx_events_and_roundtrips_v1(frame)
        ...
        entry_event_mask = tuple(r.event is VdbxEventV1.ENTRY_EVENT for r in bar_results)
