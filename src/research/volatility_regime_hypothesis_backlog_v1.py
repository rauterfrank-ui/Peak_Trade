...
REQUIRED_VTSR_HYPOTHESIS_ID = "VOLATILITY_TERM_STRUCTURE_REVERSION_NON_BITCOIN_PERPETUALS_V1"
REQUIRED_VTSR_STRATEGY_IDENTITY = "VOLATILITY_TERM_STRUCTURE_REVERSION_V1"
REQUIRED_VTDC_HYPOTHESIS_ID = (
    "VOLATILITY_TERM_STRUCTURE_DEPRESSED_CONTINUATION_NON_BITCOIN_PERPETUALS_V1"
)
REQUIRED_VTDC_STRATEGY_IDENTITY = "VOLATILITY_TERM_STRUCTURE_DEPRESSED_CONTINUATION_V1"
REQUIRED_CSHRVF_HYPOTHESIS_ID = (
    "CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_NON_BITCOIN_PERPETUALS_V1"
)
REQUIRED_CSHRVF_STRATEGY_IDENTITY = "CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_V1"
...
REQUIRED_NEXT_STEP = "REVIEW_AND_MERGE_DEFINITION_ONLY_PREREGISTRATION_THEN_SEPARATE_OPERATOR_GO_FOR_STRATEGY_IMPLEMENTATION_THEN_DEVELOPMENT_EVALUATION"
...
def validate_backlog_contract(
    ...
    vtsr = next(
        t for t in terminals if t.get("strategy_identity") == REQUIRED_VTSR_STRATEGY_IDENTITY
    )
    _require(vtsr.get("hypothesis_id") == REQUIRED_VTSR_HYPOTHESIS_ID, "VTSR_HYPOTHESIS_ID")
    _require(vtsr.get("status") == "TERMINAL_FAIL", "VTSR_STATUS")
    _require(vtsr.get("terminal_result") == "FAIL_CLOSED_NO_RETRY", "VTSR_TERMINAL_RESULT")
    _require(vtsr.get("retry_allowed") is False, "VTSR_RETRY_ALLOWED")
    _require(vtsr.get("reopen_allowed") is False, "VTSR_REOPEN_ALLOWED")
    _require(vtsr.get("run_slot_consumed") is True, "VTSR_SLOT_NOT_CONSUMED")
    vtdc = next(
        t for t in terminals if t.get("strategy_identity") == REQUIRED_VTDC_STRATEGY_IDENTITY
    )
    _require(vtdc.get("hypothesis_id") == REQUIRED_VTDC_HYPOTHESIS_ID, "VTDC_HYPOTHESIS_ID")
    _require(vtdc.get("status") == "TERMINAL_FAIL", "VTDC_STATUS")
    _require(vtdc.get("terminal_result") == "FAIL_CLOSED_NO_RETRY", "VTDC_TERMINAL_RESULT")
    _require(vtdc.get("retry_allowed") is False, "VTDC_RETRY_ALLOWED")
    _require(vtdc.get("reopen_allowed") is False, "VTDC_REOPEN_ALLOWED")
    _require(vtdc.get("run_slot_consumed") is True, "VTDC_SLOT_NOT_CONSUMED")
    cshrvf = next(
        t for t in terminals if t.get("strategy_identity") == REQUIRED_CSHRVF_STRATEGY_IDENTITY
    )
    _require(cshrvf.get("hypothesis_id") == REQUIRED_CSHRVF_HYPOTHESIS_ID, "CSHRVF_HYPOTHESIS_ID")
    _require(cshrvf.get("status") == "TERMINAL_FAIL", "CSHRVF_STATUS")
    _require(cshrvf.get("terminal_result") == "FAIL_CLOSED_NO_RETRY", "CSHRVF_TERMINAL_RESULT")
    _require(cshrvf.get("retry_allowed") is False, "CSHRVF_RETRY_ALLOWED")
    _require(cshrvf.get("reopen_allowed") is False, "CSHRVF_REOPEN_ALLOWED")
    _require(cshrvf.get("run_slot_consumed") is True, "CSHRVF_SLOT_NOT_CONSUMED")
