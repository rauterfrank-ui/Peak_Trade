...
EVALUATION_RUN_ID = "evaluate_ma_trend_alignment_mr_eligibility_development_v1"
...
def _optional_treatment_entry_eligibility_gate(
    *, enabled: bool, bars: pd.DataFrame | None
    ...
    warmup-observation bind path), looks up ``close``/``SMA(50)`` at that
    ...
    admission rule (``is_entry_eligible``): a mapped ``+1`` (long
    entry-candidate) requires ``close > SMA(50)``; a mapped ``-1`` (short
    entry-candidate) requires ``close < SMA(50)``. Mapped ``0`` (flat/exit)
    ...
    def _map_gated(evidence):  # type: ignore[no-untyped-def]
        ...
        ts = state["current_ts"]
        if ts is None or len(close_s) == 0:
            eligible = False
        else:
            close_val = close_s.asof(ts)
            sma_val = sma_s.asof(ts)
            in_warmup = bool(warmup_mask.asof(ts)) if len(warmup_mask) else True
            close_f = float(close_val) if pd.notna(close_val) else float("nan")
            sma_f = float(sma_val) if pd.notna(sma_val) else float("nan")
            eligible = is_entry_eligible(
                signal=raw_signal, close=close_f, sma=sma_f, in_warmup=in_warmup
            )
...
def _extract_member_economics(
    ...
    *,
    ...
    # MA trend-alignment eligibility attribution on the decision segment.
...
def run_arm(
    *,
    ...
    for i, member in enumerate(members, start=1):
        ...
        # run_arm loops members and close/SMA series must match the bars
...
def run_development_evaluation(
    *,
    ...
    # (MA trend-alignment side-aware eligibility gated) — two arms, one run.
    baseline = run_arm(
        arm="baseline",
        cfg=cfg,
        archive_root=root,
    ...
    config_snapshot = {
        "schema_version": "evaluate_ma_trend_alignment_mr_eligibility_development_config_snapshot.v1",
    ...
    code_hashes = {
        "ma_trend_alignment_eligibility_filter_v1.py": _sha256_file(
            repo
            / "src/research/ma_trend_alignment_mr_eligibility_development_evaluation_v1"
            / "ma_trend_alignment_eligibility_filter_v1.py"
        ),
    ...
    command = (
        ...
        "run_evaluate_ma_trend_alignment_mr_eligibility_development_v1.py "
        f"--output-dir {output_dir}"
    )
    base_sha = _git_base_sha(repo)
    summary = {
        "schema_version": "evaluate_ma_trend_alignment_mr_eligibility_development_summary.v1",
