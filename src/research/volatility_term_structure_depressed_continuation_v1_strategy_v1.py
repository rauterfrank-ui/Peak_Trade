"""VOLATILITY_TERM_STRUCTURE_DEPRESSED_CONTINUATION_V1 strategy producer.

Implements depressed short/long RV term-structure ratio → continuation with
the short-horizon signed return, with term-structure-normalization exits.
...
STRATEGY_IDENTITY_V1 = "VOLATILITY_TERM_STRUCTURE_DEPRESSED_CONTINUATION_V1"
STRATEGY_ID_V1 = "volatility_term_structure_depressed_continuation"
...
PREDECESSOR_STRATEGY_ID_V1 = "VOLATILITY_TERM_STRUCTURE_REVERSION_V1"
HYPOTHESIS_ID_V1 = "VOLATILITY_TERM_STRUCTURE_DEPRESSED_CONTINUATION_NON_BITCOIN_PERPETUALS_V1"

DEPRESSED_RATIO_PERCENTILE_INCLUSIVE_MAX_V1 = 0.20
DEPRESSED_MIN_CONSECUTIVE_BARS_V1 = 2
REARM_RATIO_PERCENTILE_STRICTLY_ABOVE_V1 = 0.50
ELEVATED_ENTRY_FORBIDDEN_IN_V1 = True
MAX_ENTRIES_PER_DEPRESSED_EPISODE_V1 = 1
...
EXIT_PARAMS_V1 = {
    ...
    "term_structure_normalization_ratio_percentile_gt": 0.45,
    "regime_invalidation_ratio_percentile_gt": 0.55,
...
class VtdcEventV1(str, Enum):
    ENTRY_EVENT = "ENTRY_EVENT"
    EXIT_EVENT = "EXIT_EVENT"
    NONE = "NONE"


class VtdcReasonV1(str, Enum):
    ...
    DEPRESSED_MONITORING = "DEPRESSED_MONITORING"
...
class VtdcRoundtripV1:
    ...
    entry_price: float
    exit_price: float
    exit_reason: VtdcExitReasonV1
    depressed_confirmation_index: int
    short_horizon_signed_return: float


@dataclass
class _DepressedEpisodeState:
    confirmation_index: int
    entry_consumed: bool = False


def _depressed_through(
    ranks: np.ndarray,
    *,
    end_inclusive: int,
    bars: int = DEPRESSED_MIN_CONSECUTIVE_BARS_V1,
    thr: float = DEPRESSED_RATIO_PERCENTILE_INCLUSIVE_MAX_V1,
) -> bool:
...
def _none_result(
    ...
    *,
    ...
    return VtdcBarResultV1(
        event=VtdcEventV1.NONE,
        ...
        depressed_confirmation_index=depressed_confirmation_index,
        short_horizon_signed_return=short_horizon_signed_return,
    )


def generate_vtdc_events_and_roundtrips_v1(
    data: pd.DataFrame,
    *,
    high_col: str = "high",
    low_col: str = "low",
...
) -> tuple[list[VtdcBarResultV1], list[VtdcRoundtripV1]]:
    """Generate entry/exit events with guaranteed exit reachability for admitted entries.

    Depressed confirmation on completed bar t_conf (>=2 bars ratio percentile <= 0.20).
    Direction is with the short-horizon signed return over 8 completed bars.
    Fill at open of signal+1. Elevated-ratio entries forbidden. No trailing exits.
    """
    ...
    results: list[VtdcBarResultV1] = [_none_result(VtdcReasonV1.NO_EVENT) for _ in range(n)]
    roundtrips: list[VtdcRoundtripV1] = []
    ...
    episode: Optional[_DepressedEpisodeState] = None
    ...
    for i in range(n):
        ...
        if pending_signal is not None:
            sig_i, side, dep_i, signed_ret = pending_signal
            ...
            if i == fill_i:
                ...
                pending_roundtrip_meta = (sig_i, fill_i, entry_px, dep_i, signed_ret)
                pending_signal = None
                results[i] = VtdcBarResultV1(
                    event=VtdcEventV1.NONE,
                    entry_side=StrategyEntrySideCarrierV1.NONE,
                    event_kind=StrategyAgreementEventKindV1.NONE,
                    reason=VtdcReasonV1.POSITION_OPEN,
                    fill_index=fill_i,
                    signal_index=sig_i,
                    depressed_confirmation_index=dep_i,
                    ratio_percentile_120=rank_i,
                    rv_term_structure_ratio=ratio_i,
                    realized_volatility_short_8=short_i,
                    realized_volatility_long_48=long_i,
            ...
            if i < fill_i:
                results[i] = VtdcBarResultV1(
                    ...
                    entry_side=StrategyEntrySideCarrierV1.NONE,
                    event_kind=StrategyAgreementEventKindV1.NONE,
                    reason=VtdcReasonV1.AWAITING_FILL,
                    signal_index=sig_i,
                    depressed_confirmation_index=dep_i,
                    ratio_percentile_120=rank_i,
                    rv_term_structure_ratio=ratio_i,
        ...
        if position is not None and pending_roundtrip_meta is not None:
            if i <= position.fill_index:
                results[i] = VtdcBarResultV1(
                    ...
                    depressed_confirmation_index=pending_roundtrip_meta[3],
                    ratio_percentile_120=rank_i,
                    rv_term_structure_ratio=ratio_i,
                    realized_volatility_short_8=short_i,
                    realized_volatility_long_48=long_i,
            ...
            if decision is not None:
                sig_i, fill_i, entry_px, dep_i, signed_ret = pending_roundtrip_meta
                roundtrips.append(
                    VtdcRoundtripV1(
                        side=decision.side,
                        signal_index=sig_i,
                ...
                results[i] = VtdcBarResultV1(
                    event=VtdcEventV1.EXIT_EVENT,
                    entry_side=StrategyEntrySideCarrierV1.NONE,
                    event_kind=StrategyAgreementEventKindV1.NONE,
                    reason=VtdcReasonV1.EXIT_EMITTED,
                    exit_reason=decision.reason,
                    exit_price=decision.exit_price,
                    fill_index=fill_i,
                    signal_index=sig_i,
        ...
        if awaiting_rearm:
            if rank_i > REARM_RATIO_PERCENTILE_STRICTLY_ABOVE_V1:
                awaiting_rearm = False
            else:
                results[i] = _none_result(
                    VtdcReasonV1.AWAITING_REARM,
                    ratio_percentile_120=rank_i,
                    rv_term_structure_ratio=ratio_i,
                    realized_volatility_short_8=short_i,
                    realized_volatility_long_48=long_i,
        ...
        if episode is not None and episode.entry_consumed:
            if not _depressed_through(ranks_np, end_inclusive=i):
                episode = None
                awaiting_rearm = True
                results[i] = _none_result(
                    VtdcReasonV1.AWAITING_REARM,
                ...
                results[i] = _none_result(
                    VtdcReasonV1.EPISODE_CONSUMED,
                    ratio_percentile_120=rank_i,
                    rv_term_structure_ratio=ratio_i,
                    realized_volatility_short_8=short_i,
                    realized_volatility_long_48=long_i,
                    depressed_confirmation_index=episode.confirmation_index,
                )
            continue
        ...
        if episode is None:
            if not _depressed_through(ranks_np, end_inclusive=i):
                ...
                continue
            episode = _DepressedEpisodeState(confirmation_index=i)

        if i != episode.confirmation_index:
            results[i] = _none_result(
                VtdcReasonV1.DEPRESSED_MONITORING
                if _depressed_through(ranks_np, end_inclusive=i)
                else VtdcReasonV1.NO_EVENT,
                ratio_percentile_120=rank_i,
                rv_term_structure_ratio=ratio_i,
                realized_volatility_short_8=short_i,
                realized_volatility_long_48=long_i,
        ...
        if signed_ret is None:
            ...
            results[i] = _none_result(
                VtdcReasonV1.INVALID_INPUT_FAIL_CLOSED,
                ratio_percentile_120=rank_i,
                rv_term_structure_ratio=ratio_i,
                realized_volatility_short_8=short_i,
                realized_volatility_long_48=long_i,
        ...
        cont_side = _continuation_side_from_signed_return(signed_ret)
        if cont_side is None:
            episode.entry_consumed = True
            results[i] = _none_result(
                VtdcReasonV1.AMBIGUOUS_DIRECTION_FAIL_CLOSED,
                ratio_percentile_120=rank_i,
                rv_term_structure_ratio=ratio_i,
                realized_volatility_short_8=short_i,
                realized_volatility_long_48=long_i,
        ...
        if not entry_exit_reachable_ex_ante_v1(signal_index=i, series_length=n):
            episode.entry_consumed = True
            results[i] = _none_result(
                VtdcReasonV1.ENTRY_SUPPRESSED_EXIT_UNREACHABLE,
                ratio_percentile_120=rank_i,
                rv_term_structure_ratio=ratio_i,
                realized_volatility_short_8=short_i,
                realized_volatility_long_48=long_i,
        ...
        episode.entry_consumed = True
        pending_signal = (i, cont_side, episode.confirmation_index, float(signed_ret))
        results[i] = VtdcBarResultV1(
            event=VtdcEventV1.ENTRY_EVENT,
            entry_side=cont_side,
            event_kind=StrategyAgreementEventKindV1.ENTRY,
            reason=VtdcReasonV1.SUCCESSFUL_ENTRY,
            signal_index=i,
            depressed_confirmation_index=episode.confirmation_index,
            ratio_percentile_120=rank_i,
            rv_term_structure_ratio=ratio_i,
...
__all__ = [
    ...
    "HYPOTHESIS_ID_V1",
    "MAX_ENTRIES_PER_DEPRESSED_EPISODE_V1",
