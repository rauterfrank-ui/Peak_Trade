"""Admission gates for Momentum V2 vol-scaled development evaluation.

Applies preregistered measurement-contract thresholds only.
"""
...
class AdmissionGateBundleV1:
    ...
    segment_trade_sufficiency: GateResultV1
    ...
    def all_pass(self) -> bool:
        return all(
            ...
            for g in (
                ...
                self.segment_trade_sufficiency,
                self.gross_edge,
                self.canonical_cost,
                self.cost_stress,
                self.max_drawdown,
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "segment_trade_sufficiency": _one(self.segment_trade_sufficiency),
            ...
            "concentration": _one(self.concentration),
...
def _thresholds_from_contract(contract: Mapping[str, Any]) -> dict[str, float | int]:
    ...
    return {
        ...
        "min_trades_per_time_segment": int(thr["min_trades_per_time_segment"]["value"]),
        ...
        "max_single_instrument_positive_gross_pnl_share_max": float(
            thr["max_single_instrument_positive_gross_pnl_share_max"]["value"]
        ),
    }
...
def evaluate_admission_gates_v1(
    *,
    ...
    assert thr["minimum_trade_count"] == MIN_EXECUTED_TREATMENT_TRADES
    assert thr["min_trades_per_time_segment"] == MIN_TRADES_PER_TIME_SEGMENT
    ...
    assert (
        thr["max_single_instrument_positive_gross_pnl_share_max"]
        == MAX_SINGLE_INSTRUMENT_POSITIVE_GROSS_PNL_SHARE_MAX
    )

    sample_ok = evaluable_treatment_breakout_events >= int(thr["minimum_trade_count"])
    trades_ok = trade_count >= int(thr["minimum_trade_count"])
    per_seg_ok = True
    for seg in segment_results:
        events = int(
            seg.get("evaluable_treatment_breakout_events")
            or seg.get("executed_treatment_trades")
            or 0
        )
        if events < int(thr["min_trades_per_time_segment"]):
            per_seg_ok = False
            break
    ...
    dd_ok = abs(max_drawdown) <= float(thr["maximum_max_drawdown"]) and abs(max_drawdown) <= abs(
        baseline_max_drawdown
    )
    baseline_ok = (
        net_profit_factor > baseline_net_profit_factor and net_return > baseline_net_return
    )
    concentration_ok = max_single_instrument_positive_gross_pnl_share <= float(
        thr["max_single_instrument_positive_gross_pnl_share_max"]
    )
    ...
    return AdmissionGateBundleV1(
        sample_sufficiency=GateResultV1(
            "SAMPLE_SUFFICIENCY",
            sample_ok,
            None if sample_ok else "INSUFFICIENT_EXECUTED_TRADES",
            evaluable_treatment_breakout_events,
            thr["minimum_trade_count"],
        ),
        trades_sufficiency=GateResultV1(
            "TRADES_SUFFICIENCY",
            trades_ok,
        ...
        segment_trade_sufficiency=GateResultV1(
            "SEGMENT_TRADE_SUFFICIENCY",
            per_seg_ok,
            None if per_seg_ok else "INSUFFICIENT_TRADES_PER_TIME_SEGMENT",
            [s.get("evaluable_treatment_breakout_events") for s in segment_results],
            thr["min_trades_per_time_segment"],
        ),
        gross_edge=GateResultV1(
            "GROSS_EDGE",
            gross_ok,
        ...
        max_drawdown=GateResultV1(
            ...
            if dd_ok
                ...
                "MAX_DRAWDOWN_ABS_BREACH"
                if abs(max_drawdown) > float(thr["maximum_max_drawdown"])
                else "MAX_DRAWDOWN_NOT_IMPROVED_VS_BASELINE"
            ),
            {
                "treatment_max_drawdown": max_drawdown,
        ...
        baseline_improvement=GateResultV1(
            ...
            if baseline_ok
                ...
                "NET_NOT_IMPROVED_VS_BASELINE"
                if not (net_profit_factor > baseline_net_profit_factor)
                else "NET_RETURN_NOT_IMPROVED_VS_BASELINE"
            ),
            {
                "treatment_net_profit_factor": net_profit_factor,
                "baseline_net_profit_factor": baseline_net_profit_factor,
                "treatment_net_return": net_return,
                "baseline_net_return": baseline_net_return,
            },
            "treatment > baseline (net_pf and net_return)",
        ),
        concentration=GateResultV1(
            "INSTRUMENT_PNL_CONCENTRATION",
            concentration_ok,
            None if concentration_ok else "INSTRUMENT_PNL_CONCENTRATION_BREACH",
...
def evaluate_segment_local_result_v1(
    *,
    ...
    thr = _thresholds_from_contract(contract)
    if evaluable_treatment_breakout_events < int(thr["min_trades_per_time_segment"]):
