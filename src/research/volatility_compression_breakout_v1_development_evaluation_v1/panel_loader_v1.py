"""Load DEVELOPMENT-only panel into InstrumentPanelSeriesV1 for VCB v1 eval.

Reuses the sealed independent DEVELOPMENT panel loader owners. Holdout paths are
rejected. BTC/spot exclusion is enforced by the sealed panel membership.
"""
...
def load_instrument_panel_series_from_development_archive_v1(
    ...
    *,
    ...
    content_hash = str(hashes.get("content_hash") or "")
    if hashes["dataset_id"] != expected_dataset_id or REQUIRED_DATASET_ID != expected_dataset_id:
        raise ValueError("DATASET_ID_MISMATCH")
    if expected_dataset_id != DATASET_ID:
        raise ValueError("DATASET_ID_NOT_BOUND")
    if expected_dataset_digest is not None and content_hash != expected_dataset_digest:
    ...
    for canon, frame in sorted(frames.items()):
        ...
        series_list.append(
            InstrumentPanelSeriesV1(
                ...
                bars=tuple(bars),
                series_digest=content_hash,
            )
        )
    return tuple(series_list), timestamps, content_hash
