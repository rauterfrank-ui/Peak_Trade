...
REQUIRED_TARGET_PHENOMENON = "CROSS_SECTIONAL_LOW_REALIZED_VOLATILITY_CONTINUATION"
...
REQUIRED_PRIOR_VTSR = "VOLATILITY_TERM_STRUCTURE_REVERSION_V1"
REQUIRED_PRIOR_VTDC = "VOLATILITY_TERM_STRUCTURE_DEPRESSED_CONTINUATION_V1"
REQUIRED_PRIOR_CSHRVF = "CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_V1"
...
def validate_program_contract(
    ...
    _require(
        payload.get("strategy_id") == "cross_sectional_low_realized_volatility_continuation",
        "STRATEGY_ID_DRIFT",
    )
    _require(
        payload.get("lane_backlog_status") == REQUIRED_LANE_STATUS,
    ...
    _require(
        payload.get("operator_decision_token")
        == "GO_VOLATILITY_REGIME_POST_CSLRVC_DEVELOPMENT_FAIL_LANE_LIFECYCLE_OPERATOR_DECISION_V1",
        "OPERATOR_DECISION_TOKEN",
    )
    _require(payload.get("active_hypothesis_inventory_empty") is True, "INVENTORY_NONEMPTY")
    ...
    _require(
        independence.get("not_a_retry_of_terminal_volatility_term_structure_reversion_v1") is True,
        "VTSR_RETRY_NOT_FORBIDDEN",
    )
    _require(
        independence.get(
    ...
    for required in (
        ...
        "cross_sectional_low_realized_volatility_continuation/v1_retry",
    ):
    ...
    md_vtsr = payload.get("material_difference_vs_volatility_term_structure_reversion_v1") or {}
    _require(md_vtsr.get("prior_strategy_identity") == REQUIRED_PRIOR_VTSR, "PRIOR_VTSR_MISMATCH")
    _require(md_vtsr.get("vtsr_retry_forbidden") is True, "VTSR_RETRY_ALLOWED")
    _require(md_vtsr.get("not_a_parameter_change_of_vtsr_v1") is True, "VTSR_PARAM_CHANGE")
    _require(md_vtsr.get("not_a_repair_or_retry_of_vtsr_v1") is True, "VTSR_REPAIR")
    vtsr_diffs = md_vtsr.get("differences") or {}
    for key in (
        "admission_polarity",
        "causal_claim",
        "direction_rule",
        ...
        _require(bool(vtsr_diffs.get(key)), f"MISSING_VTSR_MATERIAL_DIFFERENCE:{key}")

    md_vtdc = (
        payload.get("material_difference_vs_volatility_term_structure_depressed_continuation_v1")
        or {}
    )
    _require(md_vtdc.get("prior_strategy_identity") == REQUIRED_PRIOR_VTDC, "PRIOR_VTDC_MISMATCH")
    _require(md_vtdc.get("vtdc_retry_forbidden") is True, "VTDC_RETRY_ALLOWED")
    _require(md_vtdc.get("not_a_parameter_change_of_vtdc_v1") is True, "VTDC_PARAM_CHANGE")
    _require(md_vtdc.get("not_a_repair_or_retry_of_vtdc_v1") is True, "VTDC_REPAIR")
    vtdc_diffs = md_vtdc.get("differences") or {}
    for key in (
        "admission_polarity",
        "causal_claim",
        "direction_rule",
        ...
        _require(bool(vtdc_diffs.get(key)), f"MISSING_VTDC_MATERIAL_DIFFERENCE:{key}")
    ...
    if repo_root is not None:
        ...
        _require(backlog.get("status") == REQUIRED_LANE_STATUS, "LANE_BACKLOG_NOT_CLOSED")
        ...
        _require(REQUIRED_STRATEGY_IDENTITY in terminals, "BACKLOG_CSLRVC_NOT_TERMINAL")
    ...
    return {
        ...
        "material_difference_from_vtsr_v1": True,
