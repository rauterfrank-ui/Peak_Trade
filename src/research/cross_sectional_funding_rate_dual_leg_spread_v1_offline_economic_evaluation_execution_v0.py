"""Cross-sectional funding-rate dual-leg spread v1 offline economic evaluation execution v0.

Deterministic, fail-closed execution infrastructure for the bounded funding-rate dual-leg spread
...
from src.research.cross_sectional_funding_rate_dual_leg_spread_research_orchestrator_v1 import (
    default_dual_leg_spread_operator_binding_v1,
    run_cross_sectional_funding_rate_dual_leg_spread_orchestrator_v1,
)
...
PACKAGE_MARKER = (
    "CROSS_SECTIONAL_FUNDING_RATE_DUAL_LEG_SPREAD_V1_OFFLINE_ECONOMIC_EVALUATION_EXECUTION_V0=true"
)

SCHEMA_VERSION = (
    "cross_sectional_funding_rate_dual_leg_spread_v1_offline_economic_evaluation_execution.v0"
)
EXECUTION_ID = (
    "cross_sectional_funding_rate_dual_leg_spread_v1_offline_economic_evaluation_execution_v0"
)
EXECUTION_VERSION = "v0"
CANONICAL_SERIALIZATION_VERSION = "cross_sectional_execution_canonical_json_v1"

CONFIRM_GO = "GO_CROSS_SECTIONAL_FUNDING_RATE_DUAL_LEG_SPREAD_V1_OFFLINE_ECONOMIC_EVALUATION_EXECUTION_NO_RUNTIME_AUTHORITY_V0"
...
ALLOWED_EXECUTION_CONFIRM_GOS: frozenset[str] = frozenset({CONFIRM_GO})
...
RUNNER_OWNER = "scripts.ops.run_cross_sectional_funding_rate_dual_leg_spread_v1_offline_economic_evaluation_execution_v0"
RUNNER_SCRIPT = "scripts/ops/run_cross_sectional_funding_rate_dual_leg_spread_v1_offline_economic_evaluation_execution_v0.py"
...
def run_contract_smoke_evaluation_v0(
    *,
    ...
    binding = default_dual_leg_spread_operator_binding_v1()
    ...
    materialization = materialize_bound_funding_panel_dataset_v1(
    ...
    orchestrator = run_cross_sectional_funding_rate_dual_leg_spread_orchestrator_v1(
        binding=binding,
        funding_panel_series=funding_panel_series,
    )
    backtest = run_dual_leg_spread_panel_backtest_v1(
        orchestrator,
        panel_series,
...
def verify_full_evaluation_precheck_v1(
    *,
    ...
    if require_execution_go:
        if go_token not in ALLOWED_EXECUTION_CONFIRM_GOS:
...
def build_stage_wiring_status_v1() -> tuple[StageWiringStatusV1, ...]:
    return (
        StageWiringStatusV1(
            ...
            wired=True,
            owner="cross_sectional_dual_leg_spread_backtest_wiring_v1",
        ),
        StageWiringStatusV1(
...
def run_full_evaluation_entrypoint_dry_run_v1(
    *,
    ...
    _ = wire_robustness_stages_v0(
        backtest,  # type: ignore[arg-type]
        period_binding=envelope["period_binding"],
...
class FullEconomicEvaluationResultV0:
    ...
    stage_wiring: tuple[StageWiringStatusV1, ...]
    backtest: DualLegSpreadBacktestResultV1 | None
...
def _compute_single_trade_contribution(backtest: DualLegSpreadBacktestResultV1) -> float | None:
...
def _compute_single_regime_contribution(backtest: DualLegSpreadBacktestResultV1) -> float | None:
    if backtest.trades.empty:
        return None
...
def _compute_long_short_contribution(
    backtest: DualLegSpreadBacktestResultV1,
) -> tuple[float | None, float | None]:
    if backtest.long_contribution is not None and backtest.short_contribution is not None:
        return backtest.long_contribution, backtest.short_contribution
    if backtest.trades.empty:
        return 0.0, 0.0
    long_pnl = 0.0
    short_pnl = 0.0
...
def materialize_economic_viability_evidence(
    *,
    ...
    """Persist cross-sectional funding-rate dual-leg spread EconomicViabilityEvidenceV1-shaped payload."""
    envelope = dict(versioned_binding)
    ...
    body: dict[str, Any] = {
        "schema_version": "economic_viability_evidence_cross_sectional_funding_dual_leg_spread_v1",
        "strategy_id": STRATEGY_ID,
        "strategy_version": STRATEGY_VERSION,
        ...
        "portfolio_contribution": {"dual_leg_simultaneous": 1.0},
