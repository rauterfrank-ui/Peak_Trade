"""Single-run DEVELOPMENT panel evaluation v2: baseline vs midband exit-efficiency gate.

New independently preregistered DEVELOPMENT_ONLY measurement
(HYPOTHESIS_ID ...DEVELOPMENT_V2). Not a rerun of V1. Reuses the frozen V1
mechanism/gate/decision modules by import only (definition identical); no V1
partial result, checkpoint, or economic claim is transferred.
"""
...
from src.research.bollinger_mr_midband_exit_efficiency_hypothesis_preregistration_v2 import (
    FORBIDDEN_V1_PARTIAL_TRANSFER_KEYS,
...
def _assert_no_v1_partial_result_reuse(contract: Mapping[str, Any]) -> None:
    for banned in FORBIDDEN_V1_PARTIAL_TRANSFER_KEYS:
        if banned in contract:
            raise RuntimeError(f"V1_PARTIAL_TRANSFER_FORBIDDEN:{banned}")
    predecessor = contract.get("predecessor_development_v1") or {}
    if predecessor.get("hypothesis_id") != REQUIRED_PREDECESSOR_HYPOTHESIS_ID:
        raise RuntimeError("PREDECESSOR_V1_HYPOTHESIS_ID_MISMATCH")
    if predecessor.get("partial_results_reused") is not False:
        raise RuntimeError("PREDECESSOR_V1_PARTIAL_RESULTS_REUSED")
...
def run_development_evaluation(
    *,
    ...
    # Validate against the V2 preregistration validator BEFORE any measurement /
    # contract mutation. The V2 validator currently only accepts
    # DEFINITION_ONLY_PREREGISTERED with evaluation_run_count=0, which is exactly
    # the required state at the start of this run.
    report = load_and_validate_repo_contract(repo)
    if report.get("hypothesis_id") != HYPOTHESIS_ID:
        raise RuntimeError("CONTRACT_HYPOTHESIS_ID_NOT_V2")
    ...
    _assert_no_v1_partial_result_reuse(contract)
    ...
    (output_dir / "trade_ledger_baseline.json").write_text(
        json.dumps(
            {
                "schema_version": "bollinger_mr_midband_exit_efficiency_trade_ledger.v2",
                "arm": "baseline",
    ...
    config_snapshot = {
        "schema_version": "evaluate_bollinger_mr_midband_exit_efficiency_development_config_snapshot.v2",
    ...
    v2_pkg = repo / "src/research/bollinger_mr_midband_exit_efficiency_development_evaluation_v2"
    code_hashes = {
        ...
        "panel_runner_v2.py": _sha256_file(v2_pkg / "panel_runner_v2.py"),
    ...
    command = (
        ...
        "run_evaluate_bollinger_mr_midband_exit_efficiency_development_v2.py "
        f"--output-dir {output_dir}"
    )
    summary = {
        "schema_version": "evaluate_bollinger_mr_midband_exit_efficiency_development_summary.v2",
    ...
    (output_dir / "safety_attestation.md").write_text(
        "\n".join(
            [
                "# Safety attestation — midband exit-efficiency DEVELOPMENT evaluation v2",
                "",
