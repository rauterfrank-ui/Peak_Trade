"""Definition-only preregistration validator for ADX DI direction-confirmation MR eligibility v1.
...
Orthogonal to six prior FAILs on the same DEVELOPMENT panel:
regime-gated absolute-threshold classifier, ATR-percentile mid-band eligibility,
RSI(14) exhaustion eligibility, ADX(14) level range-admission eligibility,
MA SMA(50) with-trend admission, and MACD histogram-sign countertrend.
This filter uses Wilder +DI/−DI directional ordering only (long iff minus_DI > plus_DI;
short iff plus_DI > minus_DI) from TrendFollowingStrategy._compute_adx; ADX level unused.
"""
...
PACKAGE_MARKER = "ADX_DI_DIRECTION_CONFIRMATION_MR_ELIGIBILITY_HYPOTHESIS_PREREGISTRATION_V1=true"
CONTRACT_REL_PATH = (
    "config/research/"
...
def validate_preregistration_contract(
    ...
    *,
    ...
    feature_ids = [str(feat.get("feature_id") or "") for feat in features]
    if set(feature_ids) != {"plus_di_14h", "minus_di_14h"}:
        raise HypothesisPreregistrationError("DI_FEATURE_IDS_MUST_BE_EXACTLY_PLUS_AND_MINUS_14H")
    if eligibility.get("calculator_method") != "WILDER_ADX_PLUS_DI_MINUS_DI":
        raise HypothesisPreregistrationError("CALCULATOR_METHOD_MISMATCH")
    if eligibility.get("max_feature_lookback_hours") != 28:
        raise HypothesisPreregistrationError("FILTER_LOOKBACK_MISMATCH")
    for feat in features:
        ...
        if fid == "adx_14h" or (fid.startswith("adx_") and "di" not in fid):
            raise HypothesisPreregistrationError(f"ADX_LEVEL_FEATURE_REUSE_FORBIDDEN:{fid}")
        if fid.startswith("macd_"):
            raise HypothesisPreregistrationError(f"MACD_FEATURE_REUSE_FORBIDDEN:{fid}")
        if fid == "close_vs_sma_50h" or fid.startswith("close_vs_sma_"):
            raise HypothesisPreregistrationError(f"MA_TREND_FEATURE_REUSE_FORBIDDEN:{fid}")
        if fid not in {"plus_di_14h", "minus_di_14h"} and fid.startswith(("plus_di_", "minus_di_")):
            # only the frozen 14h DI features are admitted in this contract
            raise HypothesisPreregistrationError(f"UNEXPECTED_DI_FEATURE:{fid}")
    ...
    if not isinstance(prior_failed, list) or len(prior_failed) < 6:
