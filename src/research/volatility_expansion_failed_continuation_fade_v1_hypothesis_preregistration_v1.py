"""Definition-only preregistration validator for VEFCF v1."""
...
PACKAGE_MARKER = "VOLATILITY_EXPANSION_FAILED_CONTINUATION_FADE_V1_HYPOTHESIS_PREREGISTRATION=true"
CONTRACT_REL_PATH = (
    "config/research/"
    "volatility_expansion_failed_continuation_fade_v1_preregistered_economic_hypothesis_"
    "measurement_contract_v1.json"
)
GOVERNANCE_REL_PATH = (
    "docs/governance/VOLATILITY_EXPANSION_FAILED_CONTINUATION_FADE_V1_"
    "PREREGISTERED_HYPOTHESIS_MEASUREMENT_V1.md"
)
EVIDENCE_REL_PATH = (
    "docs/evidence/preregister_volatility_expansion_failed_continuation_fade_hypothesis_v1/"
)
...
REQUIRED_TARGET = "VOLATILITY_EXPANSION_THEN_FAILED_CONTINUATION_FADE"
REQUIRED_DIRECTIONAL_FORM = "OWN_INSTRUMENT_MUTUALLY_EXCLUSIVE_FAILED_CONTINUATION_FADE"
...
def validate_measurement_contract(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(
        ...
        == "VOLATILITY_EXPANSION_FAILED_CONTINUATION_FADE_ADMISSION",
        "BASELINE_SOLE_DIFF",
    )
    ...
    expansion = admission.get("expansion_state") or {}
    _require(expansion.get("percentile_inclusive_min") == 0.65, "EXPANSION_THR")
    _require(expansion.get("min_consecutive_completed_bars") == 4, "EXPANSION_BARS")
    _require(expansion.get("tolerance_gap_bars") == 0, "EXPANSION_TOLERANCE")

    pullback = admission.get("pullback_requirement") or {}
    _require(pullback.get("max_monitoring_bars_inclusive") == 8, "MONITOR_BARS")
    _require(
        pullback.get("min_pullback_fraction_of_impulse_range_qualifying") == 0.15,
        "PULLBACK_MIN",
    )
    _require(
        pullback.get("max_pullback_fraction_of_impulse_range_deep_fail") == 0.50,
        "PULLBACK_DEEP",
    )
    _require(
        pullback.get("successful_continuation_cancels_fade_for_sequence") is True,
        "CONTINUATION_CANCEL_MISSING",
    )
    _require(
        pullback.get("immediate_breakout_entry_without_failure_forbidden") is True,
        "IMMEDIATE_BREAKOUT_ALLOWED",
    )

    fade = admission.get("failed_continuation_fade_entry") or {}
    _require(
        fade.get("entry_only_after_failed_continuation_trigger") is True,
        "FADE_ORDER",
    )
    ...
    _require(fade.get("no_entry_on_expansion_confirmation_bar") is True, "ENTRY_ON_EXPANSION")
    _require(
        fade.get("no_immediate_post_expansion_breakout_entry") is True,
        "IMMEDIATE_POST_EXPANSION",
    )
    _require(
        fade.get("no_entry_on_successful_continuation_confirmation") is True,
        "CONTINUATION_ENTRY_ALLOWED",
    )
    _require(fade.get("vepc_continuation_entry_forbidden") is True, "VEPC_ENTRY_ALLOWED")
    _require(fade.get("direction_rule") == "opposite_to_failed_impulse", "DIRECTION_RULE")
    ...
    triggers = fade.get("fade_triggers_first_wins") or []
    _require(
        triggers
        == [
    ...
    _require(lifecycle.get("contraction_admission_not_required") is True, "CONTRACTION_REQUIRED")
    _require(
        lifecycle.get("vceb_joint_contraction_expansion_break_forbidden") is True,
        "VCEB_JOINT_ALLOWED",
    )
    _require(
        lifecycle.get("vepc_pullback_continuation_entry_forbidden") is True,
        "VEPC_CONTINUATION_ALLOWED",
    )
    _require(
        lifecycle.get("immediate_joint_breakout_on_expansion_bar_forbidden") is True,
        "IMMEDIATE_JOINT_ALLOWED",
    )
    ...
    reclaim = exits.get("impulse_reclaim_invalidation") or {}
    _require(reclaim.get("authorized") is True, "IMPULSE_RECLAIM_NOT_AUTHORIZED")
    ...
    _require(md_vepc.get("prior_strategy_identity") == REQUIRED_PREDECESSOR, "PRIOR_VEPC")
