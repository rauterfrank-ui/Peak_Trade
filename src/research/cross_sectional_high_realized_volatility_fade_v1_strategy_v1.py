"""CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_V1 strategy producer.

Implements confirmed high cross-sectional RV-level rank → fade opposite the
...
STRATEGY_IDENTITY_V1 = "CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_V1"
STRATEGY_ID_V1 = "cross_sectional_high_realized_volatility_fade"
...
PREDECESSOR_STRATEGY_ID_V1 = "VOLATILITY_TERM_STRUCTURE_DEPRESSED_CONTINUATION_V1"
HYPOTHESIS_ID_V1 = "CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_NON_BITCOIN_PERPETUALS_V1"

HIGH_RANK_PERCENTILE_INCLUSIVE_MIN_V1 = 0.80
HIGH_RANK_MIN_CONSECUTIVE_BARS_V1 = 2
REARM_RANK_PERCENTILE_STRICTLY_BELOW_V1 = 0.50
LOW_CROSS_SECTIONAL_VOL_ENTRY_FORBIDDEN_IN_V1 = True
MAX_ENTRIES_PER_HIGH_VOL_EPISODE_V1 = 1
...
EXIT_PARAMS_V1 = {
    ...
    "cs_vol_rank_normalization_percentile_lt": 0.55,
    "regime_invalidation_cs_rv_rank_percentile_lt": 0.40,
...
class CshrvfEventV1(str, Enum):
    ENTRY_EVENT = "ENTRY_EVENT"
    EXIT_EVENT = "EXIT_EVENT"
    NONE = "NONE"


class CshrvfReasonV1(str, Enum):
    ...
    HIGH_RANK_MONITORING = "HIGH_RANK_MONITORING"
...
class CshrvfRoundtripV1:
    ...
    entry_price: float
    exit_price: float
    exit_reason: CshrvfExitReasonV1
    high_rank_confirmation_index: int
    short_horizon_signed_return: float


@dataclass
class _HighRankEpisodeState:
    confirmation_index: int
    entry_consumed: bool = False


def _high_rank_through(
    ranks: np.ndarray,
    *,
    end_inclusive: int,
    bars: int = HIGH_RANK_MIN_CONSECUTIVE_BARS_V1,
    thr: float = HIGH_RANK_PERCENTILE_INCLUSIVE_MIN_V1,
) -> bool:
...
def _none_result(
    ...
    *,
    ...
    return CshrvfBarResultV1(
        event=CshrvfEventV1.NONE,
        ...
        high_rank_confirmation_index=high_rank_confirmation_index,
        short_horizon_signed_return=short_horizon_signed_return,
    )


def generate_cshrvf_events_and_roundtrips_v1(
    data: pd.DataFrame,
    *,
...
) -> tuple[list[CshrvfBarResultV1], list[CshrvfRoundtripV1]]:
    """Generate entry/exit events with guaranteed exit reachability for admitted fades.

    High-rank confirmation on completed bar t_conf (>=2 bars CS RV rank >= 0.80).
    Direction is opposite the short-horizon signed return over 8 completed bars.
    Fill at open of signal+1. Low CS-vol-rank entries forbidden. No trailing exits.
    ...
    results: list[CshrvfBarResultV1] = [_none_result(CshrvfReasonV1.NO_EVENT) for _ in range(n)]
    roundtrips: list[CshrvfRoundtripV1] = []
    ...
    episode: Optional[_HighRankEpisodeState] = None
    ...
    for i in range(n):
        ...
        if pending_signal is not None:
            sig_i, side, high_i, signed_ret = pending_signal
            ...
            if i == fill_i:
                ...
                pending_roundtrip_meta = (sig_i, fill_i, entry_px, high_i, signed_ret)
                pending_signal = None
                results[i] = CshrvfBarResultV1(
                    event=CshrvfEventV1.NONE,
                    entry_side=StrategyEntrySideCarrierV1.NONE,
                    event_kind=StrategyAgreementEventKindV1.NONE,
                    reason=CshrvfReasonV1.POSITION_OPEN,
                    fill_index=fill_i,
                    signal_index=sig_i,
                    high_rank_confirmation_index=high_i,
                    cs_rv_rank=rank_i,
                    realized_volatility_24=rv_i,
            ...
            if i < fill_i:
                results[i] = CshrvfBarResultV1(
                    ...
                    entry_side=StrategyEntrySideCarrierV1.NONE,
                    event_kind=StrategyAgreementEventKindV1.NONE,
                    reason=CshrvfReasonV1.AWAITING_FILL,
                    signal_index=sig_i,
                    high_rank_confirmation_index=high_i,
                    cs_rv_rank=rank_i,
                    realized_volatility_24=rv_i,
        ...
        if position is not None and pending_roundtrip_meta is not None:
            if i <= position.fill_index:
                results[i] = CshrvfBarResultV1(
                    ...
                    high_rank_confirmation_index=pending_roundtrip_meta[3],
                    cs_rv_rank=rank_i,
                    realized_volatility_24=rv_i,
            ...
            if decision is not None:
                sig_i, fill_i, entry_px, high_conf, signed_ret = pending_roundtrip_meta
                roundtrips.append(
                    CshrvfRoundtripV1(
                        side=decision.side,
                        signal_index=sig_i,
                        ...
                        high_rank_confirmation_index=high_conf,
                        short_horizon_signed_return=signed_ret,
                    )
                )
                results[i] = CshrvfBarResultV1(
                    event=CshrvfEventV1.EXIT_EVENT,
                    entry_side=StrategyEntrySideCarrierV1.NONE,
                    event_kind=StrategyAgreementEventKindV1.NONE,
                    reason=CshrvfReasonV1.EXIT_EMITTED,
                    exit_reason=decision.reason,
                    exit_price=decision.exit_price,
                    fill_index=fill_i,
                    signal_index=sig_i,
        ...
        if rank_i is None or not np.isfinite(c_i):
            reason = (
                CshrvfReasonV1.INSUFFICIENT_CROSS_SECTION
                if rv_i is not None
                else CshrvfReasonV1.WARMUP
            )
            results[i] = _none_result(
                reason,
                cs_rv_rank=rank_i,
        ...
        if awaiting_rearm:
            if rank_i < REARM_RANK_PERCENTILE_STRICTLY_BELOW_V1:
                awaiting_rearm = False
            else:
                results[i] = _none_result(
                    CshrvfReasonV1.AWAITING_REARM,
                    cs_rv_rank=rank_i,
                    realized_volatility_24=rv_i,
                )
                continue
        ...
        if episode is not None and episode.entry_consumed:
            if not _high_rank_through(ranks_np, end_inclusive=i):
                episode = None
                awaiting_rearm = True
                results[i] = _none_result(
                    CshrvfReasonV1.AWAITING_REARM,
                ...
                results[i] = _none_result(
                    CshrvfReasonV1.EPISODE_CONSUMED,
                    cs_rv_rank=rank_i,
                    realized_volatility_24=rv_i,
                    high_rank_confirmation_index=episode.confirmation_index,
                )
            continue
        ...
        if episode is None:
            if not _high_rank_through(ranks_np, end_inclusive=i):
                results[i] = _none_result(
                    CshrvfReasonV1.NO_EVENT,
                    cs_rv_rank=rank_i,
                    realized_volatility_24=rv_i,
                )
                continue
            episode = _HighRankEpisodeState(confirmation_index=i)

        if i != episode.confirmation_index:
            results[i] = _none_result(
                CshrvfReasonV1.HIGH_RANK_MONITORING
                if _high_rank_through(ranks_np, end_inclusive=i)
                else CshrvfReasonV1.NO_EVENT,
                cs_rv_rank=rank_i,
                realized_volatility_24=rv_i,
                high_rank_confirmation_index=episode.confirmation_index,
            )
        ...
        if signed_ret is None:
            ...
            results[i] = _none_result(
                CshrvfReasonV1.INVALID_INPUT_FAIL_CLOSED,
                cs_rv_rank=rank_i,
                realized_volatility_24=rv_i,
                high_rank_confirmation_index=episode.confirmation_index,
            )
        ...
        if fade_side is None:
            episode.entry_consumed = True
            results[i] = _none_result(
                CshrvfReasonV1.AMBIGUOUS_DIRECTION_FAIL_CLOSED,
                cs_rv_rank=rank_i,
                realized_volatility_24=rv_i,
                high_rank_confirmation_index=episode.confirmation_index,
                short_horizon_signed_return=signed_ret,
        ...
        if not entry_exit_reachable_ex_ante_v1(signal_index=i, series_length=n):
            episode.entry_consumed = True
            results[i] = _none_result(
                CshrvfReasonV1.ENTRY_SUPPRESSED_EXIT_UNREACHABLE,
                cs_rv_rank=rank_i,
                realized_volatility_24=rv_i,
                high_rank_confirmation_index=episode.confirmation_index,
                short_horizon_signed_return=signed_ret,
        ...
        results[i] = CshrvfBarResultV1(
            event=CshrvfEventV1.ENTRY_EVENT,
            entry_side=fade_side,
            event_kind=StrategyAgreementEventKindV1.ENTRY,
            reason=CshrvfReasonV1.SUCCESSFUL_ENTRY,
            signal_index=i,
            high_rank_confirmation_index=episode.confirmation_index,
            cs_rv_rank=rank_i,
            realized_volatility_24=rv_i,
...
__all__ = [
    ...
    "MAX_ENTRIES_PER_HIGH_VOL_EPISODE_V1",
