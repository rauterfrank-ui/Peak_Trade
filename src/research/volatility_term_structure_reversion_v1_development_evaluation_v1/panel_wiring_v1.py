"""Panel wiring: bind VTSR treatment (strategy-emitted exits) + unconditional baseline.

Does not invent metrics or exit PnL. Treatment roundtrips are produced entirely by the
strategy's own exit state machine (`generate_vtsr_events_and_roundtrips_v1`); the
...
__all__ = (
    ...
    "VtsrTreatmentBaselineWiringHandoffV1",
...
@dataclass(frozen=True)
class VtsrTreatmentBaselineWiringHandoffV1:
...
def wire_treatment_and_baseline_panel_events_v1(
    ...
    *,
    ...
    for series in panel_series:
        ...
        bar_results, roundtrips = generate_vtsr_events_and_roundtrips_v1(frame)
        ...
        entry_event_mask = tuple(r.event is VtsrEventV1.ENTRY_EVENT for r in bar_results)
    ...
    return VtsrTreatmentBaselineWiringHandoffV1(
