"""Panel wiring: bind VEPC treatment (strategy-emitted exits) + unconditional baseline.

Does not invent metrics or exit PnL. Treatment roundtrips are produced entirely by the
strategy's own exit state machine (`generate_vepc_events_and_roundtrips_v1`); the
...
@dataclass(frozen=True)
class VepcTreatmentBaselineWiringHandoffV1:
...
def wire_treatment_and_baseline_panel_events_v1(
    ...
    *,
    ...
    for series in panel_series:
        ...
        bar_results, roundtrips = generate_vepc_events_and_roundtrips_v1(frame)
        ...
        entry_event_mask = tuple(r.event is VepcEventV1.ENTRY_EVENT for r in bar_results)
        ...
        if roundtrips:
            ...
            for rt in roundtrips:
                ...
                position = open_position_from_fill_v1(
                    ...
                    pullback_swing_high=float(rt.pullback_swing_high),
                    pullback_swing_low=float(rt.pullback_swing_low),
                )
    ...
    return VepcTreatmentBaselineWiringHandoffV1(
