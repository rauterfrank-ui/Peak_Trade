"""Deterministic binding/digest resolution for VEPC v1 eval entry point."""
...
def resolve_measurement_contract(repo_root: Path) -> dict[str, Any]:
    ...
    # Historical slot CONSUMED_NO_RETRY; evaluation not re-executed.
    _require(
    ...
    _require(
        contract.get("development_evaluation_executed") is False,
        "DEV_EVAL_EXECUTED_TRUE",
    )
...
def resolve_strategy_params(contract: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not frozen:
        frozen = {
            ...
            "continuation_requires_pullback_first": True,
            "event_consumption": "SINGLE_USE",
            "ex_ante_exit_reachability_required": True,
            "expansion_min_consecutive_bars": 4,
            "expansion_percentile_inclusive_min": 0.65,
            "immediate_breakout_without_pullback_forbidden": True,
            "initial_stop_atr_multiple": 1.5,
            "initial_stop_atr_period": 14,
            "max_entries_per_expansion_pullback_sequence": 1,
            "max_pullback_bars_inclusive": 8,
            "max_pullback_fraction_of_impulse_range": 0.5,
            "min_post_fill_bars_required_inclusive": 48,
            "min_pullback_fraction_of_impulse_range": 0.15,
...
def assert_shared_channel_core_bound() -> None:
    ...
    from src.research import volatility_expansion_pullback_continuation_v1_strategy_v1 as strategy
...
def assert_exit_state_machine_bound() -> None:
    """The strategy producer must import the frozen VEPC exit state machine (no local reimpl)."""
    from src.research import (
        volatility_expansion_pullback_continuation_v1_exit_state_machine_v1 as exit_sm,
    )
    from src.research import (
        volatility_expansion_pullback_continuation_v1_strategy_v1 as strategy,
    )
...
def materialize_entry_point_binding_payload(repo_root: Path) -> dict[str, Any]:
    ...
    return {
        "artifact_kind": (
            "volatility_expansion_pullback_continuation_v1_development_evaluation_entry_point_binding"
        ),
        "artifact_version": "v1",
        ...
        "evidence_ref": "docs/evidence/evaluate_volatility_expansion_pullback_continuation_development_v1/",
        ...
        "governance_ref": (
            ...
            "HISTORICAL_SLOT_CONSUMED_NO_RETRY_AND_BASELINE_END_OF_SERIES_PAIRING_V1.md"
        ),
        ...
        "schema_version": (
            "volatility_expansion_pullback_continuation_v1_development_evaluation_entry_point_binding.v1"
        ),
        "shared_channel_core_bound": True,
        "shared_channel_core_ref": SHARED_CHANNEL_CORE_REL_PATH,
        "signal_family": SIGNAL_FAMILY,
        "slice_class": "STRATEGY_IMPLEMENTATION_PLUS_UNAUTHORIZED_DEVELOPMENT_ENTRY_POINT",
        ...
        "time_segment_definition_id": TIME_SEGMENT_DEFINITION_ID,
        "verdict": (
            "HISTORICAL_DEVELOPMENT_SLOT_CONSUMED_NO_RETRY_AWAITING_SEPARATE_OPERATOR_GO_"
            "FOR_SUCCESSOR_OR_INFRASTRUCTURE_SCOPE"
        ),
    }
