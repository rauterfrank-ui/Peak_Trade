"""Deterministic exit state machine for CROSS_SECTIONAL_LOW_REALIZED_VOLATILITY_CONTINUATION_V1.

Ex-ante, first-event-wins exits for LONG and SHORT independently.
No trailing stop. CS-vol-rank normalization uses cs_rv_rank > 0.45.
Regime invalidation uses cs_rv_rank > 0.60 or cs_rv_rank < 0.05.
...
CS_VOL_RANK_NORMALIZATION_PERCENTILE_GT_V1 = 0.45
REGIME_INVALIDATION_CS_RV_RANK_PERCENTILE_GT_V1 = 0.60
REGIME_INVALIDATION_CS_RV_RANK_PERCENTILE_LT_V1 = 0.05
...
class CslrvcExitReasonV1(str, Enum):
...
def evaluate_exit_on_bar_v1(
    ...
    *,
    ...
    candidates: list[tuple[int, CslrvcExitReasonV1, float]] = []
        ...
        if low <= position.stop_price:
            candidates.append(
                (
                    ...
                    CslrvcExitReasonV1.INITIAL_STOP,
                    position.stop_price,
                )
            )
    else:
    ...
    if cs_rv_rank is not None and cs_rv_rank == cs_rv_rank:
        if cs_rv_rank > CS_VOL_RANK_NORMALIZATION_PERCENTILE_GT_V1:
            candidates.append(
                (
                    _PRECEDENCE_RANK["CROSS_SECTIONAL_VOL_RANK_NORMALIZATION_INVALIDATION"],
                    CslrvcExitReasonV1.CROSS_SECTIONAL_VOL_RANK_NORMALIZATION_INVALIDATION,
                    close,
                )
            )
        if (
            cs_rv_rank > REGIME_INVALIDATION_CS_RV_RANK_PERCENTILE_GT_V1
            or cs_rv_rank < REGIME_INVALIDATION_CS_RV_RANK_PERCENTILE_LT_V1
        ):
            candidates.append(
                (
                    _PRECEDENCE_RANK["REGIME_INVALIDATION"],
                    CslrvcExitReasonV1.REGIME_INVALIDATION,
                    close,
                )
            )
    ...
    if held >= TIME_EXIT_MAX_BARS_V1:
        candidates.append((_PRECEDENCE_RANK["TIME_EXIT"], CslrvcExitReasonV1.TIME_EXIT, close))

    if is_last_instrument_bar:
        candidates.append(
            (
                _PRECEDENCE_RANK["END_OF_INSTRUMENT_LIQUIDATION"],
                CslrvcExitReasonV1.END_OF_INSTRUMENT_LIQUIDATION,
                close,
            )
        )
    if is_last_panel_bar:
        candidates.append(
            (
                ...
                CslrvcExitReasonV1.END_OF_PANEL_LIQUIDATION,
                close,
            )
        )
