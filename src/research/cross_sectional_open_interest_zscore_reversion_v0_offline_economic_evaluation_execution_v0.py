"""Cross-sectional open-interest z-score-reversion v0 offline economic evaluation execution v0.

Deterministic, fail-closed execution infrastructure for the bounded open-interest z-score-reversion
...
PACKAGE_MARKER = "CROSS_SECTIONAL_OPEN_INTEREST_ZSCORE_REVERSION_V0_OFFLINE_ECONOMIC_EVALUATION_EXECUTION_V0=true"

SCHEMA_VERSION = (
    "cross_sectional_open_interest_zscore_reversion_v0_offline_economic_evaluation_execution.v0"
)
EXECUTION_ID = (
...
RUNNER_OWNER = (
    "scripts.ops.run_cross_sectional_open_interest_zscore_reversion_v0_offline_economic_"
...
def run_offline_evaluation_adapter_precheck_v0(
    *,
    ...
    validation_verdict, binding_reasons = validate_versioned_hypothesis_binding_v0(envelope)
...
def _resolve_binding_reference_field_v0(
    *,
    ...
    legacy_key: str,
    zscore_reversion_key: str,
    missing_reason: str,
    conflict_reason: str,
) -> dict[str, Any]:
    legacy_value = container.get(legacy_key)
    zscore_reversion_value = container.get(zscore_reversion_key)
    if legacy_value is not None and zscore_reversion_value is not None:
        if _stable_digest(legacy_value) != _stable_digest(zscore_reversion_value):
            raise EconomicViabilityEvidenceBindingResolutionError(
                conflict_reason,
                detail=f"{legacy_key}!={zscore_reversion_key}",
            )
    if legacy_value is not None:
        return dict(legacy_value)
    if zscore_reversion_value is not None:
        return dict(zscore_reversion_value)
...
def resolve_economic_viability_binding_references_v0(
    ...
    universe_binding = _resolve_binding_reference_field_v0(
        ...
        zscore_reversion_key="pit_universe_binding",
    ...
    fee_model_binding = _resolve_binding_reference_field_v0(
        ...
        zscore_reversion_key="fee_binding",
    ...
    slippage_model_binding = _resolve_binding_reference_field_v0(
        ...
        zscore_reversion_key="slippage_binding",
    ...
    funding_model_binding = _resolve_binding_reference_field_v0(
        ...
        zscore_reversion_key="funding_binding",
...
def materialize_ops_evaluation_config_v0(
    versioned_binding: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    ranking_binding = envelope["ranking_policy_binding"]
    selection_binding = envelope["selection_hold_exit_rotation_binding"]
    return {
        "binding_digest": envelope["binding_digest"],
        "config_digest": envelope["config_digest"],
        "config_schema_version": (
            "cross_sectional_open_interest_zscore_reversion_v0_economic_evaluation_admissibility_v1"
        ),
        "config_version": "v1",
        "cross_sectional_evaluation_binding_v1": {
            ...
            "config_digest": envelope["config_digest"],
            "data_contract_digest": envelope["data_digest"],
            "data_digest": envelope["data_digest"],
            "dataset_binding": {
                "bar_interval": dataset_binding["bar_interval"],
                "binding_version": dataset_binding["binding_version"],
                "credential_access_forbidden": True,
                "dataset_extension": dataset_binding["dataset_extension"],
                ...
                "open_interest_fields": ["open_interest"],
                "open_interest_usage": "open_interest_level_for_score_computation",
                ...
                "panel_dataset_schema": dataset_binding["panel_dataset_schema"],
                "panel_open_interest_dataset_manifest_ref": dataset_binding[
                    "panel_open_interest_manifest_ref"
                ],
                "panel_schema": dataset_binding["panel_dataset_schema"],
                "timestamp_key": "timestamp_utc",
                "timezone": "UTC",
                "warmup_bars": PANEL_WARMUP_BARS,
            },
            "hypothesis_id": RESEARCH_HYPOTHESIS_ID,
            "implementation_digest": envelope["implementation_digest"],
            "parameter_binding": {
                "binding_version": parameter_binding["binding_version"],
                "minimum_panel_bars": parameter_binding["minimum_panel_bars"],
                "no_instrument_substitution": parameter_binding["no_instrument_substitution"],
                "no_universe_expansion": parameter_binding["no_universe_expansion"],
                "open_interest_level_definition": parameter_binding[
                    "open_interest_level_definition"
                ],
                "open_interest_observation_field": parameter_binding[
                    "open_interest_observation_field"
                ],
                "parameter_search_forbidden": parameter_binding["parameter_search_forbidden"],
                "parameters": {
                    "max_bar_staleness_bars": 1,
                    "min_eligible_members_for_rank": ranking_binding[
                        "minimum_rankable_instrument_count"
                    ],
                    "rebalance_interval_bars": selection_binding["rebalance_interval_bars"],
                    "signal_lag_bars": parameter_binding["signal_lag_bars"],
                    "switch_entry_delay_epochs": selection_binding["switch_entry_delay_epochs"],
                },
                "score_formula_expression": parameter_binding["score_formula_expression"],
                "score_formula_version": parameter_binding["score_formula_version"],
                "signal_lag_bars": parameter_binding["signal_lag_bars"],
                "unchanged_retry_of_failed_bindings_forbidden": parameter_binding[
                    "unchanged_retry_of_failed_bindings_forbidden"
                ],
            },
            "period_binding_id": period_binding["period_binding_id"],
        },
...
def verify_execution_start_state_v0(
    *,
    ...
    if envelope.get("parameter_binding", {}).get("rank_lookback_k") is not None:
        reasons.append("RANK_LOOKBACK_K_FORBIDDEN_VIOLATION")
    if envelope.get("parameter_binding", {}).get("delta_lookback_k") is not None:
        reasons.append("DELTA_LOOKBACK_K_FORBIDDEN_VIOLATION")
    selection_mode = (
        envelope.get("binding", {}).get("direction_semantics", {}).get("selection_mode")
    )
    if selection_mode == "open_interest_level_rank_extremes_single_leg_rotation_v0":
    ...
    if selection_mode != "open_interest_zscore_reversion_extremes_single_leg_rotation_v0":
        reasons.append("ZSCORE_SELECTION_MODE_MISMATCH")
...
def run_contract_smoke_evaluation_v0(
    *,
    ...
    binding = default_open_interest_zscore_reversion_operator_binding_v0()
    ...
    orchestrator = run_cross_sectional_open_interest_zscore_reversion_orchestrator_v0(
        binding=binding,
    ...
    backtest = run_single_slot_panel_backtest_v0(
        orchestrator,
        panel_series,
        cost_execution_binding=_normalize_cost_execution_binding_for_backtest_v0(cost_binding),
    )
...
def materialize_economic_viability_evidence(
    *,
    ...
    body: dict[str, Any] = {
        "schema_version": "economic_viability_evidence_cross_sectional_open_interest_zscore_reversion_v0",
        "strategy_id": STRATEGY_ID,
        "strategy_version": STRATEGY_VERSION,
