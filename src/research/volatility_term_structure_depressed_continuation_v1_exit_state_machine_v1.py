"""Deterministic exit state machine for VTDC v1.

Ex-ante, first-event-wins exits for LONG and SHORT independently.
No trailing stop. Term-structure normalization uses ratio percentile > 0.45.
Regime invalidation uses ratio percentile > 0.55.
...
TERM_STRUCTURE_NORMALIZATION_RATIO_PERCENTILE_GT_V1 = 0.45
REGIME_INVALIDATION_RATIO_PERCENTILE_GT_V1 = 0.55
...
class VtdcExitReasonV1(str, Enum):
    INITIAL_STOP = "INITIAL_STOP"
    TERM_STRUCTURE_NORMALIZATION_INVALIDATION = "TERM_STRUCTURE_NORMALIZATION_INVALIDATION"
...
def evaluate_exit_on_bar_v1(
    ...
    *,
    ...
    low: float,
    close: float,
    ratio_percentile: Optional[float],
    ...
    candidates: list[tuple[int, VtdcExitReasonV1, float]] = []
    ...
    if ratio_percentile is not None and ratio_percentile == ratio_percentile:
        if ratio_percentile > TERM_STRUCTURE_NORMALIZATION_RATIO_PERCENTILE_GT_V1:
            candidates.append(
                (
                    _PRECEDENCE_RANK["TERM_STRUCTURE_NORMALIZATION_INVALIDATION"],
                    VtdcExitReasonV1.TERM_STRUCTURE_NORMALIZATION_INVALIDATION,
                    close,
                )
            )
        if ratio_percentile > REGIME_INVALIDATION_RATIO_PERCENTILE_GT_V1:
            candidates.append(
                (
                    _PRECEDENCE_RANK["REGIME_INVALIDATION"],
                    VtdcExitReasonV1.REGIME_INVALIDATION,
                    close,
                )
            )
    ...
    if held >= TIME_EXIT_MAX_BARS_V1:
        candidates.append((_PRECEDENCE_RANK["TIME_EXIT"], VtdcExitReasonV1.TIME_EXIT, close))

    if is_last_instrument_bar:
        candidates.append(
            (
                _PRECEDENCE_RANK["END_OF_INSTRUMENT_LIQUIDATION"],
                VtdcExitReasonV1.END_OF_INSTRUMENT_LIQUIDATION,
                close,
            )
        )
    if is_last_panel_bar:
        candidates.append(
            (
                ...
                VtdcExitReasonV1.END_OF_PANEL_LIQUIDATION,
                close,
            )
        )
