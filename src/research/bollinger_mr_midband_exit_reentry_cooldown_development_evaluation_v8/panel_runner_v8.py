...
class _LifecycleCheckpointTrackerV8:
...
def _claim_run_slot_atomic_v8(
    ...
    payload = {
        "schema_version": "bollinger_mr_midband_exit_reentry_cooldown_development_v8_run_slot_claim.v1",
...
def run_arm(
    *,
    ...
    lc_tracker: _LifecycleCheckpointTrackerV8 | None = None,
) -> ArmResult:
...
def _assert_no_v6_partial_reuse(contract: Mapping[str, Any]) -> None:
    if contract.get("predecessor_hypothesis_id") != REQUIRED_PREDECESSOR_HYPOTHESIS_ID:
        raise RuntimeError("PREDECESSOR_HYPOTHESIS_ID_MISMATCH")
    ...
    if contract.get("v7_partial_results_reused") is True:
        raise RuntimeError("V7_PARTIAL_RESULTS_REUSED_FORBIDDEN")
    if contract.get("v7_reopen_forbidden") is not True:
        raise RuntimeError("V7_REOPEN_FORBIDDEN_REQUIRED")
    if contract.get("v7_rerun_forbidden") is not True:
        raise RuntimeError("V7_RERUN_FORBIDDEN_REQUIRED")
    if contract.get("not_a_v7_reopen") is not True:
        raise RuntimeError("NOT_A_V7_REOPEN_REQUIRED")


def assert_v8_frozen_exit_parameters_unchanged(contract: Mapping[str, Any] | None = None) -> None:
    """Bind runner freeze to V8 contract SSOT (+ V6 composite identity). No defaults."""
    ...
    if contract is not None:
        try:
            ...
            raise MidbandExitMechanismError(str(exc)) from exc


def assert_v8_authority_and_prereg_gates(
    *,
    repo: Path,
    ...
    # Pre-authorization frozen-parameter parity BEFORE authority/slot/panel.
    contract_pre = json.loads((repo / CONTRACT_REL_PATH).read_text(encoding="utf-8"))
    try:
        validate_pre_authorization_frozen_parameter_parity(contract_pre)
    except PreAuthorizationParityError as exc:
        raise RuntimeError(f"PRE_AUTHORIZATION_PARITY:{exc}") from exc
    report = load_and_validate_repo_contract(repo)
    if report.get("hypothesis_id") != HYPOTHESIS_ID:
        raise RuntimeError("CONTRACT_HYPOTHESIS_ID_NOT_V8")
    ...
    if require_evaluation_authorized and not eval_auth:
        raise RuntimeError(
            f"V8_EVALUATION_NOT_AUTHORIZED:{effective.get('reason') or 'effective_false'}"
        )
...
def run_preflight_only(
    *,
    ...
    gates = assert_v8_authority_and_prereg_gates(
        repo=repo,
    ...
    payload = {
        "schema_version": "bollinger_mr_v8_preflight_only.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "mode": "preflight",
...
def run_development_evaluation(
    *,
    ...
    started_at = _utc_now()
    lc_tracker = _LifecycleCheckpointTrackerV8(
    ...
    claim = _claim_run_slot_atomic_v8(output_dir, repo=repo, started_at=started_at)
    ...
    try:
        assert_v8_frozen_exit_parameters_unchanged(contract)
    except MidbandExitMechanismError as exc:
        summary = {
            "schema_version": "evaluate_bollinger_mr_midband_exit_reentry_cooldown_development_summary.v8",
            "evaluation_run_id": EVALUATION_RUN_ID,
    ...
    try:
        ...
        decision_out = decide_development_evaluation_v8(
