"""Definition-only preregistration validator for RSI-exhaustion MR eligibility v1.
...
Orthogonal to the failed ATR-percentile mid-band entry-effective contract
(``entry_effective_mr_eligibility_preregistered_economic_hypothesis_measurement_contract.v1``,
FAIL: net_profit_factor_not_improved_despite_entry_eligibility_divergence) and to the
failed regime-gated stand-aside contract
(``regime_gated_standaside_mr_preregistered_economic_hypothesis_measurement_contract.v1``,
FAIL: identical_arms_gate_inactive_on_entries). This filter uses RSI(14) momentum-exhaustion
confirmation instead of a volatility-regime or absolute-threshold regime mechanism.
"""
...
PACKAGE_MARKER = "RSI_EXHAUSTION_MR_ELIGIBILITY_HYPOTHESIS_PREREGISTRATION_V1=true"
CONTRACT_REL_PATH = (
    "config/research/"
    "rsi_exhaustion_mr_eligibility_preregistered_economic_hypothesis_measurement_contract_v1.json"
)
...
REQUIRED_HYPOTHESIS_ID = "RSI_EXHAUSTION_MR_ENTRY_ELIGIBILITY_NON_BITCOIN_PERPETUALS_V1"
...
REQUIRED_FILTER_ID = "canonical_rsi_exhaustion_entry_eligibility_v1"
...
def validate_preregistration_contract(
    ...
    *,
    ...
    if not isinstance(prior_failed, list) or len(prior_failed) < 2:
        raise HypothesisPreregistrationError("PRIOR_FAILED_HYPOTHESES_MUST_LIST_BOTH_PRIORS")
