"""Valid evaluable rebalance-observation semantics for CS open-gap fade v1.
...
def is_valid_evaluable_rebalance_observation(
    intent: CrossSectionalOpenGapRankIntentV1,
) -> bool:
...
def classify_rebalance_observation(
    intent: CrossSectionalOpenGapRankIntentV1, *, timestamp_utc: str
...
def collect_valid_evaluable_rebalance_observations(
    ...
    *,
    ...
    if len(timestamps_utc) == 0:
        return ()
    lengths: list[int] = []
    for series in panel_by_instrument.values():
        opens = series.get("open")
        closes = series.get("close")
        if opens is None or closes is None:
            continue
        lengths.append(min(len(opens), len(closes)))
    bar_count = min(lengths) if lengths else 0
    bar_count = min(bar_count, len(timestamps_utc))
    observations: list[RebalanceObservationV1] = []
    prior: CrossSectionalOpenGapRankIntentV1 | None = None
