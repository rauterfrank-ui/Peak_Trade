"""Single-run DEVELOPMENT panel evaluation v6: baseline vs composite exit-efficiency gate.

New independently preregistered DEVELOPMENT_ONLY measurement
(HYPOTHESIS_ID ...DEVELOPMENT_V6). Not a rerun of V1–V5. Uses composite
midband-cross OR max-holding-horizon=48h treatment; no V5 partial result,
...
def _claim_run_slot_atomic_v6(
    ...
    *,
    ...
    payload = {
        "schema_version": "bollinger_mr_midband_exit_efficiency_development_v6_run_slot_claim.v1",
...
def run_arm(
    *,
    ...
    for i, member in enumerate(members, start=1):
        ...
        midband_exit_total += int(counters["midband_exit_count"])
        max_holding_exit_total += int(counters["max_holding_exit_count"])
...
def _assert_no_predecessor_partial_result_reuse(contract: Mapping[str, Any]) -> None:
    ...
    predecessor = contract.get("predecessor_development_v5") or {}
    if predecessor.get("hypothesis_id") != REQUIRED_PREDECESSOR_HYPOTHESIS_ID:
        raise RuntimeError("PREDECESSOR_V5_HYPOTHESIS_ID_MISMATCH")
    if predecessor.get("partial_results_reused") is not False:
        raise RuntimeError("PREDECESSOR_V5_PARTIAL_RESULTS_REUSED")
...
def run_development_evaluation(
    *,
    ...
    # Validate against the V6 preregistration validator BEFORE any measurement /
    # contract mutation. The V6 validator accepts DEFINITION_ONLY_PREREGISTERED
    ...
    if report.get("hypothesis_id") != HYPOTHESIS_ID:
        raise RuntimeError("CONTRACT_HYPOTHESIS_ID_NOT_V6")
    ...
    _claim_run_slot_atomic_v6(output_dir, repo=repo, started_at=started_at)
    lc_tracker.mark(
        LIFECYCLE_STATE_RUN_SLOT_CLAIMED,
    ...
    if validity.get("passed") is not True:
        ...
        command = (
            ...
            "run_evaluate_bollinger_mr_midband_exit_efficiency_development_v6.py "
            f"--output-dir {output_dir}"
        )
        summary = {
            "schema_version": "evaluate_bollinger_mr_midband_exit_efficiency_development_summary.v6",
            "evaluation_run_id": EVALUATION_RUN_ID,
    ...
    exit_attribution = {
        ...
        "midband_exit_count": int(treatment.metrics["midband_exit_count"]),
        "max_holding_exit_count": int(treatment.metrics["max_holding_exit_count"]),
    ...
    (output_dir / "trade_ledger_baseline.json").write_text(
        json.dumps(
            {
                "schema_version": "bollinger_mr_midband_exit_efficiency_trade_ledger.v6",
                "arm": "baseline",
    ...
    config_snapshot = {
        "schema_version": "evaluate_bollinger_mr_midband_exit_efficiency_development_config_snapshot.v6",
        "hypothesis_id": HYPOTHESIS_ID,
        "predecessor_hypothesis_id": REQUIRED_PREDECESSOR_HYPOTHESIS_ID,
        "contract_id": contract["schema_version"],
    ...
    v6_pkg = repo / "src/research/bollinger_mr_midband_exit_efficiency_development_evaluation_v6"
    code_hashes = {
        "composite_midband_max_holding_exit_mechanism_v6.py": _sha256_file(
            v6_pkg / "composite_midband_max_holding_exit_mechanism_v6.py"
        ),
        "composite_exit_efficiency_gate_v6.py": _sha256_file(
            v6_pkg / "composite_exit_efficiency_gate_v6.py"
        ),
        "decision_v1.py": _sha256_file(v1_pkg / "decision_v1.py"),
        "panel_runner_v6.py": _sha256_file(v6_pkg / "panel_runner_v6.py"),
    ...
    (output_dir / "safety_attestation.md").write_text(
        "\n".join(
            [
                "# Safety attestation — composite exit-efficiency DEVELOPMENT evaluation v6",
                "",
