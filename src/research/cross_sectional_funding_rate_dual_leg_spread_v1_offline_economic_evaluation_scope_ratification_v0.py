"""Cross-sectional funding-rate dual-leg spread v1 offline economic evaluation scope ratification v0.

Deterministic, fail-closed ratification of bounded offline-only economic evaluation
scope for cross_sectional_funding_rate_dual_leg_spread/v1. Does not execute evaluation.
"""
...
PACKAGE_MARKER = (
    "CROSS_SECTIONAL_FUNDING_RATE_DUAL_LEG_SPREAD_V1_OFFLINE_ECONOMIC_EVALUATION_"
    "SCOPE_RATIFICATION_V0=true"
)

SCHEMA_VERSION = "cross_sectional_funding_rate_dual_leg_spread_v1_offline_economic_evaluation_scope_ratification.v0"
RATIFICATION_ID = "cross_sectional_funding_rate_dual_leg_spread_v1_offline_economic_evaluation_scope_ratification_v0"
...
RECOMMENDED_SCOPE_ID = (
    "CROSS_SECTIONAL_FUNDING_RATE_DUAL_LEG_SPREAD_V1_OFFLINE_ECONOMIC_EVALUATION_RATIFICATION_PREP"
)
...
OPERATOR_SCOPE_RATIFICATION_REF = (
    "bounded_cs_funding_rate_dual_leg_spread_v1_scope_binding_ratification_v0"
)
CONFIG_REL_PATH = (
    "config/research/"
    "cross_sectional_funding_rate_dual_leg_spread_v1_offline_economic_evaluation_scope_ratification_v0.json"
)
FUTURE_RUNNER_BINDING_REF = (
    "scripts/ops/"
    "run_cross_sectional_funding_rate_dual_leg_spread_v1_offline_economic_evaluation_execution_v0.py"
)
...
def _required_bindings_matrix_v1(
...
def _all_required_bindings_ratified_v1(matrix: Mapping[str, Mapping[str, str]]) -> bool:
    return all(entry.get("status") in {"BOUND", "COMPLETE"} for entry in matrix.values())


def materialize_dual_leg_spread_offline_economic_evaluation_scope_ratification_v1(
    *,
    repo_root: Path | None = None,
    versioned_binding: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    _ = repo_root
    envelope = dict(versioned_binding or materialize_versioned_research_binding_v1())
    binding = envelope["binding"]
    ...
    required_bindings_matrix = _required_bindings_matrix_v1(envelope)
    all_required_bindings_ratified = _all_required_bindings_ratified_v1(required_bindings_matrix)
    ...
    body: dict[str, Any] = {
        ...
        "material_difference_basis": (
            "funding_level_spread_dual_leg_simultaneous_not_pr4925_delta_single_slot_rotation"
        ),
        "material_difference_vs_pr4925_confirmed": MATERIAL_DIFFERENCE_VS_PR4925_CONFIRMED,
        ...
        "parent_scope_definition_bundle": PARENT_SCOPE_DEFINITION_BUNDLE,
        "parent_pr4925_negative_terminalization_bundle": PARENT_PR4925_NEGATIVE_TERMINALIZATION_BUNDLE,
        "parent_pr4925_evaluation_bundle": PARENT_PR4925_EVALUATION_BUNDLE,
...
def validate_dual_leg_spread_offline_economic_evaluation_scope_ratification_v1(
    ...
    *,
    ...
    if ratification.get("material_difference_vs_pr4925_confirmed") is not True:
        reasons.append("MATERIAL_DIFFERENCE_NOT_CONFIRMED")
    if ratification.get("unchanged_retry") is not False:
        reasons.append("UNCHANGED_RETRY_MUST_BE_FALSE")
    if ratification.get("parameter_rescue") is not False:
        reasons.append("PARAMETER_RESCUE_MUST_BE_FALSE")
    if ratification.get("threshold_relaxation") is not False:
        reasons.append("THRESHOLD_RELAXATION_MUST_BE_FALSE")
    ...
    if "cross_sectional_funding_rate_delta_momentum/v0" not in ratification.get(
        "terminal_failed_binding_exclusions", []
    ):
        reasons.append("PR4925_EXCLUSION_MISSING")
