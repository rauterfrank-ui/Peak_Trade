"""Definition-only preregistration validator for ADX range-admission MR eligibility v1.
...
Orthogonal to three prior FAILs on the same DEVELOPMENT panel:
regime-gated absolute-threshold classifier, ATR-percentile mid-band eligibility,
and RSI(14) exhaustion eligibility. This filter uses single-feature Wilder ADX(14)
range admission (ELIGIBLE iff ADX < 25) from TrendFollowingStrategy._compute_adx.
"""
...
PACKAGE_MARKER = "ADX_RANGE_ADMISSION_MR_ELIGIBILITY_HYPOTHESIS_PREREGISTRATION_V1=true"
CONTRACT_REL_PATH = (
    "config/research/"
    "adx_range_admission_mr_eligibility_preregistered_economic_hypothesis_measurement_contract_v1.json"
)
...
REQUIRED_HYPOTHESIS_ID = "ADX_RANGE_ADMISSION_MR_ENTRY_ELIGIBILITY_NON_BITCOIN_PERPETUALS_V1"
...
REQUIRED_FILTER_ID = "canonical_adx_range_admission_entry_eligibility_v1"
...
def validate_preregistration_contract(
    ...
    *,
    ...
    if not isinstance(prior_failed, list) or len(prior_failed) < 3:
