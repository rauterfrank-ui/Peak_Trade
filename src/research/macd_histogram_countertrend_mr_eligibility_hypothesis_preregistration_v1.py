"""Definition-only preregistration validator for MACD histogram-countertrend MR eligibility v1.
...
Orthogonal to five prior FAILs on the same DEVELOPMENT panel:
regime-gated absolute-threshold classifier, ATR-percentile mid-band eligibility,
RSI(14) exhaustion eligibility, ADX(14) range-admission eligibility, and
MA SMA(50) with-trend admission. This filter uses single-feature MACD(12,26,9)
histogram-sign countertrend admission (long iff histogram < 0; short iff histogram > 0)
from src/strategies/macd.py.
"""
...
PACKAGE_MARKER = "MACD_HISTOGRAM_COUNTERTREND_MR_ELIGIBILITY_HYPOTHESIS_PREREGISTRATION_V1=true"
CONTRACT_REL_PATH = (
    "config/research/"
    "macd_histogram_countertrend_mr_eligibility_preregistered_economic_hypothesis_measurement_contract_v1.json"
)
...
REQUIRED_HYPOTHESIS_ID = "MACD_HISTOGRAM_COUNTERTREND_ELIGIBILITY_NON_BITCOIN_PERPETUALS_V1"
...
REQUIRED_FILTER_ID = "canonical_macd_histogram_countertrend_entry_eligibility_v1"
...
def validate_preregistration_contract(
    ...
    *,
    ...
    if not isinstance(prior_failed, list) or len(prior_failed) < 5:
