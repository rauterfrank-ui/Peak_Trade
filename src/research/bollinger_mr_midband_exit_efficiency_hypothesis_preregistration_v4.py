"""Definition-only preregistration validator for Bollinger/MR midband exit-efficiency v4.
...
V4 is a new independently versioned DEVELOPMENT_ONLY measurement ID with enhanced
measurement-validity gates relative to V3 (effective-config inequality, open_side
binding, exit observability, synthetic divergence) and the same exit mechanism.
V3 remains terminal FAIL (run count 1; identical_arms_no_exit_divergence;
MEASUREMENT_BINDING_COLLAPSE) and must not be rerun. V2 remains terminal
INCONCLUSIVE_INFRASTRUCTURE_FAILURE. V1 remains terminal and unchanged.
Future evaluation (separate Operator-GO) must bind observability, falsy-zero
hygiene, and MV2_WIRING_MOD_CAPTURE_ALIAS_OPEN_SIDE_BINDING_FIX.
"""
...
PACKAGE_MARKER = "BOLLINGER_MR_MIDBAND_EXIT_EFFICIENCY_HYPOTHESIS_PREREGISTRATION_V4=true"
CONTRACT_REL_PATH = (
    "config/research/"
...
EXPECTED_DEVELOPMENT_PREREGISTRATION_DIGEST = (
    "7f0cc8abcf08610ec1a364bd4ef4861c5d4969704d90b1e041f18a8568d8c228"
)
...
def assert_v3_terminal_preserved(v3_contract: Mapping[str, Any]) -> None:
    if v3_contract.get("hypothesis_id") != REQUIRED_PREDECESSOR_HYPOTHESIS_ID:
...
def validate_preregistration_contract(
    ...
    *,
    ...
    pred = contract.get("predecessor_development_v3")
    if not isinstance(pred, Mapping):
        raise HypothesisPreregistrationError("PREDECESSOR_V3_REQUIRED")
    ...
    if pred.get("root_cause_detail") != V3_TERMINAL_ROOT_CAUSE:
        raise HypothesisPreregistrationError("PREDECESSOR_ROOT_CAUSE_MISMATCH")
    if pred.get("panel_backtest_executed") is not True:
        raise HypothesisPreregistrationError("PREDECESSOR_PANEL_BACKTEST_MUST_BE_TRUE")
    if pred.get("economic_metrics_produced") is not True:
        raise HypothesisPreregistrationError("PREDECESSOR_ECONOMIC_METRICS_MUST_BE_TRUE")
    if pred.get("decision_reason") != "identical_arms_no_exit_divergence":
        raise HypothesisPreregistrationError("PREDECESSOR_DECISION_REASON_MISMATCH")
    ...
    if owner_map is not None:
        ...
        if REQUIRED_OWNER_SURFACE not in owners:
            raise HypothesisPreregistrationError("V4_OWNER_MAP_ENTRY_MISSING")
    ...
    if terminal_result_class is None:
        return {
            ...
            "definition_semantics_identical_to_v3": False,
