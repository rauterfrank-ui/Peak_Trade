"""Definition-only preregistration validator for VEPC v1."""
...
PACKAGE_MARKER = "VOLATILITY_EXPANSION_PULLBACK_CONTINUATION_V1_HYPOTHESIS_PREREGISTRATION=true"
CONTRACT_REL_PATH = (
    "config/research/"
    "volatility_expansion_pullback_continuation_v1_preregistered_economic_hypothesis_"
    "measurement_contract_v1.json"
)
GOVERNANCE_REL_PATH = (
    "docs/governance/VOLATILITY_EXPANSION_PULLBACK_CONTINUATION_V1_"
...
REQUIRED_HYPOTHESIS_ID = "VOLATILITY_EXPANSION_PULLBACK_CONTINUATION_NON_BITCOIN_PERPETUALS_V1"
REQUIRED_STRATEGY_IDENTITY = "VOLATILITY_EXPANSION_PULLBACK_CONTINUATION_V1"
REQUIRED_PREDECESSOR = "VOLATILITY_CONTRACTION_EXPANSION_BREAKOUT_V1"
REQUIRED_PROGRAM_ID = "VOLATILITY_REGIME_RESEARCH_PROGRAM_V1"
REQUIRED_SIGNAL_FAMILY = "VOLATILITY_REGIME"
REQUIRED_TARGET = "VOLATILITY_EXPANSION_THEN_LIMITED_PULLBACK_CONTINUATION"
REQUIRED_DIRECTIONAL_FORM = "OWN_INSTRUMENT_MUTUALLY_EXCLUSIVE_PULLBACK_CONTINUATION"
...
def validate_measurement_contract(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(
        ...
        == "VOLATILITY_EXPANSION_LIMITED_PULLBACK_CONTINUATION_ADMISSION",
        "BASELINE_SOLE_DIFF",
    )
    ...
    _require(pullback.get("max_pullback_bars_inclusive") == 8, "PULLBACK_BARS")
    _require(pullback.get("min_pullback_fraction_of_impulse_range") == 0.15, "PULLBACK_MIN")
    _require(pullback.get("max_pullback_fraction_of_impulse_range") == 0.50, "PULLBACK_MAX")
    _require(pullback.get("pullback_required_before_entry") is True, "PULLBACK_REQUIRED")
    _require(
        pullback.get("immediate_breakout_entry_without_pullback_forbidden") is True,
        "IMMEDIATE_BREAKOUT_ALLOWED",
    )

    cont = admission.get("continuation_entry") or {}
    _require(cont.get("entry_only_after_pullback_then_continuation") is True, "CONTINUATION_ORDER")
    ...
    _require(cont.get("no_entry_on_expansion_confirmation_bar") is True, "ENTRY_ON_EXPANSION")
    _require(
        cont.get("no_immediate_post_expansion_breakout_entry") is True,
    ...
    pb_inv = exits.get("pullback_structure_invalidation") or {}
    _require(pb_inv.get("authorized") is True, "PULLBACK_INVALIDATION_NOT_AUTHORIZED")
    ...
    _require(frozen.get("continuation_requires_pullback_first") is True, "FROZEN_PULLBACK_FIRST")
    ...
    _require(md_vceb.get("prior_strategy_identity") == REQUIRED_PREDECESSOR, "PRIOR_VCEB")
    ...
    return {
        ...
        "material_difference_from_vceb_v1": True,
