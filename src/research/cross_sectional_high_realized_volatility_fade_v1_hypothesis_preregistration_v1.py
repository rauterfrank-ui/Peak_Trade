"""Definition-only preregistration validator for CSHRVF v1."""
...
PACKAGE_MARKER = "CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_V1_HYPOTHESIS_PREREGISTRATION=true"
CONTRACT_REL_PATH = (
    "config/research/"
    "cross_sectional_high_realized_volatility_fade_v1_preregistered_economic_hypothesis_"
    "measurement_contract_v1.json"
)
GOVERNANCE_REL_PATH = (
    "docs/governance/CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_V1_"
    "PREREGISTERED_HYPOTHESIS_MEASUREMENT_V1.md"
)
EVIDENCE_REL_PATH = (
    "docs/evidence/preregister_cross_sectional_high_realized_volatility_fade_hypothesis_v1/"
)
REQUIRED_HYPOTHESIS_ID = "CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_NON_BITCOIN_PERPETUALS_V1"
...
REQUIRED_TARGET = "CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE"
REQUIRED_DIRECTIONAL_FORM = (
    "OWN_INSTRUMENT_MUTUALLY_EXCLUSIVE_CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE"
)
...
REQUIRED_TREATMENT = "OWN_INSTRUMENT_CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_ADMISSION"
...
def validate_measurement_contract(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(
        ...
        == "CROSS_SECTIONAL_HIGH_REALIZED_VOLATILITY_FADE_ADMISSION",
        "BASELINE_SOLE_DIFF",
    )
    ...
    rank_metric = vol.get("cross_sectional_rank_metric") or {}
    _require(
        rank_metric.get("metric") == "CROSS_SECTIONAL_PERCENTILE_RANK_OF_RV_LEVEL",
        "CS_RANK_METRIC",
    )
    _require(rank_metric.get("lookahead_forbidden") is True, "CS_RANK_LOOKAHEAD")

    state = admission.get("cross_sectional_vol_rank_state") or {}
    _require(state.get("high_rank_percentile_inclusive_min") == 0.80, "CS_HIGH_THR")
    _require(state.get("min_consecutive_high_rank_bars") == 2, "CS_HIGH_BARS")
    _require(state.get("low_rank_entry_forbidden_in_v1") is True, "LOW_RANK_ENTRY_ALLOWED")
    _require(state.get("pit_cross_section_only") is True, "PIT_CS_ONLY")

    fade = admission.get("cross_sectional_high_vol_fade_entry") or {}
    _require(
        fade.get("entry_only_after_cross_sectional_high_rv_rank_state") is True,
        "FADE_ORDER",
    )
    ...
    _require(fade.get("no_term_structure_ratio_required") is True, "TS_RATIO_REQUIRED")
    _require(
        fade.get("vtdc_depressed_continuation_entry_forbidden") is True,
        "VTDC_ENTRY_ALLOWED",
    )
    _require(
        fade.get("vtsr_elevated_reversion_fade_entry_forbidden") is True,
        "VTSR_ENTRY_ALLOWED",
    )
    _require(
    ...
    _require(fade.get("low_cross_sectional_vol_entry_forbidden") is True, "LOW_CS_VOL_ALLOWED")
    ...
    _require(lifecycle.get("vtdc_retry_forbidden") is True, "VTDC_RETRY_ALLOWED")
    ...
    _require(
        reclaim.get("rule") == "cs_rv_rank_percentile_falls_strictly_below_0_55",
        "CS_NORM_RULE",
    )
    _require(exits.get("regime_invalidation_cs_rv_rank_percentile_lt") == 0.40, "REGIME_THR")
    ...
    md = (
        payload.get("material_difference_vs_volatility_term_structure_depressed_continuation_v1")
        or {}
    )
    _require(md.get("prior_strategy_identity") == REQUIRED_PREDECESSOR, "MD_VTDC_PRIOR")
    _require(md.get("not_a_parameter_change_of_vtdc_v1") is True, "MD_VTDC_PARAM")
    _require(md.get("not_a_repair_or_retry_of_vtdc_v1") is True, "MD_VTDC_RETRY")
    _require(md.get("vtdc_retry_forbidden") is True, "MD_VTDC_RETRY_FLAG")
    ...
    for key in (
        ...
        _require(bool(diffs.get(key)), f"MD_VTDC_MISSING:{key}")
    ...
    _require(bias.get("pit_panel_only") is True, "PIT_PANEL")
    leak = bias.get("leakage_controls") or {}
    _require(leak.get("no_holdout_access") is True, "LEAK_HOLDOUT")
    _require(leak.get("dataset_binding_only_no_panel_load") is True, "LEAK_PANEL_LOAD")
    ...
    return {
        ...
        "materially_distinct_from_vtdc": True,
    }
