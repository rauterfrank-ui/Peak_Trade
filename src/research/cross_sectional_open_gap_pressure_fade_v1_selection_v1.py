"""Deterministic single-top1 rank-intent selection for CS open-gap fade v1.

Directional form D: mutually exclusive directional selection via
``single_top1_by_score_desc`` + ``symmetric_top1_sign``.

Fade semantics:
- positive top1 score (extreme gap-down) → LONG_TOP1
- negative top1 score (extreme gap-up) → SHORT_TOP1
...
PACKAGE_MARKER = "CROSS_SECTIONAL_OPEN_GAP_PRESSURE_FADE_V1_SELECTION_V1=true"
...
class CrossSectionalOpenGapRankIntentV1:
    strategy_id: str
...
def select_single_top1_rank_intent_v1(
    panel_by_instrument: Mapping[str, Mapping[str, Sequence[float]]],
    *,
    epoch_index: int,
    ...
    prior_intent: CrossSectionalOpenGapRankIntentV1 | None = None,
) -> CrossSectionalOpenGapRankIntentV1:
    """Select single top1 rank intent; hold prior intent between rebalance epochs.

    ``panel_by_instrument`` maps instrument_id → ``{"open","close"}`` sequences
    (minimal PT1H panel wiring for open-gap scoring).
    """
    ...
    if not is_rebalance_epoch_v1(epoch_index, rebalance_interval_bars=rebalance_interval_bars):
        if prior_intent is None:
            return CrossSectionalOpenGapRankIntentV1(
    ...
    scores: list[CrossSectionalOpenGapScoreResultV1] = []
    for instrument_id, series in panel_by_instrument.items():
