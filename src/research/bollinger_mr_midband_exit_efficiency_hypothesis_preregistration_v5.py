"""Definition-only preregistration validator for Bollinger/MR midband exit-efficiency v5.
...
V5 is a new independently versioned DEVELOPMENT_ONLY measurement ID that preserves
the V4 economic hypothesis/exit mechanism/measurement-validity gates and adds
generic process-lifecycle observability with atomic checkpoints and read-only
recovery diagnostics. V4 remains terminal INFRASTRUCTURE_FAILURE
(PROCESS_DIED_INCOMPLETE_PANEL_RUN_NO_LIFECYCLE_TERMINAL; run count 1) and must
not be rerun or reinterpreted as economic PASS/FAIL. V3 remains terminal FAIL.
...
PACKAGE_MARKER = "BOLLINGER_MR_MIDBAND_EXIT_EFFICIENCY_HYPOTHESIS_PREREGISTRATION_V5=true"
CONTRACT_REL_PATH = (
    "config/research/"
...
def assert_v4_terminal_preserved(v4_contract: Mapping[str, Any]) -> None:
    if v4_contract.get("hypothesis_id") != REQUIRED_PREDECESSOR_HYPOTHESIS_ID:
...
def validate_preregistration_contract(
    ...
    *,
    ...
    for flag in (
        ...
        "enhanced_process_lifecycle_observability_relative_to_development_v4",
    ):
    ...
    if contract.get("hypothesis_id") == REQUIRED_PREDECESSOR_HYPOTHESIS_ID:
        raise HypothesisPreregistrationError("HYPOTHESIS_ID_MUST_NOT_EQUAL_V4")
    ...
    pred = contract.get("predecessor_development_v4")
    if not isinstance(pred, Mapping):
        raise HypothesisPreregistrationError("PREDECESSOR_V4_REQUIRED")
    ...
    if pred.get("root_cause_detail") != V4_TERMINAL_ROOT_CAUSE:
    ...
    if pred.get("diagnostic_class") != V4_TERMINAL_ROOT_CAUSE:
    ...
    if life_bind.get("does_not_authorize_v4_rerun") is not True:
        raise HypothesisPreregistrationError("LIFECYCLE_MUST_NOT_AUTHORIZE_V4_RERUN")
    if life_bind.get("v4_terminal_unchanged") is not True:
        raise HypothesisPreregistrationError("LIFECYCLE_V4_TERMINAL_UNCHANGED_REQUIRED")
    if owner_map is not None:
        owners = owner_map.get("allowed_optimization_surfaces") or {}
    ...
    if terminal_result_class is None:
        return {
            ...
            "definition_semantics_identical_to_v4_economic": True,
            "identical_exit_mechanism_to_development_v4": True,
            "identical_economic_hypothesis_to_development_v4": True,
            "enhanced_process_lifecycle_observability_relative_to_development_v4": True,
            "identical_exit_mechanism_to_development_v3": True,
