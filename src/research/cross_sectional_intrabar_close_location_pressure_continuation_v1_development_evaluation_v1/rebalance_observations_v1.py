"""Valid evaluable rebalance-observation semantics for CS intrabar CLV v1.
...
def is_valid_evaluable_rebalance_observation(
    intent: CrossSectionalIntrabarClvRankIntentV1,
) -> bool:
...
def classify_rebalance_observation(
    intent: CrossSectionalIntrabarClvRankIntentV1, *, timestamp_utc: str
...
def collect_valid_evaluable_rebalance_observations(
    ...
    *,
    ...
    if len(timestamps_utc) == 0:
        return ()
    bar_count = 0
    if ohlc_by_instrument:
        bar_count = min(len(v.get("close", ())) for v in ohlc_by_instrument.values())
    bar_count = min(bar_count, len(timestamps_utc))
    observations: list[RebalanceObservationV1] = []
    prior: CrossSectionalIntrabarClvRankIntentV1 | None = None
