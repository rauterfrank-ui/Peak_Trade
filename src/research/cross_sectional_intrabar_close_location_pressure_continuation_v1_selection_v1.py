"""Deterministic single-top1 rank-intent selection for CS intrabar CLV v1.
...
PACKAGE_MARKER = (
    "CROSS_SECTIONAL_INTRABAR_CLOSE_LOCATION_PRESSURE_CONTINUATION_V1_SELECTION_V1=true"
)
...
class CrossSectionalIntrabarClvRankIntentV1:
    strategy_id: str
...
def select_single_top1_rank_intent_v1(
    ohlc_by_instrument: Mapping[str, Mapping[str, Sequence[float]]],
    *,
    epoch_index: int,
    ...
    prior_intent: CrossSectionalIntrabarClvRankIntentV1 | None = None,
) -> CrossSectionalIntrabarClvRankIntentV1:
    """Select single top1 rank intent; hold prior intent between rebalance epochs.

    ``ohlc_by_instrument`` maps instrument_id → ``{"high","low","close"}`` sequences.
    """
    ...
    if not is_rebalance_epoch_v1(epoch_index, rebalance_interval_bars=rebalance_interval_bars):
        if prior_intent is None:
            return CrossSectionalIntrabarClvRankIntentV1(
    ...
    scores: list[CrossSectionalIntrabarClvScoreResultV1] = []
    for instrument_id, ohlc in ohlc_by_instrument.items():
