"""Definition-only preregistration validator for Bollinger/MR midband exit-efficiency v6.
...
V6 introduces a genuine economic exit-mechanism change versus V5: composite
side-aware midband-cross OR frozen max-holding-horizon=48h. V5 remains terminal
INFRASTRUCTURE_FAILURE (PROCESS_DIED_INCOMPLETE_PANEL_RUN_NO_LIFECYCLE_TERMINAL;
run count 1; partial metrics non-authoritative). No V5 partial metrics used.
"""
...
REQUIRED_V4_LINEAGE_HYPOTHESIS_ID = (
...
REQUIRED_BINDING_FIX_SHA = "364f51b6c9619e087396c881a8a670caff616d7c"
...
REQUIRED_DIAGNOSTICS = (
    "baseline_declared_config_digest",
    "treatment_declared_config_digest",
    "baseline_effective_config_digest",
    "treatment_effective_config_digest",
    "declared_configs_differ",
    "effective_configs_differ",
    "open_side_binding_observed",
    "exit_bars_observed",
    "synthetic_divergence_expected",
    "synthetic_divergence_observed",
...
REQUIRED_PASS_SUBSTRINGS = (
    ...
    "synthetic_divergence_observed == true",
    "exit_divergence_observed == true",
    "net_return_after_costs_treatment not degraded beyond canonical tolerance vs control",
    "profit_factor_treatment not degraded beyond canonical tolerance vs control",
    "max_drawdown_treatment not worsened beyond canonical tolerance vs control",
...
V5_TERMINAL_ROOT_CAUSE = "PROCESS_DIED_INCOMPLETE_PANEL_RUN_NO_LIFECYCLE_TERMINAL"
V4_TERMINAL_ROOT_CAUSE = "PROCESS_DIED_INCOMPLETE_PANEL_RUN_NO_LIFECYCLE_TERMINAL"
V3_TERMINAL_ROOT_CAUSE = "FEEDBACK_MOD_ONLY_PATCH_LEFT_WIRING_MOD_FROM_IMPORT_ALIAS_UNBOUND"
...
V5_TERMINAL_STATE = "DEVELOPMENT_EVALUATION_EXECUTED_TERMINAL/INFRASTRUCTURE_FAILURE"
V4_TERMINAL_STATE = "DEVELOPMENT_EVALUATION_EXECUTED_TERMINAL/INFRASTRUCTURE_FAILURE"
V3_TERMINAL_STATE = "DEVELOPMENT_EVALUATION_EXECUTED_TERMINAL/FAIL"
...
INVALID_MEASUREMENT_RESULT_CLASSES = frozenset(
    {
        "INVALID_MEASUREMENT_BINDING_MISSING",
        "INVALID_MEASUREMENT_IDENTICAL_EFFECTIVE_CONFIGS",
        "INVALID_MEASUREMENT_NO_EXIT_OBSERVABILITY",
...
FORBIDDEN_PARTIAL_TRANSFER_KEYS = frozenset(
    {
        ...
        "v3_checkpoint_ref",
        "v3_partial_result_ref",
        "v4_checkpoint_ref",
        "v4_partial_result_ref",
        "v5_partial_result_ref",
        "v5_checkpoint_ref",
    }
)
...
def assert_v5_terminal_preserved(v5_contract: Mapping[str, Any]) -> None:
    if v5_contract.get("hypothesis_id") != REQUIRED_PREDECESSOR_HYPOTHESIS_ID:
        raise HypothesisPreregistrationError("V5_HYPOTHESIS_ID_MISMATCH")
    if v5_contract.get("status") != V5_TERMINAL_STATE:
        raise HypothesisPreregistrationError(
            "V5_STATUS_MUST_REMAIN_TERMINAL_INFRASTRUCTURE_FAILURE"
        )
    if v5_contract.get("preregistration_state") != V5_TERMINAL_STATE:
        raise HypothesisPreregistrationError("V5_PREREGISTRATION_STATE_MUST_REMAIN_TERMINAL")
    if int(v5_contract.get("evaluation_run_count", -1)) != 1:
        raise HypothesisPreregistrationError("V5_EVALUATION_RUN_COUNT_MUST_REMAIN_1")
    if v5_contract.get("result_class") != "INFRASTRUCTURE_FAILURE":
        raise HypothesisPreregistrationError("V5_RESULT_CLASS_MUST_REMAIN_INFRASTRUCTURE_FAILURE")
    if v5_contract.get("economic_verdict") != "NOT_EVALUATED":
        raise HypothesisPreregistrationError("V5_ECONOMIC_VERDICT_MUST_REMAIN_NOT_EVALUATED")
    if v5_contract.get("rerun_allowed") is not False:
        raise HypothesisPreregistrationError("V5_RERUN_MUST_REMAIN_FORBIDDEN")
    if v5_contract.get("evaluation_started") is not True:
        raise HypothesisPreregistrationError("V5_EVALUATION_STARTED_MUST_REMAIN_TRUE")
    if v5_contract.get("evaluation_completed") is not False:
        raise HypothesisPreregistrationError("V5_EVALUATION_COMPLETED_MUST_REMAIN_FALSE")
    if v5_contract.get("pass") is not False:
        raise HypothesisPreregistrationError("V5_PASS_MUST_REMAIN_FALSE")
    if v5_contract.get("fail") is not False:
        raise HypothesisPreregistrationError("V5_FAIL_MUST_REMAIN_FALSE")
    closeout = v5_contract.get("terminal_closeout") or {}
    if isinstance(closeout, Mapping):
        if closeout.get("diagnostic_class") != V5_TERMINAL_ROOT_CAUSE:
            raise HypothesisPreregistrationError("V5_DIAGNOSTIC_CLASS_MISMATCH")
        if closeout.get("baseline_members_completed") != "3/46":
            raise HypothesisPreregistrationError("V5_BASELINE_MEMBERS_MISMATCH")


def assert_v4_terminal_preserved(v4_contract: Mapping[str, Any]) -> None:
    if v4_contract.get("hypothesis_id") != REQUIRED_V4_LINEAGE_HYPOTHESIS_ID:
        raise HypothesisPreregistrationError("V4_HYPOTHESIS_ID_MISMATCH")
    if v4_contract.get("status") != V4_TERMINAL_STATE:
        raise HypothesisPreregistrationError(
            "V4_STATUS_MUST_REMAIN_TERMINAL_INFRASTRUCTURE_FAILURE"
        )
    if v4_contract.get("preregistration_state") != V4_TERMINAL_STATE:
        raise HypothesisPreregistrationError("V4_PREREGISTRATION_STATE_MUST_REMAIN_TERMINAL")
    if int(v4_contract.get("evaluation_run_count", -1)) != 1:
        raise HypothesisPreregistrationError("V4_EVALUATION_RUN_COUNT_MUST_REMAIN_1")
    if v4_contract.get("result_class") != "INFRASTRUCTURE_FAILURE":
        raise HypothesisPreregistrationError("V4_RESULT_CLASS_MUST_REMAIN_INFRASTRUCTURE_FAILURE")
    if v4_contract.get("economic_verdict") != "NOT_EVALUATED":
        raise HypothesisPreregistrationError("V4_ECONOMIC_VERDICT_MUST_REMAIN_NOT_EVALUATED")
    if v4_contract.get("rerun_allowed") is not False:
        raise HypothesisPreregistrationError("V4_RERUN_MUST_REMAIN_FORBIDDEN")
    if v4_contract.get("evaluation_started") is not True:
        raise HypothesisPreregistrationError("V4_EVALUATION_STARTED_MUST_REMAIN_TRUE")
    if v4_contract.get("evaluation_completed") is not False:
        raise HypothesisPreregistrationError("V4_EVALUATION_COMPLETED_MUST_REMAIN_FALSE")
    if v4_contract.get("pass") is not False:
        raise HypothesisPreregistrationError("V4_PASS_MUST_REMAIN_FALSE")
    if v4_contract.get("fail") is not False:
        raise HypothesisPreregistrationError("V4_FAIL_MUST_REMAIN_FALSE")
    closeout = v4_contract.get("terminal_closeout") or {}
    if isinstance(closeout, Mapping):
        if closeout.get("diagnostic_class") != V4_TERMINAL_ROOT_CAUSE:
            raise HypothesisPreregistrationError("V4_DIAGNOSTIC_CLASS_MISMATCH")


def assert_v3_terminal_preserved(v3_contract: Mapping[str, Any]) -> None:
    if v3_contract.get("hypothesis_id") != REQUIRED_V3_HYPOTHESIS_ID:
        raise HypothesisPreregistrationError("V3_HYPOTHESIS_ID_MISMATCH")
    if v3_contract.get("status") != V3_TERMINAL_STATE:
        raise HypothesisPreregistrationError("V3_STATUS_MUST_REMAIN_TERMINAL_FAIL")
    if v3_contract.get("preregistration_state") != V3_TERMINAL_STATE:
        raise HypothesisPreregistrationError("V3_PREREGISTRATION_STATE_MUST_REMAIN_TERMINAL")
    if int(v3_contract.get("evaluation_run_count", -1)) != 1:
        raise HypothesisPreregistrationError("V3_EVALUATION_RUN_COUNT_MUST_REMAIN_1")
    if v3_contract.get("result_class") != "FAIL":
        raise HypothesisPreregistrationError("V3_RESULT_CLASS_MUST_REMAIN_FAIL")
    if v3_contract.get("economic_verdict") != "FAIL":
        raise HypothesisPreregistrationError("V3_ECONOMIC_VERDICT_MUST_REMAIN_FAIL")
    if v3_contract.get("rerun_allowed") is not False:
        raise HypothesisPreregistrationError("V3_RERUN_MUST_REMAIN_FORBIDDEN")
    if v3_contract.get("evaluation_started") is not True:
        raise HypothesisPreregistrationError("V3_EVALUATION_STARTED_MUST_REMAIN_TRUE")
    if v3_contract.get("evaluation_completed") is not True:
        raise HypothesisPreregistrationError("V3_EVALUATION_COMPLETED_MUST_REMAIN_TRUE")
    if v3_contract.get("fail") is not True:
        raise HypothesisPreregistrationError("V3_FAIL_MUST_REMAIN_TRUE")
    if v3_contract.get("pass") is not False:
        raise HypothesisPreregistrationError("V3_PASS_MUST_REMAIN_FALSE")


def assert_v2_terminal_preserved(v2_contract: Mapping[str, Any]) -> None:
    if v2_contract.get("hypothesis_id") != REQUIRED_V2_HYPOTHESIS_ID:
...
def _assert_measurement_validity_prerequisites(mvp: Mapping[str, Any]) -> None:
    if mvp.get("required_before_real_panel_run") is not True:
        raise HypothesisPreregistrationError("MVP_REQUIRED_BEFORE_REAL_PANEL_RUN")
    if mvp.get("block_real_panel_run_if_any_fails") is not True:
        raise HypothesisPreregistrationError("MVP_BLOCK_REAL_PANEL_RUN_REQUIRED")

    eff = mvp.get("effective_config_digest_inequality")
    if not isinstance(eff, Mapping):
        raise HypothesisPreregistrationError("MVP_EFFECTIVE_CONFIG_DIGEST_REQUIRED")
    if eff.get("required") is not True:
        raise HypothesisPreregistrationError("MVP_EFFECTIVE_CONFIG_REQUIRED_FLAG")
    if eff.get("baseline_and_treatment_effective_digests_must_differ") is not True:
        raise HypothesisPreregistrationError("MVP_EFFECTIVE_DIGESTS_MUST_DIFFER")
    if eff.get("if_identical_result_class") != "INVALID_MEASUREMENT_IDENTICAL_EFFECTIVE_CONFIGS":
        raise HypothesisPreregistrationError("MVP_EFFECTIVE_IDENTICAL_CLASS_MISMATCH")
    if eff.get("if_identical_block_real_panel_run") is not True:
        raise HypothesisPreregistrationError("MVP_EFFECTIVE_BLOCK_REQUIRED")

    open_side = mvp.get("open_side_binding")
    if not isinstance(open_side, Mapping):
        raise HypothesisPreregistrationError("MVP_OPEN_SIDE_BINDING_REQUIRED")
    if open_side.get("required") is not True:
        raise HypothesisPreregistrationError("MVP_OPEN_SIDE_REQUIRED_FLAG")
    if open_side.get("open_side_must_be_bound_in_per_bar_exit_decision_input") is not True:
        raise HypothesisPreregistrationError("MVP_OPEN_SIDE_MUST_BE_BOUND")
    if open_side.get("if_absent_result_class") != "INVALID_MEASUREMENT_BINDING_MISSING":
        raise HypothesisPreregistrationError("MVP_OPEN_SIDE_ABSENT_CLASS_MISMATCH")
    if open_side.get("if_absent_block_real_panel_run") is not True:
        raise HypothesisPreregistrationError("MVP_OPEN_SIDE_BLOCK_REQUIRED")

    exit_obs = mvp.get("exit_observability")
    if not isinstance(exit_obs, Mapping):
        raise HypothesisPreregistrationError("MVP_EXIT_OBSERVABILITY_REQUIRED")
    if exit_obs.get("required") is not True:
        raise HypothesisPreregistrationError("MVP_EXIT_OBS_REQUIRED_FLAG")
    if (
        exit_obs.get(
        ...
        raise HypothesisPreregistrationError("MVP_EXIT_BARS_MUST_BE_GT_0")
    if exit_obs.get("if_absent_result_class") != "INVALID_MEASUREMENT_NO_EXIT_OBSERVABILITY":
        raise HypothesisPreregistrationError("MVP_EXIT_OBS_ABSENT_CLASS_MISMATCH")
    if exit_obs.get("if_absent_block_real_panel_run") is not True:
        raise HypothesisPreregistrationError("MVP_EXIT_OBS_BLOCK_REQUIRED")

    synth = mvp.get("synthetic_divergence")
    if not isinstance(synth, Mapping):
        raise HypothesisPreregistrationError("MVP_SYNTHETIC_DIVERGENCE_REQUIRED")
    if synth.get("required") is not True:
        raise HypothesisPreregistrationError("MVP_SYNTH_REQUIRED_FLAG")
    if (
        synth.get("baseline_and_treatment_must_synthetically_diverge_at_exit_decision_boundary")
        is not True
    ):
        raise HypothesisPreregistrationError("MVP_SYNTH_MUST_DIVERGE")
    if synth.get("synthetic_divergence_expected") is not True:
        raise HypothesisPreregistrationError("MVP_SYNTH_EXPECTED_REQUIRED")
    if synth.get("if_absent_result_class") != "INVALID_MEASUREMENT_BINDING_MISSING":
        raise HypothesisPreregistrationError("MVP_SYNTH_ABSENT_CLASS_MISMATCH")
    if synth.get("if_absent_block_real_panel_run") is not True:
        raise HypothesisPreregistrationError("MVP_SYNTH_BLOCK_REQUIRED")
...
def validate_preregistration_contract(
    ...
    *,
    ...
    if terminal_result_class is not None:
        ...
        evaluation_completed_expected = terminal_result_class != "INFRASTRUCTURE_FAILURE"
        ...
        if terminal_result_class == "PASS":
            economic_verdict_expected = "PASS"
            pass_expected, fail_expected = True, False
        elif terminal_result_class == "FAIL":
            economic_verdict_expected = "FAIL"
            pass_expected, fail_expected = False, True
        else:
            economic_verdict_expected = "NOT_EVALUATED"
            pass_expected, fail_expected = False, False
    ...
    for flag in (
        ...
        "v4_rerun_forbidden",
        "v5_rerun_forbidden",
        "enhanced_measurement_validity_relative_to_development_v3",
        "reuses_process_lifecycle_checkpoint_surface_from_development_v5",
        "economic_change_vs_development_v5",
    ):
    ...
    for flag in (
        ...
        "identical_measurement_rules_to_development_v3",
        "identical_measurement_rules_to_development_v4",
        "identical_measurement_rules_to_development_v5",
        "identical_exit_mechanism_to_development_v3",
        "identical_exit_mechanism_to_development_v4",
        "identical_exit_mechanism_to_development_v5",
        "identical_economic_hypothesis_to_development_v4",
        "identical_economic_hypothesis_to_development_v5",
        "predecessor_partial_metrics_used",
    ):
        if contract.get(flag) is not False:
            raise HypothesisPreregistrationError(f"{flag.upper()}_MUST_BE_FALSE")
    ...
    if contract.get("hypothesis_id") == REQUIRED_PREDECESSOR_HYPOTHESIS_ID:
        raise HypothesisPreregistrationError("HYPOTHESIS_ID_MUST_NOT_EQUAL_V5")
    if contract.get("hypothesis_id") == REQUIRED_V3_HYPOTHESIS_ID:
        raise HypothesisPreregistrationError("HYPOTHESIS_ID_MUST_NOT_EQUAL_V3")
    if contract.get("hypothesis_id") == REQUIRED_V2_HYPOTHESIS_ID:
    ...
    if treatment.get("includes_max_holding_horizon_exit") is not True:
        raise HypothesisPreregistrationError("MAX_HOLDING_HORIZON_EXIT_REQUIRED")
    if treatment.get("composite_exit_efficiency") is not True:
        raise HypothesisPreregistrationError("COMPOSITE_EXIT_EFFICIENCY_REQUIRED")
    if treatment.get("bollinger_event_only") is not False:
        raise HypothesisPreregistrationError("BOLLINGER_EVENT_ONLY_MUST_BE_FALSE_FOR_COMPOSITE")
    ...
    diagnostics = list(metrics.get("diagnostics") or [])
    if diagnostics != list(REQUIRED_DIAGNOSTICS):
        raise HypothesisPreregistrationError("DIAGNOSTICS_MISMATCH")
    ...
    for required_guard in (
        ...
        "cost_multiplier",
        "effective_config_digest_inequality",
        "open_side_binding",
        "exit_observability",
    ...
    mvp = contract.get("measurement_validity_prerequisites")
    if not isinstance(mvp, Mapping):
        raise HypothesisPreregistrationError("MEASUREMENT_VALIDITY_PREREQUISITES_REQUIRED")
    _assert_measurement_validity_prerequisites(mvp)
    ...
    for required_state in (
        ...
        "DEVELOPMENT_EVALUATION_EXECUTED_TERMINAL/INVALID_MEASUREMENT_BINDING_MISSING",
        "DEVELOPMENT_EVALUATION_EXECUTED_TERMINAL/INVALID_MEASUREMENT_IDENTICAL_EFFECTIVE_CONFIGS",
        "DEVELOPMENT_EVALUATION_EXECUTED_TERMINAL/INVALID_MEASUREMENT_NO_EXIT_OBSERVABILITY",
    ...
    pred = contract.get("predecessor_development_v5")
    if not isinstance(pred, Mapping):
        raise HypothesisPreregistrationError("PREDECESSOR_V5_REQUIRED")
    ...
    if pred.get("result_class") != "INFRASTRUCTURE_FAILURE":
    ...
    if pred.get("partial_metrics_authoritative") is not False:
        raise HypothesisPreregistrationError(
            "PREDECESSOR_PARTIAL_METRICS_AUTHORITATIVE_MUST_BE_FALSE"
        )
    if pred.get("root_cause_detail") != V5_TERMINAL_ROOT_CAUSE:
    ...
    if pred.get("economic_verdict") != "NOT_EVALUATED":
        raise HypothesisPreregistrationError("PREDECESSOR_ECONOMIC_VERDICT_MISMATCH")
    if pred.get("diagnostic_class") != V5_TERMINAL_ROOT_CAUSE:
        raise HypothesisPreregistrationError("PREDECESSOR_DIAGNOSTIC_CLASS_MISMATCH")
    if pred.get("baseline_members_completed") != "3/46":
        raise HypothesisPreregistrationError("PREDECESSOR_BASELINE_MEMBERS_MISMATCH")
    ...
    for key in (
        ...
        "invalid_measurement_distinct_from_economic_failure",
        "auto_resume_forbidden",
        "auto_rerun_on_infrastructure_failure_forbidden",
        "measurement_validity_prerequisites_must_pass_before_real_panel_run",
    ):
    ...
    if life.get("infrastructure_failure_result_class") != "INFRASTRUCTURE_FAILURE":
    ...
    if life.get("invalid_measurement_economic_verdict") != "NOT_EVALUATED":
        raise HypothesisPreregistrationError("LIFECYCLE_INVALID_ECONOMIC_VERDICT_MISMATCH")
    ...
    for key in (
        "exactly_once_run_slot_semantics",
        "checkpoint_never_authorizes_automatic_rerun",
        "checkpoint_cannot_reclaim_or_create_run_slot",
        "recovery_inspection_read_only_diagnostic_only",
        "dead_process_before_lifecycle_terminal_is_infrastructure_failure",
        "partial_metrics_must_not_promote_to_baseline_treatment_or_delta",
        "summary_must_distinguish_complete_metrics_from_partial_diagnostic_counters",
        "import_or_validate_definition_must_not_start_runner",
    ):
        if life.get(key) is not True:
            raise HypothesisPreregistrationError(f"LIFECYCLE_{key.upper()}_REQUIRED")
    mono = list(life.get("monotonic_lifecycle_states") or [])
    if mono != list(REQUIRED_LIFECYCLE_STATES):
        raise HypothesisPreregistrationError("LIFECYCLE_STATES_MISMATCH")
    progress_fields = list(life.get("required_progress_metadata_fields") or [])
    if progress_fields != list(REQUIRED_PROGRESS_METADATA_FIELDS):
        raise HypothesisPreregistrationError("LIFECYCLE_PROGRESS_FIELDS_MISMATCH")
    atomic = life.get("atomic_checkpoint_persistence")
    if not isinstance(atomic, Mapping) or atomic.get("required") is not True:
        raise HypothesisPreregistrationError("ATOMIC_CHECKPOINT_REQUIRED")
    ...
    if obs.get("required_result_class_on_incomplete_run") != "INFRASTRUCTURE_FAILURE":
        raise HypothesisPreregistrationError("INCOMPLETE_RUN_RESULT_CLASS_MISMATCH")
    durable = list(obs.get("required_durable_diagnostics") or [])
    for required_diag in REQUIRED_DURABLE_DIAGNOSTICS:
        if required_diag not in durable:
    ...
    if owner_map is not None:
        ...
        if REQUIRED_OWNER_SURFACE not in owners:
            raise HypothesisPreregistrationError("V5_OWNER_MAP_ENTRY_MISSING")

    life_bind = infra.get("bollinger_mr_midband_exit_efficiency_process_lifecycle_checkpoint_v5")
    if not isinstance(life_bind, Mapping):
        raise HypothesisPreregistrationError("LIFECYCLE_CHECKPOINT_BINDING_REQUIRED")
    if life_bind.get("surface_id") != REQUIRED_LIFECYCLE_CHECKPOINT_SURFACE:
        raise HypothesisPreregistrationError("LIFECYCLE_CHECKPOINT_SURFACE_MISMATCH")
    if life_bind.get("binding_required_for_future_evaluation") is not True:
        raise HypothesisPreregistrationError("LIFECYCLE_CHECKPOINT_BINDING_MUST_BE_REQUIRED")
    if life_bind.get("checkpoint_never_authorizes_automatic_rerun") is not True:
        raise HypothesisPreregistrationError("LIFECYCLE_CHECKPOINT_NO_AUTO_RERUN_REQUIRED")
    if life_bind.get("checkpoint_cannot_reclaim_or_create_run_slot") is not True:
        raise HypothesisPreregistrationError("LIFECYCLE_CHECKPOINT_NO_SLOT_RECLAIM_REQUIRED")
    if life_bind.get("dead_process_before_lifecycle_terminal_result_class") != (
        "INFRASTRUCTURE_FAILURE"
    ):
        raise HypothesisPreregistrationError("LIFECYCLE_DEAD_PROCESS_CLASS_MISMATCH")
    if life_bind.get("partial_metrics_authoritative") is not False:
        raise HypothesisPreregistrationError("PARTIAL_METRICS_MUST_BE_NON_AUTHORITATIVE")
    if life_bind.get("does_not_authorize_v5_rerun") is not True:
        raise HypothesisPreregistrationError("LIFECYCLE_MUST_NOT_AUTHORIZE_V5_RERUN")
    if life_bind.get("v5_terminal_unchanged") is not True:
        raise HypothesisPreregistrationError("LIFECYCLE_V5_TERMINAL_UNCHANGED_REQUIRED")
    if owner_map is not None:
        owners = owner_map.get("allowed_optimization_surfaces") or {}
        if REQUIRED_LIFECYCLE_CHECKPOINT_SURFACE not in owners:
            raise HypothesisPreregistrationError("LIFECYCLE_CHECKPOINT_OWNER_MAP_ENTRY_MISSING")
    ...
    binding_fix = infra.get("mv2_wiring_mod_capture_alias_open_side_binding_fix")
    if not isinstance(binding_fix, Mapping):
        raise HypothesisPreregistrationError("BINDING_FIX_REQUIRED")
    if binding_fix.get("surface_id") != REQUIRED_BINDING_FIX_SURFACE:
        raise HypothesisPreregistrationError("BINDING_FIX_SURFACE_MISMATCH")
    if binding_fix.get("binding_required_for_future_evaluation") is not True:
        raise HypothesisPreregistrationError("BINDING_FIX_MUST_BE_REQUIRED")
    if binding_fix.get("merged_on_main_sha") != REQUIRED_BINDING_FIX_SHA:
        raise HypothesisPreregistrationError("BINDING_FIX_SHA_MISMATCH")
    if binding_fix.get("root_cause_addressed") != REQUIRED_BINDING_FIX_ROOT_CAUSE:
        raise HypothesisPreregistrationError("BINDING_FIX_ROOT_CAUSE_MISMATCH")
    if binding_fix.get("does_not_authorize_v3_rerun") is not True:
        raise HypothesisPreregistrationError("BINDING_FIX_MUST_NOT_AUTHORIZE_V3_RERUN")
    if binding_fix.get("v3_terminal_unchanged") is not True:
        raise HypothesisPreregistrationError("BINDING_FIX_V3_TERMINAL_UNCHANGED_REQUIRED")
    ...
    if v5_contract is not None:
        assert_v5_terminal_preserved(v5_contract)
    if v4_contract is not None:
        assert_v4_terminal_preserved(v4_contract)
    if v3_contract is not None:
        assert_v3_terminal_preserved(v3_contract)
    ...
    if terminal_result_class is None:
        return {
            ...
            "definition_semantics_identical_to_v4_economic": False,
            "identical_exit_mechanism_to_development_v5": False,
            "identical_economic_hypothesis_to_development_v5": False,
            "reuses_process_lifecycle_checkpoint_surface_from_development_v5": True,
            "economic_change_vs_development_v5": True,
            "identical_exit_mechanism_to_development_v3": False,
            "enhanced_measurement_validity_relative_to_development_v3": True,
...
def load_and_validate_repo_contract(repo_root: Path | None = None) -> dict[str, Any]:
    ...
    v5_path = root / V5_CONTRACT_REL_PATH
    if not v5_path.is_file():
        raise HypothesisPreregistrationError(f"V5_CONTRACT_MISSING:{v5_path}")
    v4_path = root / V4_CONTRACT_REL_PATH
    if not v4_path.is_file():
        raise HypothesisPreregistrationError(f"V4_CONTRACT_MISSING:{v4_path}")
    v3_path = root / V3_CONTRACT_REL_PATH
    if not v3_path.is_file():
        raise HypothesisPreregistrationError(f"V3_CONTRACT_MISSING:{v3_path}")
    ...
    v5_contract = load_json(v5_path)
    v4_contract = load_json(v4_path)
    v3_contract = load_json(v3_path)
    ...
    report["v5_contract_path"] = V5_CONTRACT_REL_PATH
    report["v4_contract_path"] = V4_CONTRACT_REL_PATH
    report["v3_contract_path"] = V3_CONTRACT_REL_PATH
