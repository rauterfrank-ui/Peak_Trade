"""Single-run DEVELOPMENT panel evaluation: baseline vs ADX DI direction-confirmation eligibility gate."""
...
EVALUATION_RUN_ID = "evaluate_adx_di_direction_confirmation_mr_eligibility_development_v1"
...
def _optional_treatment_entry_eligibility_gate(
    *, enabled: bool, bars: pd.DataFrame | None
    ...
    warmup-observation bind path), looks up Wilder +DI/−DI at that
    ...
    DI direction-confirmation rule (``is_entry_eligible``): a mapped ``+1`` (long
    entry-candidate) requires ``minus_DI > plus_DI``; a mapped ``-1`` (short
    entry-candidate) requires ``plus_DI > minus_DI``. Mapped ``0`` (flat/exit)
...
def _extract_member_economics(
    ...
    *,
    ...
    # ADX DI direction-confirmation eligibility attribution on the decision segment.
...
def run_arm(
    *,
    ...
    for i, member in enumerate(members, start=1):
        ...
        # run_arm loops members and DI series must match the bars
...
def run_development_evaluation(
    *,
    ...
    # (ADX DI direction-confirmation side-aware eligibility gated) — two arms, one run.
    baseline = run_arm(
        arm="baseline",
        cfg=cfg,
        archive_root=root,
    ...
    config_snapshot = {
        "schema_version": "evaluate_adx_di_direction_confirmation_mr_eligibility_development_config_snapshot.v1",
    ...
    command = (
        ...
        "run_evaluate_adx_di_direction_confirmation_mr_eligibility_development_v1.py "
        f"--output-dir {output_dir}"
    )
    base_sha = _git_base_sha(repo)
    summary = {
        "schema_version": "evaluate_adx_di_direction_confirmation_mr_eligibility_development_summary.v1",
