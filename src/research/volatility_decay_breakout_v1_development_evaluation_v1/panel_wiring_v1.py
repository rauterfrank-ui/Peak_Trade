"""Panel wiring: bind VDB treatment + unconditional baseline to panel series.

Does not invent metrics or exit PnL. Maps each instrument panel into the existing
strategy/baseline event producers and returns a typed handoff for the productive
evaluator (VCB handoff types; no second PnL truth).
"""
...
def wire_treatment_and_baseline_panel_events_v1(
    ...
    *,
    ...
    for series in panel_series:
        ...
        treatment_rows = generate_volatility_decay_breakout_events_v1(frame)
        ...
        treatment_arms.append(
            ArmEventSeriesV1(
                ...
                entry_event_mask=tuple(
                    r.event is VolatilityDecayBreakoutEventV1.ENTRY_EVENT for r in treatment_rows
                ),
            )
        )
