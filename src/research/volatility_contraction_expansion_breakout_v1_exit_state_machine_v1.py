"""Deterministic exit state machine for VOLATILITY_CONTRACTION_EXPANSION_BREAKOUT_V1.

Ex-ante, first-event-wins exits for LONG and SHORT independently.
No trailing stop. Opposite-break invalidation uses shared 20-bar channel.
...
class VcebExitReasonV1(str, Enum):
    INITIAL_STOP = "INITIAL_STOP"
    OPPOSITE_BREAK_INVALIDATION = "OPPOSITE_BREAK_INVALIDATION"
...
def evaluate_exit_on_bar_v1(
    ...
    *,
    ...
    upper_channel: Optional[float],
    lower_channel: Optional[float],
    ...
    candidates: list[tuple[int, VcebExitReasonV1, float]] = []
    ...
    if position.side == "LONG":
        if low <= position.stop_price:
            candidates.append(
                (
                    ...
                    VcebExitReasonV1.INITIAL_STOP,
                    position.stop_price,
                )
            )
        if lower_channel is not None and lower_channel == lower_channel and close < lower_channel:
            candidates.append(
                (
                    _PRECEDENCE_RANK["OPPOSITE_BREAK_INVALIDATION"],
                    VcebExitReasonV1.OPPOSITE_BREAK_INVALIDATION,
                    close,
                )
            )
    else:
        ...
        if upper_channel is not None and upper_channel == upper_channel and close > upper_channel:
            candidates.append(
                (
                    _PRECEDENCE_RANK["OPPOSITE_BREAK_INVALIDATION"],
                    VcebExitReasonV1.OPPOSITE_BREAK_INVALIDATION,
        ...
        if percentile_rank < REGIME_INVALIDATION_PERCENTILE_LT_V1:
            candidates.append(
                (
                    ...
                    VcebExitReasonV1.REGIME_INVALIDATION,
                    close,
                )
            )
    ...
    if held >= TIME_EXIT_MAX_BARS_V1:
        candidates.append((_PRECEDENCE_RANK["TIME_EXIT"], VcebExitReasonV1.TIME_EXIT, close))

    if is_last_instrument_bar:
        candidates.append(
            (
                _PRECEDENCE_RANK["END_OF_INSTRUMENT_LIQUIDATION"],
                VcebExitReasonV1.END_OF_INSTRUMENT_LIQUIDATION,
                close,
            )
        )
    if is_last_panel_bar:
        candidates.append(
            (
                ...
                VcebExitReasonV1.END_OF_PANEL_LIQUIDATION,
                close,
            )
        )
