"""Deterministic binding/digest resolution for CSHRVF v1 eval entry point."""
...
from src.research.cross_sectional_high_realized_volatility_fade_v1_development_evaluation_v1.constants_v1 import (
    ALLOWED_DATASET_IDS,
    BASELINE_ID,
...
def resolve_strategy_params(contract: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not frozen:
        frozen = {
            "cross_sectional_high_rank_percentile_inclusive_min": 0.8,
            "cross_sectional_rearm_rank_percentile_below": 0.5,
            "direction_opposite_short_horizon_signed_return": True,
            ...
            "low_cross_sectional_vol_entry_forbidden": True,
            "max_entries_per_high_vol_episode": 1,
            "min_consecutive_high_rank_bars": 2,
            "min_post_fill_bars_required_inclusive": 48,
            "normalization_rank_percentile_lt": 0.55,
            "panel_members_required_min": 10,
            "realized_volatility_period_completed_bars": 24,
            "regime_invalidation_rank_percentile_lt": 0.4,
            "short_horizon_return_lookback_completed_bars": 8,
            "signal_lag_bars": 1,
            "time_exit_max_bars": 48,
            "trailing_stop_forbidden": True,
            "vol_estimator_family": "REALIZED_VOLATILITY_CROSS_SECTIONAL_RANK",
        }
    return {
...
def assert_shared_channel_core_bound() -> None:
    """Baseline reuses shared channel core; CSHRVF treatment has no channel prerequisite."""
...
def assert_exit_state_machine_bound() -> None:
    """The strategy producer must import the frozen CSHRVF exit state machine (no local reimpl)."""
    from src.research import (
        cross_sectional_high_realized_volatility_fade_v1_exit_state_machine_v1 as exit_sm,
    )
    from src.research import (
        cross_sectional_high_realized_volatility_fade_v1_strategy_v1 as strategy,
    )
...
def materialize_entry_point_binding_payload(repo_root: Path) -> dict[str, Any]:
    ...
    return {
        "artifact_kind": (
            "cross_sectional_high_realized_volatility_fade_v1_development_evaluation_entry_point_binding"
        ),
        "artifact_version": "v1",
        ...
        "evidence_ref": "docs/evidence/evaluate_cross_sectional_high_realized_volatility_fade_development_v1/",
        ...
        "schema_version": (
            "cross_sectional_high_realized_volatility_fade_v1_development_evaluation_entry_point_binding.v1"
        ),
