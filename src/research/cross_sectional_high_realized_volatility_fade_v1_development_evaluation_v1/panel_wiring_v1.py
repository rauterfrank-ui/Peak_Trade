"""Panel wiring: bind CSHRVF treatment (strategy-emitted exits) + unconditional baseline.

Does not invent metrics or exit PnL. Treatment roundtrips are produced entirely by the
strategy's own exit state machine (`generate_cshrvf_events_and_roundtrips_v1`); the
...
CSHRVF requires same-timestamp cross-sectional RV-level ranks over the full panel
before per-instrument event generation (PIT, no lookahead).
"""
...
from src.research.cross_sectional_high_realized_volatility_fade_v1_development_evaluation_v1.binding_v1 import (
...
__all__ = (
    ...
    "CshrvfTreatmentBaselineWiringHandoffV1",
...
@dataclass(frozen=True)
class CshrvfTreatmentBaselineWiringHandoffV1:
...
def wire_treatment_and_baseline_panel_events_v1(
    ...
    *,
    ...
    for series in panel_series:
        ...
        frames[series.instrument_id] = frame

    assert reference_timestamps is not None

    eligible_ids = filter_eligible_instrument_ids_v1(tuple(frames.keys()))
    if not eligible_ids:
        raise ValueError("NO_ELIGIBLE_PANEL_INSTRUMENTS")

    rv_wide = pd.DataFrame(
        {
            instrument_id: compute_realized_volatility_24_v1(frames[instrument_id]["close"])
            for instrument_id in eligible_ids
        },
        index=frames[eligible_ids[0]].index,
    )
    cs_rank_wide = compute_cross_sectional_rv_rank_wide_panel_v1(rv_wide)
    ...
    for series in panel_series:
        frame = frames[series.instrument_id]
        timestamps = tuple(str(ts) for ts in frame["timestamp_utc"].tolist())

        if series.instrument_id in cs_rank_wide.columns:
            cs_rv_rank = cs_rank_wide[series.instrument_id].astype(float)
        else:
            # BTC/spot excluded from ranking universe → no treatment events.
            cs_rv_rank = pd.Series(float("nan"), index=frame.index, dtype=float)

        bar_results, roundtrips = generate_cshrvf_events_and_roundtrips_v1(
            frame, cs_rv_rank=cs_rv_rank
        )
        ...
        entry_event_mask = tuple(r.event is CshrvfEventV1.ENTRY_EVENT for r in bar_results)
    ...
    return CshrvfTreatmentBaselineWiringHandoffV1(
