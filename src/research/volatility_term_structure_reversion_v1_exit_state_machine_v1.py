"""Deterministic exit state machine for VOLATILITY_TERM_STRUCTURE_REVERSION_V1.

Ex-ante, first-event-wins exits for LONG and SHORT independently.
No trailing stop. Term-structure normalization uses ratio percentile < 0.55.
...
TERM_STRUCTURE_NORMALIZATION_RATIO_PERCENTILE_LT_V1 = 0.55
REGIME_INVALIDATION_RATIO_PERCENTILE_LT_V1 = 0.40
...
class VtsrExitReasonV1(str, Enum):
...
def evaluate_exit_on_bar_v1(
    ...
    *,
    ...
    candidates: list[tuple[int, VtsrExitReasonV1, float]] = []
    ...
    if ratio_percentile is not None and ratio_percentile == ratio_percentile:
        if ratio_percentile < TERM_STRUCTURE_NORMALIZATION_RATIO_PERCENTILE_LT_V1:
            candidates.append(
                (
                    _PRECEDENCE_RANK["TERM_STRUCTURE_NORMALIZATION_INVALIDATION"],
                    VtsrExitReasonV1.TERM_STRUCTURE_NORMALIZATION_INVALIDATION,
                    close,
                )
            )
        if ratio_percentile < REGIME_INVALIDATION_RATIO_PERCENTILE_LT_V1:
            candidates.append(
                (
                    _PRECEDENCE_RANK["REGIME_INVALIDATION"],
                    VtsrExitReasonV1.REGIME_INVALIDATION,
                    close,
                )
            )
    ...
    if held >= TIME_EXIT_MAX_BARS_V1:
        candidates.append((_PRECEDENCE_RANK["TIME_EXIT"], VtsrExitReasonV1.TIME_EXIT, close))

    if is_last_instrument_bar:
        candidates.append(
            (
                _PRECEDENCE_RANK["END_OF_INSTRUMENT_LIQUIDATION"],
                VtsrExitReasonV1.END_OF_INSTRUMENT_LIQUIDATION,
                close,
            )
        )
    if is_last_panel_bar:
        candidates.append(
            (
                ...
                VtsrExitReasonV1.END_OF_PANEL_LIQUIDATION,
                close,
            )
        )
