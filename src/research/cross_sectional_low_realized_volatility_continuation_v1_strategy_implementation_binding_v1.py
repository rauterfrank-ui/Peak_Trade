"""Validator for CSLRVC v1 strategy-implementation binding."""
...
PACKAGE_MARKER = (
    "CROSS_SECTIONAL_LOW_REALIZED_VOLATILITY_CONTINUATION_V1_STRATEGY_IMPLEMENTATION_BINDING=true"
)
BINDING_REL_PATH = (
    "config/research/cross_sectional_low_realized_volatility_continuation_v1_"
    "strategy_implementation_binding_v1.json"
)
REQUIRED_DIGEST = "c0b116530a1bd9dd11bd9004d76c3fbfa74036348b3f6949483f6b0890b86872"
...
REQUIRED_IMPL_FILES = (
    ...
    "src/research/cross_sectional_low_realized_volatility_continuation_v1_exit_state_machine_v1.py",
...
def validate_implementation_binding(
    ...
    _require(notes.get("direction_rule") == "with_signed_return", "DIRECTION_NOT_CONTINUATION")
    _require(notes.get("fade_logic_present") is False, "FADE_LOGIC_PRESENT")
    non_actions = set(payload.get("explicit_non_actions") or [])
    _require("NO_SECOND_PNL_TRUTH" in non_actions, "SECOND_PNL_NOT_FORBIDDEN")
    _require("NO_CSHRVF_V1_RETRY" in non_actions, "CSHRVF_RETRY_NOT_FORBIDDEN")
    ...
    _require("NO_HIGH_CS_VOL_ENTRY_IN_V1" in non_actions, "HIGH_CS_VOL_NOT_FORBIDDEN")
    ...
    if repo_root is not None:
        ...
        _require(contract.get("development_run_count") == 0, "CONTRACT_RUN_COUNT_NONZERO")
        _require(contract.get("runner_start_count") == 0, "CONTRACT_RUNNER_START_NONZERO")
        _require(
            contract.get("development_evaluation_executed") is False,
            "CONTRACT_EVAL_EXECUTED",
        )
