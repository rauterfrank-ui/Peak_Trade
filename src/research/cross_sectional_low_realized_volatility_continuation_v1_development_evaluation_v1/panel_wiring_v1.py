"""Panel wiring: bind CSLRVC treatment (strategy-emitted exits) + unconditional baseline.

Does not invent metrics or exit PnL. Treatment roundtrips are produced entirely by the
strategy's own exit state machine (`generate_cslrvc_events_and_roundtrips_v1`); the
...
CSLRVC requires same-timestamp cross-sectional RV-level ranks over the full panel
...
__all__ = (
    ...
    "CslrvcTreatmentBaselineWiringHandoffV1",
...
@dataclass(frozen=True)
class CslrvcTreatmentBaselineWiringHandoffV1:
...
def wire_treatment_and_baseline_panel_events_v1(
    ...
    *,
    ...
    for series in panel_series:
        ...
        bar_results, roundtrips = generate_cslrvc_events_and_roundtrips_v1(
        ...
        entry_event_mask = tuple(r.event is CslrvcEventV1.ENTRY_EVENT for r in bar_results)
    ...
    return CslrvcTreatmentBaselineWiringHandoffV1(
