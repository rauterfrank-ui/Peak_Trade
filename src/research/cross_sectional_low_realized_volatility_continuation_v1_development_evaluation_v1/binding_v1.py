"""Deterministic binding/digest resolution for CSLRVC v1 eval entry point."""
...
def resolve_measurement_contract(repo_root: Path) -> dict[str, Any]:
    ...
    _require(contract.get("run_slot_consumed") is True, "RUN_SLOT_NOT_CONSUMED")
...
def resolve_strategy_params(contract: Mapping[str, Any] | None = None) -> dict[str, Any]:
    """Bind strategy params from preregistered admission_mechanism (SSOT for CSLRVC).

    The measurement-contract ``parameter_governance.frozen_parameters`` block retained a
    predecessor-copy artifact; admission_mechanism is the validator-checked authority
    and must drive digests/execution without retune.
    """
    admission = dict((contract or {}).get("admission_mechanism") or {})
    vol = dict(admission.get("vol_estimator") or {})
    state = dict(admission.get("cross_sectional_vol_rank_state") or {})
    entry = dict(admission.get("cross_sectional_low_vol_continuation_entry") or {})
    exits = dict((contract or {}).get("exit_semantics") or {})
    frozen = {
        "cross_sectional_low_rank_percentile_inclusive_max": state.get(
            "low_rank_percentile_inclusive_max", 0.2
        ),
        "cross_sectional_rearm_rank_percentile_above": state.get(
            "rearm_rank_percentile_strictly_above", 0.5
        ),
        "direction_with_short_horizon_signed_return": entry.get("direction_rule")
        == "with_signed_return_over_short_horizon_window",
        "event_consumption": (admission.get("transition_event_lifecycle") or {}).get(
            "event_consumption", "SINGLE_USE"
        ),
        "ex_ante_exit_reachability_required": entry.get("ex_ante_exit_reachability_required", True),
        "high_cross_sectional_vol_entry_forbidden": entry.get(
            "high_cross_sectional_vol_entry_forbidden", True
        ),
        "initial_stop_atr_multiple": exits.get("initial_stop_atr_multiple", 1.5),
        "initial_stop_atr_period": exits.get("initial_stop_atr_period", 14),
        "max_entries_per_low_vol_episode": entry.get("max_entries_per_low_vol_episode", 1),
        "min_consecutive_low_rank_bars": state.get("min_consecutive_low_rank_bars", 2),
        "min_post_fill_bars_required_inclusive": entry.get(
            "min_post_fill_bars_required_inclusive", 48
        ),
        "normalization_rank_percentile_gt": 0.45,
        "panel_members_required_min": state.get("panel_members_required_min", 10),
        "realized_volatility_period_completed_bars": vol.get(
            "realized_volatility_period_completed_bars", 24
        ),
        "regime_invalidation_rank_percentile_gt": exits.get(
            "regime_invalidation_cs_rv_rank_percentile_gt", 0.6
        ),
        "regime_invalidation_rank_percentile_lt": exits.get(
            "regime_invalidation_cs_rv_rank_percentile_lt", 0.05
        ),
        "short_horizon_return_lookback_completed_bars": entry.get(
            "short_horizon_return_lookback_completed_bars", 8
        ),
        "signal_lag_bars": entry.get("signal_lag_bars", 1),
        "time_exit_max_bars": exits.get("time_exit_max_bars", 48),
        "trailing_stop_forbidden": exits.get("trailing_stop_forbidden", True),
        "vol_estimator_family": vol.get("family", "REALIZED_VOLATILITY_CROSS_SECTIONAL_RANK"),
    }
    return {
...
def assert_shared_channel_core_bound() -> None:
    """Baseline reuses shared channel core; CSLRVC treatment has no channel prerequisite."""
...
def assert_exit_state_machine_bound() -> None:
    """The strategy producer must import the frozen CSLRVC exit state machine (no local reimpl)."""
    from src.research import (
        cross_sectional_low_realized_volatility_continuation_v1_exit_state_machine_v1 as exit_sm,
    )
    from src.research import (
        cross_sectional_low_realized_volatility_continuation_v1_strategy_v1 as strategy,
    )
...
def materialize_entry_point_binding_payload(repo_root: Path) -> dict[str, Any]:
    ...
    return {
        "artifact_kind": (
            "cross_sectional_low_realized_volatility_continuation_v1_development_evaluation_entry_point_binding"
        ),
        "artifact_version": "v1",
        ...
        "entry_point_status": "RUN_SLOT_CONSUMED_DEVELOPMENT_FAIL",
        "evaluation_authorized": False,
        "evidence_ref": "docs/evidence/evaluate_cross_sectional_low_realized_volatility_continuation_development_v1/",
        ...
        "schema_version": (
            "cross_sectional_low_realized_volatility_continuation_v1_development_evaluation_entry_point_binding.v1"
        ),
        ...
        "status": "RUN_SLOT_CONSUMED_DEVELOPMENT_FAIL",
        "strategy_id": STRATEGY_ID,
        "strategy_identity": STRATEGY_IDENTITY,
        "strategy_params_digest": strategy_params_digest,
        "strategy_version": STRATEGY_VERSION,
        "time_segment_definition_id": TIME_SEGMENT_DEFINITION_ID,
        "verdict": "DEVELOPMENT_EVALUATION_EXECUTED_TERMINAL/FAIL",
    }
