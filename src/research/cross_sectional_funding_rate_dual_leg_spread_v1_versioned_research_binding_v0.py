"""Cross-sectional funding-rate dual-leg spread v1 versioned research binding materializer."""
...
from src.research.cross_sectional_funding_rate_dual_leg_spread_ranking_semantics_binding_v1 import (
    ...
    materialize_versioned_funding_rate_dual_leg_spread_ranking_semantics_binding_v1,
    serialize_versioned_binding_artifact_json_v1,
)
...
PACKAGE_MARKER = (
    "CROSS_SECTIONAL_FUNDING_RATE_DUAL_LEG_SPREAD_V1_VERSIONED_RESEARCH_BINDING_V0=true"
)
BINDING_ARTIFACT_VERSION = "v0"
BINDING_SCHEMA_VERSION = (
    "cross_sectional_funding_rate_dual_leg_spread_v1_versioned_research_binding.v0"
)
CONFIG_REL_PATH = (
    "config/research/"
    "cross_sectional_funding_rate_dual_leg_spread_v1_versioned_research_binding_v0.json"
)
...
SCORE_FORMULA_EXPRESSION = (
    "spread_i(t) = funding_rate_max_panel(t) - funding_rate_min_panel(t); "
...
class VersionedResearchBindingResultV1:
...
def compute_implementation_digest_v1() -> str:
    return _stable_digest(
        {
            "module": "cross_sectional_funding_rate_dual_leg_spread_v1_versioned_research_binding_v0",
...
def build_parameter_binding_v1() -> dict[str, Any]:
    from src.research.cross_sectional_funding_rate_dual_leg_spread_ranking_semantics_binding_v1 import (
    ...
    return {
        ...
        "funding_delta_lookback_k_forbidden": True,
    }


def build_pit_universe_binding_v1() -> dict[str, Any]:
    return {
        "binding_version": "v1",
        "venue": "OKX",
...
def build_panel_dataset_binding_v1() -> dict[str, Any]:
    return {
        "binding_version": "v1",
        ...
        "funding_usage": "funding_rate_for_spread_computation",
        "narrow_adapter": "pit_okx_pt1h_panel_funding_field_materialization.v0",
...
def build_period_binding_v1() -> dict[str, Any]:
...
def build_instrument_binding_v1() -> dict[str, Any]:
    return {
        "binding_version": "v1",
        "selection_mode": SELECTION_MODE,
...
def build_cost_execution_binding_v1() -> dict[str, Any]:
    return {
        "binding_version": "v1",
        "fee_model_binding": {
...
def build_economic_policy_binding_v1() -> dict[str, Any]:
    return {
        "binding_version": "v1",
...
def apply_complete_external_bindings_v1(binding: dict[str, Any]) -> dict[str, Any]:
    ...
    impl_digest = compute_implementation_digest_v1()
    config_bytes = json.dumps(
        {
            "strategy_id": STRATEGY_ID,
            "strategy_version": STRATEGY_VERSION,
            "parameter_binding": build_parameter_binding_v1(),
            "pit_universe_binding": build_pit_universe_binding_v1(),
            "panel_dataset_binding": build_panel_dataset_binding_v1(),
            "period_binding": build_period_binding_v1(),
            "cost_execution_binding": build_cost_execution_binding_v1(),
        },
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
    config_digest = compute_config_digest_v1(config_bytes)
...
def materialize_versioned_research_binding_v1() -> dict[str, Any]:
    base = materialize_funding_rate_dual_leg_spread_ranking_semantics_binding_v1()
    with_numeric = apply_ratified_operator_bindings_v1(base)
    complete_binding = apply_complete_external_bindings_v1(with_numeric)
    binding_digest = complete_binding.pop("_binding_digest", "")
    validation = validate_funding_rate_dual_leg_spread_ranking_semantics_binding_v1(
        complete_binding
    )
    return {
        "artifact_kind": "cross_sectional_funding_rate_dual_leg_spread_v1_versioned_research_binding",
        ...
        "panel_dataset_binding": build_panel_dataset_binding_v1(),
        "period_binding": build_period_binding_v1(),
        "instrument_binding": build_instrument_binding_v1(),
        "cost_execution_binding": build_cost_execution_binding_v1(),
        "economic_policy_binding": build_economic_policy_binding_v1(),
...
def materialize_and_validate_versioned_research_binding_v1() -> VersionedResearchBindingResultV1:
    envelope = materialize_versioned_research_binding_v1()
    binding = envelope["binding"]
    validation = validate_funding_rate_dual_leg_spread_ranking_semantics_binding_v1(binding)
    if not validation.valid:
        return VersionedResearchBindingResultV1(
...
def serialize_versioned_research_binding_json_v1(envelope: Mapping[str, Any]) -> str:
    return json.dumps(dict(envelope), indent=2, sort_keys=True, ensure_ascii=False) + "\n"


def write_versioned_research_binding_artifacts_v1(repo_root: Path) -> tuple[Path, Path]:
    envelope = materialize_versioned_research_binding_v1()
    ranking_envelope = (
        materialize_versioned_funding_rate_dual_leg_spread_ranking_semantics_binding_v1()
    )
    ...
    config_path.write_text(serialize_versioned_research_binding_json_v1(envelope), encoding="utf-8")
    ranking_path.write_text(
        serialize_versioned_binding_artifact_json_v1(ranking_envelope),
        encoding="utf-8",
    )
