"""Definition-only preregistration validator for Bollinger/MR midband exit-efficiency v2.
...
V2 is a new independently versioned DEVELOPMENT_ONLY measurement ID with
identical definition semantics to V1. V1 remains terminal as
INCONCLUSIVE_INFRASTRUCTURE_FAILURE (run count 1) and must not be rerun,
reset, or partially reused. Future evaluation (separate Operator-GO) must bind
...
PACKAGE_MARKER = "BOLLINGER_MR_MIDBAND_EXIT_EFFICIENCY_HYPOTHESIS_PREREGISTRATION_V2=true"
CONTRACT_REL_PATH = (
    "config/research/"
...
EXPECTED_DEVELOPMENT_PREREGISTRATION_DIGEST = (
    "8a33ef16946034ab9da71e69634f225ef5179ef69c19914f61f9faf588f589b1"
)
...
def assert_v1_terminal_preserved(v1_contract: Mapping[str, Any]) -> None:
    ...
    if v1_contract.get("preregistration_state") != V1_TERMINAL_STATE:
        raise HypothesisPreregistrationError("V1_PREREGISTRATION_STATE_MUST_REMAIN_TERMINAL")
    ...
    if v1_contract.get("economic_verdict") != "NOT_EVALUATED":
        raise HypothesisPreregistrationError("V1_ECONOMIC_VERDICT_MUST_REMAIN_NOT_EVALUATED")
    if v1_contract.get("rerun_allowed") is not False:
        raise HypothesisPreregistrationError("V1_RERUN_MUST_REMAIN_FORBIDDEN")
    if v1_contract.get("evaluation_started") is not True:
        raise HypothesisPreregistrationError("V1_EVALUATION_STARTED_MUST_REMAIN_TRUE")
    if v1_contract.get("evaluation_completed") is not False:
...
def validate_preregistration_contract(
    ...
    *,
    ...
    for banned_key in FORBIDDEN_V1_PARTIAL_TRANSFER_KEYS:
        if banned_key in contract:
            raise HypothesisPreregistrationError(f"V1_PARTIAL_TRANSFER_FORBIDDEN:{banned_key}")
    ...
    pred = contract.get("predecessor_development_v1")
    if not isinstance(pred, Mapping):
        raise HypothesisPreregistrationError("PREDECESSOR_V1_REQUIRED")
    ...
    if pred.get("historical_process_death_cause") != "UNKNOWN":
        raise HypothesisPreregistrationError("PREDECESSOR_DEATH_CAUSE_MUST_BE_UNKNOWN")
