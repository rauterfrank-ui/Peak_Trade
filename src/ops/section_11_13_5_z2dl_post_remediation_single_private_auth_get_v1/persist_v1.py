"""Sanitized evidence persist for the Z2DL one-shot Auth GET. No secrets."""
...
class Z2DLPrivateAuthGetPersistError(RuntimeError):
...
def assert_no_secrets_in_payload_v1(payload: Mapping[str, Any]) -> None:
    def _walk(value: Any, key: str = "") -> None:
        ...
        if key_l in _SECRET_VALUE_KEYS and text and text not in {"<REDACTED>", "<REF_ONLY>"}:
            raise Z2DLPrivateAuthGetPersistError("SECRET_IN_EVIDENCE")
...
def persist_z2dl_private_auth_get_evidence_v1(
    *,
    pack: Path,
    origin_main_sha: str,
    ...
    if str(origin_main_sha or "").strip() != EXPECTED_ORIGIN_MAIN_SHA:
        raise Z2DLPrivateAuthGetPersistError("ORIGIN_MAIN_SHA_MISMATCH")
    ...
    if int(verified.get("MANIFEST_VERIFY_RC", 1)) != 0:
        raise Z2DLPrivateAuthGetPersistError("MANIFEST_VERIFY_FAILED")
    return verified
