"""Exclusive single-writer authority for dynamic scope state root."""
...
class ConflictingWriterError(RuntimeError):
    def __init__(self, detail: str = "") -> None:
        self.code = DynamicScopeBindingFailureCodeV1.CONFLICTING_WRITER
        super().__init__(f"{self.code.value}:{detail}" if detail else self.code.value)


@dataclass
class DynamicScopeStateSingleWriterV1:
    state_root: Path
    ...
    def assert_held(self) -> None:
        if not self._held:
            raise ConflictingWriterError(
                DynamicScopeBindingFailureCodeV1.WRITER_LOCK_NOT_HELD.value
            )
