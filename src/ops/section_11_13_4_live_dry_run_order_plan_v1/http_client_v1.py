"""Dedicated LIVE dry-run with exchange reconciliation HTTP client (GET-only; no Testnet client reuse)."""
...
class LiveDryRunOrderPlanHttpError(RuntimeError):
    """Fail-closed Live dry-run order plan HTTP violation."""


@dataclass(frozen=True)
class LiveDryRunOrderPlanHttpRequestV1:
    method: str
    url: str
    host: str
    endpoint: str
...
@dataclass(frozen=True)
class LiveDryRunOrderPlanHttpResponseV1:
    status_code: int
    body_bytes: bytes
...
class LiveDryRunOrderPlanTransportV1(Protocol):
    transport_class: str
    venue_live_contact: bool

    def send(self, request: LiveDryRunOrderPlanHttpRequestV1) -> LiveDryRunOrderPlanHttpResponseV1:
        """Send exactly one HTTP request."""


@dataclass
class LiveDryRunOrderPlanRequestCountersV1:
    request_count: int = 0
...
def assert_method_allowlisted_v1(method: str) -> str:
    ...
    if m in FORBIDDEN_HTTP_METHODS or m not in METHOD_ALLOWLIST:
        raise LiveDryRunOrderPlanHttpError(f"HTTP_METHOD_HARD_BLOCK_BEFORE_WIRE:{m or '<empty>'}")
    return m
...
def assert_endpoint_allowlisted_v1(endpoint: str) -> str:
    ...
    if not ep:
        raise LiveDryRunOrderPlanHttpError("ENDPOINT_REQUIRED")
    ...
    for marker in FORBIDDEN_MUTATION_ENDPOINT_MARKERS:
        ...
        if marker.rstrip("?") in {"/trade/order", "/api/v5/trade/order"} and (
            "/trade/orders-" in lowered or path.endswith("/trade/orders-pending")
        ):
            continue
        raise LiveDryRunOrderPlanHttpError(f"MUTATION_ENDPOINT_HARD_BLOCK:{ep}")
    if path not in ENDPOINT_ALLOWLIST:
        raise LiveDryRunOrderPlanHttpError(f"ENDPOINT_NOT_ALLOWLISTED:{ep}")
    try:
        assert_treasury_shadow_http_endpoint_allowed_v1(
            endpoint=path,
            method="GET",
            shadow_surface=SHADOW_HTTP_SURFACE_11_13_4,
        )
    except TreasuryPhase3ShadowEnforcementError as exc:
        raise LiveDryRunOrderPlanHttpError(str(exc)) from exc
    return ep
...
def assert_no_demo_simulation_headers_v1(headers: Mapping[str, str] | None) -> None:
    ...
    for key, value in headers.items():
        ...
        if key_l in FORBIDDEN_DEMO_SIMULATION_HEADERS:
            raise LiveDryRunOrderPlanHttpError(f"DEMO_SIMULATION_HEADER_FORBIDDEN:{key_l}")
...
def assert_host_matches_binding_v1(
    *,
    ...
    if host != binding.rest_host:
        raise LiveDryRunOrderPlanHttpError(f"HOST_MISMATCH:{host}!={binding.rest_host}")
...
@dataclass
class LiveDryRunOrderPlanHttpClientV1:
    """GET-only Live dry-run order plan client with hard mutation blocks before wire-send."""
    ...
    counters: LiveDryRunOrderPlanRequestCountersV1 = field(
        default_factory=LiveDryRunOrderPlanRequestCountersV1
    )
    ...
    def _build_request(
        ...
        *,
    ...
    ) -> LiveDryRunOrderPlanHttpRequestV1:
        m = assert_method_allowlisted_v1(method)
        ep = assert_endpoint_allowlisted_v1(endpoint)
        path = _endpoint_path_only(ep)
        if path not in self.endpoint_allowlist:
            raise LiveDryRunOrderPlanHttpError(f"ENDPOINT_NOT_IN_RUNTIME_ALLOWLIST:{ep}")
        ...
        return LiveDryRunOrderPlanHttpRequestV1(
            method=m,
            url=url,
            host=host,
            endpoint=ep,
    ...
    def get(
        ...
        *,
        endpoint: str,
        headers: Mapping[str, str] | None = None,
    ) -> LiveDryRunOrderPlanHttpResponseV1:
        if self.counters.request_count >= self.max_request_count:
            raise LiveDryRunOrderPlanHttpError("MAX_REQUEST_COUNT_EXCEEDED")
        ...
        while attempts <= self.max_retries:
            ...
            try:
                ...
                if response.method != "GET":
                    raise LiveDryRunOrderPlanHttpError("TRANSPORT_RETURNED_NON_GET")
                ...
                return LiveDryRunOrderPlanHttpResponseV1(
            ...
            except LiveDryRunOrderPlanHttpError:
                raise
            except TimeoutError as exc:
                last_exc = exc
                if attempts > self.max_retries:
                    raise LiveDryRunOrderPlanHttpError("TIMEOUT") from exc
                time.sleep(RETRY_BACKOFF_SECONDS)
            except (URLError, OSError) as exc:
                last_exc = exc
                if attempts > self.max_retries:
                    raise LiveDryRunOrderPlanHttpError(f"NETWORK_ERROR:{exc}") from exc
                time.sleep(RETRY_BACKOFF_SECONDS)
        raise LiveDryRunOrderPlanHttpError(f"RETRY_EXHAUSTED:{last_exc}")

    def post(self, *, endpoint: str, **_: Any) -> None:
        self.counters.write_request_count += 1
        assert_method_allowlisted_v1("POST")

    def request(self, *, method: str, endpoint: str, **_: Any) -> LiveDryRunOrderPlanHttpResponseV1:
        m = str(method or "").strip().upper()
        if m != "GET":
...
class RecordingFakeTransportV1:
    ...
    calls: list[LiveDryRunOrderPlanHttpRequestV1] = field(default_factory=list)
    ...
    def send(self, request: LiveDryRunOrderPlanHttpRequestV1) -> LiveDryRunOrderPlanHttpResponseV1:
        ...
        if request.method != "GET":
            raise LiveDryRunOrderPlanHttpError("FAKE_TRANSPORT_GET_ONLY")
...
class UrllibLiveTransportV1:
    ...
    def send(self, request: LiveDryRunOrderPlanHttpRequestV1) -> LiveDryRunOrderPlanHttpResponseV1:
        if request.method != "GET":
            raise LiveDryRunOrderPlanHttpError("URLLIB_TRANSPORT_GET_ONLY")
...
def parse_json_object_v1(body: bytes) -> dict[str, Any]:
    try:
        ...
        raise LiveDryRunOrderPlanHttpError("MALFORMED_NON_JSON_RESPONSE") from exc
    if not isinstance(payload, dict):
        raise LiveDryRunOrderPlanHttpError("RESPONSE_NOT_JSON_OBJECT")
    return payload
