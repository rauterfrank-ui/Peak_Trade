"""Bounded 1:1 Risk/Sizing/Capital archive sibling exporter V1.

CAPABILITY_ID=CAPABILITY_RISK_SIZING_CAPITAL_ARCHIVE_SIBLING_EXPORTER_V1

Writes already-mapped Risk/Sizing/Capital field dicts atomically to:

    archive_root/readmodels/risk_sizing_capital.v1.json

Invariants:
- AUTHORITY_EFFECT=NONE
- no capital/risk/sizing evaluator imports or recomputation
- no presentation / dashboard / materializer imports
- no invented statuses, quantity, or timestamps
...
class RiskSizingCapitalArchiveSiblingExportResultV1:
    exported: bool
    source_path: str
    target_path: str
    ...
    risk_sizing_authority_effect: str = RISK_SIZING_AUTHORITY_EFFECT
    owner: str = OWNER
...
def _fail(
    *,
    ...
    return RiskSizingCapitalArchiveSiblingExportResultV1(
        exported=False,
...
def coerce_risk_sizing_capital_fields_export_payload_v1(
    source: object,
    ...
    coerced, errors = coerce_risk_sizing_capital_fields_mapping_v1(source)
    if coerced is None:
...
def load_risk_sizing_capital_fields_export_payload_v1(
    ...
    fields, error = coerce_risk_sizing_capital_fields_export_payload_v1(payload)
    if fields is None:
...
def export_risk_sizing_capital_fields_payload_to_archive_sibling_v1(
    *,
    ...
    fields, error = coerce_risk_sizing_capital_fields_export_payload_v1(fields_payload)
    if fields is None:
        return _fail(
            source_path=label,
    ...
    return RiskSizingCapitalArchiveSiblingExportResultV1(
        ...
        source_path=label,
        target_path=str(target_path),
        risk_status=str(fields.get("risk_status") or ""),
...
def export_risk_sizing_capital_fields_to_archive_sibling_v1(
    *,
    fields_source_path: str | Path,
    archive_root: str | Path,
) -> RiskSizingCapitalArchiveSiblingExportResultV1:
    ...
    fields, source_str, load_error = load_risk_sizing_capital_fields_export_payload_v1(
        fields_source_path
    )
    if fields is None:
        return _fail(
    ...
    return export_risk_sizing_capital_fields_payload_to_archive_sibling_v1(
...
def export_risk_sizing_capital_to_archive_sibling_from_replay_commit_v1(
    *,
    archive_root: str | Path,
    ...
    payload, build_errors = build_risk_sizing_capital_sibling_payload_from_replay_commit_v1(
