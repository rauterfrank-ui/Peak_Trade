"""Exclusive single-writer authority for productive ranking snapshots."""
...
class DuplicateRankingWriterError(RuntimeError):
    """Raised when another ranking producer writer holds or claims the lock."""

    def __init__(self, message: str) -> None:
        super().__init__(message)
        self.failure_code = RankingFailureCodeV1.DUPLICATE_PRODUCER_WRITER.value


@dataclass
class ProductiveRankingSingleWriterV1:
    state_root: Path
    ...
    def acquire(self, *, now_unix: Optional[float] = None) -> None:
        ...
        try:
            ...
            raise DuplicateRankingWriterError(
    ...
    def assert_held(self) -> None:
        if not self._held:
            raise DuplicateRankingWriterError("WRITER_LOCK_NOT_HELD")
        if not self.lock_path.is_file():
            raise DuplicateRankingWriterError("WRITER_LOCK_MISSING")
        existing = json.loads(self.lock_path.read_text(encoding="utf-8"))
        if existing.get("writer_identity") != self.writer_identity:
            raise DuplicateRankingWriterError("CONFLICTING_WRITER_IDENTITY")
        if existing.get("session_id") != self.session_id:
            raise DuplicateRankingWriterError("CONFLICTING_WRITER_SESSION")
