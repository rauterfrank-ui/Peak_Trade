"""Sanitized offline evidence persist for the P12 EXECUTION_PREREQUISITE_11 pack."""
...
class P12PositionSidePersistError(RuntimeError):
...
def assert_no_secrets_in_payload_v1(payload: Mapping[str, Any]) -> None:
    def _walk(value: Any, key: str = "") -> None:
        ...
        if key_l in _SECRET_VALUE_KEYS and text and text not in {"<REDACTED>", "<REF_ONLY>"}:
            raise P12PositionSidePersistError("SECRET_IN_EVIDENCE")
        lowered = text.lower()
...
def persist_p12_position_side_evidence_v1(
    *,
    pack: Path,
    origin_main_sha: str,
    ...
    if str(origin_main_sha or "").strip() != EXPECTED_ORIGIN_MAIN_SHA:
        raise P12PositionSidePersistError("ORIGIN_MAIN_SHA_MISMATCH")
    if adjudication.get("EXECUTION_PREREQUISITE_11_POSITION_SIDE_POSSIDE") != "PASS":
        raise P12PositionSidePersistError("PREREQUISITE_11_MUST_PASS")
    if adjudication.get("P11_CLOSED") is not True:
        raise P12PositionSidePersistError("P11_MUST_BE_CLOSED")
    if adjudication.get("POST_PERFORMED") is True or summary.get("POST_PERFORMED") is True:
        raise P12PositionSidePersistError("FORBIDDEN_POST_CLAIM")
    if int(adjudication.get("THIS_GO_GET_COUNT") or 0) != 0:
        raise P12PositionSidePersistError("FORBIDDEN_NEW_GET_CLAIM")
    if adjudication.get("PRIVATE_AUTH_USED") is True:
        raise P12PositionSidePersistError("FORBIDDEN_PRIVATE_AUTH_CLAIM")
    if adjudication.get("REQUEST_POS_SIDE") != "OMITTED":
        raise P12PositionSidePersistError("REQUEST_POS_SIDE_MUST_REMAIN_OMITTED")
    documents = {
        "CENSUS.json": census,
    ...
    for payload in documents.values():
        ...
        if payload.get("LIVE_EXECUTION") is True:
            raise P12PositionSidePersistError("FORBIDDEN_LIVE_EXECUTION_CLAIM")
    ...
    if int(verified.get("MANIFEST_VERIFY_RC", 1)) != 0:
        raise P12PositionSidePersistError("MANIFEST_VERIFY_FAILED")
    return verified
