...
class Z2DGFundingBalanceGetError(RuntimeError):
    """Fail-closed Z2DG one-shot GET violation."""
...
def _assert_one_get_zero_writes(client: LiveCanaryHttpClientV1) -> dict[str, Any]:
    ...
    if int(counters.get("GET_REQUEST_COUNT", 0) or 0) != 1:
        raise Z2DGFundingBalanceGetError("GET_COUNT_NOT_ONE")
    if int(counters.get("REQUEST_COUNT", 0) or 0) != 1:
        raise Z2DGFundingBalanceGetError("REQUEST_COUNT_NOT_ONE")
    if int(counters.get("WRITE_REQUEST_COUNT", 0) or 0) != 0:
        raise Z2DGFundingBalanceGetError("WRITE_REQUEST_DETECTED")
    if int(counters.get("TRANSFER_REQUEST_COUNT", 0) or 0) != 0:
        raise Z2DGFundingBalanceGetError("TRANSFER_REQUEST_DETECTED")
    if int(counters.get("ORDER_REQUEST_COUNT", 0) or 0) != 0:
        raise Z2DGFundingBalanceGetError("ORDER_REQUEST_DETECTED")
    if int(counters.get("ENTRY_SUBMIT_COUNT", 0) or 0) != 0:
        raise Z2DGFundingBalanceGetError("ENTRY_SUBMIT_DETECTED")
    if int(counters.get("FLATTEN_SUBMIT_COUNT", 0) or 0) != 0:
        raise Z2DGFundingBalanceGetError("FLATTEN_SUBMIT_DETECTED")
    if list(client.counters.endpoints_used) != [ENDPOINT]:
        raise Z2DGFundingBalanceGetError("ENDPOINT_SET_MISMATCH")
    if list(client.counters.methods_used) != ["GET"]:
        raise Z2DGFundingBalanceGetError("NON_GET_METHOD_DETECTED")
    return counters
...
def execute_single_actual_funding_balance_get_v1(
    *,
    ...
    if owned != OWNER_GO:
        raise Z2DGFundingBalanceGetError("OWNER_GO_MISMATCH")
    bound_sha = str(origin_main_sha or "").strip()
    if bound_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise Z2DGFundingBalanceGetError("ORIGIN_MAIN_SHA_MISMATCH")
    if "?" in ENDPOINT or ENDPOINT != "/api/v5/asset/balances":
        raise Z2DGFundingBalanceGetError("ENDPOINT_CONTRACT_DRIFT")
    if FORBIDDEN_TRANSFER in ENDPOINT or FORBIDDEN_WITHDRAWAL in ENDPOINT:
        raise Z2DGFundingBalanceGetError("MUTATION_ENDPOINT_FORBIDDEN")

    productive = transport is None
    if productive:
        if vault_file is None or not str(vault_file).strip():
            raise Z2DGFundingBalanceGetError("VAULT_FILE_REQUIRED")
    ...
    if isinstance(transport, UrllibLiveCanaryTransportV1) and not bool(
        ...
        raise Z2DGFundingBalanceGetError("PRODUCTIVE_WIRE_DISABLED")
    ...
    try:
        ...
        if response.method != "GET":
            raise Z2DGFundingBalanceGetError("NON_GET_RESPONSE")
    ...
    if get_error is None:
        ...
        if productive and wire_count != 1:
            raise Z2DGFundingBalanceGetError("NETWORK_REQUEST_COUNT_NOT_ONE")
    ...
    snapshot = {
        "DOCUMENT_CLASS": "Z2DG_SINGLE_ACTUAL_READ_ONLY_FUNDING_BALANCE_GET_V1",
    ...
    verified = persist_z2dg_funding_balance_get_evidence_v1(
        pack=pack,
    ...
    if get_error:
        raise Z2DGFundingBalanceGetError(f"FUNDING_BALANCE_GET_FAILED:{get_error}")
    if parse_error:
        raise Z2DGFundingBalanceGetError(f"FUNDING_BALANCE_OBSERVATION_FAIL_CLOSED:{parse_error}")
    return {
