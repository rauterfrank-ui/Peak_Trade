"""Execute the bounded Z2DS post-Z2DR read-only runtime evidence GET package.
...
from src.ops.section_11_13_5_z2ds_post_z2dr_runtime_read_only_evidence_max_leverage_v1.persist_v1 import (
    persist_z2ds_runtime_read_only_evidence_v1,
)
...
class Z2DSRuntimeReadOnlyEvidenceGetError(RuntimeError):
    """Fail-closed Z2DS GET-package violation."""
...
def _assert_get_only_client(client: LiveCanaryHttpClientV1) -> dict[str, Any]:
    ...
    if int(counters.get("WRITE_REQUEST_COUNT", 0) or 0) != 0:
        raise Z2DSRuntimeReadOnlyEvidenceGetError("WRITE_REQUEST_DETECTED")
    if int(counters.get("TRANSFER_REQUEST_COUNT", 0) or 0) != 0:
        raise Z2DSRuntimeReadOnlyEvidenceGetError("TRANSFER_REQUEST_DETECTED")
    if int(counters.get("ORDER_REQUEST_COUNT", 0) or 0) != 0:
        raise Z2DSRuntimeReadOnlyEvidenceGetError("ORDER_REQUEST_DETECTED")
    if int(counters.get("ENTRY_SUBMIT_COUNT", 0) or 0) != 0:
        raise Z2DSRuntimeReadOnlyEvidenceGetError("ENTRY_SUBMIT_DETECTED")
    if int(counters.get("FLATTEN_SUBMIT_COUNT", 0) or 0) != 0:
        raise Z2DSRuntimeReadOnlyEvidenceGetError("FLATTEN_SUBMIT_DETECTED")
    methods = list(client.counters.methods_used)
    if any(method != "GET" for method in methods):
        raise Z2DSRuntimeReadOnlyEvidenceGetError("NON_GET_METHOD_DETECTED")
    for endpoint in client.counters.endpoints_used:
        path = _endpoint_path_v1(str(endpoint))
        if path in FORBIDDEN_ENDPOINTS:
            raise Z2DSRuntimeReadOnlyEvidenceGetError(f"FORBIDDEN_ENDPOINT_USED:{path}")
    return counters


def execute_runtime_read_only_evidence_v1(
    *,
    owner_go: str,
    origin_main_sha: str,
    evidence_root: Path,
    ...
    if owned != OWNER_GO:
        raise Z2DSRuntimeReadOnlyEvidenceGetError("OWNER_GO_MISMATCH")
    bound_sha = str(origin_main_sha or "").strip()
    if bound_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise Z2DSRuntimeReadOnlyEvidenceGetError("ORIGIN_MAIN_SHA_MISMATCH")
    if REUSED_REST_HOST != AUTHORIZED_HOST:
        raise Z2DSRuntimeReadOnlyEvidenceGetError("HOST_MISMATCH")
    if FUNDING_GET_REQUIRED:
        raise Z2DSRuntimeReadOnlyEvidenceGetError("FUNDING_GET_REQUIRED_DRIFT")

    productive = transport is None
    if productive:
        if vault_file is None or not str(vault_file).strip():
            raise Z2DSRuntimeReadOnlyEvidenceGetError("VAULT_FILE_REQUIRED")
    ...
    if isinstance(transport, UrllibLiveCanaryTransportV1) and not bool(
        ...
        raise Z2DSRuntimeReadOnlyEvidenceGetError("PRODUCTIVE_WIRE_DISABLED")
    ...
    def _record(
        *,
        ...
        if http_status in {401, 403} or venue_code == "50110":
    ...
    def _issue_get(*, endpoint: str, signed: bool) -> dict[str, Any]:
        ...
        if path in FORBIDDEN_ENDPOINTS:
            raise Z2DSRuntimeReadOnlyEvidenceGetError(f"FORBIDDEN_ENDPOINT:{path}")
        ...
        try:
            if signed and handle is not None:
                ...
                raise Z2DSRuntimeReadOnlyEvidenceGetError("PRIVATE_GET_REQUIRES_CREDENTIAL_HANDLE")
            ...
            if response.method != "GET":
                raise Z2DSRuntimeReadOnlyEvidenceGetError("NON_GET_RESPONSE")
        ...
        except Z2DSRuntimeReadOnlyEvidenceGetError:
            raise
        finally:
            headers.clear()
        record = _record(
    ...
    try:
        ...
        if limit_px:
            ...
            observations["MAX_AVAILABLE_SKIPPED"] = "LIMIT_PX_UNAVAILABLE_NO_PRICE_SYNTHESIS"
            parsed_surfaces["MAX_AVAILABLE_SKIPPED"] = observations["MAX_AVAILABLE_SKIPPED"]
    ...
    for method in FORBIDDEN_HTTP_METHODS:
        if method in list(client.counters.methods_used):
            raise Z2DSRuntimeReadOnlyEvidenceGetError(f"FORBIDDEN_METHOD:{method}")
    ...
    snapshot = {
        "DOCUMENT_CLASS": "Z2DS_POST_Z2DR_RUNTIME_READ_ONLY_EVIDENCE_MAX_LEVERAGE_V1",
    ...
    adjudication = adjudicate_runtime_read_only_evidence_v1(
    ...
    summary = {
        ...
        **{
            ...
            for k in (
                ...
                "CREATE_PATH_BLOCKER_COUNT",
                "READ_ONLY_CLOSABLE_BLOCKER_COUNT_AFTER_GETS",
                "REMAINING_MUTATION_REQUIRED_BLOCKER_COUNT",
                "MAX_SAFE_READ_ONLY_RUNTIME_BUNDLE_REMAINING",
    ...
    verified = persist_z2ds_runtime_read_only_evidence_v1(
        pack=pack,
