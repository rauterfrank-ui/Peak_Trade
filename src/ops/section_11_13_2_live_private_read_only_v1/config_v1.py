"""Versioned config schema for §11.13.2 LIVE_PRIVATE_READ_ONLY."""
...
class LivePrivateRoConfigError(RuntimeError):
    """Fail-closed config violation."""


@dataclass(frozen=True)
class LivePrivateRoConfigV1:
    environment: str
    venue: str
    entity: str
    region: str
...
def _as_tuple_str(value: Any, *, field: str) -> tuple[str, ...]:
    ...
    if isinstance(value, (list, tuple)):
        ...
        return out
    raise LivePrivateRoConfigError(f"CONFIG_FIELD_TYPE_INVALID:{field}")


def load_live_private_ro_config_v1(payload: Mapping[str, Any]) -> LivePrivateRoConfigV1:
    """Load config. Productive fields may be empty; execute/preflight gates fail closed."""
    if not isinstance(payload, Mapping):
        raise LivePrivateRoConfigError("CONFIG_PAYLOAD_MUST_BE_MAPPING")
    ...
    evidence_version = str(
        payload.get("evidence_version", "section_11_13_2_live_private_read_only_proven_v1")
    ).strip()
    ...
    if methods != METHOD_ALLOWLIST:
        raise LivePrivateRoConfigError("METHOD_ALLOWLIST_MUST_BE_GET_ONLY")
    if max_request_count < 1 or max_request_count > MAX_REQUEST_COUNT_HARD_CAP:
        raise LivePrivateRoConfigError("MAX_REQUEST_COUNT_OUT_OF_BOUNDS")
    if max_retries < 0 or max_retries > MAX_RETRIES_HARD_CAP:
        raise LivePrivateRoConfigError("MAX_RETRIES_OUT_OF_BOUNDS")
    if timeout_seconds <= 0 or timeout_seconds > MAX_TIMEOUT_SECONDS:
        raise LivePrivateRoConfigError("TIMEOUT_OUT_OF_BOUNDS")
    if not endpoints:
        raise LivePrivateRoConfigError("ENDPOINT_ALLOWLIST_REQUIRED")
    for ep in endpoints:
        if ep not in ENDPOINT_ALLOWLIST:
            raise LivePrivateRoConfigError(f"ENDPOINT_NOT_IN_CANONICAL_ALLOWLIST:{ep}")

    return LivePrivateRoConfigV1(
...
def require_execute_time_fields_v1(config: LivePrivateRoConfigV1) -> None:
    ...
    if missing:
        raise LivePrivateRoConfigError("FAIL_CLOSED_PRE_FLIGHT:MISSING:" + ",".join(missing))
    if config.environment != REQUIRED_ENVIRONMENT:
        raise LivePrivateRoConfigError("FAIL_CLOSED_PRE_FLIGHT:ENVIRONMENT_NOT_LIVE")
    if config.credential_class != REQUIRED_CREDENTIAL_CLASS:
        raise LivePrivateRoConfigError("FAIL_CLOSED_PRE_FLIGHT:CREDENTIAL_CLASS")
    if not config.expected_demo_marker_absent:
        raise LivePrivateRoConfigError("FAIL_CLOSED_PRE_FLIGHT:DEMO_MARKER_MUST_BE_ABSENT")


def load_live_private_ro_config_from_json_file_v1(path: Path | str) -> LivePrivateRoConfigV1:
    text = Path(path).read_text(encoding="utf-8")
    payload = json.loads(text)
    if not isinstance(payload, Mapping):
        raise LivePrivateRoConfigError("CONFIG_FILE_MUST_BE_OBJECT")
    return load_live_private_ro_config_v1(payload)
