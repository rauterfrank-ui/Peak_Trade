"""LIVE private read-only OKX request signing for §11.13.2.
...
class LivePrivateRoSignerError(RuntimeError):
    """Fail-closed LIVE RO signer violation."""
...
def build_okx_live_ro_get_auth_headers_v1(
    *,
    ...
    if method_u not in METHOD_ALLOWLIST:
        raise LivePrivateRoSignerError(f"SIGNER_METHOD_FORBIDDEN:{method_u or '<empty>'}")
    if body:
        raise LivePrivateRoSignerError("SIGNER_BODY_FORBIDDEN_FOR_GET")
    if extra_headers:
        for key in extra_headers:
            if str(key).strip().lower() in FORBIDDEN_DEMO_SIMULATION_HEADERS:
                raise LivePrivateRoSignerError(f"DEMO_SIMULATION_HEADER_FORBIDDEN:{key}")
    ...
    try:
        ...
        headers = {
            ...
            "User-Agent": "PeakTrade-Section-11-13-2-LivePrivateRO/1",
        }
    except LivePrivateRoCredentialError as exc:
        raise LivePrivateRoSignerError(str(exc)) from exc
    finally:
        material = None
        creds = None
