...
def assert_okx_business_code_success_v1(payload: Mapping[str, Any]) -> str:
    ...
    if raw is None:
        raise LivePrivateRoAssertionError("OKX_CODE_MISSING")
    code = str(raw).strip()
    if code != "0":
        raise LivePrivateRoAssertionError(f"OKX_CODE_NOT_SUCCESS:{code}")
    return code


def _payload_has_demo_testnet_marker(payload: Mapping[str, Any]) -> bool:
    """Targeted marker check — avoids naive whole-payload substring false positives."""
...
def assert_authenticated_private_read_success_v1(
    *,
    ...
    if require_account_identity and not account_raw:
        raise LivePrivateRoAssertionError("ACCOUNT_IDENTITY_REQUIRED")
    ...
    if expected_account_scope is not None and str(expected_account_scope).strip():
        ...
        if not account_raw:
            raise LivePrivateRoAssertionError("ACCOUNT_SCOPE_CROSSCHECK_IDENTITY_MISSING")
        if account_raw != expected:
            raise LivePrivateRoAssertionError("ACCOUNT_SCOPE_MISMATCH")
        account_scope_match = True
...
def validate_permission_attestation_v1(attestation: Mapping[str, Any] | None) -> dict[str, bool]:
    if not isinstance(attestation, Mapping):
        raise LivePrivateRoAssertionError("PERMISSION_ATTESTATION_REQUIRED")
    read = attestation.get("READ")
    trade = attestation.get("TRADE")
    withdraw = attestation.get("WITHDRAW")
    if read is not True:
        raise LivePrivateRoAssertionError("PERMISSION_ATTESTATION_READ_MUST_BE_TRUE")
    if trade is not False:
        raise LivePrivateRoAssertionError("PERMISSION_ATTESTATION_TRADE_MUST_BE_FALSE")
    if withdraw is not False:
        raise LivePrivateRoAssertionError("PERMISSION_ATTESTATION_WITHDRAW_MUST_BE_FALSE")
