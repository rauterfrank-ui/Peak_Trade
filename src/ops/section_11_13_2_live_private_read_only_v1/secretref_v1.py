"""SecretRef / credential-class isolation for §11.13.2 Live private RO."""
...
class LivePrivateRoSecretRefError(RuntimeError):
...
class LivePrivateRoSecretRefMetadataV1:
    secretref_uri: str
...
def validate_live_private_ro_secretref_uri_v1(secretref_uri: str) -> str:
    ref = str(secretref_uri or "").strip()
    if not ref:
        raise LivePrivateRoSecretRefError("SECRETREF_REQUIRED")
    if ref.startswith("plaintext:") or ref.startswith("sk-"):
        raise LivePrivateRoSecretRefError("PLAINTEXT_SECRET_FORBIDDEN")
    if not ref.startswith(SECRETREF_URI_PREFIX):
        raise LivePrivateRoSecretRefError("SECRETREF_URI_PREFIX_REQUIRED")
    if not _SAFE_REF_RE.match(ref):
        raise LivePrivateRoSecretRefError("SECRETREF_URI_MALFORMED")
    lowered = ref.lower()
    if SECRETREF_LIVE_PATH_MARKER not in lowered:
        raise LivePrivateRoSecretRefError("SECRETREF_LIVE_PATH_MARKER_REQUIRED")
    for marker in SECRETREF_FORBIDDEN_PATH_MARKERS:
        if marker in lowered:
            raise LivePrivateRoSecretRefError(f"SECRETREF_FORBIDDEN_PATH:{marker}")
    return ref


def validate_live_private_ro_credential_class_v1(credential_class: str) -> str:
    klass = str(credential_class or "").strip()
    if not klass:
        raise LivePrivateRoSecretRefError("CREDENTIAL_CLASS_REQUIRED")
    upper = klass.upper()
    for marker in FORBIDDEN_CREDENTIAL_CLASS_MARKERS:
        if marker in upper:
            raise LivePrivateRoSecretRefError(f"FORBIDDEN_CREDENTIAL_CLASS:{klass}")
    if klass != REQUIRED_CREDENTIAL_CLASS:
        raise LivePrivateRoSecretRefError("LIVE_PRIVATE_RO_CREDENTIAL_CLASS_REQUIRED")
    return klass
    ...
    *,
    ...
    if env == "LIVE":
        if has_non_live or not has_live:
            raise LivePrivateRoSecretRefError("CROSS_BIND_DEMO_TESTNET_REF_TO_LIVE_REJECT")
        return
    if env in {"DEMO", "TESTNET", "PAPER", "SIMULATED"}:
        if has_live:
            raise LivePrivateRoSecretRefError("CROSS_BIND_LIVE_REF_TO_DEMO_TESTNET_REJECT")
        return
    raise LivePrivateRoSecretRefError(f"UNSUPPORTED_ENVIRONMENT_FOR_SECRETREF:{env}")


def build_live_private_ro_secretref_metadata_v1(
    *,
    secretref_uri: str,
    credential_class: str,
) -> LivePrivateRoSecretRefMetadataV1:
    ref = validate_live_private_ro_secretref_uri_v1(secretref_uri)
    klass = validate_live_private_ro_credential_class_v1(credential_class)
    digest = _digest(ref)
    return LivePrivateRoSecretRefMetadataV1(
        secretref_uri=ref,
...
def refuse_credential_material_borrow_v1(*, reason: str) -> None:
    raise LivePrivateRoSecretRefError(f"CREDENTIAL_MATERIAL_BORROW_REFUSED:{reason}")
