"""Sanitized offline evidence persist for the P10 qty-unit forensic pack."""
...
class P10QtyUnitPersistError(RuntimeError):
...
def assert_no_secrets_in_payload_v1(payload: Mapping[str, Any]) -> None:
    def _walk(value: Any, key: str = "") -> None:
        ...
        if key_l in _SECRET_VALUE_KEYS and text and text not in {"<REDACTED>", "<REF_ONLY>"}:
            raise P10QtyUnitPersistError("SECRET_IN_EVIDENCE")
        lowered = text.lower()
...
def persist_p10_qty_unit_adjudication_evidence_v1(
    *,
    pack: Path,
    origin_main_sha: str,
    ...
    if str(origin_main_sha or "").strip() != EXPECTED_ORIGIN_MAIN_SHA:
        raise P10QtyUnitPersistError("ORIGIN_MAIN_SHA_MISMATCH")
    if adjudication.get("TARGET_POSITION_QTY_UNIT") != "UNPROVEN":
        raise P10QtyUnitPersistError("UNIT_MUST_REMAIN_UNPROVEN")
    if adjudication.get("POST_PERFORMED") is True or summary.get("POST_PERFORMED") is True:
        raise P10QtyUnitPersistError("FORBIDDEN_POST_CLAIM")
    if int(adjudication.get("THIS_GO_GET_COUNT") or 0) != 0:
        raise P10QtyUnitPersistError("FORBIDDEN_NEW_GET_CLAIM")
    documents = {
        "CENSUS.json": census,
    ...
    for payload in documents.values():
        ...
        if payload.get("LIVE_EXECUTION") is True:
            raise P10QtyUnitPersistError("FORBIDDEN_LIVE_EXECUTION_CLAIM")
        if payload.get("TARGET_POSITION_QTY_UNIT") not in {None, "UNPROVEN"}:
            raise P10QtyUnitPersistError("UNIT_MUST_REMAIN_UNPROVEN")
    ...
    if int(verified.get("MANIFEST_VERIFY_RC", 1)) != 0:
        raise P10QtyUnitPersistError("MANIFEST_VERIFY_FAILED")
    return verified
