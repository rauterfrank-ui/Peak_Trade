"""Sanitized evidence persist for authenticated private read / permit issuance."""
...
_MANIFEST_FILES: tuple[str, ...] = (
    ...
    "SUMMARY.json",
    "claims.json",
    "GET_ACCOUNT_POSITIONS.sanitized.json",
    "RUNTIME_PERMIT.json",
)
...
class AuthenticatedPrivateRuntimeReadPersistError(RuntimeError):
...
def assert_no_secrets_in_payload_v1(payload: Mapping[str, Any]) -> None:
    def _walk(value: Any, key: str = "") -> None:
        ...
        if key_l in _SECRET_VALUE_KEYS and text and text not in {"<REDACTED>", "<REF_ONLY>"}:
            raise AuthenticatedPrivateRuntimeReadPersistError("SECRET_IN_EVIDENCE")
        lowered = text.lower()
...
def persist_authenticated_private_runtime_read_evidence_v1(
    *,
    pack: Path,
    origin_main_sha: str,
    ...
    get_sanitized: Mapping[str, Any],
    runtime_permit: Mapping[str, Any],
    claims: Mapping[str, Any],
) -> dict[str, Any]:
    if str(origin_main_sha or "").strip() != EXPECTED_ORIGIN_MAIN_SHA:
        raise AuthenticatedPrivateRuntimeReadPersistError("ORIGIN_MAIN_SHA_MISMATCH")
    if adjudication.get("POST_PERFORMED") is True or summary.get("POST_PERFORMED") is True:
        raise AuthenticatedPrivateRuntimeReadPersistError("FORBIDDEN_POST_CLAIM")
    if adjudication.get("FLATTEN_EXECUTE_AUTHORIZED") is True:
        raise AuthenticatedPrivateRuntimeReadPersistError(
    ...
    if adjudication.get("LIVE_AUTHORIZED") is True:
        raise AuthenticatedPrivateRuntimeReadPersistError("FORBIDDEN_LIVE_AUTHORIZED_TRUE")
    if adjudication.get("PRODUCTIVE_FLATTEN_POST_AUTHORIZED") is True:
        raise AuthenticatedPrivateRuntimeReadPersistError("FORBIDDEN_PRODUCTIVE_FLATTEN_POST")
    documents = {
        "CENSUS.json": census,
        ...
        "GET_ACCOUNT_POSITIONS.sanitized.json": get_sanitized,
    ...
    for payload in documents.values():
        ...
        if payload.get("LIVE_EXECUTION") is True:
            raise AuthenticatedPrivateRuntimeReadPersistError("FORBIDDEN_LIVE_EXECUTION_CLAIM")
    ...
    if int(verified.get("MANIFEST_VERIFY_RC", 1)) != 0:
        raise AuthenticatedPrivateRuntimeReadPersistError("MANIFEST_VERIFY_FAILED")
    return verified
