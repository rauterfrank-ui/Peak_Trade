"""Bounded Futures Testnet admission presentation ↔ operator closure lifecycle bridge (v0).

Deterministic, offline, explicit-input-only fail-closed bridge composing the canonical
PE-34/35/36/37 operator review admission presentation lifecycle integration (PR #4436)
and the PE-25 operator closure lifecycle integration.
...
PACKAGE_MARKER = "BOUNDED_FUTURES_TESTNET_ADMISSION_PRESENTATION_OPERATOR_CLOSURE_LIFECYCLE_BRIDGE_CONTRACT_V0=true"
CONTRACT_VERSION = (
    "bounded_futures_testnet_admission_presentation_operator_closure_lifecycle_bridge.v0"
)
SERIALIZATION_VERSION = "bounded_futures_testnet_admission_presentation_operator_closure_lifecycle_bridge.serialization.v0"
HASH_ALGORITHM = "sha256"
REPOSITORY_IDENTITY = "Peak_Trade"

BRIDGE_MODE = "static_admission_presentation_operator_closure_lifecycle_bridge_proof_only"
...
@dataclass(frozen=True)
class AdmissionPresentationOperatorClosureLifecycleBridgeInput:
    source_revision: str
...
def _validate_pe25_operator_closure_proof(
    ...
    *,
    ...
    fail_reasons: list[str] = []
    proof = bridge_input.pe25_operator_closure_proof
    pe25_input = bridge_input.pe25_closure_integration_input

    if proof.closure_owner != PE25_CONTRACT_VERSION:
        fail_reasons.append(
            f"pe25_operator_closure_proof: closure_owner must be {PE25_CONTRACT_VERSION!r}"
        )
    if proof.source_revision != bridge_input.source_revision:
        fail_reasons.append("pe25_operator_closure_proof: source_revision mismatch")
    if proof.closure_input_digest != compute_pe25_closure_input_digest(pe25_input):
        fail_reasons.append("pe25_operator_closure_proof: closure_input_digest mismatch")
    expected_closure_digest = pe25_result.get("closure_result_digest")
    if not proof.closure_result_digest:
        fail_reasons.append("pe25_operator_closure_proof: closure_result_digest required")
    elif expected_closure_digest is None:
        fail_reasons.append("pe25_operator_closure_proof: closure_result_digest unavailable")
    elif proof.closure_result_digest != expected_closure_digest:
    ...
    if not proof.admission_integration_proof_digest:
        fail_reasons.append(
            "pe25_operator_closure_proof: admission_integration_proof_digest required"
        )
    elif expected_admission_proof is None:
        fail_reasons.append(
            "pe25_operator_closure_proof: admission_integration_proof_digest unavailable"
        )
    elif proof.admission_integration_proof_digest != expected_admission_proof:
        fail_reasons.append(
            "pe25_operator_closure_proof: admission_integration_proof_digest mismatch"
        )
    ...
    if proof.operator_closure_lifecycle_bound is not True:
        fail_reasons.append(
            "pe25_operator_closure_proof: operator_closure_lifecycle_bound must be true"
        )
    if proof.pe25_operator_closure_bound is not True:
        fail_reasons.append("pe25_operator_closure_proof: pe25_operator_closure_bound must be true")
    return fail_reasons
...
def _validate_cross_lifecycle_coherence(
    ...
    pe25 = bridge_input.pe25_closure_integration_input
    ...
    if pe25.source_revision != bridge_input.source_revision:
        fail_reasons.append("composition: PE-25 source_revision mismatch")
    if admission.adapter_id != bridge_input.adapter_id:
        fail_reasons.append("composition: admission adapter_id mismatch")
    if pe25.adapter_id != bridge_input.adapter_id:
        fail_reasons.append("composition: PE-25 adapter_id mismatch")
    if admission.instrument != bridge_input.instrument:
        fail_reasons.append("composition: admission instrument mismatch")
    if pe25.instrument != bridge_input.instrument:
        fail_reasons.append("composition: PE-25 instrument mismatch")
...
def _validate_proof_chain_binding(
    ...
    *,
    ...
    admission_proof = admission_result.get("integration_proof_digest") or ""

    expected_slots = {
        "admission_integration_proof_digest": admission_proof,
        ...
        "pe25_closure_proof_digest": pe25_result.get("closure_result_digest") or "",
        "pe25_closure_input_digest": compute_pe25_closure_input_digest(pe25),
        "closure_referenced_admission_proof_digest": admission_proof,
    ...
    if chain.closure_referenced_admission_proof_digest != chain.admission_integration_proof_digest:
        fail_reasons.append(
            "proof_chain: closure_referenced_admission_proof_digest must match "
            "admission_integration_proof_digest"
        )
...
def validate_admission_presentation_operator_closure_lifecycle_bridge_input(
    bridge_input: AdmissionPresentationOperatorClosureLifecycleBridgeInput,
) -> list[str]:
    """Fail-closed validation of explicit admission presentation ↔ PE-25 closure bridge."""
    fail_reasons: list[str] = []
    ...
    fail_reasons.extend(
        validate_operator_closure_lifecycle_integration_input(
            bridge_input.pe25_closure_integration_input
        )
    )
...
def _bridge_input_dict(
    ...
    return {
        ...
        "pe25_operator_closure_proof": asdict(bridge_input.pe25_operator_closure_proof),
...
def _bridge_result_dict(
    ...
    *,
    ...
    admission_presentation_operator_closure_lifecycle_bridge_eligibility: bool = False,
) -> dict[str, Any]:
    payload: dict[str, Any] = {
        ...
        "pe25_closure_proof_digest": bridge_input.proof_chain.pe25_closure_proof_digest,
        "closure_referenced_admission_proof_digest": (
            bridge_input.proof_chain.closure_referenced_admission_proof_digest
        ),
        "shared_pe37_traceability_identity": bridge_input.proof_chain.shared_pe37_traceability_identity,
        "admission_presentation_operator_closure_lifecycle_bridge_eligibility": (
            admission_presentation_operator_closure_lifecycle_bridge_eligibility
        ),
        "admission_presentation_lifecycle_bound": (
            admission_presentation_operator_closure_lifecycle_bridge_eligibility
        ),
        "operator_closure_lifecycle_bound": (
            admission_presentation_operator_closure_lifecycle_bridge_eligibility
        ),
        "pe25_operator_closure_bound": (
            admission_presentation_operator_closure_lifecycle_bridge_eligibility
        ),
        "pe34_handoff_bound": admission_presentation_operator_closure_lifecycle_bridge_eligibility,
...
def evaluate_admission_presentation_operator_closure_lifecycle_bridge(
    bridge_input: AdmissionPresentationOperatorClosureLifecycleBridgeInput,
    *,
    expected_source_revision: str | None = None,
    expected_admission_integration_proof_digest: str | None = None,
    expected_pe25_closure_proof_digest: str | None = None,
    ...
    """Evaluate explicit admission presentation ↔ PE-25 operator closure lifecycle bridge."""
    fail_reasons = validate_admission_presentation_operator_closure_lifecycle_bridge_input(
        bridge_input
    )
    ...
    if expected_pe25_closure_proof_digest is not None:
        if chain.pe25_closure_proof_digest != expected_pe25_closure_proof_digest:
            fail_reasons.append("proof_chain: pe25_closure_proof_digest mismatch")
    ...
    return {
        ...
        "bridge_proof_digest": (
            compute_bridge_proof_digest(
                bridge_input,
                admission_presentation_operator_closure_lifecycle_bridge_eligibility=eligibility,
            )
            if bridge_pass
            else None
        ),
        ...
        "pe25_closure_proof_digest": chain.pe25_closure_proof_digest,
        "closure_referenced_admission_proof_digest": chain.closure_referenced_admission_proof_digest,
        ...
        "operator_closure_lifecycle_bound": eligibility,
...
def default_minimal_bridge_input(
    *,
    ...
    bridge_id: str = "admission-presentation-operator-closure-lifecycle-bridge-001",
    ...
    from src.ops.bounded_futures_testnet_operator_closure_lifecycle_integration_contract_v0 import (
        ...
        evaluate_operator_closure_lifecycle_integration as evaluate_pe25,
    )
    from src.ops.bounded_futures_testnet_operator_review_admission_presentation_lifecycle_integration_contract_v0 import (
    ...
    admission_input = default_minimal_admission_input(
    ...
    pe25_result = evaluate_pe25(pe25_input)

    admission_proof_digest = admission_result["integration_proof_digest"]
    ...
    bridge_proof_chain = BridgeProofChainBinding(
        ...
        closure_referenced_admission_proof_digest=admission_proof_digest,
        shared_pe35_boundary_result_digest=admission_input.proof_chain.pe35_boundary_result_digest,
        shared_pe37_traceability_identity=admission_input.proof_chain.pe37_traceability_identity,
    )

    return AdmissionPresentationOperatorClosureLifecycleBridgeInput(
        source_revision=source_revision,
        ...
        admission_presentation_lifecycle_proof=AdmissionPresentationLifecycleProofBinding(
            ...
            source_revision=source_revision,
            integration_input_digest=compute_admission_integration_input_digest(admission_input),
            integration_proof_digest=admission_proof_digest,
