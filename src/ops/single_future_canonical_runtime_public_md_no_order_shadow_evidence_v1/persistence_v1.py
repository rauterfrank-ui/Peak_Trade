"""Evidence persistence for Cap 5.2 public-MD no-order shadow evidence."""
...
def persist_public_md_shadow_evidence_atomic_v1(
    *,
    evidence_root: Path,
    ...
    failure_injection: Mapping[str, Any] | None = None,
    ...
    try:
        files: list[str] = [EVIDENCE_FILENAME, RESULT_FILENAME, GATE_FILENAME]
        ...
        if telemetry is not None:
            (staging / TELEMETRY_FILENAME).write_text(
                json.dumps(dict(telemetry), sort_keys=True, indent=2) + "\n", encoding="utf-8"
            )
            files.append(TELEMETRY_FILENAME)
        if restart is not None:
            (staging / RESTART_FILENAME).write_text(
                json.dumps(dict(restart), sort_keys=True, indent=2) + "\n", encoding="utf-8"
            )
            files.append(RESTART_FILENAME)
        if failure_injection is not None:
            (staging / FAILURE_INJECTION_FILENAME).write_text(
                json.dumps(dict(failure_injection), sort_keys=True, indent=2) + "\n",
                encoding="utf-8",
            )
            files.append(FAILURE_INJECTION_FILENAME)
        if capture is not None:
            (staging / CAPTURE_FILENAME).write_text(
                json.dumps(dict(capture), sort_keys=True, indent=2) + "\n", encoding="utf-8"
            )
            files.append(CAPTURE_FILENAME)
        if authorization_consumption is not None:
            (staging / AUTH_CONSUMPTION_FILENAME).write_text(
                json.dumps(dict(authorization_consumption), sort_keys=True, indent=2) + "\n",
                encoding="utf-8",
            )
            files.append(AUTH_CONSUMPTION_FILENAME)
        write_manifest(staging, tuple(files))
        for name in (*files, MANIFEST_FILENAME):
            src = staging / name
