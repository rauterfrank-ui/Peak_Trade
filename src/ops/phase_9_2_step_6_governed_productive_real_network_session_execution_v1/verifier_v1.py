...
REQUIRED_TRUE_SESSION_IMPL_CLAIMS = {
    ...
    "BINDING_ONLY_EXECUTOR_NOT_USED_AS_PRODUCTIVE_EXECUTOR",
    "EXACTLY_ONE_SESSION_START_ENFORCED",
...
def verify_session_execution_implementation_manifest_v1(
    ...
    if bool(claims.get("STEP6_PRODUCTIVE_REAL_NETWORK_EXECUTION_PATH_ABSENT")):
        blockers.append("PATH_ABSENT_CLAIM_MUST_BE_FALSE")
    if (
        str(claims.get("PRODUCTIVE_SESSION_INVOKE_SYMBOL") or "")
        != "execute_governed_step6_session_v1"
    ):
        blockers.append("PRODUCTIVE_SESSION_INVOKE_SYMBOL_MISMATCH")
    return {
        "ok": not blockers,
        "blockers": blockers,
