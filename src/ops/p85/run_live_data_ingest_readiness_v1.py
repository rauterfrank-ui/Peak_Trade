...
class P85ContextV1:
    ...
    def from_env(cls) -> "P85ContextV1":
        ...
        return cls(
            ...
            connectivity_url=os.environ.get("P85_CONNECTIVITY_URL", ""),
        )


def _check_connectivity(ctx: P85ContextV1) -> dict[str, Any]:
    """Check connectivity to live data endpoint (REST)."""
    if not str(ctx.connectivity_url or "").strip():
        return {
            "ok": False,
            "error": "connectivity_url_not_configured",
            "detail": {"url": None},
        }
    try:
...
def _check_rate_limit(resp_headers: dict[str, str]) -> dict[str, Any]:
    ...
    # Rate-limit headers are optional on public endpoints.
