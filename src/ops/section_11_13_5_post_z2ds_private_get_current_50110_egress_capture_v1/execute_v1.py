"""Execute exactly one authenticated GET /api/v5/account/config for 50110 capture.
...
HTTP 401/50110 is an authorized capture result, not a runner failure.
"""
...
from src.ops.section_11_13_5_post_z2ds_private_get_current_50110_egress_capture_v1.persist_v1 import (
    persist_50110_egress_capture_evidence_v1,
)

_EGRESS_IP_RE = re.compile(
    r"Your IP\s+(\d{1,3}(?:\.\d{1,3}){3})\s+is not included",
    re.IGNORECASE,
)
_UUID_RE = re.compile(
...
class PostZ2DS50110EgressCaptureError(RuntimeError):
    """Fail-closed one-shot 50110 egress-capture GET violation."""


def extract_okx_reported_egress_ipv4_v1(message: str | None) -> str | None:
    if not message:
        return None
    match = _EGRESS_IP_RE.search(str(message))
    if match is None:
        return None
    return str(match.group(1))


def sanitize_okx_message_v1(message: str | None) -> str | None:
    if message is None:
        return None
    clipped = str(message)[:200]
    return _UUID_RE.sub("<REDACTED_KEY_ID>", clipped)
...
def _assert_one_get_zero_writes(client: LiveCanaryHttpClientV1) -> dict[str, Any]:
    ...
    if int(counters.get("GET_REQUEST_COUNT", 0) or 0) != 1:
        raise PostZ2DS50110EgressCaptureError("GET_COUNT_NOT_ONE")
    if int(counters.get("REQUEST_COUNT", 0) or 0) != 1:
        raise PostZ2DS50110EgressCaptureError("REQUEST_COUNT_NOT_ONE")
    if int(counters.get("WRITE_REQUEST_COUNT", 0) or 0) != 0:
        raise PostZ2DS50110EgressCaptureError("WRITE_REQUEST_DETECTED")
    if int(counters.get("TRANSFER_REQUEST_COUNT", 0) or 0) != 0:
        raise PostZ2DS50110EgressCaptureError("TRANSFER_REQUEST_DETECTED")
    if int(counters.get("ORDER_REQUEST_COUNT", 0) or 0) != 0:
        raise PostZ2DS50110EgressCaptureError("ORDER_REQUEST_DETECTED")
    if int(counters.get("ENTRY_SUBMIT_COUNT", 0) or 0) != 0:
        raise PostZ2DS50110EgressCaptureError("ENTRY_SUBMIT_DETECTED")
    if int(counters.get("FLATTEN_SUBMIT_COUNT", 0) or 0) != 0:
        raise PostZ2DS50110EgressCaptureError("FLATTEN_SUBMIT_DETECTED")
    if list(client.counters.endpoints_used) != [ENDPOINT]:
        raise PostZ2DS50110EgressCaptureError("ENDPOINT_SET_MISMATCH")
    if list(client.counters.methods_used) != ["GET"]:
        raise PostZ2DS50110EgressCaptureError("NON_GET_METHOD_DETECTED")
    return counters


def adjudicate_capture_result_v1(
    *,
    ...
    if http_status == 401 and code == "50110":
        ...
        return {
            ...
            "PRIVATE_AUTH_50110_IS_NOT_CURRENTLY_REPRODUCED": False,
            "OKX_REPORTED_EGRESS_IPV4_CAPTURED": captured,
            "WHITELIST_GO_INPUT_CAPTURED": captured,
            "FRESH_ADJUDICATION_REQUIRED": not captured,
            "FAIL_CLOSED_RECORD": not captured,
            "ROOT_CAUSE": "UNPROVEN",
            "DO_NOT_INFER_ROOT_CAUSE": False,
        }
    if http_status == 200 and code == "0":
        return {
            ...
            "PRIVATE_AUTH_50110_IS_NOT_CURRENTLY_REPRODUCED": True,
            "OKX_REPORTED_EGRESS_IPV4_CAPTURED": False,
            "WHITELIST_GO_INPUT_CAPTURED": False,
            "FRESH_ADJUDICATION_REQUIRED": True,
...
def execute_single_50110_egress_capture_get_v1(
    *,
    owner_go: str,
    origin_main_sha: str,
    evidence_root: Path,
    ...
    """Perform exactly one allowlisted authenticated GET and persist capture evidence."""
    owned = str(owner_go or "").strip()
    if owned != OWNER_GO:
        raise PostZ2DS50110EgressCaptureError("OWNER_GO_MISMATCH")
    bound_sha = str(origin_main_sha or "").strip()
    if bound_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise PostZ2DS50110EgressCaptureError("ORIGIN_MAIN_SHA_MISMATCH")
    if REUSED_REST_HOST != AUTHORIZED_HOST:
        raise PostZ2DS50110EgressCaptureError("HOST_MISMATCH")
    if "?" in ENDPOINT or ENDPOINT != "/api/v5/account/config":
        raise PostZ2DS50110EgressCaptureError("ENDPOINT_CONTRACT_DRIFT")
    if ENDPOINT in FORBIDDEN_ENDPOINTS:
        raise PostZ2DS50110EgressCaptureError("MUTATION_ENDPOINT_FORBIDDEN")

    productive = transport is None
    if productive:
        if vault_file is None or not str(vault_file).strip():
            raise PostZ2DS50110EgressCaptureError("VAULT_FILE_REQUIRED")
    ...
    if isinstance(transport, UrllibLiveCanaryTransportV1) and not bool(
        ...
        raise PostZ2DS50110EgressCaptureError("PRODUCTIVE_WIRE_DISABLED")
    ...
    try:
        ...
        if parsed.path != ENDPOINT or parsed.query:
            raise PostZ2DS50110EgressCaptureError("SIGNED_REQUEST_TARGET_MISMATCH")
        ...
        if response.method != "GET":
            raise PostZ2DS50110EgressCaptureError("NON_GET_RESPONSE")
        if redirect_followed:
            raise PostZ2DS50110EgressCaptureError("REDIRECT_FOLLOWED")
    ...
    if get_error is None:
        ...
        if http_exchange_count != MAX_HTTP_EXCHANGE_COUNT:
            raise PostZ2DS50110EgressCaptureError("HTTP_EXCHANGE_COUNT_NOT_ONE")
        if productive and http_exchange_count != 1:
            raise PostZ2DS50110EgressCaptureError("NETWORK_REQUEST_COUNT_NOT_ONE")
    elif http_exchange_count > MAX_HTTP_EXCHANGE_COUNT:
        raise PostZ2DS50110EgressCaptureError("HTTP_EXCHANGE_COUNT_EXCEEDED")
    ...
    outcome = adjudicate_capture_result_v1(
    ...
    snapshot = {
        "DOCUMENT_CLASS": "POST_Z2DS_PRIVATE_GET_CURRENT_50110_EGRESS_CAPTURE_V1",
        ...
        "OKX_REPORTED_EGRESS_IPV4": egress_ipv4,
    ...
    verified = persist_50110_egress_capture_evidence_v1(
        pack=pack,
    ...
    if get_error:
        raise PostZ2DS50110EgressCaptureError(f"PRIVATE_AUTH_GET_FAILED:{get_error}")
    if parse_error:
        raise PostZ2DS50110EgressCaptureError(f"PRIVATE_AUTH_GET_PARSE_FAIL_CLOSED:{parse_error}")
    return {
