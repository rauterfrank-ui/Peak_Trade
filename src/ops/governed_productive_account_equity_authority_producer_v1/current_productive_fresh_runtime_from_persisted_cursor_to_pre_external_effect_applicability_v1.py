"""CURRENT_PRODUCTIVE runtime cycle from persisted cursor to pre-external-effect.

Consumes Owner-GO
OWNER_GO_CURRENT_PRODUCTIVE_FRESH_RUNTIME_FROM_PERSISTED_CURSOR_TO_PRE_EXTERNAL_EFFECT_APPLICABILITY_V1.

Re-proves occupancy/pending/config via READ-ONLY GETs, restores the persisted
CURRENT SideState/confirmation cursor only on exact schema/version/native/
instrument/lineage match, binds Cap-23/24 through the current Cap-2.1–2.4
producers without forcing the prior instrument, and runs one current
Master-V2 cycle through the canonical host. Does not reuse DQ/DR/DS/DT packs
as current market authority. Does not fabricate ENTER. Does not POST. Does
not create or consume a live permit. Does not carry confirmation across
instruments.
...
OWNER_GO = (
    "OWNER_GO_CURRENT_PRODUCTIVE_FRESH_RUNTIME_FROM_PERSISTED_CURSOR_TO_"
    "PRE_EXTERNAL_EFFECT_APPLICABILITY_V1"
)
THIS_SLICE = (
    "11.2.1.DU.FULL_CORE_CURRENT_PRODUCTIVE_FRESH_RUNTIME_FROM_PERSISTED_"
    "CURSOR_TO_PRE_EXTERNAL_EFFECT_APPLICABILITY"
)
EXPECTED_ORIGIN_MAIN_SHA = "a3fa2cb1fd6e9922f94b9dd8f4693e7fbef95796"
...
_OCCUPANCY_DECISION_ID = "du-occupancy-reproof-from-persisted-cursor-v1"
MAX_GET_REQUEST_COUNT = 24


class CurrentProductiveFreshRuntimeFromPersistedCursorError(
    CurrentProductiveFreshCap23Cap24ReadinessError
):
    """Fail-closed persisted-cursor pre-external-effect boundary violation."""


@dataclass(frozen=True)
class CurrentProductiveFreshRuntimeFromPersistedCursorResultV1:
    store_root: str
...
def _assert_du_pins() -> None:
    _assert_standing_pins()
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_V3_CREATED is not True:
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError("DQ_ADAPTER_NOT_CREATED")
    if CURRENT_PRODUCTIVE_ONESHOT_SIDESTATE_CONFIRMATION_CURSOR_JOIN_CREATED is not True:
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError("DR_ADAPTER_NOT_CREATED")
    if CURRENT_PRODUCTIVE_HOST_ENTER_29P_INVALID_STOP_PRICE_REPAIR_CREATED is not True:
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError("DS_ADAPTER_NOT_CREATED")
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_TO_PRE_EXTERNAL_EFFECT_APPLICABILITY_CREATED is not True:
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError("DT_ADAPTER_NOT_CREATED")
    if (
        CURRENT_PRODUCTIVE_FRESH_RUNTIME_FROM_PERSISTED_CURSOR_TO_PRE_EXTERNAL_EFFECT_APPLICABILITY_CREATED
        is not True
    ):
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError("DU_ADAPTER_NOT_CREATED")
...
def execute_current_productive_fresh_runtime_from_persisted_cursor_to_pre_external_effect_applicability_v1(
    *,
    owner_go: str,
    origin_main_sha: str,
...
) -> CurrentProductiveFreshRuntimeFromPersistedCursorResultV1:
    if owner_go != OWNER_GO:
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError("OWNER_GO_MISMATCH")
    if origin_main_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError("ORIGIN_MAIN_SHA_MISMATCH")
    if execute_network is not True and acquisition_result is None and acquisition_transport is None:
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError(
    ...
    if occupancy_blocker:
        ...
        if acquisition_result is None:
            try:
                ...
                raise CurrentProductiveFreshRuntimeFromPersistedCursorError(str(exc)) from exc
                ...
                if not market_blocker:
                    try:
                        cycle_result = run_current_productive_master_v2_runtime_cycle_v1(
                            ...
                            cycle_id=f"du-{native_id}-{package_started}",
                            observed_unix=observed_unix,
    ...
    send_handle = FullCoreSendCredentialHandleV1(
        handle_id="full-core-du-readiness-handle", bound=True
    )
    ...
    try:
        http_transport.post_trade_order(
            ...
            permit_id="eep-du-must-not-post",
            envelope_id="env-du-must-not-post",
        ...
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError("HTTP_MUST_NOT_POST")
    if http_transport.post_count != 0:
        raise CurrentProductiveFreshRuntimeFromPersistedCursorError("HTTP_TRANSPORT_SIDE_EFFECT")
    ...
    if plan is not None and envelope_readiness == TRUE_TOKEN:
        envelope = bind_final_order_envelope_from_venue_plan_v1(
            plan,
            admission_ref="DU_FRESH_CAP24_AND_CURRENT_PRODUCTIVE_MASTER_V2",
    ...
    return CurrentProductiveFreshRuntimeFromPersistedCursorResultV1(
...
def main() -> int:
    ...
    result = execute_current_productive_fresh_runtime_from_persisted_cursor_to_pre_external_effect_applicability_v1(
