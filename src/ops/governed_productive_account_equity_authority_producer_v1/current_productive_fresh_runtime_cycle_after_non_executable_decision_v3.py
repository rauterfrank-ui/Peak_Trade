"""CURRENT_PRODUCTIVE fresh runtime cycle after non-executable DP decision.
...
Master-V2 cycle. Does not reuse DN/DO/DP facts as current authority. Does not
...
EXPECTED_ORIGIN_MAIN_SHA = "04a3f1e9b36f9d2f4c5fd9b544e5c523140a32b8"
...
_OCCUPANCY_DECISION_ID = "dq-occupancy-reproof-after-non-executable-v3"
MAX_GET_REQUEST_COUNT = 24


class CurrentProductiveFreshRuntimeCycleAfterNonExecutableV3Error(
    CurrentProductiveFreshCap23Cap24ReadinessError
):
    """Fail-closed post-non-executable V3 runtime-cycle boundary violation."""


@dataclass(frozen=True)
class CurrentProductiveFreshRuntimeCycleAfterNonExecutableResultV3:
    store_root: str
...
def _assert_dq_pins() -> None:
    _assert_standing_pins()
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_CREATED is not True:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV3Error("DO_ADAPTER_NOT_CREATED")
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_V2_CREATED is not True:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV3Error("DP_ADAPTER_NOT_CREATED")
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_V3_CREATED is not True:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV3Error("DQ_ADAPTER_NOT_CREATED")
...
def execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v3(
    *,
    owner_go: str,
    origin_main_sha: str,
    ...
    if origin_main_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV3Error(
    ...
    if occupancy_blocker:
        ...
        if acquisition_result is None:
            try:
                ...
                raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV3Error(str(exc)) from exc
    ...
    send_handle = FullCoreSendCredentialHandleV1(
        handle_id="full-core-dq-readiness-handle", bound=True
    )
    ...
    try:
        http_transport.post_trade_order(
            ...
            permit_id="eep-dq-must-not-post",
            envelope_id="env-dq-must-not-post",
        ...
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV3Error("HTTP_MUST_NOT_POST")
    ...
    if plan is not None and envelope_readiness == TRUE_TOKEN:
        envelope = bind_final_order_envelope_from_venue_plan_v1(
            plan,
            admission_ref="DQ_FRESH_CAP24_AND_CURRENT_PRODUCTIVE_MASTER_V2",
    ...
    return CurrentProductiveFreshRuntimeCycleAfterNonExecutableResultV3(
...
def main() -> int:
    ...
    result = execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v3(
