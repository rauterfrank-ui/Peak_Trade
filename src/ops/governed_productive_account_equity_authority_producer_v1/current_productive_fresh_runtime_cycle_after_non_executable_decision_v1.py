"""CURRENT_PRODUCTIVE fresh runtime cycle after non-executable DN decision.
...
Master-V2 cycle. Does not reuse DN facts as current authority. Does not
...
EXPECTED_ORIGIN_MAIN_SHA = "aadd3c9acb8e34fab98bfda245525ef3926d7412"
...
_OCCUPANCY_DECISION_ID = "do-occupancy-reproof-after-non-executable-v1"
MAX_GET_REQUEST_COUNT = 24


class CurrentProductiveFreshRuntimeCycleAfterNonExecutableError(
    CurrentProductiveFreshCap23Cap24ReadinessError
):
    """Fail-closed post-non-executable runtime-cycle boundary violation."""


@dataclass(frozen=True)
class CurrentProductiveFreshRuntimeCycleAfterNonExecutableResultV1:
    store_root: str
...
def _assert_do_pins() -> None:
    _assert_standing_pins()
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_CREATED is not True:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableError("DO_ADAPTER_NOT_CREATED")
...
def execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v1(
    *,
    owner_go: str,
    origin_main_sha: str,
...
) -> CurrentProductiveFreshRuntimeCycleAfterNonExecutableResultV1:
    if owner_go != OWNER_GO:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableError("OWNER_GO_MISMATCH")
    if origin_main_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableError("ORIGIN_MAIN_SHA_MISMATCH")
    if execute_network is not True and acquisition_result is None and acquisition_transport is None:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableError(
    ...
    if occupancy_blocker:
        ...
        if acquisition_result is None:
            try:
                ...
                raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableError(str(exc)) from exc
            ...
            if missing and not market_blocker:
                ...
                try:
                    cycle_result = run_current_productive_master_v2_runtime_cycle_v1(
                        ...
                        cycle_id=f"do-{native_id}-{package_started}",
                        observed_unix=observed_unix,
    ...
    if bound is not None and not occupancy_blocker and not market_blocker:
        status, reasons, plan = try_bind_current_productive_venue_plan_v1(
            replay=cycle_replay,
            bound_instrument=bound,
            session_id="current-productive-do-session",
            run_id="current-productive-do-run",
    ...
    send_handle = FullCoreSendCredentialHandleV1(
        handle_id="full-core-do-readiness-handle", bound=True
    )
    ...
    try:
        http_transport.post_trade_order(
            ...
            permit_id="eep-do-must-not-post",
            envelope_id="env-do-must-not-post",
        ...
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableError("HTTP_MUST_NOT_POST")
    ...
    if plan is not None and envelope_readiness == TRUE_TOKEN:
        envelope = bind_final_order_envelope_from_venue_plan_v1(
            plan,
            admission_ref="DO_FRESH_CAP24_AND_CURRENT_PRODUCTIVE_MASTER_V2",
    ...
    return CurrentProductiveFreshRuntimeCycleAfterNonExecutableResultV1(
...
def main() -> int:
    ...
    result = execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v1(
