"""CURRENT_PRODUCTIVE LIVE_ENABLED standing-gate closure.

Consumes Owner-GO
OWNER_GO_CURRENT_PRODUCTIVE_LIVE_ENABLED_STANDING_GATE_EVALUATION_V1.

Sets the existing §11.2.1.O Full-Core standing predicate LIVE_ENABLED=true.
That satisfies only the LIVE_ENABLED deny predicate. It does not admit Live,
arm Live, permit wire send, authorize POST, or construct LiveExecutionPort.
...
from src.ops.full_core_live_path_composition_root_v1.constants_v1 import (
    ...
    LIVE_ENABLED_DOES_NOT_IMPLY_LIVE_AUTHORIZED,
...
from src.ops.governed_productive_account_equity_authority_producer_v1.constants_v1 import (
    CURRENT_PRODUCTIVE_LIVE_ENABLED_STANDING_GATE_ADAPTER_CREATED,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.current_productive_eea_universe_inventory_to_cap24_and_29p_v1 import (
    CANONICAL_PACK_RELPATH as CZ_PACK_RELPATH,
)
...
EXPECTED_ORIGIN_MAIN_SHA = "b34db27f354a399c4d66263f3e48300b6857d363"
CANONICAL_PACK_RELPATH = (
    "evidence/ops/full_core_current_productive_live_enabled_standing_gate_v1/20260915T163400Z"
)
FALSE_TOKEN = "false"
TRUE_TOKEN = "true"
...
class CurrentProductiveLiveEnabledStandingGateError(ValueError):
    """Fail-closed LIVE_ENABLED standing-gate evaluation violation."""


@dataclass(frozen=True)
class CurrentProductiveLiveEnabledStandingGateResultV1:
    store_root: str
    live_enabled: str
    live_armed: str
...
def _assert_no_secrets(payload: Mapping[str, Any]) -> None:
    ...
    for token in _SECRET_TOKENS:
        if token in blob:
            raise CurrentProductiveLiveEnabledStandingGateError(f"SECRET_TOKEN_PRESENT:{token}")


def _load_json_object(*, path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise CurrentProductiveLiveEnabledStandingGateError(f"JSON_NOT_OBJECT:{path.name}")
    return payload
...
def _assert_standing_pins() -> None:
    if CURRENT_PRODUCTIVE_LIVE_ENABLED_STANDING_GATE_ADAPTER_CREATED is not True:
        raise CurrentProductiveLiveEnabledStandingGateError("ADAPTER_NOT_CREATED")
    if LIVE_ENABLED_STANDING_ADMISSION_SEAM_IMPLEMENTED is not True:
        raise CurrentProductiveLiveEnabledStandingGateError("SEAM_NOT_IMPLEMENTED")
    if LIVE_ENABLED_STANDING_GATE_CLOSED is not True:
        raise CurrentProductiveLiveEnabledStandingGateError("STANDING_GATE_NOT_CLOSED")
    if LIVE_ENABLED is not True:
        raise CurrentProductiveLiveEnabledStandingGateError("LIVE_ENABLED_NOT_TRUE")
    if LIVE_ENABLED_TRUE_IS_NOT_AUTOMATIC_ADMISSION is not True:
        raise CurrentProductiveLiveEnabledStandingGateError("TRUE_MUST_NOT_ADMIT")
    if LIVE_ENABLED_DOES_NOT_IMPLY_LIVE_ARMED is not True:
        raise CurrentProductiveLiveEnabledStandingGateError("MUST_NOT_IMPLY_ARMED")
    if LIVE_ENABLED_DOES_NOT_IMPLY_WIRE_SEND is not True:
        raise CurrentProductiveLiveEnabledStandingGateError("MUST_NOT_IMPLY_WIRE")
    if LIVE_ENABLED_DOES_NOT_IMPLY_PORT_CONSTRUCTION is not True:
...
def _bind_current_cz_epoch(*, repo_root: Path) -> dict[str, Any]:
    claims_path = repo_root / CZ_PACK_RELPATH / "claims.json"
    if not claims_path.is_file():
        raise CurrentProductiveLiveEnabledStandingGateError("CZ_CLAIMS_MISSING")
    claims = _load_json_object(path=claims_path)
    if str(claims.get("STEP_29P_RISK_ADMISSIBLE") or "") != TRUE_TOKEN:
        raise CurrentProductiveLiveEnabledStandingGateError("CZ_STEP_29P_NOT_TRUE")
    if str(claims.get("CAP24_BOUND_INSTRUMENT_STATUS") or "") != "PASS":
        raise CurrentProductiveLiveEnabledStandingGateError("CZ_CAP24_NOT_PASS")
    instrument_id = str(claims.get("CAP24_BOUND_INSTRUMENT_ID") or "").strip()
    if not instrument_id:
        raise CurrentProductiveLiveEnabledStandingGateError("CZ_CAP24_ID_MISSING")
    if str(claims.get("CANARY_INSTRUMENT_AUTHORITY_IMPORTED") or "") != FALSE_TOKEN:
        raise CurrentProductiveLiveEnabledStandingGateError("CZ_CANARY_IMPORTED")
    if str(claims.get("POST_COUNT") or "") != "0":
        raise CurrentProductiveLiveEnabledStandingGateError("CZ_POST_COUNT_NOT_ZERO")
    return claims


def _admission_inputs_v1() -> ExecutionAdmissionInputsV1:
    return ExecutionAdmissionInputsV1(
        plan_identity="current-productive-live-enabled-standing-gate",
        venue_plan_identity="current-productive-live-enabled-standing-gate",
...
def execute_current_productive_live_enabled_standing_gate_v1(
    *,
    owner_go: str,
    origin_main_sha: str,
    evidence_root: Path | None = None,
    repo_root: Path | None = None,
) -> CurrentProductiveLiveEnabledStandingGateResultV1:
    if owner_go != OWNER_GO:
        raise CurrentProductiveLiveEnabledStandingGateError("OWNER_GO_MISMATCH")
    if origin_main_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise CurrentProductiveLiveEnabledStandingGateError("ORIGIN_MAIN_SHA_MISMATCH")
    _assert_standing_pins()
    root = repo_root or Path(__file__).resolve().parents[3]
    cz = _bind_current_cz_epoch(repo_root=root)
    ...
    claims = {
        ...
        "LIVE_ENABLED_SEAM_STATUS": "STANDING_TRUE_NOT_AUTOMATIC_ADMISSION",
        "LIVE_ENABLED_SEMANTICS_PROVEN": TRUE_TOKEN,
        "CONTRADICTION_LOCK_REMOVED": TRUE_TOKEN,
        "LIVE_ENABLED_TRUE_IS_NOT_AUTOMATIC_ADMISSION": TRUE_TOKEN,
        "LIVE_ENABLED_DOES_NOT_IMPLY_LIVE_ARMED": TRUE_TOKEN,
        "LIVE_ENABLED_DOES_NOT_IMPLY_WIRE_SEND": TRUE_TOKEN,
        "LIVE_ENABLED_DOES_NOT_IMPLY_PORT_CONSTRUCTION": TRUE_TOKEN,
        "LIVE_ENABLED_DOES_NOT_IMPLY_LIVE_AUTHORIZED": TRUE_TOKEN,
        ...
        "LIVE_ENABLED_DENY_ABSENT": TRUE_TOKEN,
        ...
        "POST_COUNT": "0",
        "STEP_29P_RISK_ADMISSIBLE": str(cz["STEP_29P_RISK_ADMISSIBLE"]),
        "CAP24_BOUND_INSTRUMENT_ID": str(cz["CAP24_BOUND_INSTRUMENT_ID"]),
        "CAP23_SELECTED_INSTRUMENT_ID": str(cz["CAP23_SELECTED_INSTRUMENT_ID"]),
    ...
    lineage = {
        "OWNER_GO": OWNER_GO,
        "THIS_SLICE": THIS_SLICE,
        "CZ_OWNER_GO": str(cz.get("OWNER_GO") or ""),
        "CZ_FIRST_REAL_BLOCKER": str(cz.get("FIRST_REAL_BLOCKER") or ""),
        "CAP24_PROVENANCE": cz.get("CAP24_PROVENANCE") or {},
    }
    protected = {
    ...
    return CurrentProductiveLiveEnabledStandingGateResultV1(
