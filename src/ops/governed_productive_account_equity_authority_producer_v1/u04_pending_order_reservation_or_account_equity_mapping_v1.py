"""Adjudicate U04 productive scope and deny mapping without source proof.

Consumes the workpackage Owner-GO. Historical U04 algebra inclusion
IN_BASE vs NOT_IN_BASE remains UNRESOLVED. Does not INCLUDE or EXCLUDE
U04 as an equity-stock kind. Does not treat NOT_IN_CURRENT_PRODUCTIVE_KIND_SET
as EXCLUDE. Does not treat UNRESOLVED as IN_BASE or NOT_IN_BASE. Does
not GET. Does not POST. Does not Hope-GET. Does not elevate venue eq to
source authority. Mapping remains not canonically valid. KIND_SET
remains EMPTY_FAIL_CLOSED. AUTHORITY_EFFECT=NONE.

RUNTIME_AUTHORIZATION_EFFECT=NONE
"""

from __future__ import annotations

import json
from dataclasses import dataclass
from pathlib import Path
from typing import Any, Mapping

from src.ops.full_core_live_path_composition_root_v1.constants_v1 import (
    LIVE_ARMED,
    LIVE_ENABLED,
    MAPPING_PROVEN,
    WIRE_SEND_PERMITTED,
)
from src.ops.full_core_live_path_composition_root_v1.live_admission_gap_dag_v1 import (
    EARLIEST_UNRESOLVED_FULL_CORE_DEPENDENCY as DAG_PIN,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.bj_future_admissible_evidence_and_include_exclude_qualification_law_v1 import (
    OUTCOME_EXCLUDE,
    OUTCOME_INCLUDE,
    TARGET_RESIDUAL,
    TARGET_U05,
    TARGET_U06,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.classified_event_kind_set_and_source_seam_contract_v1 import (
    DISPOSITION_NOT_EQUITY_STOCK,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.constants_v1 import (
    CURRENT_PRODUCTIVE_ACCOUNT_EQUITY_SOURCE_MAPPING_CLOSURE_CLOSED,
    ACCOUNT_BILLS_REMAINS_CURRENT_NONCANONICAL,
    ACCOUNT_EQUITY_SOURCE_MAPPING_DENIED_THIS_WORKPACKAGE,
    AUTHORIZED_PRODUCTIVE_EVENT_SOURCE_SEAM_PRESENT,
    C17_CREATED,
    CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_MAPPING,
    COMPLETE_EVENT_STREAM_PROVEN,
    CURRENT_PRODUCTIVE_REMAINING_NECESSARY_EQUITY_STOCK_CLASSES,
    D6_ACCOUNT_EQUITY_SOURCE_MAPPING_COVERAGE_STATUS,
    D6_FULLY_CLOSED,
    D7_AUTHORIZED,
    EQ_RECONCILIATION_TARGET_ONLY,
    EVENT_ACQUISITION_NETWORK_GET_AUTHORIZED,
    KIND_SET_RESOLVED,
    MS2_AUTHORIZED,
    NAMED_REMAINING_UNKNOWN_NECESSARY_CLASSES,
    OBSERVATION_NETWORK_GET_AUTHORIZED,
    PRODUCTIVE_RESIDUAL_EQUITY_STOCK_KIND_MEMBERSHIP,
    PRODUCTIVE_U04_AVAILABLE_CAPITAL_ROLE,
    PRODUCTIVE_U04_EQUITY_STOCK_KIND_MEMBERSHIP,
    PRODUCTIVE_U04_EQUITY_STOCK_ROLE,
    PRODUCTIVE_U05_EQUITY_STOCK_KIND_MEMBERSHIP,
    PRODUCTIVE_U06_EQUITY_STOCK_KIND_MEMBERSHIP,
    RAW_EQ_SOURCE_AUTHORITY,
    RECONSTRUCTION_ALGEBRA_COMPLETE,
    RESIDUAL_KIND_DECISION,
    SEMANTIC_MAPPING_PROVEN,
    U04_LEGACY_ALGEBRA_IN_BASE_VS_NOT_IN_BASE,
    U04_LEGACY_INCLUDE_EXCLUDE_AS_EQUITY_STOCK_KIND_RETIRED_AS_PRODUCTIVE_BLOCKER,
    U04_LEGACY_STATUS,
    U04_PLACEMENT,
    U05_KIND_DECISION,
    U06_KIND_DECISION,
    U06_PLACEMENT,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.d4_d5_genesis_rebaseline_contract_v1 import (
    EXPECTED_GENESIS_AS_OF,
    EXPECTED_GENESIS_ID,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.d4_d5_genesis_runtime_orchestrator_v1 import (
    persist_manifest_sha256_v1,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.equity_affecting_event_taxonomy_contract_v1 import (
    RATIFIED_CLASSIFIED_KIND_SET,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.named_remaining_unknown_kind_set_evidence_persist_contract_v1 import (
    DECISION_REMAIN_UNKNOWN,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.package_1_s6_mapping_classification_v1 import (
    verify_manifest_sha256_v1,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.productive_remaining_necessary_equity_stock_kind_u06_and_residual_v1 import (
    BLOCKER_ID as PARENT_BLOCKER_ID,
    CANONICAL_PACK_RELPATH as CANONICAL_CN_PACK_RELPATH,
    NEXT_OWNER_GO as PIN_OWNER_GO,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.reconstruction_algebra_contract_v1 import (
    EARLIEST_UNRESOLVED_ALGEBRA_TERM,
    U04_PENDING_ORDER_RESERVATIONS,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.remaining_necessary_kind_evidence_classification_v1 import (
    U04_KIND_SET_DISPOSITION,
    reject_u04_reclassify_as_equity_stock_kind_v1,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.residual_positive_necessary_kind_exhaustiveness_durable_unknown_pin_v1 import (
    reject_durable_unknown_as_include_or_exclude_v1,
)
from src.ops.governed_productive_account_equity_authority_producer_v1.u06_paired_fee_event_and_once_only_equity_stock_effect_primary_proof_surface_binding_v1 import (
    reject_hope_get_v1,
)

OWNER_GO = "OWNER_GO_U04_PENDING_ORDER_RESERVATION_OR_ACCOUNT_EQUITY_MAPPING_V1"
EXPECTED_ORIGIN_MAIN_SHA = "41d160425c01a6b2a45b4f096bdc9b8d5f2a952c"
CANONICAL_PACK_RELPATH = (
    "evidence/ops/full_core_u04_pending_order_reservation_or_account_equity_mapping_v1/"
    "2026-09-15T140000Z"
)
CANONICAL_PERSIST_AS_OF = "2026-09-15T14:00:00Z"
SCHEMA_CLASS = "U04_PENDING_ORDER_RESERVATION_OR_ACCOUNT_EQUITY_MAPPING_V1"
CONTRACT_VERSION = "v1"
AUTHORITY_EFFECT = "NONE"
FALSE_TOKEN = "false"
TRUE_TOKEN = "true"
NONE_TOKEN = "NONE"
STATE_UNRESOLVED = "UNRESOLVED"
KIND_SET_EMPTY = "EMPTY_FAIL_CLOSED"
PRODUCTIVE_MEMBERSHIP = "NOT_IN_CURRENT_PRODUCTIVE_KIND_SET"
U04_PROOF_LAW = (
    "PRODUCTIVE_EQUITY_STOCK_KIND_MEMBERSHIP_FORBIDDEN_U04_KIND_SET_DISPOSITION_IS_"
    "NOT_EQUITY_STOCK_AFFECTING_PRODUCTIVE_AVAILABLE_CAPITAL_ROLE_IS_AVAILABLE_FOR_"
    "SIZING_OR_RISK_SIZING_ALGEBRA_IN_BASE_VS_NOT_IN_BASE_REMAINS_UNRESOLVED_ABSENCE_"
    "IS_NOT_EXCLUDE_AND_NOT_INCLUDE_AND_NOT_IN_BASE"
)
MAPPING_DENY_LAW = (
    "CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_MAPPING_REQUIRES_RATIFIED_NON_EQ_SOURCE_"
    "KIND_AND_SEMANTIC_UNITS_TEMPORAL_COMPLETENESS_AND_CONSUMER_BINDING_U04_SIZING_"
    "SCOPE_DOES_NOT_SUPPLY_A_SOURCE_KIND_EQ_REMAINS_RECONCILIATION_TARGET_ONLY"
)
U04_EVIDENCE = (
    "SEALED_AR_U04_PLACEMENT_AVAILABLE_FOR_SIZING_OR_RISK_SIZING_PLUS_CK_KIND_SET_"
    "DISPOSITION_NOT_EQUITY_STOCK_AFFECTING_PLUS_T_POLICY_ENTRY_SUBTRACT_IF_NOT_IN_"
    "BASE_PLUS_CK_GET_CANNOT_DISCRIMINATE_IN_BASE_VS_NOT_IN_BASE_EMPTY_PENDING_ROWS_"
    "ARE_NOT_EXCLUDE_HISTORICAL_11_14_ORDERS_PENDING_NOT_UPLIFTABLE"
)
MAPPING_EVIDENCE = (
    "SEALED_BE_PARTIAL_FAIL_CLOSED_NO_RATIFIED_SOURCE_KIND_PLUS_S_NO_CANONICALLY_"
    "VALID_MAPPING_PLUS_AR_EQ_RECONCILIATION_TARGET_ONLY_PLUS_KIND_SET_EMPTY_FAIL_"
    "CLOSED_PLUS_COMPLETE_EVENT_STREAM_UNPROVEN_PLUS_SEMANTIC_MAPPING_UNPROVEN"
)
EXACT_MISSING_PREDICATE = (
    "NO_RATIFIED_EQUITY_STOCK_SOURCE_KIND_KIND_SET_UNRESOLVED_EQ_IS_RECONCILIATION_"
    "TARGET_NOT_SOURCE_COMPLETENESS_UNPROVEN_U04_SIZING_IN_BASE_VS_NOT_IN_BASE_"
    "REMAINS_UNRESOLVED"
)
BLOCKER_ID = (
    "NO_CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_MAPPING_AFTER_U04_PRODUCTIVE_SCOPE_"
    "SPLIT_NO_RATIFIED_SOURCE_KIND"
)
ARCHITECTURE_BLOCKER = (
    "U04_NOT_EQUITY_STOCK_AVAILABLE_FOR_SIZING_IN_BASE_VS_NOT_IN_BASE_UNRESOLVED_"
    "MAPPING_STILL_INVALID_KIND_SET_EMPTY"
)
NEXT_PRODUCTIVE_NODE = "NO_RATIFIED_EQUITY_STOCK_SOURCE_KIND"
NEXT_OWNER_GO = (
    "OWNER_GO_REQUIRED_TO_RATIFY_NON_EQ_EQUITY_STOCK_SOURCE_KIND_OR_COMPLETENESS_"
    "WITHOUT_KIND_SET_UPLIFT_AND_WITHOUT_TREATING_EQ_AS_SOURCE_V1"
)
NEXT_ACTION = (
    "STOP_FIRST_DEFINITIVE_BLOCK_NO_CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_"
    "MAPPING_U04_PRODUCTIVE_SCOPE_SPLIT_NO_GET"
)
CLAIMS_FILE = "claims.json"
SECRET_MARKERS: tuple[str, ...] = (
    "ok-access",
    "api_secret",
    "api-secret",
    "passphrase",
    "secretref://",
)
_FORBIDDEN_INCLUSION_TOKENS = frozenset(
    {
        OUTCOME_INCLUDE,
        OUTCOME_EXCLUDE,
        "INCLUDE",
        "EXCLUDE",
        "IN_BASE",
        "NOT_IN_BASE",
        "true",
        "TRUE",
        "ABSENT",
        "absent",
        "0",
        "zero",
        "NONE",
    }
)
_REPO_ROOT = Path(__file__).resolve().parents[3]


class U04PendingOrderReservationOrAccountEquityMappingError(ValueError):
    """Fail-closed U04/mapping productive-scope violation."""


@dataclass(frozen=True)
class U04PendingOrderReservationOrAccountEquityMappingResultV1:
    genesis_id: str
    persist_as_of: str
    store_root: str
    u04_legacy_status: str
    u04_productive_scope_status: str
    u04_equity_stock_role: str
    u04_available_capital_role: str
    kind_set_resolved: str
    canonically_valid_account_equity_source_mapping: str
    account_equity_source_mapping_status: str
    reconciliation_target_status: str
    first_definitive_block: str
    exact_missing_predicate: str
    authorized_get_count: str
    actual_get_count: str
    post_count: str
    evidence_manifest: str


def _canonical_json(payload: Mapping[str, Any]) -> str:
    return json.dumps(payload, sort_keys=True, separators=(",", ":"), ensure_ascii=True)


def _persist_json(*, path: Path, payload: Mapping[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    tmp.write_text(_canonical_json(payload) + "\n", encoding="utf-8")
    tmp.replace(path)


def _load_json_object(*, path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise U04PendingOrderReservationOrAccountEquityMappingError(f"JSON_NOT_OBJECT:{path.name}")
    return payload


def _require_token(*, field: str, payload: Mapping[str, Any], expected: str) -> None:
    actual = str(payload.get(field) or "")
    if actual != expected:
        raise U04PendingOrderReservationOrAccountEquityMappingError(f"{field}_DRIFT:{actual}")


def reject_unresolved_inclusion_as_in_base_or_not_in_base_v1(*, claimed: str) -> None:
    if claimed in _FORBIDDEN_INCLUSION_TOKENS:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            f"UNRESOLVED_INCLUSION_IS_NOT_IN_BASE_OR_NOT_IN_BASE:{claimed}"
        )


def reject_productive_not_in_kind_set_as_exclude_v1(*, claimed: str) -> None:
    if claimed in {OUTCOME_EXCLUDE, "EXCLUDE", "IN_BASE"}:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            f"PRODUCTIVE_NOT_IN_KIND_SET_IS_NOT_EXCLUDE:{claimed}"
        )


def reject_eq_as_source_authority_v1(*, claimed: str) -> None:
    if claimed in {"true", "TRUE", "SOURCE", "RAW_EQ_SOURCE_AUTHORITY"}:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            f"EQ_IS_RECONCILIATION_TARGET_NOT_SOURCE:{claimed}"
        )


def _assert_standing_pins() -> None:
    if U05_KIND_DECISION != DECISION_REMAIN_UNKNOWN:
        raise U04PendingOrderReservationOrAccountEquityMappingError("U05_UNKNOWN_PRESERVED")
    if PRODUCTIVE_U05_EQUITY_STOCK_KIND_MEMBERSHIP != PRODUCTIVE_MEMBERSHIP:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "PRODUCTIVE_U05_MEMBERSHIP_DRIFT"
        )
    if U06_KIND_DECISION != DECISION_REMAIN_UNKNOWN:
        raise U04PendingOrderReservationOrAccountEquityMappingError("U06_UNKNOWN_PRESERVED")
    if PRODUCTIVE_U06_EQUITY_STOCK_KIND_MEMBERSHIP != PRODUCTIVE_MEMBERSHIP:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "PRODUCTIVE_U06_MEMBERSHIP_DRIFT"
        )
    if RESIDUAL_KIND_DECISION != DECISION_REMAIN_UNKNOWN:
        raise U04PendingOrderReservationOrAccountEquityMappingError("RESIDUAL_UNKNOWN_PRESERVED")
    if PRODUCTIVE_RESIDUAL_EQUITY_STOCK_KIND_MEMBERSHIP != PRODUCTIVE_MEMBERSHIP:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "PRODUCTIVE_RESIDUAL_MEMBERSHIP_DRIFT"
        )
    if U04_LEGACY_STATUS != STATE_UNRESOLVED:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "LEGACY_U04_UNRESOLVED_PRESERVED"
        )
    if U04_LEGACY_ALGEBRA_IN_BASE_VS_NOT_IN_BASE != STATE_UNRESOLVED:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "U04_INCLUSION_MUST_REMAIN_UNRESOLVED"
        )
    if U04_LEGACY_INCLUDE_EXCLUDE_AS_EQUITY_STOCK_KIND_RETIRED_AS_PRODUCTIVE_BLOCKER is not True:
        raise U04PendingOrderReservationOrAccountEquityMappingError("U04_LEGACY_NOT_RETIRED")
    if PRODUCTIVE_U04_EQUITY_STOCK_KIND_MEMBERSHIP != PRODUCTIVE_MEMBERSHIP:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "PRODUCTIVE_U04_MEMBERSHIP_DRIFT"
        )
    if PRODUCTIVE_U04_EQUITY_STOCK_ROLE != DISPOSITION_NOT_EQUITY_STOCK:
        raise U04PendingOrderReservationOrAccountEquityMappingError("U04_STOCK_ROLE_DRIFT")
    if PRODUCTIVE_U04_AVAILABLE_CAPITAL_ROLE != "AVAILABLE_FOR_SIZING_OR_RISK_SIZING":
        raise U04PendingOrderReservationOrAccountEquityMappingError("U04_SIZING_ROLE_DRIFT")
    if U04_KIND_SET_DISPOSITION != DISPOSITION_NOT_EQUITY_STOCK:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "U04_KIND_SET_DISPOSITION_DRIFT"
        )
    if U04_PLACEMENT != "AVAILABLE_FOR_SIZING_OR_RISK_SIZING":
        raise U04PendingOrderReservationOrAccountEquityMappingError("U04_PLACEMENT_DRIFT")
    if U04_PENDING_ORDER_RESERVATIONS != (
        "ENTRY_SUBTRACT_IF_NOT_IN_BASE_REDUCE_ONLY_NO_PRE_FILL_INCREASE"
    ):
        raise U04PendingOrderReservationOrAccountEquityMappingError("U04_POLICY_PIN_DRIFT")
    if CURRENT_PRODUCTIVE_REMAINING_NECESSARY_EQUITY_STOCK_CLASSES != ():
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "CURRENT_PRODUCTIVE_REMAINING_MUST_BE_EMPTY"
        )
    if NAMED_REMAINING_UNKNOWN_NECESSARY_CLASSES != (TARGET_U05, TARGET_U06, TARGET_RESIDUAL):
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "LEGACY_NAMED_REMAINING_MUST_REMAIN"
        )
    if U06_PLACEMENT != "EVENT_OR_RECONCILIATION_NOT_BLIND_SUBTRACTION":
        raise U04PendingOrderReservationOrAccountEquityMappingError("U06_PLACEMENT_DRIFT")
    if EARLIEST_UNRESOLVED_ALGEBRA_TERM != "U04_PENDING_ORDER_RESERVATION_INCLUSION_UNRESOLVED":
        raise U04PendingOrderReservationOrAccountEquityMappingError("U04_ALGEBRA_TERM_DRIFT")
    if KIND_SET_RESOLVED is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError("KIND_SET_RESOLVED_NOT_FALSE")
    if RATIFIED_CLASSIFIED_KIND_SET:
        raise U04PendingOrderReservationOrAccountEquityMappingError("KIND_SET_MUST_REMAIN_EMPTY")
    if CURRENT_PRODUCTIVE_ACCOUNT_EQUITY_SOURCE_MAPPING_CLOSURE_CLOSED is True:
        raise U04PendingOrderReservationOrAccountEquityMappingError("MAPPING_CLOSURE_CONSUMED_REEXECUTE_FORBIDDEN")
    if CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_MAPPING is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError("MAPPING_MUST_REMAIN_INVALID")
    if MAPPING_PROVEN is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError("MAPPING_PROVEN_NOT_FALSE")
    if SEMANTIC_MAPPING_PROVEN is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError("SEMANTIC_MAPPING_NOT_FALSE")
    if RECONSTRUCTION_ALGEBRA_COMPLETE is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "ALGEBRA_MUST_REMAIN_INCOMPLETE"
        )
    if RAW_EQ_SOURCE_AUTHORITY is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "RAW_EQ_SOURCE_AUTHORITY_NOT_FALSE"
        )
    if EQ_RECONCILIATION_TARGET_ONLY is not True:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "EQ_MUST_REMAIN_RECONCILIATION_TARGET"
        )
    if D6_ACCOUNT_EQUITY_SOURCE_MAPPING_COVERAGE_STATUS != (
        "PARTIAL_FAIL_CLOSED_NO_RATIFIED_SOURCE_KIND"
    ):
        raise U04PendingOrderReservationOrAccountEquityMappingError("MAPPING_COVERAGE_DRIFT")
    if ACCOUNT_EQUITY_SOURCE_MAPPING_DENIED_THIS_WORKPACKAGE is not True:
        raise U04PendingOrderReservationOrAccountEquityMappingError("MAPPING_DENY_PIN_DRIFT")
    if AUTHORIZED_PRODUCTIVE_EVENT_SOURCE_SEAM_PRESENT is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "SOURCE_SEAM_MUST_REMAIN_ABSENT"
        )
    if COMPLETE_EVENT_STREAM_PROVEN is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "COMPLETE_STREAM_MUST_REMAIN_UNPROVEN"
        )
    if EVENT_ACQUISITION_NETWORK_GET_AUTHORIZED is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError("EVENT_GET_NOT_AUTHORIZED")
    if OBSERVATION_NETWORK_GET_AUTHORIZED is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "OBSERVATION_GET_NOT_AUTHORIZED"
        )
    if MS2_AUTHORIZED is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError("MS2_AUTHORIZED_NOT_FALSE")
    if D6_FULLY_CLOSED is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError("D6_FULLY_CLOSED_NOT_FALSE")
    if D7_AUTHORIZED is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError("D7_AUTHORIZED_NOT_FALSE")
    if C17_CREATED is not False:
        raise U04PendingOrderReservationOrAccountEquityMappingError("C17_CREATED_NOT_FALSE")
    if ACCOUNT_BILLS_REMAINS_CURRENT_NONCANONICAL is not True:
        raise U04PendingOrderReservationOrAccountEquityMappingError("BILLS_CANONICALIZED_FORBIDDEN")
    if DAG_PIN != "NO_CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_MAPPING":
        raise U04PendingOrderReservationOrAccountEquityMappingError("DAG_PIN_DRIFT")


def _assert_parent_cn_pack(*, sealed_cn_pack: Path) -> None:
    if not sealed_cn_pack.is_dir():
        raise U04PendingOrderReservationOrAccountEquityMappingError("PARENT_CN_PACK_MISSING")
    claims = _load_json_object(path=sealed_cn_pack / CLAIMS_FILE)
    _require_token(field="U04_STATUS", payload=claims, expected=STATE_UNRESOLVED)
    _require_token(field="KIND_SET_RESOLVED", payload=claims, expected=FALSE_TOKEN)
    _require_token(
        field="CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_MAPPING",
        payload=claims,
        expected=FALSE_TOKEN,
    )
    _require_token(field="NEXT_OWNER_GO_REQUIRED", payload=claims, expected=PIN_OWNER_GO)
    if verify_manifest_sha256_v1(store_root=sealed_cn_pack) != 0:
        raise U04PendingOrderReservationOrAccountEquityMappingError("PARENT_CN_MANIFEST_INVALID")


def classify_u04_productive_scope_and_mapping_deny_v1() -> dict[str, str]:
    reject_u04_reclassify_as_equity_stock_kind_v1(claimed=DISPOSITION_NOT_EQUITY_STOCK)
    reject_productive_not_in_kind_set_as_exclude_v1(claimed=PRODUCTIVE_MEMBERSHIP)
    reject_unresolved_inclusion_as_in_base_or_not_in_base_v1(claimed=STATE_UNRESOLVED)
    reject_eq_as_source_authority_v1(claimed=FALSE_TOKEN)
    return {
        "layer": "ADJUDICATED_CONCLUSION",
        "u04_legacy_status": STATE_UNRESOLVED,
        "u04_legacy_algebra_in_base_vs_not_in_base": STATE_UNRESOLVED,
        "u04_policy_status": "DECIDED",
        "u04_policy_pin": U04_PENDING_ORDER_RESERVATIONS,
        "u04_kind_set_disposition": DISPOSITION_NOT_EQUITY_STOCK,
        "u04_include_proven": FALSE_TOKEN,
        "u04_exclude_proven": FALSE_TOKEN,
        "u04_in_base_proven": FALSE_TOKEN,
        "u04_not_in_base_proven": FALSE_TOKEN,
        "u04_productive_scope_status": PRODUCTIVE_MEMBERSHIP,
        "u04_equity_stock_role": DISPOSITION_NOT_EQUITY_STOCK,
        "u04_available_capital_role": "AVAILABLE_FOR_SIZING_OR_RISK_SIZING",
        "u04_proof_law": U04_PROOF_LAW,
        "u04_evidence": U04_EVIDENCE,
        "mapping_denied": TRUE_TOKEN,
        "mapping_deny_law": MAPPING_DENY_LAW,
        "mapping_evidence": MAPPING_EVIDENCE,
        "source_status": "NO_RATIFIED_SOURCE_KIND",
        "semantics_status": "SEMANTIC_MAPPING_UNPROVEN",
        "units_status": "UNBOUND",
        "temporal_status": "UNBOUND",
        "completeness_status": "COMPLETE_EVENT_STREAM_UNPROVEN",
        "consumer_binding_status": "UNBOUND_29P_SEMANTIC_MAPPING_UNPROVEN",
        "reconciliation_target_status": "EQ_RECONCILIATION_TARGET_ONLY",
        "synthetic_witness_used": FALSE_TOKEN,
        "durable_unknown_reopened": FALSE_TOKEN,
    }


def reevaluate_downstream_v1(*, split: Mapping[str, str]) -> dict[str, Any]:
    if split["u04_legacy_status"] != STATE_UNRESOLVED:
        raise U04PendingOrderReservationOrAccountEquityMappingError(
            "U04_STATUS_MUST_REMAIN_UNRESOLVED"
        )
    if split["mapping_denied"] != TRUE_TOKEN:
        raise U04PendingOrderReservationOrAccountEquityMappingError("MAPPING_MUST_REMAIN_DENIED")
    reject_durable_unknown_as_include_or_exclude_v1(claimed="DURABLE_UNKNOWN")
    return {
        "layer": "ADJUDICATED_CONCLUSION",
        "kind_set": KIND_SET_EMPTY,
        "kind_set_resolved": FALSE_TOKEN,
        "legacy_named_remaining_unknown_necessary_classes": ",".join(
            NAMED_REMAINING_UNKNOWN_NECESSARY_CLASSES
        ),
        "current_productive_remaining_necessary_equity_stock_classes": ",".join(
            CURRENT_PRODUCTIVE_REMAINING_NECESSARY_EQUITY_STOCK_CLASSES
        ),
        "canonically_valid_account_equity_source_mapping": FALSE_TOKEN,
        "account_equity_source_mapping_status": DAG_PIN,
        "reconstruction_algebra_complete": FALSE_TOKEN,
        "reconstruction_status": (
            "INCOMPLETE_U04_SIZING_INCLUSION_UNRESOLVED_U05_U06_NOT_CURRENT_PRODUCTIVE_TERMS"
        ),
        "u04_status": STATE_UNRESOLVED,
        "u04_kind_set_disposition": DISPOSITION_NOT_EQUITY_STOCK,
        "u05_legacy_status": DECISION_REMAIN_UNKNOWN,
        "u06_legacy_status": DECISION_REMAIN_UNKNOWN,
        "residual_legacy_status": DECISION_REMAIN_UNKNOWN,
        "equity_stock_readiness": (
            "NOT_READY_KIND_SET_UNRESOLVED_AND_MAPPING_NOT_CANONICALLY_VALID"
        ),
        "running_account_equity_available_for_sizing_status": (
            "UNBOUND_29P_SEMANTIC_MAPPING_UNPROVEN_U04_IN_BASE_VS_NOT_IN_BASE_UNRESOLVED"
        ),
        "risk_sizing_readiness": (
            "NOT_READY_NO_CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_MAPPING_"
            "AND_29P_SEMANTIC_MAPPING_UNPROVEN"
        ),
        "local_advancement_exhausted": TRUE_TOKEN,
        "first_definitive_block": DAG_PIN,
        "exact_missing_predicate": EXACT_MISSING_PREDICATE,
        "next_productive_node": NEXT_PRODUCTIVE_NODE,
        "standing_full_core_dag_pin": DAG_PIN,
        "earliest_unresolved_algebra_term": EARLIEST_UNRESOLVED_ALGEBRA_TERM,
    }


def execute_u04_pending_order_reservation_or_account_equity_mapping_v1(
    *,
    owner_go: str,
    origin_main_sha: str,
    evidence_root: Path | str,
    repo_root: Path | str | None = None,
    sealed_cn_pack: Path | str | None = None,
    persist_as_of: str | None = None,
) -> U04PendingOrderReservationOrAccountEquityMappingResultV1:
    if owner_go != OWNER_GO:
        raise U04PendingOrderReservationOrAccountEquityMappingError("OWNER_GO_MISMATCH")
    if origin_main_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise U04PendingOrderReservationOrAccountEquityMappingError("ORIGIN_MAIN_SHA_MISMATCH")
    _assert_standing_pins()
    reject_hope_get_v1(authorized_get_count="0", actual_get_count="0")
    reject_u04_reclassify_as_equity_stock_kind_v1(claimed=DISPOSITION_NOT_EQUITY_STOCK)
    reject_productive_not_in_kind_set_as_exclude_v1(claimed=PRODUCTIVE_MEMBERSHIP)
    reject_unresolved_inclusion_as_in_base_or_not_in_base_v1(claimed=STATE_UNRESOLVED)
    reject_eq_as_source_authority_v1(claimed=FALSE_TOKEN)
    repo = Path(repo_root) if repo_root is not None else _REPO_ROOT
    cn_pack = (
        Path(sealed_cn_pack) if sealed_cn_pack is not None else repo / CANONICAL_CN_PACK_RELPATH
    )
    _assert_parent_cn_pack(sealed_cn_pack=cn_pack)
    as_of = persist_as_of or CANONICAL_PERSIST_AS_OF
    if as_of is None or not isinstance(as_of, str) or as_of.strip() == "" or as_of != as_of.strip():
        raise U04PendingOrderReservationOrAccountEquityMappingError("FIELD_MISSING:persist_as_of")
    store = Path(evidence_root)
    store.mkdir(parents=True, exist_ok=True)
    split = classify_u04_productive_scope_and_mapping_deny_v1()
    downstream = reevaluate_downstream_v1(split=split)
    claims = {
        "OWNER_GO": OWNER_GO,
        "OWNER_GO_STATUS": "CONSUMED",
        "PIN_OWNER_GO": PIN_OWNER_GO,
        "PIN_OWNER_GO_STATUS": "SATISFIED_BY_U04_PRODUCTIVE_SCOPE_SPLIT_AND_MAPPING_DENY",
        "SCHEMA_CLASS": SCHEMA_CLASS,
        "CONTRACT_VERSION": CONTRACT_VERSION,
        "AUTHORITY_EFFECT": AUTHORITY_EFFECT,
        "GENESIS_ID": EXPECTED_GENESIS_ID,
        "GENESIS_AS_OF": EXPECTED_GENESIS_AS_OF,
        "PERSIST_AS_OF": as_of,
        "PARENT_CN_PACK": CANONICAL_CN_PACK_RELPATH,
        "WORKPACKAGE": SCHEMA_CLASS,
        "START_BLOCK": DAG_PIN,
        "U04_LEGACY_STATUS": STATE_UNRESOLVED,
        "U04_PRODUCTIVE_SCOPE_STATUS": PRODUCTIVE_MEMBERSHIP,
        "U04_EQUITY_STOCK_ROLE": DISPOSITION_NOT_EQUITY_STOCK,
        "U04_AVAILABLE_CAPITAL_ROLE": "AVAILABLE_FOR_SIZING_OR_RISK_SIZING",
        "U04_EVIDENCE": U04_EVIDENCE,
        "U04_PROOF_LAW": U04_PROOF_LAW,
        "U04_POLICY_STATUS": "DECIDED",
        "U04_POLICY_PIN": U04_PENDING_ORDER_RESERVATIONS,
        "U04_KIND_SET_DISPOSITION": DISPOSITION_NOT_EQUITY_STOCK,
        "U04_LEGACY_KIND_DECISION": STATE_UNRESOLVED,
        "U04_LEGACY_ALGEBRA_IN_BASE_VS_NOT_IN_BASE": STATE_UNRESOLVED,
        "U04_LEGACY_INCLUDE_EXCLUDE_AS_EQUITY_STOCK_KIND_RETIRED_AS_PRODUCTIVE_BLOCKER": TRUE_TOKEN,
        "PRODUCTIVE_U04_EQUITY_STOCK_KIND_MEMBERSHIP": PRODUCTIVE_MEMBERSHIP,
        "PRODUCTIVE_U04_EQUITY_STOCK_ROLE": DISPOSITION_NOT_EQUITY_STOCK,
        "PRODUCTIVE_U04_AVAILABLE_CAPITAL_ROLE": "AVAILABLE_FOR_SIZING_OR_RISK_SIZING",
        "U04_INCLUDE_PROVEN": FALSE_TOKEN,
        "U04_EXCLUDE_PROVEN": FALSE_TOKEN,
        "U04_IN_BASE_PROVEN": FALSE_TOKEN,
        "U04_NOT_IN_BASE_PROVEN": FALSE_TOKEN,
        "AUTHORITY_CHANGE": "ADDITIVE_PRODUCTIVE_SCOPE_SPLIT_NO_INCLUDE_OR_EXCLUDE_MAPPING_DENIED",
        "MIGRATION_STRATEGY": (
            "ADDITIVE_VERSIONED_SPLIT_LEGACY_U04_UNRESOLVED_PRESERVED_PRODUCTIVE_NOT_STOCK"
        ),
        "NAMED_REMAINING_UNKNOWN_NECESSARY_CLASSES": ",".join(
            NAMED_REMAINING_UNKNOWN_NECESSARY_CLASSES
        ),
        "CURRENT_PRODUCTIVE_REMAINING_NECESSARY_EQUITY_STOCK_CLASSES": ",".join(
            CURRENT_PRODUCTIVE_REMAINING_NECESSARY_EQUITY_STOCK_CLASSES
        ),
        "U04_STATUS": STATE_UNRESOLVED,
        "EARLIEST_UNRESOLVED_ALGEBRA_TERM": EARLIEST_UNRESOLVED_ALGEBRA_TERM,
        "U05_STATUS": DECISION_REMAIN_UNKNOWN,
        "U05_LEGACY_STATUS": DECISION_REMAIN_UNKNOWN,
        "U06_STATUS": DECISION_REMAIN_UNKNOWN,
        "U06_LEGACY_STATUS": DECISION_REMAIN_UNKNOWN,
        "RESIDUAL_STATUS": DECISION_REMAIN_UNKNOWN,
        "RESIDUAL_LEGACY_STATUS": DECISION_REMAIN_UNKNOWN,
        "DURABLE_UNKNOWN_NOT_INCLUDE": TRUE_TOKEN,
        "DURABLE_UNKNOWN_NOT_EXCLUDE": TRUE_TOKEN,
        "DURABLE_UNKNOWN_NOT_ABSENT_OR_ZERO": TRUE_TOKEN,
        "DURABLE_UNKNOWN_REOPENED": FALSE_TOKEN,
        "SYNTHETIC_WITNESS_USED": FALSE_TOKEN,
        "KIND_SET": KIND_SET_EMPTY,
        "KIND_SET_RESOLVED": FALSE_TOKEN,
        "KIND_SET_STATUS": KIND_SET_EMPTY,
        "ACCOUNT_EQUITY_SOURCE_MAPPING": DAG_PIN,
        "ACCOUNT_EQUITY_SOURCE_MAPPING_STATUS": DAG_PIN,
        "ACCOUNT_EQUITY_SOURCE_MAPPING_EVIDENCE": MAPPING_EVIDENCE,
        "ACCOUNT_EQUITY_SOURCE_MAPPING_DENIED_THIS_WORKPACKAGE": TRUE_TOKEN,
        "CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_MAPPING": FALSE_TOKEN,
        "MAPPING_PROVEN": FALSE_TOKEN,
        "SEMANTIC_MAPPING_PROVEN": FALSE_TOKEN,
        "RECONCILIATION_TARGET_STATUS": "EQ_RECONCILIATION_TARGET_ONLY",
        "RAW_EQ_SOURCE_AUTHORITY": FALSE_TOKEN,
        "EQ_RECONCILIATION_TARGET_ONLY": TRUE_TOKEN,
        "RECONSTRUCTION_ALGEBRA_COMPLETE": FALSE_TOKEN,
        "RECONSTRUCTION_STATUS": downstream["reconstruction_status"],
        "EQUITY_STOCK_READINESS": downstream["equity_stock_readiness"],
        "RUNNING_ACCOUNT_EQUITY_AVAILABLE_FOR_SIZING_STATUS": downstream[
            "running_account_equity_available_for_sizing_status"
        ],
        "RISK_SIZING_READINESS": downstream["risk_sizing_readiness"],
        "NO_GET_REQUIRED": TRUE_TOKEN,
        "NO_EQ_SOURCE_AUTHORITY": TRUE_TOKEN,
        "NO_ALGEBRAIC_UPLIFT": TRUE_TOKEN,
        "NO_HOPE_GET": TRUE_TOKEN,
        "NO_RETROACTIVE_SEALED_EVIDENCE_UPLIFT": TRUE_TOKEN,
        "MAX_GET_COUNT": "0",
        "AUTHORIZED_GET_COUNT": "0",
        "ACTUAL_GET_COUNT": "0",
        "VENUE_GET_COUNT": "0",
        "VENUE_GET_SURFACES": NONE_TOKEN,
        "RETRY_COUNT": "0",
        "POST_COUNT": "0",
        "SECRET_RESOLUTION_STATUS": "NOT_ATTEMPTED",
        "VENUE_EQ_SOURCE_AUTHORITY": FALSE_TOKEN,
        "ACCOUNT_BILLS_CANONICALIZED": FALSE_TOKEN,
        "GATE_A_REOPENED": FALSE_TOKEN,
        "GATE_B_REEXECUTED": FALSE_TOKEN,
        "MS2_AUTHORIZED": FALSE_TOKEN,
        "D6_FULLY_CLOSED": FALSE_TOKEN,
        "D7_AUTHORIZED": FALSE_TOKEN,
        "C17_CREATED": FALSE_TOKEN,
        "PRODUCTIVE_ACQUISITION_AUTHORIZED": FALSE_TOKEN,
        "PRODUCTIVE_ACQUISITION_EXECUTED": FALSE_TOKEN,
        "HOPE_GET_FORBIDDEN": TRUE_TOKEN,
        "LOCAL_ADVANCEMENT_EXHAUSTED": TRUE_TOKEN,
        "FIRST_DEFINITIVE_BLOCK": DAG_PIN,
        "EXACT_MISSING_PREDICATE": EXACT_MISSING_PREDICATE,
        "STANDING_FULL_CORE_DAG_PIN": DAG_PIN,
        "NEXT_PRODUCTIVE_NODE": NEXT_PRODUCTIVE_NODE,
        "BLOCKER_ID": BLOCKER_ID,
        "ARCHITECTURE_BLOCKER": ARCHITECTURE_BLOCKER,
        "PARENT_BLOCKER_ID": PARENT_BLOCKER_ID,
        "NEXT_OWNER_GO_REQUIRED": NEXT_OWNER_GO,
        "NEXT_ACTION": NEXT_ACTION,
        "PROTECTED_SURFACES_UNCHANGED": TRUE_TOKEN,
        "ATLAS_AUTHORITY": NONE_TOKEN,
    }
    lineage = {
        "layer": "HISTORICAL",
        "origin_main_sha": origin_main_sha,
        "owner_go": OWNER_GO,
        "pin_owner_go": PIN_OWNER_GO,
        "genesis_id": EXPECTED_GENESIS_ID,
        "genesis_as_of": EXPECTED_GENESIS_AS_OF,
        "persist_as_of": as_of,
        "parent_cn_pack": CANONICAL_CN_PACK_RELPATH,
        "historical_artifacts_rewritten": FALSE_TOKEN,
        "synthetic_witness_used": FALSE_TOKEN,
        "hope_get_used": FALSE_TOKEN,
        "eq_source_authority_used": FALSE_TOKEN,
        "include_or_exclude_normalized": FALSE_TOKEN,
        "in_base_or_not_in_base_normalized": FALSE_TOKEN,
        "mapping_ratified": FALSE_TOKEN,
        "kind_set_resolved": FALSE_TOKEN,
    }
    protected = {
        "master_v2_unchanged": TRUE_TOKEN,
        "double_play_unchanged": TRUE_TOKEN,
        "bull_bear_state_switch_unchanged": TRUE_TOKEN,
        "self_learning_unchanged": TRUE_TOKEN,
        "top20_unchanged": TRUE_TOKEN,
        "full_core_autonomy_unchanged": TRUE_TOKEN,
        "step_29p_unchanged": TRUE_TOKEN,
        "trading_logic_unchanged": TRUE_TOKEN,
        "u04_not_reclassified_as_equity_stock_kind": TRUE_TOKEN,
        "u04_algebra_inclusion_not_normalized": TRUE_TOKEN,
        "legacy_u04_unresolved_preserved": TRUE_TOKEN,
        "legacy_u05_unknown_preserved": TRUE_TOKEN,
        "legacy_u06_unknown_preserved": TRUE_TOKEN,
        "legacy_residual_unknown_preserved": TRUE_TOKEN,
        "named_remaining_historical_tuple_unchanged": TRUE_TOKEN,
        "eq_not_elevated_to_source": TRUE_TOKEN,
        "option_d_architecture_unchanged": TRUE_TOKEN,
        "gate_a_not_retried": TRUE_TOKEN,
        "gate_b_not_executed": TRUE_TOKEN,
        "bills_authority_unchanged": TRUE_TOKEN,
        "venue_eq_source_authority": FALSE_TOKEN,
        "ms2_authorized": FALSE_TOKEN,
        "reconstruction_implemented": FALSE_TOKEN,
        "dag_pin": DAG_PIN,
    }
    architecture = {
        "layer": "CANONICAL_AUTHORITY",
        "blocker_id": BLOCKER_ID,
        "architecture_blocker": ARCHITECTURE_BLOCKER,
        "parent_blocker_id": PARENT_BLOCKER_ID,
        "kind_set_resolved": FALSE_TOKEN,
        "canonically_valid_account_equity_source_mapping": FALSE_TOKEN,
        "u04_productive_scope_status": PRODUCTIVE_MEMBERSHIP,
        "u04_equity_stock_role": DISPOSITION_NOT_EQUITY_STOCK,
        "u04_available_capital_role": "AVAILABLE_FOR_SIZING_OR_RISK_SIZING",
        "u04_status": STATE_UNRESOLVED,
        "first_definitive_block": DAG_PIN,
        "exact_missing_predicate": EXACT_MISSING_PREDICATE,
        "next_productive_node": NEXT_PRODUCTIVE_NODE,
        "next_owner_go_required": NEXT_OWNER_GO,
        "next_action": NEXT_ACTION,
        "atlas_authority": NONE_TOKEN,
    }
    layers = {
        "CANONICAL_AUTHORITY": "architecture_blocker_v1.json",
        "FORENSIC_RAW_EVIDENCE": "NONE_NEW_RAW_BYTES_SEALED_AR_T_CK_BE_CN_CONSUMED",
        "ADJUDICATED_CONCLUSION": "qualification_v1.json,downstream_dependency_tree_v1.json",
        "HISTORICAL": "LINEAGE.json",
        "NAVIGATION": "FORBIDDEN_AS_AUTHORITY",
        "INTERPRETATION": "FORBIDDEN",
        "HYPOTHESIS": "FORBIDDEN",
        "UNRESOLVED_OR_CONTRADICTORY": BLOCKER_ID,
        "MIGRATION_RATIFICATION": "claims.json",
        "CURRENT_PRODUCTIVE_MAPPING_STATUS": DAG_PIN,
    }
    _persist_json(path=store / CLAIMS_FILE, payload=claims)
    _persist_json(path=store / "qualification_v1.json", payload=split)
    _persist_json(path=store / "downstream_dependency_tree_v1.json", payload=downstream)
    _persist_json(path=store / "architecture_blocker_v1.json", payload=architecture)
    _persist_json(path=store / "protected_surfaces_v1.json", payload=protected)
    _persist_json(path=store / "layers_v1.json", payload=layers)
    _persist_json(path=store / "LINEAGE.json", payload=lineage)
    persist_manifest_sha256_v1(store_root=store)
    joined = "\n".join(path.read_text(encoding="utf-8") for path in store.glob("*.json"))
    lowered = joined.lower()
    if any(marker in lowered for marker in SECRET_MARKERS):
        raise U04PendingOrderReservationOrAccountEquityMappingError("SECRET_MARKER_PERSISTED")
    if verify_manifest_sha256_v1(store_root=store) != 0:
        raise U04PendingOrderReservationOrAccountEquityMappingError("MANIFEST_VERIFY_NOT_ZERO")
    return U04PendingOrderReservationOrAccountEquityMappingResultV1(
        genesis_id=EXPECTED_GENESIS_ID,
        persist_as_of=as_of,
        store_root=str(store),
        u04_legacy_status=STATE_UNRESOLVED,
        u04_productive_scope_status=PRODUCTIVE_MEMBERSHIP,
        u04_equity_stock_role=DISPOSITION_NOT_EQUITY_STOCK,
        u04_available_capital_role="AVAILABLE_FOR_SIZING_OR_RISK_SIZING",
        kind_set_resolved=FALSE_TOKEN,
        canonically_valid_account_equity_source_mapping=FALSE_TOKEN,
        account_equity_source_mapping_status=DAG_PIN,
        reconciliation_target_status="EQ_RECONCILIATION_TARGET_ONLY",
        first_definitive_block=DAG_PIN,
        exact_missing_predicate=EXACT_MISSING_PREDICATE,
        authorized_get_count="0",
        actual_get_count="0",
        post_count="0",
        evidence_manifest=str(store / "MANIFEST.sha256"),
    )


__all__ = (
    "ARCHITECTURE_BLOCKER",
    "BLOCKER_ID",
    "CANONICAL_PACK_RELPATH",
    "CANONICAL_PERSIST_AS_OF",
    "EXACT_MISSING_PREDICATE",
    "EXPECTED_ORIGIN_MAIN_SHA",
    "MAPPING_DENY_LAW",
    "MAPPING_EVIDENCE",
    "NEXT_ACTION",
    "NEXT_OWNER_GO",
    "NEXT_PRODUCTIVE_NODE",
    "OWNER_GO",
    "PARENT_BLOCKER_ID",
    "PIN_OWNER_GO",
    "U04PendingOrderReservationOrAccountEquityMappingError",
    "U04_EVIDENCE",
    "classify_u04_productive_scope_and_mapping_deny_v1",
    "execute_u04_pending_order_reservation_or_account_equity_mapping_v1",
    "reject_eq_as_source_authority_v1",
    "reject_productive_not_in_kind_set_as_exclude_v1",
    "reject_unresolved_inclusion_as_in_base_or_not_in_base_v1",
)
