"""CURRENT_PRODUCTIVE fresh runtime cycle to pre-external-effect applicability.

Consumes Owner-GO
OWNER_GO_CURRENT_PRODUCTIVE_FRESH_RUNTIME_TO_PRE_EXTERNAL_EFFECT_APPLICABILITY_V1.

Re-proves occupancy/pending/config via READ-ONLY GETs, restores the current
SideState/confirmation cursor when schema/lineage/instrument match, binds
Cap-23/24 through the current Cap-2.1–2.4 producers, and runs one current
Master-V2 cycle through the canonical host. Does not reuse DQ/DR/DS packs as
current market authority. Does not fabricate ENTER. Does not POST. Does not
create or consume a live permit.
...
_OCCUPANCY_DECISION_ID = "dt-occupancy-reproof-pre-external-effect-v1"
MAX_GET_REQUEST_COUNT = 24


class CurrentProductiveFreshRuntimeToPreExternalEffectError(
    CurrentProductiveFreshCap23Cap24ReadinessError
):
    """Fail-closed pre-external-effect runtime-cycle boundary violation."""


@dataclass(frozen=True)
class CurrentProductiveFreshRuntimeToPreExternalEffectResultV1:
    store_root: str
...
def _assert_dt_pins() -> None:
    _assert_standing_pins()
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_V3_CREATED is not True:
        raise CurrentProductiveFreshRuntimeToPreExternalEffectError("DQ_ADAPTER_NOT_CREATED")
    if CURRENT_PRODUCTIVE_ONESHOT_SIDESTATE_CONFIRMATION_CURSOR_JOIN_CREATED is not True:
        raise CurrentProductiveFreshRuntimeToPreExternalEffectError("DR_ADAPTER_NOT_CREATED")
    if CURRENT_PRODUCTIVE_HOST_ENTER_29P_INVALID_STOP_PRICE_REPAIR_CREATED is not True:
        raise CurrentProductiveFreshRuntimeToPreExternalEffectError("DS_ADAPTER_NOT_CREATED")
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_TO_PRE_EXTERNAL_EFFECT_APPLICABILITY_CREATED is not True:
        raise CurrentProductiveFreshRuntimeToPreExternalEffectError("DT_ADAPTER_NOT_CREATED")
...
def execute_current_productive_fresh_runtime_to_pre_external_effect_applicability_v1(
    *,
    owner_go: str,
    origin_main_sha: str,
...
) -> CurrentProductiveFreshRuntimeToPreExternalEffectResultV1:
    if owner_go != OWNER_GO:
        raise CurrentProductiveFreshRuntimeToPreExternalEffectError("OWNER_GO_MISMATCH")
    if origin_main_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise CurrentProductiveFreshRuntimeToPreExternalEffectError("ORIGIN_MAIN_SHA_MISMATCH")
    if execute_network is not True and acquisition_result is None and acquisition_transport is None:
        raise CurrentProductiveFreshRuntimeToPreExternalEffectError(
    ...
    if occupancy_blocker:
        ...
        if acquisition_result is None:
            try:
                ...
                raise CurrentProductiveFreshRuntimeToPreExternalEffectError(str(exc)) from exc
            ...
            if missing and not market_blocker:
                ...
                if not market_blocker:
                    try:
                        ...
                        if cycle_result.outgoing_cursor is not None:
                            cursor_state_after = _cursor_identity_v1(cycle_result.outgoing_cursor)
                            confirmation_progress = _confirmation_progress_v1(
    ...
    if market_blocker:
        hold_class = "C_TECHNICAL_BLOCKER"
    elif cycle_result is None and not occupancy_blocker:
        hold_class = "C_TECHNICAL_BLOCKER"
    elif decision_result == "EXECUTABLE_VENUE_PLAN_BOUND":
        hold_class = ""
    elif cursor_persisted == TRUE_TOKEN and cursor_state_before != cursor_state_after:
        hold_class = "B_CONFIRMATION_STATE_ADVANCED_AND_PERSISTED"
    else:
        hold_class = "A_HOLD_MISSING_CURRENT_TRADING_CONDITION"
    ...
    send_handle = FullCoreSendCredentialHandleV1(
        handle_id="full-core-dt-readiness-handle", bound=True
    )
    ...
    try:
        http_transport.post_trade_order(
            ...
            permit_id="eep-dt-must-not-post",
            envelope_id="env-dt-must-not-post",
        ...
        raise CurrentProductiveFreshRuntimeToPreExternalEffectError("HTTP_MUST_NOT_POST")
    if http_transport.post_count != 0:
        raise CurrentProductiveFreshRuntimeToPreExternalEffectError("HTTP_TRANSPORT_SIDE_EFFECT")
    ...
    if plan is not None and envelope_readiness == TRUE_TOKEN:
        envelope = bind_final_order_envelope_from_venue_plan_v1(
            plan,
            admission_ref="DT_FRESH_CAP24_AND_CURRENT_PRODUCTIVE_MASTER_V2",
    ...
    return CurrentProductiveFreshRuntimeToPreExternalEffectResultV1(
...
def main() -> int:
    ...
    result = execute_current_productive_fresh_runtime_to_pre_external_effect_applicability_v1(
