"""CURRENT_PRODUCTIVE fresh runtime cycle after non-executable DO decision.
...
Master-V2 cycle. Does not reuse DN/DO facts as current authority. Does not
fabricate ENTER. Does not POST. Does not create or consume a live permit.
...
from src.ops.governed_productive_account_equity_authority_producer_v1.constants_v1 import (
    CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_CREATED,
...
EXPECTED_ORIGIN_MAIN_SHA = "0f3eb713c2facf2c0d3b311f52bbd6e3e97869f2"
CANONICAL_PACK_RELPATH = (
    "evidence/ops/full_core_current_productive_fresh_runtime_cycle_after_"
    "non_executable_decision_v2/20260916T011500Z"
)
FALSE_TOKEN = "false"
TRUE_TOKEN = "true"
...
DO_PACK = (
    "evidence/ops/full_core_current_productive_fresh_runtime_cycle_after_"
    "non_executable_decision_v1/20260916T010000Z"
)
_REPO_ROOT = Path(__file__).resolve().parents[3]
_OCCUPANCY_DECISION_ID = "dp-occupancy-reproof-after-non-executable-v2"
MAX_GET_REQUEST_COUNT = 24


class CurrentProductiveFreshRuntimeCycleAfterNonExecutableV2Error(
    CurrentProductiveFreshCap23Cap24ReadinessError
):
    """Fail-closed post-non-executable V2 runtime-cycle boundary violation."""


@dataclass(frozen=True)
class CurrentProductiveFreshRuntimeCycleAfterNonExecutableResultV2:
    store_root: str
...
def _assert_dp_pins() -> None:
    _assert_standing_pins()
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_CREATED is not True:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV2Error("DO_ADAPTER_NOT_CREATED")
    if CURRENT_PRODUCTIVE_FRESH_RUNTIME_CYCLE_AFTER_NON_EXECUTABLE_DECISION_V2_CREATED is not True:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV2Error("DP_ADAPTER_NOT_CREATED")
...
def execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v2(
    *,
    owner_go: str,
    origin_main_sha: str,
...
) -> CurrentProductiveFreshRuntimeCycleAfterNonExecutableResultV2:
    if owner_go != OWNER_GO:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV2Error("OWNER_GO_MISMATCH")
    if origin_main_sha != EXPECTED_ORIGIN_MAIN_SHA:
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV2Error(
    ...
    if occupancy_blocker:
        ...
        if acquisition_result is None:
            try:
                ...
                raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV2Error(str(exc)) from exc
            ...
            if missing and not market_blocker:
                ...
                try:
                    cycle_result = run_current_productive_master_v2_runtime_cycle_v1(
                        ...
                        cycle_id=f"dp-{native_id}-{package_started}",
                        observed_unix=observed_unix,
    ...
    if bound is not None and not occupancy_blocker and not market_blocker:
        status, reasons, plan = try_bind_current_productive_venue_plan_v1(
            replay=cycle_replay,
            bound_instrument=bound,
            session_id="current-productive-dp-session",
            run_id="current-productive-dp-run",
    ...
    send_handle = FullCoreSendCredentialHandleV1(
        handle_id="full-core-dp-readiness-handle", bound=True
    )
    ...
    try:
        http_transport.post_trade_order(
            ...
            permit_id="eep-dp-must-not-post",
            envelope_id="env-dp-must-not-post",
        ...
        raise CurrentProductiveFreshRuntimeCycleAfterNonExecutableV2Error("HTTP_MUST_NOT_POST")
    ...
    if plan is not None and envelope_readiness == TRUE_TOKEN:
        envelope = bind_final_order_envelope_from_venue_plan_v1(
            plan,
            admission_ref="DP_FRESH_CAP24_AND_CURRENT_PRODUCTIVE_MASTER_V2",
    ...
    return CurrentProductiveFreshRuntimeCycleAfterNonExecutableResultV2(
...
def main() -> int:
    ...
    result = execute_current_productive_fresh_runtime_cycle_after_non_executable_decision_v2(
