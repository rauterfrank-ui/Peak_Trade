"""Minimum governed private GETs for §11.14 LIVE_FEE_OBSERVED.

Exactly two GETs on eea.okx.com, identity-scoped to the bound Live submit:
1. GET /api/v5/trade/order?instId=<bound>&ordId=<bound> (supporting only)
2. GET /api/v5/trade/fills?instType=FUTURES&instId=<bound>&ordId=<bound>
   (authoritative fee-row source)

Does not POST, cancel, amend, retry, or second-submit. Secrets are never
logged, printed, or persisted. Does not infer fee from a static rate or
fillPx times fillSz. Historical fill evidence is not substituted.
"""
...
from src.ops.section_11_14_live_order_and_economic_evidence_ladder_v1.fill_observed_gets_v1 import (
    FILL_GET_MAX_REQUEST_COUNT,
    FILL_GET_MAX_RETRIES,
    FILL_GET_METHOD,
    FILL_GET_REST_BASE,
    FILL_GET_REST_HOST,
...
FEE_GET_REST_HOST = FILL_GET_REST_HOST
FEE_GET_REST_BASE = FILL_GET_REST_BASE
FEE_GET_METHOD = FILL_GET_METHOD
FEE_GET_MAX_REQUEST_COUNT = FILL_GET_MAX_REQUEST_COUNT
FEE_GET_MAX_RETRIES = FILL_GET_MAX_RETRIES
FEE_GET_TIMEOUT_SECONDS = FILL_GET_TIMEOUT_SECONDS


def bind_fee_observed_gets_before_request_v1() -> dict[str, Any]:
    return {
        "WHY_GET_REQUIRED": (
            "LIVE_FEE_OBSERVED requires a current venue-native fee bound to the "
            "observed Peak_Trade Live fill/submit identity. GET /api/v5/trade/fills "
            "scoped to the bound ordId/instId is the fee-row source. GET "
            "/api/v5/trade/order scoped to the same identity is supporting "
            "contradiction context only. Static rates and fillPx*fillSz are not fee."
        ),
        "REQUIRED_FACT": "CURRENT_VENUE_FEE_BOUND_TO_FILL_IDENTITY",
        "ENDPOINTS": [bound_order_get_endpoint_v1(), bound_fills_get_endpoint_v1()],
        "ENDPOINT_PATHS": [ENDPOINT_ORDER_GET, ENDPOINT_TRADE_FILLS],
        "METHOD": FEE_GET_METHOD,
        "EXPECTED_MUTATION": "NONE",
        "MAXIMUM_REQUEST_COUNT": FEE_GET_MAX_REQUEST_COUNT,
        "RETRY": False,
        "PUBLIC_GET": False,
        "PRIVATE_GET": True,
        "POST": False,
...
def execute_fee_observed_gets_v1(
    *,
    owner_go: str,
    origin_main_sha: str,
    ...
    binding = bind_fee_observed_gets_before_request_v1()
    ...
    client = LiveCanaryHttpClientV1(
        rest_base=FEE_GET_REST_BASE,
        rest_host=FEE_GET_REST_HOST,
        transport=live_transport,
        max_request_count=FEE_GET_MAX_REQUEST_COUNT,
        max_retries=FEE_GET_MAX_RETRIES,
        timeout_seconds=FEE_GET_TIMEOUT_SECONDS,
    )
    planned = (
    ...
    try:
        for role, expected_path, endpoint, allowlist in planned:
            ...
            try:
                if productive:
                    url = f"{FEE_GET_REST_BASE}{endpoint}"
                    parsed = urlparse(url)
                    if parsed.hostname != FEE_GET_REST_HOST:
        ...
        if client.counters.request_count > FEE_GET_MAX_REQUEST_COUNT:
            raise Section1114OfflineSurfaceError("MAX_REQUEST_COUNT_EXCEEDED")
        if "POST" in client.counters.methods_used:
            raise Section1114OfflineSurfaceError("POST_INVOKED_BY_FEE_OBSERVATION")
