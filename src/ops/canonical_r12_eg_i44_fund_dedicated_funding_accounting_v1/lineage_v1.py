"""Deterministic lineage helpers for R12 EG-I44 funding accounting v1."""
...
def load_layer_config_v1(root: Path | None = None) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise R12EgI44FundError(f"contract_config_missing:{path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise R12EgI44FundError("contract_config_not_object")
    return payload
