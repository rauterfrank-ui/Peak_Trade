"""Deterministic lineage helpers for R6 S3 runtime architecture v1."""
...
def load_layer_config_v1(root: Path | None = None) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise R6S3RuntimeArchitectureError(f"contract_config_missing:{path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise R6S3RuntimeArchitectureError("contract_config_not_object")
    return payload
