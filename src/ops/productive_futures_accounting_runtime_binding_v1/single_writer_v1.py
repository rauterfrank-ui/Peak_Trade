"""Exclusive single-writer authority for productive futures accounting state."""
...
class ConflictingWriterError(RuntimeError):
    """Raised when another productive accounting writer holds the lock."""

    def __init__(self, detail: str = "") -> None:
        self.code = AccountingFailureCodeV1.CONFLICTING_WRITER
        super().__init__(f"{self.code.value}:{detail}" if detail else self.code.value)


@dataclass
class ProductiveFuturesAccountingSingleWriterV1:
    state_root: Path
