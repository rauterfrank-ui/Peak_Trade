"""Hidden-PTY confirm-token handoff + digest/ID-only validation for Step-5 execution."""
...
def acquire_confirm_token_via_hidden_pty_v1(
    *,
    ...
    """Acquire plaintext only via injected getpass (hidden PTY/stdin). Never persist."""
    blockers: list[str] = []
...
def validate_confirm_token_binding_v1(
    *,
    ...
    # Digest of token plaintext must match binding when provided as raw sha256 of token
    # OR as binding sha that includes session scope. Accept exact equality of
    # fingerprint(token) to binding OR sha256(token) to binding for test fixtures.
    ...
    if binding and token and binding not in {fp, hashlib.sha256(token.encode()).hexdigest()}:
        # Also accept precomputed binding sha as opaque expected digest equality check
        # via explicit compare of expected_binding_sha256 to itself when tests inject
        # matching values — mismatch when fp/raw don't match and caller didn't equalize.
        # Treat as digest mismatch unless binding equals fp (canonical fingerprint path).
        if binding != fp:
