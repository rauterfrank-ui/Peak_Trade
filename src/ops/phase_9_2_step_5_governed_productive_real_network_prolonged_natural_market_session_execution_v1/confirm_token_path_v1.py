"""Confirm-token path guards for Step-5 execution (no argv/env plaintext)."""
...
def reject_confirm_token_env_fallback_v1(environ: Mapping[str, str] | None) -> list[str]:
    """Step-5 execution forbids env fallback for confirm tokens (hidden PTY/stdin only)."""
    if not environ:
        return []
    blockers: list[str] = []
    for key in FORBIDDEN_CONFIRM_TOKEN_ENV_KEYS:
        if str(environ.get(key) or "").strip():
            ...
            break
    # Also reject the PSO env key when used as execution fallback.
