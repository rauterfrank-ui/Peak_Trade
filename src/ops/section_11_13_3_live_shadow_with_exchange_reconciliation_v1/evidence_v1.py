"""Evidence generator + manifest helpers for §11.13.3."""
...
class LiveShadowReconEvidenceError(RuntimeError):
...
def verify_manifest_v1(root: Path) -> dict[str, Any]:
    ...
    if not manifest.is_file():
        raise LiveShadowReconEvidenceError("MANIFEST_MISSING")
    errors: list[str] = []
    ...
    if errors:
        raise LiveShadowReconEvidenceError(";".join(errors))
