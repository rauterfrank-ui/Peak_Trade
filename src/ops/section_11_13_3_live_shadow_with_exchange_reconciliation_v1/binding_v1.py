"""Fail-closed LIVE venue binding for §11.13.3 (strictly separated from Demo/Testnet)."""
...
class LiveShadowReconBindingError(RuntimeError):
    """Fail-closed Live shadow reconciliation binding violation."""


@dataclass(frozen=True)
class LiveShadowReconVenueBindingV1:
    environment: str
    venue: str
    entity: str
    region: str
...
def normalize_rest_host(rest_base_or_host: str) -> str:
    ...
    if not raw:
        raise LiveShadowReconBindingError("REST_HOST_REQUIRED")
    if "://" in raw:
        parsed = urlparse(raw)
    ...
    if not host:
        raise LiveShadowReconBindingError("REST_HOST_UNPARSEABLE")
    return host
...
def _assert_not_forbidden_host(host: str) -> None:
    for marker in FORBIDDEN_HOST_MARKERS:
        if marker in host:
            raise LiveShadowReconBindingError(f"FORBIDDEN_NON_LIVE_HOST:{host}")


def _assert_environment(environment: str) -> None:
    env = str(environment or "").strip().upper()
    if env != REQUIRED_ENVIRONMENT:
        raise LiveShadowReconBindingError(f"ENVIRONMENT_MUST_BE_LIVE:{env or '<empty>'}")
    if env in FORBIDDEN_ENVIRONMENTS:
        raise LiveShadowReconBindingError(f"FORBIDDEN_ENVIRONMENT:{env}")
...
def reject_cross_binding_v1(
    *,
    ...
    if live_env != REQUIRED_ENVIRONMENT:
        raise LiveShadowReconBindingError("CROSS_BINDING_LIVE_SIDE_INVALID")
    if peer_env in FORBIDDEN_ENVIRONMENTS or peer_env in {"DEMO", "TESTNET"}:
        raise LiveShadowReconBindingError(f"CROSS_BINDING_LIVE_DEMO_TESTNET_REJECT:{peer_env}")
    ...
    if any(m in peer_klass for m in ("DEMO", "TESTNET", "SIMULATED", "PAPER")):
        raise LiveShadowReconBindingError(
...
def build_live_shadow_recon_venue_binding_v1(
    *,
    environment: str,
    venue: str,
    entity: str,
    ...
    if not venue_s:
        raise LiveShadowReconBindingError("VENUE_REQUIRED")
    if not entity_s:
        raise LiveShadowReconBindingError("ENTITY_REQUIRED")
    if not region_s:
        raise LiveShadowReconBindingError("REGION_REQUIRED")
    if not account_s:
        raise LiveShadowReconBindingError("ACCOUNT_SCOPE_REQUIRED")
    if OWNER_SUPPLIED_LIVE_HOST_REQUIRED and not str(rest_host or "").strip():
        raise LiveShadowReconBindingError("OWNER_SUPPLIED_LIVE_HOST_REQUIRED")
    if HARDCODED_PRODUCTION_HOST:
        # Preparation package intentionally leaves this empty.
        raise LiveShadowReconBindingError("HARDCODED_PRODUCTION_HOST_MUST_REMAIN_EMPTY")
    ...
    if base_host != host:
        raise LiveShadowReconBindingError("REST_BASE_HOST_MISMATCH")

    allowlist = tuple(normalize_rest_host(h) for h in (owner_declared_host_allowlist or (host,)))
    if not allowlist:
        raise LiveShadowReconBindingError("LIVE_HOST_ALLOWLIST_EMPTY")
    if host not in allowlist:
        raise LiveShadowReconBindingError(f"LIVE_HOST_NOT_IN_ALLOWLIST:{host}")
    for allowed in allowlist:
        _assert_not_forbidden_host(allowed)

    return LiveShadowReconVenueBindingV1(
...
def validate_binding_against_payload_v1(
    ...
    if env and env != REQUIRED_ENVIRONMENT:
        raise LiveShadowReconBindingError(f"PAYLOAD_ENVIRONMENT_REJECT:{env}")
    host = str(payload.get("rest_host", "")).strip().lower()
    if host and normalize_rest_host(host) != binding.rest_host:
        raise LiveShadowReconBindingError("PAYLOAD_HOST_MISMATCH")
    if bool(payload.get("order_capability")):
        raise LiveShadowReconBindingError("ORDER_CAPABILITY_FORBIDDEN_IN_LIVE_SHADOW_RECON_BINDING")
