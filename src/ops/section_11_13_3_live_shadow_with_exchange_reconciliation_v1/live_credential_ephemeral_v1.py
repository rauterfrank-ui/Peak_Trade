"""LIVE-scoped ephemeral SecretRef credential borrow/release for §11.13.3.
...
from src.ops.section_11_13_3_live_shadow_with_exchange_reconciliation_v1.secretref_v1 import (
    ...
    validate_live_shadow_recon_secretref_uri_v1,
)


class LiveShadowReconCredentialError(RuntimeError):
    """Fail-closed LIVE shadow-recon credential borrow/release violation."""
...
def build_file_secretref_vault_backend_v1(*, vault_file: Path | str) -> FileSecretRefVaultBackendV1:
    ...
    if not path.is_file():
        raise LiveShadowReconCredentialError("VAULT_FILE_MISSING")
...
def parse_okx_live_ro_material_v1(material: str) -> dict[str, str]:
    try:
        ...
        raise LiveShadowReconCredentialError("CREDENTIAL_MATERIAL_NOT_JSON") from exc
    if not isinstance(payload, dict):
        raise LiveShadowReconCredentialError("CREDENTIAL_MATERIAL_NOT_OBJECT")
    key = str(payload.get("api_key") or "").strip()
    secret = str(payload.get("api_secret") or "").strip()
    passphrase = str(payload.get("passphrase") or "").strip()
    if not key or not secret or not passphrase:
        raise LiveShadowReconCredentialError("CREDENTIAL_FIELDS_INCOMPLETE")
...
def resolve_and_load_live_secretref_ephemeral_v1(
    *,
    ...
    if str(runtime_mode or "").strip().upper() != REQUIRED_ENVIRONMENT:
        raise LiveShadowReconCredentialError("SECRETREF_SCOPE_MUST_BE_LIVE")
    if str(credential_class or "").strip() != REQUIRED_CREDENTIAL_CLASS:
        raise LiveShadowReconCredentialError("LIVE_SHADOW_RECON_CREDENTIAL_CLASS_REQUIRED")
    try:
        ref = validate_live_shadow_recon_secretref_uri_v1(secret_reference)
    except LiveShadowReconSecretRefError as exc:
        raise LiveShadowReconCredentialError(str(exc)) from exc
    if vault_backend is None:
        raise LiveShadowReconCredentialError("REAL_VAULT_BACKEND_REQUIRED")
    try:
        ...
        raise LiveShadowReconCredentialError(f"VAULT_RESOLVE_FAILED:{type(exc).__name__}") from exc
    if material.startswith("plaintext:") or "\nAuthorization:" in material:
        raise LiveShadowReconCredentialError("VAULT_MATERIAL_SHAPE_FORBIDDEN")
...
def borrow_live_ephemeral_material_for_session_auth_v1(
    ...
    if material is None:
        raise LiveShadowReconCredentialError("EPHEMERAL_MATERIAL_GONE")
    return material
...
def assert_no_plaintext_in_payload_v1(payload: Any) -> None:
    ...
    for needle in ("sk-", "plaintext:", "authorization: bearer"):
        if needle in text:
            raise LiveShadowReconCredentialError(f"PLAINTEXT_LEAK_DETECTED:{needle}")
            ...
            if kl in {"api_secret", "passphrase", "api_key", "secret", "ok-access-key"}:
                if isinstance(value, str) and value and value not in {"<REDACTED>", "<REF_ONLY>"}:
                    if not value.startswith("secretref-digest:") and not value.startswith("<"):
                        raise LiveShadowReconCredentialError(f"PLAINTEXT_LEAK_DETECTED:{kl}")
