"""Scoped authorization gate for §11.13.3 LIVE_SHADOW_WITH_EXCHANGE_RECONCILIATION.
...
class LiveShadowReconAuthorizationError(RuntimeError):
...
class LiveShadowReconAuthorizationBindingV1:
    live_shadow_with_exchange_reconciliation_authorized: bool
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "LIVE_SHADOW_WITH_EXCHANGE_RECONCILIATION_AUTHORIZED": self.live_shadow_with_exchange_reconciliation_authorized,
...
def default_authorization_is_false_v1() -> bool:
    return LIVE_SHADOW_WITH_EXCHANGE_RECONCILIATION_AUTHORIZED_DEFAULT is False
...
def validate_live_shadow_with_exchange_reconciliation_authorization_v1(
    *,
    owner_go: str | None,
...
) -> LiveShadowReconAuthorizationBindingV1:
    """Validate scoped GO. Missing/wrong GO → no credential load, no network."""
    if live_shadow_with_exchange_reconciliation_authorized is None:
        authorized_flag = False
    else:
        authorized_flag = bool(live_shadow_with_exchange_reconciliation_authorized)

    if not authorized_flag:
        raise LiveShadowReconAuthorizationError(
            "LIVE_SHADOW_WITH_EXCHANGE_RECONCILIATION_AUTHORIZED_FALSE"
        )

    token = str(owner_go or "").strip()
    if not token:
        raise LiveShadowReconAuthorizationError("OWNER_GO_MISSING")
    if token != OWNER_GO_EXECUTE:
        raise LiveShadowReconAuthorizationError(f"OWNER_GO_MISMATCH:{token}")

    scope = str(authorization_scope or "").strip()
    if not scope:
        raise LiveShadowReconAuthorizationError("AUTHORIZATION_SCOPE_MISSING")
    if scope != AUTHORIZATION_SCOPE:
        raise LiveShadowReconAuthorizationError(f"AUTHORIZATION_SCOPE_MISMATCH:{scope}")
    if scope in AUTHORIZATION_SCOPE_ALIASES_FORBIDDEN:
        raise LiveShadowReconAuthorizationError(f"AUTHORIZATION_SCOPE_FORBIDDEN:{scope}")
    ...
    if not sha or not _SHA_RE.match(sha):
        raise LiveShadowReconAuthorizationError("BOUND_ORIGIN_MAIN_SHA_INVALID")
    if not expected_sha or not _SHA_RE.match(expected_sha):
        raise LiveShadowReconAuthorizationError("EXPECTED_ORIGIN_MAIN_SHA_INVALID")
    if sha != expected_sha:
        raise LiveShadowReconAuthorizationError("ORIGIN_MAIN_SHA_MISMATCH")

    cfg = str(bound_config_digest or "").strip().lower()
    expected_cfg = str(expected_config_digest or "").strip().lower()
    if not cfg or len(cfg) != 64:
        raise LiveShadowReconAuthorizationError("BOUND_CONFIG_DIGEST_INVALID")
    if not expected_cfg or len(expected_cfg) != 64:
        raise LiveShadowReconAuthorizationError("EXPECTED_CONFIG_DIGEST_INVALID")
    if cfg != expected_cfg:
        raise LiveShadowReconAuthorizationError("CONFIG_DIGEST_MISMATCH")
    ...
    if LIVE_AUTHORIZED or LIVE_ENABLED or LIVE_ARMED or LIVE_ORDER_AUTHORIZED:
        raise LiveShadowReconAuthorizationError("STANDING_LIVE_GATES_MUST_REMAIN_FALSE")

    return LiveShadowReconAuthorizationBindingV1(
...
def authorization_binding_digest_v1(binding: LiveShadowReconAuthorizationBindingV1) -> str:
