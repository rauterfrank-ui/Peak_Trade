...
def reconcile(
    ...
    # balances
    assets = set(expected_balances.balances.keys()) | set(
        observed_balances.balances.keys()
    )
    ...
    if expected_positions is not None and observed_positions is not None:
        syms = set(expected_positions.positions.keys()) | set(
            observed_positions.positions.keys()
        )
