"""Offline evidence persist for Route-C net-mode posSide first-party contract census."""
...
class Z2DQPersistError(RuntimeError):
    """Fail-closed evidence persist violation."""


def persist_route_c_net_mode_posside_first_party_evidence_v1(
    *,
    pack: Path,
    origin_main_sha: str,
    ...
    if str(origin_main_sha or "").strip() != EXPECTED_ORIGIN_MAIN_SHA:
        raise Z2DQPersistError("ORIGIN_MAIN_SHA_MISMATCH")
    ...
    if int(verified.get("MANIFEST_VERIFY_RC", 1)) != 0:
        raise Z2DQPersistError("MANIFEST_VERIFY_FAILED")
    return verified
