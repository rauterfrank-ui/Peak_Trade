"""Evidence persistence for Cap 2.4 runtime binding."""
...
from src.ops.single_selected_future_runtime_binding_v1.constants_v1 import (
    BINDING_RESULT_FILENAME,
    EVIDENCE_FILENAME,
    MANIFEST_FILENAME,
    STAGING_DIRNAME_PREFIX,
)
from src.ops.single_selected_future_runtime_binding_v1.models_v1 import sha256_hex
...
def persist_binding_evidence_atomic_v1(
    *,
    evidence_root: Path,
    ...
    try:
        ...
        result_text = json.dumps(dict(result), sort_keys=True, indent=2) + "\n"
        (staging / EVIDENCE_FILENAME).write_text(evidence_text, encoding="utf-8")
        (staging / BINDING_RESULT_FILENAME).write_text(result_text, encoding="utf-8")
        write_manifest(staging, (EVIDENCE_FILENAME, BINDING_RESULT_FILENAME))
        for name in (EVIDENCE_FILENAME, BINDING_RESULT_FILENAME, MANIFEST_FILENAME):
            src = staging / name
