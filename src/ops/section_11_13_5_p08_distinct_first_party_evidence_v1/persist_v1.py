"""Sanitized plus first-party raw evidence persist for P08 distinct evidence."""
...
_RAW_FILENAMES = {
    GET_ROLE_TARGET_HISTORY: "GET_TARGET_HISTORY.raw.json",
    GET_ROLE_TYPED_HISTORY: "GET_TYPED_HISTORY.raw.json",
    GET_ROLE_HISTORY_PAGINATION: "GET_TARGET_HISTORY_AFTER.raw.json",
    GET_ROLE_ACCOUNT_POSITION_RISK: "GET_ACCOUNT_POSITION_RISK.raw.json",
    GET_ROLE_POSID_POSITIONS: "GET_POSID_POSITIONS.raw.json",
}


class P08DistinctFirstPartyPersistError(RuntimeError):
...
def assert_no_secrets_in_payload_v1(payload: Mapping[str, Any]) -> None:
    def _walk(value: Any, key: str = "") -> None:
        ...
        if key_l in _SECRET_VALUE_KEYS and text and text not in {"<REDACTED>", "<REF_ONLY>"}:
            raise P08DistinctFirstPartyPersistError("SECRET_IN_EVIDENCE")
...
def persist_p08_distinct_first_party_evidence_v1(
    *,
    pack: Path,
    origin_main_sha: str,
    ...
    if str(origin_main_sha or "").strip() != EXPECTED_ORIGIN_MAIN_SHA:
        raise P08DistinctFirstPartyPersistError("ORIGIN_MAIN_SHA_MISMATCH")
    ...
    for raw in raw_exchanges:
        ...
        if name is None:
            raise P08DistinctFirstPartyPersistError("RAW_GET_ROLE_INVALID")
        if name in used_names:
            raise P08DistinctFirstPartyPersistError("RAW_GET_ROLE_DUPLICATE")
        used_names.add(name)
    ...
    if int(verified.get("MANIFEST_VERIFY_RC", 1)) != 0:
        raise P08DistinctFirstPartyPersistError("MANIFEST_VERIFY_FAILED")
    return verified
