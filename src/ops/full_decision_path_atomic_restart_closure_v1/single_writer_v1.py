"""Exclusive single-writer fencing for the Cap 6.4 coordinator root."""
...
class ConflictingWriterError(RuntimeError):
    def __init__(self, detail: str = "") -> None:
        self.code = DecisionPathAtomicFailureCodeV1.CONFLICTING_WRITER
        super().__init__(f"{self.code.value}:{detail}" if detail else self.code.value)


@dataclass
class DecisionPathAtomicSingleWriterV1:
    state_root: Path
    ...
    def assert_held(self) -> None:
        if not self._held:
            raise ConflictingWriterError(DecisionPathAtomicFailureCodeV1.WRITER_LOCK_NOT_HELD.value)
