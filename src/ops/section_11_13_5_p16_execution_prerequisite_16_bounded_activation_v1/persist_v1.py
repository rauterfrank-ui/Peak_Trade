"""Sanitized offline evidence persist for the P16 EXECUTION_PREREQUISITE_16 pack."""
...
class P16BoundedActivationPersistError(RuntimeError):
...
def assert_no_secrets_in_payload_v1(payload: Mapping[str, Any]) -> None:
    def _walk(value: Any, key: str = "") -> None:
        ...
        if key_l in _SECRET_VALUE_KEYS and text and text not in {"<REDACTED>", "<REF_ONLY>"}:
            raise P16BoundedActivationPersistError("SECRET_IN_EVIDENCE")
        lowered = text.lower()
...
def persist_p16_bounded_activation_evidence_v1(
    *,
    pack: Path,
    origin_main_sha: str,
    ...
    if str(origin_main_sha or "").strip() != EXPECTED_ORIGIN_MAIN_SHA:
        raise P16BoundedActivationPersistError("ORIGIN_MAIN_SHA_MISMATCH")
    if (
        adjudication.get(
        ...
        raise P16BoundedActivationPersistError("PREREQUISITE_16_MUST_BE_PASS_OFFLINE_CONTRACT")
    if adjudication.get("PREREQUISITE_16_BOUNDED_RUNTIME_ACTIVATION_PROVEN") is True:
        raise P16BoundedActivationPersistError("RUNTIME_ACTIVATION_MUST_REMAIN_UNPROVEN")
    if adjudication.get("POST_PERFORMED") is True or summary.get("POST_PERFORMED") is True:
        raise P16BoundedActivationPersistError("FORBIDDEN_POST_CLAIM")
    if int(adjudication.get("THIS_GO_GET_COUNT") or 0) != 0:
        raise P16BoundedActivationPersistError("FORBIDDEN_NEW_GET_CLAIM")
    if adjudication.get("PRIVATE_AUTH_USED") is True:
        raise P16BoundedActivationPersistError("FORBIDDEN_PRIVATE_AUTH_CLAIM")
    if adjudication.get("LIVE_AUTHORIZED") is True:
        raise P16BoundedActivationPersistError("FORBIDDEN_LIVE_AUTHORIZED_TRUE")
    documents = {
        "CENSUS.json": census,
    ...
    for payload in documents.values():
        ...
        if payload.get("LIVE_EXECUTION") is True:
            raise P16BoundedActivationPersistError("FORBIDDEN_LIVE_EXECUTION_CLAIM")
    ...
    if int(verified.get("MANIFEST_VERIFY_RC", 1)) != 0:
        raise P16BoundedActivationPersistError("MANIFEST_VERIFY_FAILED")
    return verified
