...
def validate_settings(settings: Settings) -> Settings:
    ...
    if risk.risk_per_trade > 0.02:
        ...
        import logging
        logging.warning(
            f"⚠️  risk_per_trade={risk.risk_per_trade:.1%} ist hoch! "
            "Empfohlen: <= 2%"
        )
    ...
    if risk.max_position_size > 0.5:
        import logging
        logging.warning(
            f"⚠️  max_position_size={risk.max_position_size:.0%} ist hoch! "
            "Empfohlen: <= 50%"
        )
...
def get_strategy_cfg(name: str) -> dict[str, Any]:
    ...
    if name not in cfg.strategy:
        ...
        raise KeyError(
            f"Strategy '{name}' nicht in config.toml definiert. "
