...
class RateLimiter:
    ...
    def __init__(self, max_requests: int = 100, window_seconds: float = 60.0, name: str = "global"):
        ...
        self.bucket = TokenBucket(capacity=max_requests, refill_rate=self.refill_rate, name=name)
    ...
    def add_endpoint(
        ...
        with self.endpoints_lock:
            self.endpoints[endpoint] = TokenBucket(
                capacity=max_req, refill_rate=refill, name=f"{self.name}:{endpoint}"
            )
    ...
    def get_stats(self, endpoint: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self.stats_lock:
            stats = {
                ...
                "rejection_rate": (
                    self.rejected_requests / self.total_requests if self.total_requests > 0 else 0.0
                ),
                "last_rejection_time": (
    ...
    def reset(self) -> None:
        ...
        self.bucket = TokenBucket(
            capacity=self.max_requests, refill_rate=self.refill_rate, name=self.name
        )
