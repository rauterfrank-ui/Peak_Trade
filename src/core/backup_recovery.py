...
class StateSnapshot:
    ...
    def capture(self) -> Dict[str, Any]:
        ...
        """
        state = {"timestamp": get_utc_now().isoformat(), "providers": {}}
...
class RecoveryManager:
    ...
    def restore_backup(
        ...
        if metadata.status != BackupStatus.SUCCESS:
            logger.error(f"Cannot restore backup {backup_id} with status {metadata.status.value}")
            return False
    ...
    def list_backups(
        self, tags: Optional[List[str]] = None, backup_type: Optional[BackupType] = None
    ) -> List[BackupMetadata]:
        """
        List available backups.
