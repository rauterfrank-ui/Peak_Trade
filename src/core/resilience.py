...
class CircuitBreaker:
    ...
    def call(self, func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if self._state == CircuitState.OPEN:
                raise Exception(f"CircuitBreaker '{self.name}' is OPEN, call rejected")
            ...
            try:
                ...
                self._on_failure()
                logger.error(f"CircuitBreaker '{self.name}' caught exception: {e}", exc_info=True)
                raise
        ...
        for name, check_func in checks_snapshot.items():
            try:
                ...
                results[name] = HealthCheckResult(
                    name=name, healthy=healthy, message=message, details=details
                )
                ...
                results[name] = HealthCheckResult(
                    name=name, healthy=False, message=f"Check failed with exception: {str(e)}"
                )
