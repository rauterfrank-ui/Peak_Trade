...
class MetricsCollector:
    ...
    def __init__(self, namespace: str = "peak_trade"):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            logger.info(f"MetricsCollector initialized with Prometheus support (namespace: {namespace})")
        else:
            self.registry = None
    ...
    def record_circuit_breaker_state_change(
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            self.circuit_breaker_state.labels(name=name).set(
                state_values.get(to_state.lower(), 0)
            )
        ...
        logger.info(
            f"Metric: Circuit breaker '{name}' state change: {from_state} -> {to_state}"
        )
    ...
    def record_rate_limit_hit(self, limiter: str, endpoint: str = "") -> None:
        ...
        self._store_snapshot(
            "rate_limit_hit",
            1.0,
            {"limiter": limiter, "endpoint": endpoint}
        )
    ...
    def record_operation_failure(self, operation: str, error_type: str) -> None:
        ...
        """
        if PROMETHEUS_AVAILABLE:
            self.operation_failures.labels(
                operation=operation,
