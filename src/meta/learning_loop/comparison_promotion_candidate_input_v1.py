"""Offline LEVEL_3 neutral comparison promotion candidate input from verified cross-domain lineage."""
...
CONTRACT_NAME = "comparison_promotion_candidate_input_v1"
CONTRACT_VERSION = "v1"
PRODUCER_VERSION = "comparison_promotion_candidate_input_v1"
EVIDENCE_LEVEL = "LEVEL_3"
AUTHORITY_LEVEL = "NON_AUTHORITIZING_EVIDENCE_ONLY"
RECORD_KIND = "comparison_promotion_candidate_input_record"
INPUT_RELATION = "AGGREGATES_VERIFIED_CROSS_DOMAIN_LINEAGE_BINDING"
ARTIFACT_REL = "comparison_promotion_candidate_input_v1.json"
SELF_VERIFICATION_REL = "SELF_VERIFICATION.json"
MANIFEST_FILENAME = "MANIFEST.sha256"
STAGING_DIRNAME_PREFIX = ".comparison_promotion_candidate_input_staging_"

OFFLINE_DETERMINISTIC_CREATED_AT = "1970-01-01T00:00:00+00:00"

_VALID_CANDIDATE_INPUT_STATUS = frozenset({"PASS", "FAIL", "INCOMPLETE", "NOT_EVALUABLE"})
...
CANDIDATE_INPUT_AUTHORITY_INVARIANTS: dict[str, bool] = {
    **COMPARISON_AUTHORITY_INVARIANTS,
    "candidate_input_is_descriptive_only": True,
    "candidate_input_does_not_select": True,
    "candidate_input_does_not_choose_winner": True,
    "candidate_input_does_not_accept": True,
    "candidate_input_does_not_construct_promotion_candidate": True,
    "candidate_input_does_not_execute_operational_filter": True,
    "candidate_input_does_not_execute_policy": True,
    "candidate_input_does_not_create_configpatch": True,
    "candidate_input_does_not_modify_configpatch": True,
    "candidate_input_does_not_apply_configpatch": True,
    "candidate_input_does_not_modify_config": True,
    "candidate_input_does_not_authorize_promotion": True,
    "candidate_input_does_not_authorize_runtime": True,
    "candidate_input_does_not_authorize_live": True,
    "candidate_input_does_not_deploy": True,
    "candidate_input_does_not_activate": True,
    "candidate_input_does_not_create_order_intent": True,
    "candidate_input_does_not_modify_trading_logic": True,
    "evidence_does_not_authorize_promotion": True,
}

_REQUIRED_NON_AUTHORIZING_FLAGS: dict[str, bool] = {
    "is_comparison_promotion_candidate_input": True,
    "evidence_does_not_authorize_promotion": True,
    "evidence_does_not_authorize_runtime": True,
    "candidate_input_does_not_select": True,
    "candidate_input_does_not_choose_winner": True,
    "candidate_input_does_not_accept_candidate": True,
    "candidate_input_does_not_accept_configpatch": True,
    "candidate_input_does_not_construct_promotion_candidate": True,
    "candidate_input_does_not_execute_operational_filter": True,
    "candidate_input_does_not_execute_policy": True,
    "candidate_input_does_not_create_configpatch": True,
...
_SELF_VERIFICATION_SCHEMA_VERSION = "comparison_promotion_candidate_input_self_verification_v1"


class ComparisonPromotionCandidateInputError(ValueError):
    """Fail-closed neutral candidate input error."""


@dataclass(frozen=True)
class VerifiedCrossDomainLineageBindingBundle:
    bundle_dir: Path
    contract_name: str
...
@dataclass(frozen=True)
class ResolvedTransitiveLineage:
    model_parameter_binding_digest: str
    eligibility_evidence_ref: str
    eligibility_evidence_digest: str
    promotion_input_binding_ref: str
    promotion_input_binding_digest: str


@dataclass(frozen=True)
class ComparisonPromotionCandidateInputInputs:
    cross_domain_lineage_binding_bundle_dir: Path


@dataclass(frozen=True)
class ComparisonPromotionCandidateInputResult:
    output_dir: Path
...
def _reject_symlink(path: Path, *, label: str) -> None:
    if path.is_symlink():
        raise ComparisonPromotionCandidateInputError(f"{label} must not be a symlink")


def _reject_forbidden_index_keys(payload: Mapping[str, Any]) -> None:
    for key in payload:
        if key in _FORBIDDEN_INDEX_KEYS:
            raise ComparisonPromotionCandidateInputError(f"forbidden index key: {key}")


def _validate_regular_file(path: Path, *, label: str) -> None:
    _reject_symlink(path, label=label)
    if not path.is_file():
        raise ComparisonPromotionCandidateInputError(f"{label} must be a regular file: {path}")


def _validate_bundle_dir(bundle_dir: Path, *, label: str) -> None:
    _reject_symlink(bundle_dir, label=label)
    if not bundle_dir.is_dir():
        raise ComparisonPromotionCandidateInputError(f"{label} must be a directory: {bundle_dir}")


def _validate_output_target(output_dir: Path) -> None:
    if output_dir.exists():
        raise ComparisonPromotionCandidateInputError(
            f"output directory already exists: {output_dir}"
        )
    if is_under_tmp(output_dir):
        raise ComparisonPromotionCandidateInputError("output directory must not be under /tmp")
...
def _reject_unsafe_overlap(*, cross_domain_binding_dir: Path, output_dir: Path) -> None:
    binding_res = cross_domain_binding_dir.resolve()
    output_res = output_dir.resolve()
    if output_res == binding_res:
        raise ComparisonPromotionCandidateInputError("output directory must not equal input path")
    if _path_is_under(output_res, binding_res):
        raise ComparisonPromotionCandidateInputError(
            "output directory must not be inside input path"
        )
    if binding_res.is_dir() and _path_is_under(binding_res, output_res):
        raise ComparisonPromotionCandidateInputError(
...
def _validate_capabilities(capabilities: Any) -> list[str]:
    ...
    if not isinstance(capabilities, list):
        raise ComparisonPromotionCandidateInputError("capabilities must be a list")
    normalized: list[str] = []
    for item in capabilities:
        if not isinstance(item, str):
            raise ComparisonPromotionCandidateInputError("capabilities entries must be strings")
        if item in _FORBIDDEN_CAPABILITIES:
            raise ComparisonPromotionCandidateInputError(f"forbidden capability: {item}")
...
def _validate_non_authorizing_flags(payload: Mapping[str, Any]) -> None:
    for key, expected in _REQUIRED_NON_AUTHORIZING_FLAGS.items():
        if payload.get(key) is not expected:
            raise ComparisonPromotionCandidateInputError(f"{key} must be {expected!r}")
...
def _load_self_verification(bundle_dir: Path, *, rel: str, label: str) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ComparisonPromotionCandidateInputError(f"{label} must be a JSON object")
    return payload


def verify_cross_domain_lineage_binding_bundle(
    bundle_dir: Path | str,
) -> VerifiedCrossDomainLineageBindingBundle:
    """Fail-closed verification of exactly one Step-2 cross-domain lineage binding bundle."""
    path = Path(bundle_dir)
    _validate_bundle_dir(path, label="cross-domain lineage binding bundle")
    ok, msg = verify_manifest_sha256(path)
    if not ok:
        raise ComparisonPromotionCandidateInputError(
            f"cross-domain lineage binding MANIFEST.sha256 verification failed: {msg}"
        )
    ...
    if payload.get("contract_name") != UPSTREAM_CONTRACT_NAME:
        raise ComparisonPromotionCandidateInputError(
            "upstream cross-domain lineage binding contract_name mismatch"
        )
    if payload.get("contract_version") != UPSTREAM_CONTRACT_VERSION:
        raise ComparisonPromotionCandidateInputError(
            "upstream cross-domain lineage binding contract_version mismatch"
        )
    ...
    if self_payload.get("overall_status") != "PASS":
        raise ComparisonPromotionCandidateInputError(
            "upstream cross-domain lineage binding SELF_VERIFICATION overall_status must be PASS"
        )

    try:
        reverify_comparison_config_patch_manifest_cross_domain_lineage_binding_v1(output_dir=path)
    except ComparisonConfigPatchManifestCrossDomainLineageBindingError as exc:
        raise ComparisonPromotionCandidateInputError(str(exc)) from exc

    integrity = payload.get("integrity")
    if not isinstance(integrity, Mapping):
        raise ComparisonPromotionCandidateInputError(
            "cross-domain lineage binding integrity must be an object"
        )
    ...
    if not isinstance(artifact_digest, str) or not is_valid_sha256_hex(artifact_digest):
        raise ComparisonPromotionCandidateInputError(
            "cross-domain lineage binding integrity.content_sha256 invalid"
        )

    return VerifiedCrossDomainLineageBindingBundle(
...
def _resolve_transitive_lineage(
    step2: Mapping[str, Any],
) -> tuple[ResolvedTransitiveLineage | None, list[str]]:
    reason_codes: list[str] = []
    binding_ref = step2.get("model_parameter_identity_binding_bundle_ref")
    if not _non_empty_ref(binding_ref):
        reason_codes.append("MISSING_CROSS_DOMAIN_LINEAGE_BINDING")
        return None, reason_codes

    try:
        model_parameter_binding = verify_model_parameter_identity_binding_bundle(
            Path(str(binding_ref))
        )
    except Exception:
        reason_codes.append("MODEL_LINEAGE_MISMATCH")
        return None, reason_codes

    expected_digest = step2.get("model_parameter_identity_binding_digest")
    if expected_digest != model_parameter_binding.artifact_digest:
        reason_codes.append("MODEL_LINEAGE_MISMATCH")

    step1 = model_parameter_binding.evidence_payload
    eligibility_ref = step1.get("eligibility_evidence_bundle_ref")
    eligibility_digest = step1.get("eligibility_evidence_digest")
    if not _non_empty_ref(eligibility_ref) or not is_valid_sha256_hex(
        str(eligibility_digest or "")
    ):
        reason_codes.append("ELIGIBILITY_EVIDENCE_NOT_VERIFIED")
        return None, reason_codes
    ...
    if eligibility_digest != eligibility.artifact_digest:
        reason_codes.append("ELIGIBILITY_DIGEST_MISMATCH")

    eligibility_payload = eligibility.evidence_payload
    promotion_ref = str(eligibility_payload.get("promotion_input_binding_ref", ""))
    promotion_digest = str(eligibility_payload.get("promotion_input_binding_digest", ""))
    if not _non_empty_ref(promotion_ref) or not is_valid_sha256_hex(promotion_digest):
        reason_codes.append("ELIGIBILITY_LINEAGE_MISMATCH")

    return (
        ResolvedTransitiveLineage(
            model_parameter_binding_digest=model_parameter_binding.artifact_digest,
            eligibility_evidence_ref=str(eligibility_ref),
            eligibility_evidence_digest=str(eligibility_digest),
            promotion_input_binding_ref=promotion_ref,
            promotion_input_binding_digest=promotion_digest,
        ),
        reason_codes,
    )


def _evaluate_candidate_input(
    *,
    step2: Mapping[str, Any],
    transitive: ResolvedTransitiveLineage | None,
    resolution_reason_codes: list[str],
) -> tuple[str, str, str, str, str, str, str, list[str]]:
    reason_codes = list(resolution_reason_codes)

    if step2.get("candidate_selection_status") != _CANDIDATE_SELECTION_NOT_SELECTED:
        reason_codes.append("CANDIDATE_SELECTION_DETECTED")
    if step2.get("winner_selection_status") != _WINNER_SELECTION_NOT_SELECTED:
        reason_codes.append("WINNER_SELECTION_DETECTED")
    if step2.get("candidate_acceptance_status") != _CANDIDATE_ACCEPTANCE_NOT_ACCEPTED:
        reason_codes.append("CANDIDATE_ACCEPTANCE_DETECTED")
    if step2.get("configpatch_accepted") is not False:
        reason_codes.append("CONFIG_PATCH_ACCEPTANCE_DETECTED")
    if step2.get("configpatch_created") is not False:
        reason_codes.append("CONFIGPATCH_CREATION_DETECTED")
    if step2.get("configpatch_modified") is not False:
        reason_codes.append("CONFIGPATCH_MUTATION_DETECTED")
    if step2.get("configpatch_applied") is not False:
        reason_codes.append("CONFIGPATCH_APPLICATION_DETECTED")
    if step2.get("promotion_candidate_constructed") is not False:
        reason_codes.append("PROMOTION_CANDIDATE_CONSTRUCTION_DETECTED")
    if step2.get("operational_filter_executed") is not False:
        reason_codes.append("OPERATIONAL_FILTER_DETECTED")
    if step2.get("promotion_policy_executed") is not False:
        reason_codes.append("PROMOTION_POLICY_DETECTED")

    upstream_status = str(step2.get("cross_domain_lineage_binding_status", ""))
    ...
    if upstream_status == "INCOMPLETE":
        return (
            ...
            _ELIGIBILITY_NOT_VERIFIED,
            _IDENTITY_NOT_BOUND,
            _sorted_reason_codes(reason_codes + ["UPSTREAM_CROSS_DOMAIN_LINEAGE_INCOMPLETE"]),
        )
    if upstream_status == "FAIL":
        return (
            ...
            _ELIGIBILITY_NOT_VERIFIED,
            _IDENTITY_NOT_BOUND,
            _sorted_reason_codes(reason_codes + ["UPSTREAM_CROSS_DOMAIN_LINEAGE_FAIL"]),
        )

    candidate_ref = str(step2.get("candidate_identity_ref", ""))
    candidate_digest = str(step2.get("candidate_identity_digest", ""))
    candidate_lineage_digest = str(step2.get("candidate_lineage_digest", ""))
    ...
    if not is_valid_sha256_hex(candidate_digest):
        ...
        is_valid_sha256_hex(candidate_lineage_digest)
        and candidate_digest != candidate_lineage_digest
    ):
        reason_codes.append("CANDIDATE_DIGEST_MISMATCH")

    model_ref = str(step2.get("model_identity_ref", ""))
    model_digest = str(step2.get("model_identity_digest", ""))
    if not _non_empty_ref(model_ref) or not is_valid_sha256_hex(model_digest):
        ...
        reason_codes.append("MODEL_DIGEST_MISMATCH")

    parameter_ref = str(step2.get("parameter_set_identity_ref", ""))
    parameter_digest = str(step2.get("parameter_set_identity_digest", ""))
    if not _non_empty_ref(parameter_ref) or not is_valid_sha256_hex(parameter_digest):
        ...
        reason_codes.append("PARAMETER_SET_DIGEST_MISMATCH")

    config_patch_id = str(step2.get("config_patch_manifest_id", ""))
    config_patch_digest = str(step2.get("config_patch_manifest_digest", ""))
    config_patch_ref = str(step2.get("config_patch_manifest_ref", ""))
    if not _non_empty_ref(config_patch_id):
        reason_codes.append("CONFIG_PATCH_MANIFEST_IDENTITY_NOT_BOUND")
    if not is_valid_sha256_hex(config_patch_digest):
        reason_codes.append("CONFIG_PATCH_MANIFEST_DIGEST_MISMATCH")
    elif _non_empty_ref(config_patch_ref):
        config_path = Path(config_patch_ref)
        if config_path.is_file():
            try:
                loaded = load_config_patch_manifest_v1_from_json_path(config_path)
                if (
                    loaded.integrity is not None
                    and loaded.integrity.content_sha256 != config_patch_digest
                ):
                    reason_codes.append("CONFIG_PATCH_MANIFEST_DIGEST_MISMATCH")
            except ConfigPatchManifestError:
                reason_codes.append("CONFIG_PATCH_MANIFEST_DIGEST_MISMATCH")

    experiment_ref = str(step2.get("experiment_identity_ref", ""))
    experiment_digest = str(step2.get("experiment_identity_digest", ""))
    ...
    dataset_ref = str(step2.get("dataset_identity_ref", ""))
    dataset_digest = str(step2.get("dataset_identity_digest", ""))
    if not _non_empty_ref(dataset_ref) or not is_valid_sha256_hex(dataset_digest):
        reason_codes.append("DATASET_LINEAGE_MISMATCH")

    comparison_ref = str(step2.get("comparison_identity_ref", ""))
    comparison_digest = str(step2.get("comparison_identity_digest", ""))
    ...
    completion_ref = str(step2.get("comparison_completion_ref", ""))
    completion_digest = str(step2.get("comparison_completion_digest", ""))
    ...
    research_ref = str(step2.get("research_validity_ref", ""))
    research_digest = str(step2.get("research_validity_digest", ""))
    ...
    if step2.get("cross_domain_lineage_status") != _IDENTITY_BOUND:
        reason_codes.append("MISSING_CROSS_DOMAIN_LINEAGE_BINDING")
    if step2.get("candidate_identity_status") != _IDENTITY_BOUND:
        reason_codes.append("CANDIDATE_LINEAGE_MISMATCH")
    if step2.get("model_identity_status") != _IDENTITY_BOUND:
        reason_codes.append("MODEL_LINEAGE_MISMATCH")
    if step2.get("parameter_set_identity_status") != _IDENTITY_BOUND:
        reason_codes.append("PARAMETER_SET_LINEAGE_MISMATCH")
    if step2.get("config_patch_manifest_identity_status") != _IDENTITY_BOUND:
        reason_codes.append("CONFIG_PATCH_LINEAGE_MISMATCH")

    if transitive is None:
        reason_codes.append("ELIGIBILITY_EVIDENCE_NOT_VERIFIED")
    elif transitive.model_parameter_binding_digest != step2.get(
    ...
    fail_codes = {
        ...
        "CONFIG_PATCH_MANIFEST_IDENTITY_NOT_BOUND",
        ...
        "MISSING_CROSS_DOMAIN_LINEAGE_BINDING",
        "ELIGIBILITY_EVIDENCE_NOT_VERIFIED",
        "ELIGIBILITY_DIGEST_MISMATCH",
        "ELIGIBILITY_LINEAGE_MISMATCH",
    }
    ...
    eligibility_status = _ELIGIBILITY_VERIFIED
    ...
    if any(code in reason_codes for code in fail_codes):
        ...
        if (
            ...
            candidate_status = _IDENTITY_NOT_BOUND
        if "MODEL_IDENTITY_NOT_BOUND" in reason_codes or "MODEL_LINEAGE_MISMATCH" in reason_codes:
            model_status = _IDENTITY_NOT_BOUND
        if (
            "PARAMETER_SET_IDENTITY_NOT_BOUND" in reason_codes
            or "PARAMETER_SET_LINEAGE_MISMATCH" in reason_codes
        ):
            parameter_status = _IDENTITY_NOT_BOUND
        if (
            "CONFIG_PATCH_MANIFEST_IDENTITY_NOT_BOUND" in reason_codes
            or "CONFIG_PATCH_LINEAGE_MISMATCH" in reason_codes
        ):
            config_patch_status = _IDENTITY_NOT_BOUND
        if (
            "ELIGIBILITY_EVIDENCE_NOT_VERIFIED" in reason_codes
            or "ELIGIBILITY_DIGEST_MISMATCH" in reason_codes
        ):
            eligibility_status = _ELIGIBILITY_NOT_VERIFIED
        if "MISSING_CROSS_DOMAIN_LINEAGE_BINDING" in reason_codes:
    ...
    if input_status == "PASS":
        reason_codes.extend(
            [
                ...
                "COMPARISON_PROMOTION_CANDIDATE_INPUT_COMPLETE",
            ]
        )
...
def _input_artifact_ref_mapping(
    *, bundle: VerifiedCrossDomainLineageBindingBundle
) -> dict[str, Any]:
    return {
...
def build_comparison_promotion_candidate_input_v1(
    *,
    cross_domain_binding: VerifiedCrossDomainLineageBindingBundle,
) -> dict[str, Any]:
    step2 = cross_domain_binding.evidence_payload
    transitive, resolution_codes = _resolve_transitive_lineage(step2)
    (
        input_status,
        ...
        reason_codes,
    ) = _evaluate_candidate_input(
        step2=step2,
        transitive=transitive,
        resolution_reason_codes=resolution_codes,
    )

    input_refs = [_input_artifact_ref_mapping(bundle=cross_domain_binding)]
    parent_artifact_refs = list(step2.get("parent_artifact_refs", []))
    ...
    payload: dict[str, Any] = {
        ...
        "candidate_input_does_not_create_order_intent": True,
        "candidate_input_does_not_modify_trading_logic": True,
        "candidate_input_authority_invariants": dict(CANDIDATE_INPUT_AUTHORITY_INVARIANTS),
        "cross_domain_lineage_binding_bundle_ref": cross_domain_binding.bundle_dir.as_posix(),
        "cross_domain_lineage_binding_artifact_ref": cross_domain_binding.artifact_ref,
        "cross_domain_lineage_binding_digest": cross_domain_binding.artifact_digest,
        "cross_domain_lineage_binding_manifest_digest": cross_domain_binding.manifest_digest,
        "upstream_contract_name": cross_domain_binding.contract_name,
        "upstream_contract_version": cross_domain_binding.contract_version,
        "upstream_producer_version": cross_domain_binding.producer_version,
        "upstream_cross_domain_lineage_binding_status": str(
            step2.get("cross_domain_lineage_binding_status", "")
        ),
        "candidate_input_status": input_status,
        "candidate_input_reason_codes": reason_codes,
        "candidate_identity_ref": str(step2.get("candidate_identity_ref", "")),
        "candidate_identity_digest": str(step2.get("candidate_identity_digest", "")),
        ...
        "experiment_identity_ref": str(step2.get("experiment_identity_ref", "")),
        "experiment_identity_digest": str(step2.get("experiment_identity_digest", "")),
        "experiment_identity_id": str(step2.get("experiment_identity_id", "")),
        "dataset_identity_ref": str(step2.get("dataset_identity_ref", "")),
        "dataset_identity_digest": str(step2.get("dataset_identity_digest", "")),
        "comparison_identity_ref": str(step2.get("comparison_identity_ref", "")),
        "comparison_identity_digest": str(step2.get("comparison_identity_digest", "")),
        "comparison_definition_id": str(step2.get("comparison_definition_id", "")),
        "comparison_completion_ref": str(step2.get("comparison_completion_ref", "")),
        "comparison_completion_digest": str(step2.get("comparison_completion_digest", "")),
        "research_validity_ref": str(step2.get("research_validity_ref", "")),
        "research_validity_digest": str(step2.get("research_validity_digest", "")),
        "comparison_checkpoint_ref": str(step2.get("comparison_checkpoint_ref", "")),
        "comparison_checkpoint_digest": str(step2.get("comparison_checkpoint_digest", "")),
        ...
        "eligibility_evidence_status": eligibility_evidence_status,
        "cross_domain_lineage_status": cross_domain_lineage_status,
        "config_patch_manifest_ref": str(step2.get("config_patch_manifest_ref", "")),
        "config_patch_manifest_digest": str(step2.get("config_patch_manifest_digest", "")),
        "config_patch_manifest_id": str(step2.get("config_patch_manifest_id", "")),
        "config_patch_contract_name": str(step2.get("config_patch_contract_name", "")),
        "config_patch_contract_version": str(step2.get("config_patch_contract_version", "")),
        "model_parameter_identity_binding_bundle_ref": str(
            step2.get("model_parameter_identity_binding_bundle_ref", "")
        ),
        "model_parameter_identity_binding_digest": str(
            step2.get("model_parameter_identity_binding_digest", "")
        ),
    ...
    for field in (
        ...
        value = step2.get(field)
        if value is not None:
            payload[field] = str(value)

    if transitive is not None:
        payload["eligibility_evidence_ref"] = transitive.eligibility_evidence_ref
        payload["eligibility_evidence_digest"] = transitive.eligibility_evidence_digest
        payload["promotion_input_binding_ref"] = transitive.promotion_input_binding_ref
        payload["promotion_input_binding_digest"] = transitive.promotion_input_binding_digest

    metric_ref = step2.get("comparison_metric_input_ref")
    metric_digest = step2.get("comparison_metric_input_digest")
...
def serialize_comparison_promotion_candidate_input_v1(evidence: Mapping[str, Any]) -> str:
    ...
    candidate_input_status = evidence.get("candidate_input_status")
    if candidate_input_status not in _VALID_CANDIDATE_INPUT_STATUS:
        raise ComparisonPromotionCandidateInputError(
            f"candidate_input_status must be one of {sorted(_VALID_CANDIDATE_INPUT_STATUS)}"
        )
    reason_codes = evidence.get("candidate_input_reason_codes")
    if isinstance(reason_codes, list) and reason_codes != sorted(reason_codes):
        raise ComparisonPromotionCandidateInputError(
            "candidate_input_reason_codes must be sorted deterministically"
        )
...
def _evidence_bytes_for_manifest_digest(evidence: Mapping[str, Any]) -> bytes:
    ...
    return serialize_comparison_promotion_candidate_input_v1(body).encode("utf-8")
...
def _validate_evidence_integrity(evidence: Mapping[str, Any]) -> None:
    if evidence.get("contract_name") != CONTRACT_NAME:
        raise ComparisonPromotionCandidateInputError("contract_name mismatch")
    if evidence.get("contract_version") != CONTRACT_VERSION:
        raise ComparisonPromotionCandidateInputError("contract_version mismatch")
    integrity = evidence.get("integrity")
    if not isinstance(integrity, Mapping):
        raise ComparisonPromotionCandidateInputError("integrity must be an object")
    expected = compute_content_sha256(_integrity_body(evidence))
    actual = integrity.get("content_sha256")
    if actual != expected:
        raise ComparisonPromotionCandidateInputError("integrity.content_sha256 mismatch")
    output_digest = evidence.get("output_digest")
    if output_digest != _compute_output_digest(evidence):
        raise ComparisonPromotionCandidateInputError("output_digest mismatch")
    if evidence.get("artifact_id") != output_digest:
        raise ComparisonPromotionCandidateInputError("artifact_id must equal output_digest")
...
def build_self_verification_v1(
    *,
    ...
    checks: list[dict[str, str]] = [
        ...
        {"check_id": "cross_domain_binding_manifest_verified", "status": "PASS"},
        {"check_id": "cross_domain_binding_self_verification_pass", "status": "PASS"},
        ...
        {"check_id": "eligibility_evidence_verified_on_pass", "status": "PASS"},
    ...
    input_status = evidence.get("candidate_input_status")
    if input_status == "PASS":
        for check_id, field in (
            ...
            ("eligibility_evidence_verified_on_pass", "eligibility_evidence_status"),
            ("cross_domain_lineage_bound_on_pass", "cross_domain_lineage_status"),
        ):
            expected = (
                _ELIGIBILITY_VERIFIED if field == "eligibility_evidence_status" else _IDENTITY_BOUND
            )
    ...
    structural_failures = [
        ...
        not in {
            ...
            "config_patch_manifest_identity_bound_on_pass",
            "eligibility_evidence_verified_on_pass",
            "cross_domain_lineage_bound_on_pass",
        }
    ]
    if structural_failures:
        raise ComparisonPromotionCandidateInputError("self-verification checks failed")
    ...
    payload: dict[str, Any] = {
        ...
        "verified_cross_domain_lineage_binding_bundle_ref": cross_domain_binding.bundle_dir.as_posix(),
        "verified_candidate_input_status": input_status,
    }
...
def verify_binding_inputs(
    ...
    """Verify exactly one Step-2 cross-domain lineage binding bundle."""
    return verify_cross_domain_lineage_binding_bundle(
        inputs.cross_domain_lineage_binding_bundle_dir
    )


def reverify_comparison_promotion_candidate_input_v1(*, output_dir: Path | str) -> None:
    """Replay candidate input bundle without producer or upstream mutation."""
    bundle_dir = Path(output_dir)
    if not bundle_dir.is_dir():
        raise ComparisonPromotionCandidateInputError(
            f"candidate input directory not found: {bundle_dir}"
        )
    ok, msg = verify_manifest_sha256(bundle_dir)
    if not ok:
        raise ComparisonPromotionCandidateInputError(f"MANIFEST.sha256 verification failed: {msg}")
    ...
    if evidence.get("manifest_digest") != manifest_digest:
        raise ComparisonPromotionCandidateInputError("manifest_digest mismatch on replay")

    cross_domain_binding = verify_cross_domain_lineage_binding_bundle(
        Path(str(evidence["cross_domain_lineage_binding_bundle_ref"]))
    )
    if evidence.get("cross_domain_lineage_binding_digest") != cross_domain_binding.artifact_digest:
        raise ComparisonPromotionCandidateInputError(
            "cross_domain_lineage_binding_digest mismatch on replay"
        )


def produce_comparison_promotion_candidate_input_v1(
    *,
    inputs: ComparisonPromotionCandidateInputInputs,
    output_dir: Path | str,
) -> ComparisonPromotionCandidateInputResult:
    """Produce offline LEVEL_3 neutral comparison promotion candidate input evidence."""
    final_dir = Path(output_dir)
    _validate_output_target(final_dir)
    _reject_unsafe_overlap(
        cross_domain_binding_dir=inputs.cross_domain_lineage_binding_bundle_dir,
        output_dir=final_dir,
    )

    cross_domain_binding = verify_binding_inputs(inputs)
    try:
        reverify_comparison_config_patch_manifest_cross_domain_lineage_binding_v1(
            output_dir=cross_domain_binding.bundle_dir
        )
    except ComparisonConfigPatchManifestCrossDomainLineageBindingError as exc:
        raise ComparisonPromotionCandidateInputError(str(exc)) from exc

    evidence_body = build_comparison_promotion_candidate_input_v1(
        cross_domain_binding=cross_domain_binding,
    )

    staging = _staging_dir_for(final_dir)
    if staging.exists():
        raise ComparisonPromotionCandidateInputError(f"staging directory collision: {staging}")
    ...
    try:
        ...
        evidence_path.write_text(
            serialize_comparison_promotion_candidate_input_v1(finalized),
            encoding="utf-8",
        )
        ...
        reverify_comparison_promotion_candidate_input_v1(output_dir=staging)
    ...
    return ComparisonPromotionCandidateInputResult(
        output_dir=final_dir,
        ...
        candidate_input_status=str(finalized["candidate_input_status"]),
