    ...
    if date_section_pattern not in content:
        ...
        if existing_dates:
            ...
            if first_date_match:
                ...
                content = content[:insert_pos] + f"## {date_str}\n\n" + content[insert_pos:]
        else:
    ...
    for snippet in new_snippets:
        ...
        if date_section_start != -1:
            ...
            if line_end != -1:
                ...
                existing_under_date = content[
                    actual_insert : next_section if next_section != -1 else len(content)
                ]

                # Füge am Ende des Tages-Abschnitts ein
                if next_section != -1:
                    content = (
                        content[:next_section] + markdown_block + "\n" + content[next_section:]
                    )
                else:
                    if not content.endswith("\n"):
                        content += "\n"
