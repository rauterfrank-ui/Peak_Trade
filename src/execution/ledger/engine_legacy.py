...
# Legacy Slice-2 engine (WAC + double-entry journal).
#
# Kept to preserve existing determinism/replay-pack tests that depend on:
# - accounts + postings double-entry invariant
# - WAC position semantics
# - BETA_EXEC_V1 dict event ingestion
#
# EXEC_SLICE2 (this task) introduces a new FIFO-lot ledger engine in
# src/execution/ledger/engine.py.
...
class LegacyLedgerEngine:
    """
    Deterministic legacy ledger engine that consumes execution events and produces:
    ...
    def apply(self, event: Union[Mapping[str, Any], "FillLike"]) -> LegacyLedgerState:
        """
    ...
    def open_cash(self, *, amount: Decimal, entry_id: str = "opening_cash") -> LegacyLedgerState:
        """
    ...
    def _apply_fill_like(self, fill: Any) -> None:
        ...
        if quote != self.state.quote_currency or fee_ccy != self.state.quote_currency:
            raise ValueError("Legacy Slice-2 LedgerEngine supports a single quote currency only")
