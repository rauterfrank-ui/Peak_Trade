...
def discover_sessions(root: Path, include_compressed: bool = True) -> List[SessionMeta]:
    """
...
def build_plan(
    ...
    protected_sessions = set(s.path for s in sessions_by_mtime[: policy.keep_last_n_sessions])
    ...
    remaining_bytes = (
        sum(s.size_bytes for s in remaining_sessions) - compressed_bytes - deleted_bytes
    )
