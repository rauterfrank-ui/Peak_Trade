...
class BollingerBandsStrategy(BaseStrategy):
    ...
    def validate(self) -> None:
        ...
        if not 0 < self.entry_threshold <= 1:
            raise ValueError(f"entry_threshold ({self.entry_threshold}) muss zwischen 0 und 1 sein")
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        if len(data) < self.bb_period:
            raise ValueError(f"Brauche mind. {self.bb_period} Bars, habe nur {len(data)}")
        ...
        cross_exit = (data["close"].shift(1) < exit_level.shift(1)) & (data["close"] >= exit_level)
...
def generate_signals(df: pd.DataFrame, params: Dict) -> pd.Series:
    ...
    # Bollinger Bands berechnen
    upper, middle, lower = calculate_bollinger_bands(df["close"], period=bb_period, num_std=bb_std)
