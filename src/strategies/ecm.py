...
def calculate_ecm_phase(
    current_date: datetime, reference_date: datetime, cycle_days: int = ECM_CYCLE_DAYS
) -> Dict[str, float]:
    """
...
def generate_signals(df: pd.DataFrame, params: Dict) -> pd.Series:
    ...
    entry_crossover = entry_condition & (~entry_condition.shift(1, fill_value=False).astype(bool))
    ...
    exit_crossover = exit_condition & (~exit_condition.shift(1, fill_value=False).astype(bool))
