...
unproven. Historical next pointer superseded by §11.13.5.J below.
...
(one-shot; now **CONSUMED** for this preparation scope) authorized
non-trading diagnostics plus bounded classification/source preparation
after a **new** one-shot canary **trading POST** (separate from the
historical first POST) returned HTTP 401 with parseable OKX `50124`.
This does **not** authorize a Canary retry, a second POST
`&#47;api&#47;v5&#47;trade&#47;order`, cancel, flatten, general Live
authorization, general submit unlock, G13, R6 S5, Multi-Future, N&gt;1,
funding, S6, Withdraw enablement, or a canary instrument rebind.
...
in this GO). These GETs were HTTP 200 &#47; OKX `code=0` and are **not**
HTTP-401 facts. An empty SWAP instrument list is a 200-payload fact
only. They may be recorded as **unproven diagnostic candidates**, never
as the proven 50124 root cause:
...
ROOT_CAUSE_CANDIDATES=GENERIC_TRADE_FLAG_PRESENT;ACCOUNT_INSTRUMENTS_SWAP_HTTP_200_CODE_0_EMPTY_LIST;PUBLIC_INSTRUMENTS_HTTP_200_CODE_0_BTC_USDT_SWAP_PRESENT
...
LIVE_AUTHORIZED=false
ORDER_COUNT_SUBMITTED_THIS_STEP=0
NETWORK_EFFECT=GET_ONLY_ACCOUNT_CONFIG_AND_ACCOUNT_INSTRUMENTS_NO_TRADING_POST
ORDER_EFFECT=NONE
ACCOUNT_MUTATION_EFFECT=NONE
...
DIAGNOSTIC_CANDIDATE != ROOT_CAUSE_PROVEN
```
