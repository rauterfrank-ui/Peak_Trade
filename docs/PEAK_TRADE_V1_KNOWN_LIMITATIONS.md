...
- LB-EXE-001 Phase 2 keeps execution deny-by-default while aligning `transport_gate_v1` canary evaluation with the same explicit environment used by entry validation; this remains non-live, non-outbound, and is not evidence of live-approved execution.
- **Canonical Core Runtime Integration Remediation Slice D (PR #4747):** `src&#47;execution&#47;live_session.py` and `trading.master_v2.legacy_runtime_entrypoint_guard_v0` fail-closed deauthorize legacy `LiveSessionRunner` productive runtime authority (`SLICE_D_STATUS=LEGACY_RUNTIME_ENTRYPOINTS_DEAUTHORIZED`; `LEGACY_RUNTIME_ENTRYPOINTS_REACHABLE_FOR_AUTHORITY=false`; `CANONICAL_RUNTIME_ENTRYPOINT_STATUS=BOUND_NOT_ACTIVATED`). Historical tests may use `PT_LEGACY_RUNTIME_ENTRYPOINT_TEST_ONLY=1`; **no** canonical runtime activation, **no** live approval, **no** order/submission unlock. Progress registry: [`PEAK_TRADE_AUTONOMY_RUNBOOK_PROGRESS_V1.md`](governance/PEAK_TRADE_AUTONOMY_RUNBOOK_PROGRESS_V1.md).
