...
- **Nicht** Ziel von **`GET &#47;market`** ist das vollständige **Futures Read-only Market Dashboard** (F5‑Semantik) — Kanon dort: [Futures Read-only Market Dashboard Contract v0](../ops/specs/FUTURES_READ_ONLY_MARKET_DASHBOARD_CONTRACT_V0.md). Die dedizierte F5-SSR-Route ist **`GET &#47;market&#47;futures`** (siehe Routen-Tabelle oben); **ersetzt nicht** Market Surface v0 und **erteilt keine** Trading-/Execution-Autorität.
...
**Diagnostic markers (non-authorizing):** **`data-chartjs-cdn-load-error`**, **`data-chartjs-cdn-monitored-v0="true"`**, **`data-chartjs-cdn-script-v0="true"`**, **`data-chartjs-vendor-fallback-v0="true"`** (set only after successful onerror-triggered local load), **`data-market-chart-status`** (incl. **`#market-v0-chart-status`**), and **`data-market-v11-*`** chart render diagnostics mirror CDN load, SSR empty, client render state, and vendor-fallback activation — **display/troubleshooting only**; **`GET`** **`&#47;market`** must **not** derive Provider Truth from these markers.

**Vendor fallback boundary:** CDN-primary remains **jsdelivr** (`chart.js@4.4.1`). **Template-wired vendor fallback (PR #4108 / wiring v1)** loads **`/static/vendor/chartjs/4.4.1/chart.umd.min.js`** **only** on CDN `<script>` **`onerror`** / **`data-chartjs-cdn-load-error`** via **`peakTradeChartjsVendorFallbackV0`** — **no** eager local `<script>` in SSR. After successful local load the shell sets **`data-chartjs-vendor-fallback-v0="true"`** (**diagnostisch / non-authorizing**, distinct from CDN error markers). See **`#### Chart.js vendor fallback template wiring v1 (implemented)`** and **`tests/webui/test_chartjs_vendor_fallback_wiring_contract_v0.py`**. **`CHARTJS_LOCAL_FALLBACK_PLANNING_CHARTER_V0`** charter subsections (#4104–#4106) govern **separate authorization** for evidence capture / vendor decision — **not** wiring absence. **Do not** treat CDN load errors, vendor-fallback markers, or wiring as trading blocks, Dashboard Truth GO, Provider Truth, or readiness GO.

**Troubleshooting (CDN blocked vs SSR empty vs render error):** Walk **Chart status states** (`ready`/`empty`/`error` on **`data-market-chart-status`**) first, then **CDN attribution** (`data-chartjs-cdn-load-error`, **`data-chartjs-cdn-monitored-v0`**, script tag **`data-chartjs-cdn-script-v0`**), then **vendor fallback marker** (`data-chartjs-vendor-fallback-v0` — onerror-only, non-authorizing), then **v1.1 diagnostics** (`data-market-v11-*`). Cross-check **`tests/webui/test_market_dashboard_readonly_structure_contract_v0.py`**, **`tests/test_market_surface_api.py`**, and **`tests/webui/test_chartjs_vendor_fallback_wiring_contract_v0.py`**. Distinguish **CDN script blocked** (CDN markers set; onerror may trigger template-wired local fallback) from **SSR empty bars** (`empty` status, no bars) from **client render failure** (`error` status) — **none** of these grant dashboard or provider authority.
...
**Parity record:** Chart.js chain **#4108→#4113 closed on `main`** (`CHARTJS_CHAIN_CLOSED_ON_MAIN=true`; PR #4108 template wiring → PR #4109 CDN diagnostics parity → PR #4110 charter parity → PR #4111 stale-pointer guard closed DOCS_TRUTH_MAP/Double-Play deferred-fallback drift → PR #4112 browser-capture-criteria parity closed post-#4111 charter cross-refs → PR #4113 chain-closeout handoff protocol in canonical owners — **no parallel handoff doc/SSOT**; **chronicle/cross-ref only**, **no Truth/Readiness/Execution/Runtime authority**, **no Browser-Capture execute**; `CHARTJS_CHAIN_CLOSEOUT_HANDOFF_PR_CHECK_SAFETY_RECORD=docs-tests-only-no-authority-change`). Charter slices #4104–#4106 aligned to post-#4108 onerror-only wiring reality. Browser-Capture execute remains **separately authorized — not granted**; next allowed scope class = **Browser-Capture-Execute-Readiness-Prep** (read-only, no execute). CDN-primary **jsdelivr**; vendor fallback **onerror-only**; `data-chartjs-vendor-fallback-v0` **diagnostisch/non-authorizing**.
...
**Ein späteres Arbeitspaket** kann **z. B.** erweiterte Orderbuch-/Tiefen-**Ladder**-/Cockpit‑Visualisierung **auf** **`GET`** **`&#47;market&#47;double-play`** (über die **kompakte** Depth‑SSR v0 hinaus) **separat** planen — **v1.3** liefert **template‑gebundene** Rail‑Zuordnung (kein neues JSON‑Feld). CDN‑Ausfall‑Mitigation über **template-wired onerror-only** Vendor-Fallback ist **seit PR #4108 auf `main`** (**non-authorizing**); Browser/Capture‑Evidence bleibt **separat autorisiert — jetzt nicht erteilt**. **Kompakte** read‑only Depth‑SSR v0 existiert auf **`GET`** **`&#47;market`** und **`GET`** **`&#47;market&#47;double-play`** (**#3725**, contract **#3726**/**#3730**) — siehe [Market Depth display on GET /market (SSR v0 implemented)](#market-depth-display-on-get-market-ssr-v0-implemented).
