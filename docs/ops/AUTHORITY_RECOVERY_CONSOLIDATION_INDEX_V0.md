...
- **P0-D — Bounded / Acceptance / First-Live** authority: **landed in-repo (partial):** the **P0-D** table rows above (frontdoor, **GO_NO_GO** snapshot, **bounded** chain **index**, **review** hub **index** page, **START_HERE**, the **acceptance-oriented bounded run operator** runbook row, the **operator cheat sheet** row, the **operator verify checklist** row, and the **bounded real-money pilot entry contract (anchor spec)** row) record **docs-only** authority-boundary work **indexed** or **merged** as described; that is **not** a claim that **P0-D** is **complete** or that every bounded/acceptance surface is bounded, and **row-listing the entry contract** does **not** assert a **current** **activated**, **approved**, or **live-ready** state. **Still open as single-topic candidates** where misread is concrete: e.g. **other** operator runbooks, **other** checklists, and **review** surfaces not listed in the table — each as a **narrow** docs read-model or **single** boundary note when scope is **explicit** (no broad Sammel-audit in one PR).
