...
- **P0-D — Bounded / Acceptance / First-Live** authority: **landed in-repo (partial):** the **P0-D** table rows above (frontdoor, **GO_NO_GO** snapshot, **bounded** chain **index**, **review** hub **index** page, **START_HERE**, the **acceptance-oriented bounded run operator** runbook row, the **operator cheat sheet** row, the **operator verify checklist** row, the **bounded real-money pilot entry contract (anchor spec)** row, the **local bounded secret launcher** runbook row, and the **next bounded trial preflight** checklist row) record **docs-only** authority-boundary work **indexed** or **merged** as described; that is **not** a claim that **P0-D** is **complete** or that every bounded/acceptance surface is bounded, and **row-listing the entry contract**, the **local bounded secret launcher** runbook, or the **next bounded trial preflight** checklist does **not** assert a **current** **activated**, **approved**, or **live-ready** state. **Still open as single-topic candidates** where misread is concrete: e.g. **further** operator runbooks, **other** preflight or trial checklists **beyond** the **row-** **listed** **next** **bounded** **trial** **preflight** path, and **review** surfaces **not** **yet** **row-** **listed** in this table — each as a **narrow** docs read-model or **single** boundary note when scope is **explicit** (no broad Sammel-audit in one PR).
