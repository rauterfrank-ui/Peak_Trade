...
**GitHub (operators with admin):** Confirm **Branch protection** on **`main`** lists **`docs-drift-guard`** and **`repo-truth-claims`** as **Required status checks** if not already verified (names must match the workflow job `name:` fields in [.github/workflows/truth_gates_pr.yml](../../../.github/workflows/truth_gates_pr.yml)).
...
- **Optional:** next **small** slice — [J1 forward-pipeline stub follow-up](../runbooks/RUNBOOK_UNIMPLEMENTED_FEATURES_ORDERED.md#stufe-j--scripts--demo-daten-operativ-niedrig-priorisiert) (still **STUB** / no live: shared OHLCV loader, ``--ohlcv-source``). **Or** a **chat-led** gap pass per [RUNBOOK_CHAT_LED_OPEN_FEATURES.md](../runbooks/RUNBOOK_CHAT_LED_OPEN_FEATURES.md). Keep scope **narrow**; no new product or live unlock.
