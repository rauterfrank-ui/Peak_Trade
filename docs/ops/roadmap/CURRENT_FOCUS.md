...
- **Stufe J (Forward-Demo-Stub):** ``scripts/dev/run_forward_dummy_pipeline_demo.sh`` — offline Dummy-Pipeline Generate → Evaluate mit ``as_of``-Korrektur (wie Integrationssmoke); Runbook [Operator-Kurzreferenz (J1)](../runbooks/RUNBOOK_UNIMPLEMENTED_FEATURES_ORDERED.md#operator-kurzreferenz-j1-forward-no-live) Punkt 6; **NO-LIVE**.
...
| 2026-04-12 | Stufe J Forward-Demo-Stub (`run_forward_dummy_pipeline_demo.sh`); `CURRENT_FOCUS` + Runbook (this file) | `bash scripts/ops/pt_docs_gates_snapshot.sh --changed` |
