...
OBSERVATION_PACK_DIGEST_LEAVE_NULL_UNTIL_COMPUTED=false
...
7. records pack materialization execution under a separate Owner GO while
   keeping `CAMPAIGN_START_AUTHORIZED=false`, `INPUT_AUTHORITY=false`, and
   `RUNTIME_IMPLEMENTED=false`.
...
```text
OWNER_STA_RAW_INPUT_PACK_MATERIALIZED_OBSERVATION_DIGEST_COMPUTED_REMAINING_NULL=true
DECISION_STATUS=RATIFIED
OWNER_VALUE=AUTHORIZE_SURFACE_B_RAW_INPUT_PACK_MATERIALIZATION
AUTHORIZE_DETAIL_PROVABLE_REFS_CLOSED=true
AUTHORIZE_DETAIL_FIELDS_COMPLETE=false
...
INV_PACK_MATERIALIZATION_EXECUTED=true
INV_RAW_PACK_MATERIALIZATION_AUTHORIZED_AFTER_EXECUTION_GO=true
...
INVENTED_VALUES=false
NON_PROVABLE_INSTANCE_VALUES_PARTIALLY_FILLED=true
REMAINING_NULL_FIELDS=campaign_id,regime_coverage_counts,regime_coverage_instance
```
...
PROPOSED_VALUES=false
INVENTED_VALUES=false
RAW_INPUT_PACK_MATERIALIZATION_AUTHORIZED=true until remaining instance fields and STA proofs are fully ratified
PACK_MATERIALIZATION=true until a separate explicit materialization-execution Owner GO
RAW_INPUT_PACK_CREATED=true
CAMPAIGN_STARTED=false
INPUT_AUTHORITY=false
RUNTIME_IMPLEMENTED=false
...
## 9c. Raw input pack materialization execution (this GO)
...
EXECUTION_SURFACE=docs/ops/PRODUCTIVE_PURE_STACK_STAGE2_SURFACE_B_OWNER_STA_RAW_INPUT_PACK_MATERIALIZATION_V1.md
SEALED_OBSERVATION_PACK=docs/ops/artifacts/productive_pure_stack_stage2_surface_b_owner_sta_raw_input_pack_materialization_v1/observation_pack.json
CAMPAIGN_START=false
INPUT_AUTHORITY=false
RUNTIME_IMPLEMENTED=false
REGIME_COVERAGE_PRODUCER_AVAILABLE=false
...
Remaining null after materialization:
...
is ready under
`OWNER_STA_RAW_INPUT_PACK_MATERIALIZED_OBSERVATION_DIGEST_COMPUTED_REMAINING_NULL`.
