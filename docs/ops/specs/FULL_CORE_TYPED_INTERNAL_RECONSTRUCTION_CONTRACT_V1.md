---
docs_token: DOCS_TOKEN_FULL_CORE_TYPED_INTERNAL_RECONSTRUCTION_CONTRACT_V1
status: active
scope: Full-Core typed internal reconstruction contract schema; no productive reconstruction; no source selection; no mapping; no producer implementation; no POST; no wire; no LiveExecutionPort construction
capability: FULL_CORE_TYPED_INTERNAL_RECONSTRUCTION_CONTRACT_V1
architecture_spec: PEAK_TRADE_MASTER_RUNBOOK
last_updated: 2026-09-12
---

# Full Core Typed Internal Reconstruction Contract V1

Derived spec. Non-SSOT. Canonical persist is Master Runbook §11.2.1.Z.
...
This persist adds the typed `INTERNAL_RECONSTRUCTION_CONTRACT_V1` after
`NORMALIZATION_INCLUSION_ADJUDICATION_V1`. Schema presence is not a
productive reconstruction runtime instance, not a reconstructed value,
not source selection, not mapping, not reconciliation, and not producer
implementation. Target-dimension naming is not reconstruction proof. U02/U03
remain in-equity-base-only with no separate addend; the algebra schema is
present and incomplete. C01–C21 remain rejected. AccountingPortfolioStateV1,
LedgerSnapshot.equity_by_ccy, and SimulatedPortfolioStateV1 are not elevated.
