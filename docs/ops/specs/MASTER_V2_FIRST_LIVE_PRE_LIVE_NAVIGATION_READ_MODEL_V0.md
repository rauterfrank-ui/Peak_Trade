...
| C0f-producer | [MASTER_V2_DOUBLE_PLAY_FUTURES_INPUT_PRODUCER_CONTRACT_V0.md](MASTER_V2_DOUBLE_PLAY_FUTURES_INPUT_PRODUCER_CONTRACT_V0.md) | **Futures Input Producer** boundary (operational sources vs snapshot handoff; companion to C0f) | Trading authority, selector go, registry row as proof of readiness, Live/Testnet |
...
| C0-Class-A | [MASTER_V2_FUTURES_CLASS_A_CAPABILITY_CONTRACT_V0.md](MASTER_V2_FUTURES_CLASS_A_CAPABILITY_CONTRACT_V0.md) | **Class-A Spot/Paper scheduled probe evidence** — operator navigation pointer for the guarded Class-A Spot/Paper scheduled workflow (repository path: `.github/workflows/class-a-shadow-paper-scheduled-probe-v1.yml`) and its CI artifacts. **Purpose:** relate scheduled paper artifacts to **spot/paper infrastructure smoke** interpretation only. **Evidence boundary:** `meta.json` and `events.parquet` from those runs may support **infrastructure-smoke** reading only — not broader readiness claims. **Futures Class-A boundary:** this chain does **not** demonstrate Futures/Perp readiness; it does **not** close the Futures-specific gaps or prerequisite checklist in the linked contract (§4–§7), including public provider paths for derivatives, contract sizing, margin, leverage, funding, liquidation semantics, mark/index/last policy, short semantics versus spot inventory, futures evidence fields, or Master V2 Futures **runtime** wiring versus pure DTO readiness. **Non-authority:** navigation text only — not Testnet/Live enablement, not Double Play or execution permission, not a substitute for Scope/Capital, Risk/KillSwitch, or Execution-Gate procedures. | Futures/Perp readiness; **Futures Class-A** prerequisite satisfaction; Testnet/Live enablement; Master V2 **runtime** proof; bypass of Master V2, Scope/Capital, Risk/KillSwitch, or Execution Gates; external signoff; treating spot Class-A artifacts as permission to trade |
...
| C1a | [MASTER_V2_BOUNDED_PILOT_L1_L5_POINTER_RUNBOOK_CROSSWALK_V0.md](MASTER_V2_BOUNDED_PILOT_L1_L5_POINTER_RUNBOOK_CROSSWALK_V0.md) | **Navigation pointer** — L1–L5 **crosswalk** from ladder levels to canonical runbooks and **operator-held**, **candidate-specific** evidence **pointer** vocabulary (**read path** only; **non-authorizing**; does not create or validate evidence) | Closure of G4–G8, that evidence exists or is sufficient, bounded pilot / Testnet / Live enablement, scanner, exchange, runtime, provider, or session work; ambiguous gaps remain **OPEN** / **TBD** / **EXTERNAL** and **fail-closed** |
