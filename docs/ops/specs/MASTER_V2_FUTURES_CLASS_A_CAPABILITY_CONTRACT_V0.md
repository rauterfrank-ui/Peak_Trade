...
- `tests&#47;execution&#47;paper&#47;test_paper_engine_futures_seam_v0.py` **characterizes** WP1B seam separation from Futures Accounting **and snapshot/DTO helpers** alongside **PaperExecutionEngine** itself: paper-engine modules (excluding `futures_accounting.py`) do **not** import `futures_accounting`; `futures_accounting.py` does **not** import engine/broker/journal stacks; both modules remain loadable-side-by-side (**tests-only**, **non-authorizing** detail in that file).
- **PR #3190** (same seam suite): additionally documents `PaperExecutionEngine` (`engine.py`) source **excluding** UM-style BTC/ETH paper-prep literals (`BTCUSDT`, `ETHUSDT`), `paper_slot`, `max_loss`, plus selected snapshot-builder / wiring **name** tokens enumerated in **`test_paper_execution_engine_source_has_no_um_paper_prep_or_snapshot_wire_tokens_v0`** — worksheet vocabulary stays external to WP1B source (**tests-only** characterization in CI only; prose here **does not** change WP1B or kernel code). Reading this clause grants **nothing**: **no** Paper activation or WP1B↔Accounting wiring **on `main`** from words alone; **no** runners/workflows/providers/exchanges; **no** Testnet/Live/order path; **no** real-capital/strategy-authority implication; **no** new readiness, evidence, report, index, or handoff surface. **External** operator `/tmp` BTC/ETH paper-prep worksheets are **not** copied into canonical repo specs.
