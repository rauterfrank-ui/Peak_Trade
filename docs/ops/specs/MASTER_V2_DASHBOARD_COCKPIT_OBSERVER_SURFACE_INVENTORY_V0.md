...
| `docs/ops/market_dashboard/PEAK_TRADE_MARKET_DASHBOARD_LANDSCAPE_MASTER_RUNBOOK_V2.md` | Dashboard / Cockpit | current Landscape V2 visual read-only consumer (`LANDSCAPE_AUTHORITY_EFFECT=NONE`) | **author** / **reviewer** | **Markdown** | `DASHBOARD_AUTHORITY_EFFECT=NONE`; not a permit by itself |
| `docs/webui/MARKET_DASHBOARD_REMOVED.md` | Dashboard / Cockpit | historical legacy Market Dashboard evidence only | **reviewer** | **Markdown** | `DOCUMENT_CLASS=HISTORICAL_EVIDENCE_ONLY`; not a current contract |
