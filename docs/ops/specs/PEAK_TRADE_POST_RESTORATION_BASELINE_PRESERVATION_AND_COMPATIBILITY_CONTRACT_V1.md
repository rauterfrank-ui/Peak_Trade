...
| MAX_SIZE fresh observation and consumer wiring | Master §5.3; MAX_SIZE fresh observation and consumer wiring spec | `tests/ops/test_peak_trade_max_size_fresh_observation_and_consumer_wiring_v1.py` | true |
...
```text
SIMULATED_EXECUTION_PIPELINE_ADJUDICATION_PRESERVED=true
...
Named preservation invariants for the persisted venue-pretrade
metadata-binding alignment adjudication. This contract does not itself
perform that adjudication, does not freeze a current max-size numeric,
does not authorize a Network GET, and does not grant live or execution
authority.
...
Named preservation invariants for the persisted MAX_SIZE fresh
observation and productive consumer wiring. This contract does not
itself perform that wiring, does not implement MAX_AVAILABLE, does not
invent a TTL or cache, and does not grant live or execution authority.
The freshness Owner-policy persist remains historically
`MAX_SIZE_CONSUMER_REMAINS_UNBOUND=true` for that policy-only slice.
The #6148 GET window remains historically non-operative. The
20260829T210523Z pack is forensic evidence and is not an operative
cache.

```text
MAX_SIZE_FRESH_OBSERVATION_AND_CONSUMER_WIRING_V1_PRESERVED=true
CURRENT_MAX_SIZE_UNIT_BOUND=true
CURRENT_MAX_SIZE_UNIT=contracts
CURRENT_MAX_SIZE_NORMALIZATION_BOUND=true
CURRENT_MAX_SIZE_FRESHNESS_POLICY_BOUND=true
CURRENT_MAX_SIZE_FRESHNESS_POLICY=FRESH_GET_PER_PRETRADE_DECISION
CURRENT_MAX_SIZE_CONSUMER_BOUND=true
CURRENT_MAX_SIZE_BINDING_STATUS=PROVEN
HISTORICAL_FRESHNESS_POLICY_CONSUMER_UNBOUND_PRESERVED=true
HISTORICAL_MAX_SIZE_REUSE_ALLOWED=false
PERSISTED_OBSERVATION_IS_OPERATIVE_CACHE=false
FIXED_TTL_DEFINED=false
EVENT_INVALIDATED_CACHE_DEFINED=false
