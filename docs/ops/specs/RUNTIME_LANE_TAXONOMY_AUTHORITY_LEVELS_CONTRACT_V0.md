...
| F5 / Landscape display (non-authorizing) — **current** visual consumer is Landscape V2 `GET &#47;market` (`DASHBOARD_AUTHORITY_EFFECT=NONE`); historical legacy Market Dashboard product evidence only (`DOCUMENT_CLASS=HISTORICAL_EVIDENCE_ONLY`) | [PEAK_TRADE_MARKET_DASHBOARD_LANDSCAPE_MASTER_RUNBOOK_V2.md](../market_dashboard/PEAK_TRADE_MARKET_DASHBOARD_LANDSCAPE_MASTER_RUNBOOK_V2.md) |
...
- Market Dashboard: §6a.2 — read-only Registry v1 projection fields only; no runtime actions; **legacy** product UI routes remain intentionally absent (see [MARKET_DASHBOARD_REMOVED.md](../../webui/MARKET_DASHBOARD_REMOVED.md)). Landscape V2 `GET &#47;market` is the separate authorized read-only consumer shell; §6a.2 run-projection overlay stays disabled-by-default (`MARKET_DASHBOARD_RUN_PROJECTION_ENABLED=false`).
...
| `MARKET_DASHBOARD_READONLY_RUN_PROJECTION_V0` | §6a.2 — `market_dashboard_projection` states; **legacy** product routes intentionally absent; Landscape V2 `GET &#47;market` is the separate read-only consumer (not this overlay); Master V2 / Double Play domain authority unchanged (§9); DP JSON is `GET &#47;api&#47;master-v2&#47;double-play&#47;dashboard-display.json` (not a Market Dashboard route) |
...
**Cross-surface post-invoke result-classification parity matrix (static guards only):** `POST_INVOKE_RESULT_CLASSIFICATION_MATRIX_GUARD_V0=true`; `ALL_FIVE_ATTACH_HOOK_SURFACES_RESULT_CLASSIFICATION_COVERED=true`; Paper defines canonical `emit_bounded_adapter_durable_closeout_machine_lines` with non-authorizing `STATUS` (`blocked` / `pass` / `failed`) and `HELPER_RC` (`not_run` or helper exit code); Shadow/Testnet delegate post-invoke classification via Paper `maybe_invoke_durable_closeout_after_archive` import chain (`NEW_PARALLEL_CLASSIFICATION_LOGIC_CREATED=false`); Scheduler (`scheduler_completion`) and Supervisor (`supervisor_evidence_pack`) expose prefix-scoped emit helpers with blocked-path `STATUS`/`HELPER_RC=not_run`/`NON_AUTHORIZING` and invoked-path fail-closed `EXIT_CODE` surrogate (`SCHEDULER_RESULT_CLASSIFICATION_SURFACE_GUARDED=true`; `SUPERVISOR_RESULT_CLASSIFICATION_SURFACE_GUARDED=true`; `STATUS_CLASSIFICATION_GUARDED=true`; `HELPER_RC_CLASSIFICATION_GUARDED=true`; `NON_AUTHORIZING_CLASSIFICATION_GUARDED=true`). Static matrix: [test_post_closeout_hook_attach_readiness_bridge_v0.py](../../../tests/ops/test_post_closeout_hook_attach_readiness_bridge_v0.py); [test_post_closeout_automation_hook_owner_precheck_v0.py](../../../tests/ops/test_post_closeout_automation_hook_owner_precheck_v0.py).

**Cross-surface bounded-adapter CLI-args parity matrix (static guards only):** `VALIDATE_CLI_ARGS_CROSS_SURFACE_PARITY_MATRIX_GUARD_V0=true`; `ALL_BOUNDED_ADAPTER_ATTACH_SURFACES_CLI_ARGS_MATRIX_COVERED=true`; Paper defines canonical `validate_durable_closeout_invoke_cli_args()` and `add_bounded_adapter_durable_closeout_cli_args()` with fail-closed invoke-flag coupling (`--run-local-post-closeout-chain-v0`, pointer evidence, and `--durable-closeout-dest-dir` require `--invoke-durable-closeout-v0`; dest outside `/tmp`; `--durable-closeout-force` registered for force-scope); Shadow/Testnet delegate CLI-arg validation via Paper import chain (`PAPER_CLI_ARGS_SURFACE_GUARDED=true`; `SHADOW_CLI_ARGS_IMPORT_CHAIN_GUARDED=true`; `TESTNET_CLI_ARGS_SURFACE_GUARDED=true`; `VALIDATE_DURABLE_CLOSEOUT_INVOKE_CLI_ARGS_GUARDED=true`; `DURABLE_CLOSEOUT_INVOKE_FLAG_CLI_ARGS_GUARDED=true`; `FORCE_SCOPE_BLOCKER_HINT_CLI_ARGS_GUARDED=true`; `NEW_PARALLEL_CLI_ARGS_VALIDATION_LOGIC_CREATED=false`). Static matrix: [test_post_closeout_hook_attach_readiness_bridge_v0.py](../../../tests/ops/test_post_closeout_hook_attach_readiness_bridge_v0.py); [test_post_closeout_automation_hook_owner_precheck_v0.py](../../../tests/ops/test_post_closeout_automation_hook_owner_precheck_v0.py). Scheduler/Supervisor out of scope (distinct attach-hook CLI shapes).

**Cross-surface post-invoke dest manifest-verify RC parity matrix (static guards only):** `DEST_MANIFEST_VERIFY_RC_CROSS_SURFACE_PARITY_MATRIX_GUARD_V0=true`; `ALL_FIVE_ATTACH_HOOK_SURFACES_DEST_MANIFEST_VERIFY_RC_COVERED=true`; canonical helper [durable_closeout_copy_verify_v0.py](../../../scripts/ops/durable_closeout_copy_verify_v0.py) emits `DURABLE_CLOSEOUT_DEST_MANIFEST_VERIFY_RC` via `emit_machine_lines()`; Paper defines canonical helper invoke chain (`build_durable_closeout_invoke_argv`, `_default_durable_closeout_invoker`) with `BOUNDED_ADAPTER_DURABLE_CLOSEOUT_HELPER_RC` fail-closed surrogate (`PAPER_DEST_MANIFEST_VERIFY_RC_SURFACE_GUARDED=true`); Shadow/Testnet delegate via Paper `maybe_invoke_durable_closeout_after_archive` import chain without parallel dest manifest RC emit (`SHADOW_DEST_MANIFEST_VERIFY_RC_IMPORT_CHAIN_GUARDED=true`; `TESTNET_DEST_MANIFEST_VERIFY_RC_SURFACE_GUARDED=true`); Scheduler/Supervisor invoke the same helper chain at attach time with prefix-scoped `EXIT_CODE` surrogate (`SCHEDULER_DEST_MANIFEST_VERIFY_RC_SURFACE_GUARDED=true`; `SUPERVISOR_DEST_MANIFEST_VERIFY_RC_SURFACE_GUARDED=true`); attach surfaces must **not** emit `DURABLE_CLOSEOUT_DEST_MANIFEST_VERIFY_RC` locally (`NEW_PARALLEL_MANIFEST_RC_CLASSIFICATION_LOGIC_CREATED=false`); `HOOK_ATTACH_AFTER_MANIFEST_VERIFY_RC_ZERO_ONLY_GUARDED=true` preserves `HOOK_ATTACH_AFTER_MANIFEST_VERIFY_RC_ZERO_ONLY=true` in owner precheck markers. Static matrix: [test_post_closeout_hook_attach_readiness_bridge_v0.py](../../../tests/ops/test_post_closeout_hook_attach_readiness_bridge_v0.py); [test_post_closeout_automation_hook_owner_precheck_v0.py](../../../tests/ops/test_post_closeout_automation_hook_owner_precheck_v0.py).
...
- Removed **legacy** Market Dashboard product routes (`GET &#47;market&#47;double-play`, `GET &#47;market&#47;futures`, legacy OHLCV/depth APIs — intentionally absent; not an attach owner; see [MARKET_DASHBOARD_REMOVED.md](../../webui/MARKET_DASHBOARD_REMOVED.md)). Landscape V2 `GET &#47;market` is not a post-closeout automation attach owner.
...
Cross-reference: [PEAK_TRADE_MARKET_DASHBOARD_LANDSCAPE_MASTER_RUNBOOK_V2.md](../market_dashboard/PEAK_TRADE_MARKET_DASHBOARD_LANDSCAPE_MASTER_RUNBOOK_V2.md) (current Landscape V2 read-only consumer; `DASHBOARD_AUTHORITY_EFFECT=NONE`); historical legacy product evidence [MARKET_DASHBOARD_REMOVED.md](../../webui/MARKET_DASHBOARD_REMOVED.md) (`DOCUMENT_CLASS=HISTORICAL_EVIDENCE_ONLY`; registry overlay consumer §6a.2 remains ops-only); §6a.0.9 payload builder planning; [DOCS_TRUTH_MAP.md](../registry/DOCS_TRUTH_MAP.md); Preflight §2b.1 mandatory durable closeout; Preflight §2b.3 adapter closeout validation SSOT.
...
- No **legacy** Market Dashboard product routes (`GET &#47;market&#47;double-play`, `GET &#47;market&#47;futures`, legacy OHLCV/depth APIs intentionally absent; `MARKET_DASHBOARD_DOUBLE_PLAY_TOUCHED=false`); Landscape V2 `GET &#47;market` remains a pure read-only consumer without Double Play / Master V2 authority; Master V2 / Double Play domain decision authority remains protected (§9)
- No `lane_id=daemon_paper_24h`, no `lane_id=remote_runtime`, no Registry v2
...
- **Current Landscape V2 consumer:** [PEAK_TRADE_MARKET_DASHBOARD_LANDSCAPE_MASTER_RUNBOOK_V2.md](../market_dashboard/PEAK_TRADE_MARKET_DASHBOARD_LANDSCAPE_MASTER_RUNBOOK_V2.md) is the current read-only visual consumer (`DASHBOARD_AUTHORITY_EFFECT=NONE`; `GET &#47;market`). Historical legacy Market Dashboard product evidence remains [MARKET_DASHBOARD_REMOVED.md](../../webui/MARKET_DASHBOARD_REMOVED.md) (`DOCUMENT_CLASS=HISTORICAL_EVIDENCE_ONLY`; not a current tombstone or negative non-regression contract). Landscape V2 does **not** enable §6a.2 Registry run projection (`MARKET_DASHBOARD_RUN_PROJECTION_ENABLED=false`).
...
- Detail owner: [PEAK_TRADE_MARKET_DASHBOARD_LANDSCAPE_MASTER_RUNBOOK_V2.md](../market_dashboard/PEAK_TRADE_MARKET_DASHBOARD_LANDSCAPE_MASTER_RUNBOOK_V2.md) — current Landscape V2 read-only consumer; historical F5 / Market Surface contracts are Git history only (`DOCUMENT_CLASS=HISTORICAL_EVIDENCE_ONLY` in [MARKET_DASHBOARD_REMOVED.md](../../webui/MARKET_DASHBOARD_REMOVED.md)).
