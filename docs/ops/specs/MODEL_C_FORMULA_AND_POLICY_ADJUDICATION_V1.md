...
OQ-C3_AFTER_FREEZE_EXCEPTION_BEFORE_RUNTIME_BIND=
  Cap 6.3 200/80/120 remain the productive generator SSOT
  EFFECTIVE_NUMERIC_VALUES_UNCHANGED=true
  MODEL_C_FREEZE_EXCEPTION_AUTHORIZED=true
...
5. Cap-6.2 digest residual + Cap-6.5 adverse residual owner contract —
   ...
   — **persisted** (docs-only; no freeze-exception; no runtime bind)
...
| Canonical Master Runbook §9.2.3 | SSOT for Cap 6.2 digest / Cap 6.5 adverse residual owner persist |
...
6. Treats Cap 6.2 / 6.3 freeze-exception as mutation, runtime bind, or numeric cutover
...
EARLIEST_UNRESOLVED_MODEL_C_DEPENDENCY=CROSS_INSTRUMENT_VALIDATION
LATER_REQUIRED_GO=CROSS_INSTRUMENT_VALIDATION
