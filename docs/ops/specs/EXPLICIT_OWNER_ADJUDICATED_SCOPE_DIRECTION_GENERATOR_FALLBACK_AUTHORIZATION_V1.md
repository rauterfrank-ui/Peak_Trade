# Explicit Owner-Adjudicated ScopeDirection Generator-Fallback Authorization v1

status: ACTIVE
last_updated: 2026-09-05
owner: Peak_Trade
purpose: Fail-closed sixth Economic Guard admission class for an Owner-adjudicated generator-fallback repair so ScopeDirectionState remains a SideState projection. Not wiring, restoration, decommission, nonproductive contract change, or the fifth mapping-bind class. Not live authority.
docs_token: DOCS_TOKEN_EXPLICIT_OWNER_ADJUDICATED_SCOPE_DIRECTION_GENERATOR_FALLBACK_AUTHORIZATION_V1
...
closed fifth-class grant.

Committed machine state:

[`config/governance/explicit_owner_adjudicated_scope_direction_generator_fallback_authorization_v1.json`](../../../config/governance/explicit_owner_adjudicated_scope_direction_generator_fallback_authorization_v1.json)
...
5. `bound_authority_spec` exists and is the directional mapping contract
   that recorded the remaining generator fallback
...
Authorized runtime surface for this slice:

`src/trading/master_v2/integrated_offline_trading_logic_replay_v1.py`

Target function: `run_integrated_offline_trading_logic_replay_v1`.
The repair makes generator orientation
`scope_direction_from_side_state_v1(inp.side_state)` with the helper default
LONG fallback for Neutral/CHOP/KILL. Composition `selected_side` and
`inp.scope_direction_state` must not write `ScopeDirectionState`.
...
| Reuse fifth-class grant | fifth-class grant remains inactive; purpose mismatch fails |
...
Proof that this generator-fallback repair is offline-fixture-proven is
...
5. This class (remaining forbidden matches; also claims an exact digest-bound
   slice that a standing wiring subset grant would otherwise swallow)
6. Explicit Owner-Adjudicated Nonproductive Contract Change (unclassified only)

This class must not consume unclassified nonproductive paths. It does not
