...
create an artifact, and is not an automatic next step. Owner-GO
`OWNER_GO_MF_MEMBERSHIP_CONTEXT_ARTIFACT_MATERIALIZATION_AUTHORITY_DECISION_V1`
closes that class in §1.16 as `SET_MATERIALIZATION_AUTHORITY_GRANTED_TRUE`.
`MATERIALIZATION_AUTHORITY_GRANTED` is `true` as grant only. Grant is
not materialization, not artifact creation, and not an automatic next
step. The §1.16 persist does not name a next canonical decision.
