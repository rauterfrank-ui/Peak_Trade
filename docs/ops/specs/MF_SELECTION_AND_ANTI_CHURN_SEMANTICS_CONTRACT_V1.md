...
policy of the newly bound Active Set. Rotation/anti-churn for that
Active Set remains unratified. `N_VALUE` is not re-owned. Consumer
identity remains `UNBOUND`. Host join remains false. Cap 2.3 and Cap
2.4 remain unre-wired. G13 remains closed. This persist does **not**
...
contract. This persist does **not** re-own that close. Isolated
POLICY_A in this file remains the policy of the non-authoritative
membership selector. Adoption for the authoritative Active Set remains
`UNPROVEN`. PDF Step 5 remains `UNRESOLVED`. Workpackage
`PDF_STEP_5_ANTI_CHURN_OWNER_RATIFICATION_DECISION_PACKAGE_V1`
prepares a non-operative Owner decision surface in the subordinate
