...
The broader **`GET &#47;market`** WebUI chronicle [Market Surface v0](../../webui/MARKET_SURFACE_V0.md) (Kraken/OHLCV/dummy SSR lineage) is **orthogonal** to this F5 contract. Market Surface v0 **does not replace** F5 futures-aware display semantics; this contract remains the canonical F5 read-only dashboard boundary. **Orthogonality means separate producer/authority/display-contract ownership — not necessarily a separate HTTP route host.** F5 read-only panels may be hosted on **`GET &#47;market`** without transferring F5 or futures authority into WebUI. Market-Airport and parallel dashboard surfaces remain out of scope.
...
- [Market Surface v0](../../webui/MARKET_SURFACE_V0.md) — orthogonal WebUI technical chronicle for `&#47;market` and related read-only routes; does not replace F5 semantics; hosting on `/market` does not transfer F5 ownership
