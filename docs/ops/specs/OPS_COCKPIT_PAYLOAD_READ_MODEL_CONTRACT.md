...
| Key | Role |
|-----|------|
| `workflow_officer_state` | Latest Workflow Officer panel context — does **not** start the officer; `src&#47;ops&#47;workflow_officer.py` (panel builders). Surfaces in HTML as **Operator workflow observation** (`id=operator-workflow-observation-surface`, `_render_workflow_officer_observation_surface`) — visibility and bounded **handoff** &#47; **next-step preview** rows from the same `report.json` `summary` when present; **not** approval, **not** unlock, **not** execution authority, **not** a substitute for `policy_go_no_go_observation`. Operator Summary HTML: [`OPS_COCKPIT_OPERATOR_SUMMARY_SURFACE.md`](OPS_COCKPIT_OPERATOR_SUMMARY_SURFACE.md) — **Operator workflow (observation)** (`id=operator-summary-workflow-officer`). Supplementary traceability: [`OPS_COCKPIT_VNEXT_REQUIRED_VIEWS_COVERAGE.md`](OPS_COCKPIT_VNEXT_REQUIRED_VIEWS_COVERAGE.md) — **§ Supplementary Cockpit surfaces**. |
