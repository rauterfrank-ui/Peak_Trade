...
| Observability / Execution | Weitere Module unter `src/execution`, `src/live`, `src/observability` | Cockpit **liest** nur über vorhandene Builder/Reader, die in `ops_cockpit.py` eingebunden sind — **kein** vollständiger Repo-weiter Export aller Observability-Features ohne Codeprüfung jedes Imports. |
