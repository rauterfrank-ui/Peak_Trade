...
- The same snapshot is mirrored on `handoff_context.primary_sequencing`, per-row `handoff_context.top_followups[].sequencing`, `summary.primary_sequencing`, `next_chat_preview.primary_sequencing`, and primary follow-ups in `operator_report` / `executive_summary` where applicable. Deterministic ordering is unchanged.
