...
PE-7 establishes durable-completion applicability for both parked Order-Capability offline lanes. PE-8 closes the **adapter vs fixture-binding wiring asymmetry** at contract level (PE-4 analog): `run_order_capability_dry_validation_adapter_v1.py` is the **wired reference** — it imports `scripts/ops/primary_evidence_retention_v0.py` (`require_durable_archive_root`, `write_manifest_sha256`, `validate_order_capability_offline_durable_run_root`) for operator-gated `--write-evidence`. `run_order_capability_fixture_binding_dry_validation_v1.py` remains **plan-only** with optional flat `--output`; **primary-evidence closeout wiring is pending** (`FIXTURE_BINDING_RUNNER_PRIMARY_EVIDENCE_WIRING_PENDING=true`) until a separately scoped operator GO wires the shared helper. Until wired, fixture-binding durable run completion claims are **invalid** (`FIXTURE_BINDING_DURABLE_COMPLETION_INVALID_WITHOUT_HELPER_WIRING=true`); flat `--output` alone does not satisfy §2a.1 durable primary evidence. Static guard: `tests/ops/test_run_primary_evidence_retention_hard_gate_v0.py` (`PE8_ORDER_CAPABILITY_FIXTURE_BINDING_OFFLINE_CLOSEOUT_WIRING_LANE_MATRIX`; `test_pe8_order_capability_fixture_binding_offline_durable_closeout_wiring_guard_v0`). Crosslink: `docs/ops/planning/SECTION5_PREFLIGHT_GAP_OWNER_MAP_CONTRACT_V0.md` §2a.1. **Tests-only / contract-only** — does not authorize orderflow, cancel, execute, arming, Live, runtime start, or Preflight lift; does not activate enforcement (`GAP2A1_PRIMARY_EVIDENCE_ENFORCED=false`; `ORDER_CAPABILITY_OFFLINE_ENFORCEMENT_OPT_IN_CONFIRMED=true`).
