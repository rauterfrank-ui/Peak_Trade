...
- **Pre-Economic Zero-Order Evidence (non-equivalent):** [PRE_ECONOMIC_ZERO_ORDER_EVIDENCE_SESSION_V1.md](PRE_ECONOMIC_ZERO_ORDER_EVIDENCE_SESSION_V1.md) — optional governed evidence stage before Economic Validity; **not** Shadow-247, **not** STEP 29U, **not** a run approval; default `BLOCKED`; max 21600s; Zero-Order only; does not set Economic PASS. Implementation readiness (`PRE_ECONOMIC_ZERO_ORDER_EVIDENCE_IMPLEMENTATION_READINESS_V1`) and authorization/execution readiness (`PRE_ECONOMIC_ZERO_ORDER_EVIDENCE_SESSION_AUTHORIZATION_AND_EXECUTION`) may be present; `SESSION_EXECUTION_AUTHORIZED=false`, `OPERATOR_GO_GRANTED=false`, `SESSION_EVIDENCE_VALID=false`, `SIX_HOUR_SESSION_EXECUTED=false`, session evidence remains `NOT_AUTHORIZED`. Companion: [PRE_ECONOMIC_ZERO_ORDER_EVIDENCE_SESSION_AUTHORIZATION_AND_EXECUTION_V1.md](PRE_ECONOMIC_ZERO_ORDER_EVIDENCE_SESSION_AUTHORIZATION_AND_EXECUTION_V1.md).
