...
- **Pre-Economic Zero-Order Evidence (non-equivalent):** [PRE_ECONOMIC_ZERO_ORDER_EVIDENCE_SESSION_V1.md](PRE_ECONOMIC_ZERO_ORDER_EVIDENCE_SESSION_V1.md) — optional governed evidence stage before Economic Validity; **not** Shadow-247, **not** STEP 29U, **not** a run approval; default `BLOCKED`; max 21600s; Zero-Order only; does not set Economic PASS.
