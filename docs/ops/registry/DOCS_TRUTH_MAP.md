...
- 2026-05-26 — `docs/ops/specs/RUNTIME_LANE_TAXONOMY_AUTHORITY_LEVELS_CONTRACT_V0.md` §6a.0.6 — **Remote paper validator CLI planning contract v0** (`REMOTE_PAPER_VALIDATOR_CLI_PLANNING_ONLY`; future offline CLI input/output shape; local JSON fixtures only; no archive walker/network/AWS/SSH/systemd/Docker; binds §6a.0–§6a.0.5 + §2a/§2b.1/§2b.2 + S3 §6a.3; `PASS` non-authorizing; **no** validator CLI/dry-template/runner implementation; **docs/tests-only**; **kein** Runtime-/Gate-Unlock).
- 2026-05-26 — `docs/ops/runbooks/PAPER_SHADOW_247_PREFLIGHT_CONTRACT_V0.md` §2b.2 — **Closeout Enforcement Planning Contract v0** (`CLOSEOUT_ENFORCEMENT_PLANNING_ONLY`; classifies `post_merge_closeout.sh` + `append_closeout_index.py` as non-§2b.1-completion helpers; future copy-only helper expectations; binds §2b.1 + §2a + remote §6a.0–§6a.0.5 + S3 §6a.3; gates remote implementation charter + dry-template review; **no** copy/verify implementation/archive mutation; **docs/tests-only**; **kein** Runtime-/Gate-Unlock).
...
- 2026-05-26 — `docs/ops/specs/RUNTIME_LANE_TAXONOMY_AUTHORITY_LEVELS_CONTRACT_V0.md` §6a.3.1 — **S3 finalized evidence export implementation preflight contract v0** (local-only dry adapter/command contract shape; preflight checklist; PHASE_T/W extend-only reconciliation to `MANIFEST.sha256`; `SHA256SUMS.stable.txt` not competing truth; `evidence_transport` transition gated; download+verify before closeout; **docs/tests-only**; **kein** AWS/rclone/S3-Upload/Runtime-/Gate-Unlock).
- 2026-05-26 — `docs/ops/specs/RUNTIME_LANE_TAXONOMY_AUTHORITY_LEVELS_CONTRACT_V0.md` §6a.3 — **S3 finalized evidence export gate contract v0** (`evidence_transport` `local_only`/`s3_export_after_finalize`; default `local_only`; finalize+`MANIFEST.sha256` RC=0 required; download+verify before closeout acceptance; no active staging sync; `MANIFEST.sha256` canonical; no `SHA256SUMS.stable.txt` parallel truth; **docs/tests-only**; **kein** AWS/rclone/S3-Upload/Runtime-/Gate-Unlock).
