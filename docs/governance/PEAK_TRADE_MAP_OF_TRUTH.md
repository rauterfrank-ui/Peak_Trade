...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 LIVE_HANDOFF_CREATE_PRODUCTIVE_CAPTURE_OWNER_AND_LIFECYCLE_HOOK | Navigation-only current §11.14 create productive capture-owner and lifecycle-hook persist (`PRODUCTIVE_CAPTURE_OWNER=SECTION_11_14_LIVE_PRODUCTIVE_CAPTURE_OWNER_V1`; `PRODUCTIVE_CAPTURE_OWNER_UNIQUE=true`; `PRODUCTIVE_LIFECYCLE_HOOK=run_capture_hook_after_bound_fill_before_restart_v1`; `PRODUCTIVE_LIFECYCLE_HOOK_UNIQUE=true`; `STRUCTURAL_RUNTIME_BINDING_PROVEN=true`; `CURRENT_RUNTIME_EXECUTION_AUTHORIZED=false`; `AUTHORIZED_RUNTIME_SURFACE=NONE`; `BINDING_CASE=CASE_A_CREATED`; `PRODUCTIVE_BINDING_IMPLEMENTED=true`; `COMPLETE_CAPTURE_SEAM=UNPROVEN`; `HOST_CRASH_DURABILITY=UNPROVEN`; `CONTEMPORANEOUS_PEAK_TRADE_PRE_RESTART_HANDOFF_OBSERVED=false`; `LIVE_RESTART_RECONSTRUCTED=false`; `IMPLEMENTATION_AUTHORIZED=true`; no GET; no POST; no productive handoff write during persist; no restart execution; `OWNER_GO_FOR_LIVE_RESTART_RECONSTRUCTED` remains §11.14 evidence-domain only and is not the productive Live-execution next pointer; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_LIVE_HANDOFF_CREATE_PRODUCTIVE_CAPTURE_OWNER_AND_LIFECYCLE_HOOK_V1.md`](../ops/specs/SECTION_11_14_LIVE_HANDOFF_CREATE_PRODUCTIVE_CAPTURE_OWNER_AND_LIFECYCLE_HOOK_V1.md) | Derived §11.14 create productive capture-owner and lifecycle-hook spec (non-SSOT; unique owner created; unique hook created; structural binding proven; runtime execution unauthorized; complete seam unproven; host-crash unproven; restart false) |
