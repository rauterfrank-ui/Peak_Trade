...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 LIVE_HANDOFF_EXACT_FEE_RESTART_DURABILITY_AND_PRE_EXECUTION_READINESS_CLOSURE | Navigation-only current §11.14 exact-fee / restart / durability / pre-execution-readiness forensic closure (`EXACT_OKX_FEE_FORMULA_UNPROVEN=true`; `BOUNDED_FEE_ENVELOPE_PROVEN=false`; `LIVE_RESTART_STATIC_RECONSTRUCTION_PROVEN=true`; `LIVE_RESTART_EMPIRICAL_RECONSTRUCTION_PROVEN=false`; `LIVE_RESTART_RECONSTRUCTED=false`; `HOST_CRASH_DURABILITY=UNPROVEN`; `TECHNICAL_PRE_EXECUTION_READINESS=false`; `OWNER_EXECUTION_AUTHORIZED=false`; `CANARY_AUTHORIZED_EXACT_FIELD=INDETERMINATE_ABSENT`; `POST_ALLOWED_EXACT_FIELD=INDETERMINATE_ABSENT`; no GET; no POST; no submit; no restart execution; no crash test; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_LIVE_HANDOFF_EXACT_FEE_RESTART_DURABILITY_AND_PRE_EXECUTION_READINESS_CLOSURE_V1.md`](../ops/specs/SECTION_11_14_LIVE_HANDOFF_EXACT_FEE_RESTART_DURABILITY_AND_PRE_EXECUTION_READINESS_CLOSURE_V1.md) | Derived §11.14 exact-fee / restart / durability / pre-execution-readiness forensic closure spec (non-SSOT; exact formula unproven; bounded numeric envelope unproven; static restart fail-closed proven; empirical restart false; host-crash unproven; Owner execution false; no GET; no POST) |
