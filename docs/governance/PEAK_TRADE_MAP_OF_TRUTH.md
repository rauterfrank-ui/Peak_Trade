...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 PRODUCTIVE_WIRE_SEND_OWNER_CONTRACT_AND_ORCHESTRATOR_SPEC | Navigation-only current §11.14 productive wire-send Owner contract and orchestrator spec (`OWNER_PRODUCTIVE_WIRE_SEND_AUTHORITY_V1` defined; `ISSUED=false`; orchestrator not implemented; no POST; no arming; no GET; MOT not SSOT) |
