...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 PRODUCTIVE_SEND_ORCHESTRATOR_AND_SEND_CAPABLE_BIND_IMPLEMENTATION | Navigation-only current §11.14 productive send orchestrator and send-capable bind implementation (`verify_owner_productive_wire_send_authority_v1` implemented; `ProductiveWireSendOrchestratorV1` implemented; `ProductiveTransportBindSendCapableV1` implemented; `ProductiveFlattenSubmitSendAdapterV1` implemented; NO_SEND adapter unchanged; no POST; no inner.send; `CURRENT_CANONICAL_BOUNDARY=PRODUCTIVE_NETWORK_SESSION_NOT_AUTHORIZED`; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_PRODUCTIVE_SEND_ORCHESTRATOR_AND_SEND_CAPABLE_BIND_IMPLEMENTATION_V1.md`](../ops/specs/SECTION_11_14_PRODUCTIVE_SEND_ORCHESTRATOR_AND_SEND_CAPABLE_BIND_IMPLEMENTATION_V1.md) | Derived §11.14 productive send orchestrator and send-capable bind implementation spec (non-SSOT; evaluator/orchestrator/bind/adapter implemented; no arming; no GET; no POST; no inner.send) |
