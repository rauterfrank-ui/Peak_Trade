...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 LIVE_HANDOFF_COMPLETE_CONTEMPORANEOUS_CAPTURE_SEAM_AND_REQUIRED_FIELD_PROVENANCE | Navigation-only current §11.14 complete contemporaneous capture-seam acceptance contract and required-field provenance persist (`COMPLETE_CAPTURE_SEAM=UNPROVEN`; `COMPLETE_CAPTURE_SEAM_ACCEPTANCE_CONTRACT_BOUND=true`; `REQUIRED_FIELD_PROVENANCE_MATRIX_BOUND=true`; `PROVEN_FAIL_CLOSED_FIELD_COUNT=5`; `PROVEN_COMPLETE_FIELD_COUNT=0`; `CONTEMPORANEOUS_PRODUCTIVE_CAPTURE_EXECUTED=false`; `STRUCTURAL_RUNTIME_BINDING_PROVEN=true`; `CURRENT_RUNTIME_EXECUTION_AUTHORIZED=false`; `AUTHORIZED_RUNTIME_SURFACE=NONE`; `PARTIAL_CAPTURE_ALLOWED=false`; `HOST_CRASH_DURABILITY=UNPROVEN`; `CONTEMPORANEOUS_PEAK_TRADE_PRE_RESTART_HANDOFF_OBSERVED=false`; `LIVE_RESTART_RECONSTRUCTED=false`; `IMPLEMENTATION_AUTHORIZED=true`; no GET; no POST; no productive capture executed; no restart execution; `OWNER_GO_FOR_LIVE_RESTART_RECONSTRUCTED` remains §11.14 evidence-domain only and is not the productive Live-execution next pointer; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_LIVE_HANDOFF_COMPLETE_CONTEMPORANEOUS_CAPTURE_SEAM_AND_REQUIRED_FIELD_PROVENANCE_V1.md`](../ops/specs/SECTION_11_14_LIVE_HANDOFF_COMPLETE_CONTEMPORANEOUS_CAPTURE_SEAM_AND_REQUIRED_FIELD_PROVENANCE_V1.md) | Derived §11.14 complete contemporaneous capture-seam and required-field provenance spec (non-SSOT; acceptance contract bound; required-field provenance bound; complete seam unproven; no productive capture executed; host-crash unproven; restart false) |
