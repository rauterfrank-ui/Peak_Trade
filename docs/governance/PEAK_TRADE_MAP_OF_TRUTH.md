...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 LIVE_SUBMIT_ACK_OBSERVED_PROOF_CRITERION | Navigation-only current §11.14 ACK proof-criterion bind (`LIVE_SUBMIT_ACK_OBSERVED=false`; `CASE_ADJUDICATION=CASE_A_READY_FOR_EXACT_SINGLE_POST_OWNER_GO`; producer and criterion bound; HTTP 200 + code 0 + one data row + sCode 0 + nonempty ordId + returned clOrdId equals sent; recon is not ACK; `POST_PERFORMED=false`; next `OWNER_GO_FOR_EXACT_SINGLE_LIVE_SUBMIT_POST`; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_LIVE_SUBMIT_ACK_OBSERVED_PROOF_CRITERION_V1.md`](../ops/specs/SECTION_11_14_LIVE_SUBMIT_ACK_OBSERVED_PROOF_CRITERION_V1.md) | Derived §11.14 LIVE_SUBMIT_ACK_OBSERVED proof-criterion spec (non-SSOT; offline only; no GET; no POST; ACK remains false; CASE_A) |
