...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 SESSION_ARMING | Navigation-only current §11.14 bound session-arming seam (`arm_productive_session_v1` implemented; `SESSION_ARMING_STANDING` remains false; no send_permitted; NO_SEND adapter unchanged; no GET; no POST; no inner.send; `CURRENT_CANONICAL_BOUNDARY=SEND_PERMITTED_FALSE`; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_SESSION_ARMING_V1.md`](../ops/specs/SECTION_11_14_SESSION_ARMING_V1.md) | Derived §11.14 bound session-arming spec (non-SSOT; adapter.session_armed seam only; no send_permitted; no GET; no POST; no inner.send) |
