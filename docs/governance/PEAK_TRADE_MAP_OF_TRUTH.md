...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 PRODUCTIVE_INNER_SEND_FAIL_CLOSED_RECEIPT_BOUNDARY | Navigation-only current §11.14 productive inner.send fail-closed receipt boundary (`AuthenticatedGatedProductiveFlattenTransportV1.send` invoked; `FIRST_DENY=RECEIPT_MISSING`; no GET; no HTTP POST; no HMAC mint; no receipt mint; `CURRENT_CANONICAL_BOUNDARY=RECEIPT_MISSING`; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_PRODUCTIVE_INNER_SEND_FAIL_CLOSED_RECEIPT_BOUNDARY_V1.md`](../ops/specs/SECTION_11_14_PRODUCTIVE_INNER_SEND_FAIL_CLOSED_RECEIPT_BOUNDARY_V1.md) | Derived §11.14 productive inner.send fail-closed receipt-boundary spec (non-SSOT; productive send invoked; `RECEIPT_MISSING`; no GET; no POST) |
