...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 FRESH_PRE_SUBMIT_GET_AND_FRESHNESS_ADJUDICATION | Navigation-only current §11.14 Fresh Pre-Submit GET and freshness adjudication (`GET_PERFORMED=true`; `GET_CALL_COUNT=4`; `POSITION_VALUE=1`; `QUOTE_BID=0.8121`; `ENVELOPE_FRESHNESS_STATUS=REPRICE_REQUIRED`; `REPRICE_EXECUTED=false`; `RECEIPT_ATTACHED=false`; `CURRENT_CANONICAL_BOUNDARY=RECEIPT_MISSING`; `NEXT_OWNER_AUTHORITY_REQUIRED=ENVELOPE_REPRICE_OR_FRESHNESS_AT_SEND`; no POST; no HMAC POST; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_FRESH_PRE_SUBMIT_GET_AND_FRESHNESS_ADJUDICATION_V1.md`](../ops/specs/SECTION_11_14_FRESH_PRE_SUBMIT_GET_AND_FRESHNESS_ADJUDICATION_V1.md) | Derived §11.14 Fresh Pre-Submit GET and freshness spec (non-SSOT; four required GETs; reprice required not executed; no POST) |
