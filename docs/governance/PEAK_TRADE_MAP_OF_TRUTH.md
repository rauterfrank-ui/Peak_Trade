...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 LIVE_RESTART_HANDOFF_OWNER_BIND_AND_RETROACTIVE_SYNTHESIS_REFUSAL | Navigation-only current §11.14 Live restart handoff owner bind (`LIVE_RESTART_RECONSTRUCTED=false`; `SECTION_11_14_LIVE_HANDOFF_OWNER_CURRENT=NONE`; `RETROACTIVE_HANDOFF_SYNTHESIS_ALLOWED=false`; historical identity `UNPROVABLE_WITHOUT_CONTEMPORANEOUS_CAPTURE`; no GET; no POST; no restart execution; `OWNER_GO_FOR_LIVE_RESTART_RECONSTRUCTED` remains §11.14 evidence-domain only and is not the productive Live-execution next pointer; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_LIVE_RESTART_HANDOFF_OWNER_BIND_AND_RETROACTIVE_SYNTHESIS_REFUSAL_V1.md`](../ops/specs/SECTION_11_14_LIVE_RESTART_HANDOFF_OWNER_BIND_AND_RETROACTIVE_SYNTHESIS_REFUSAL_V1.md) | Derived §11.14 Live restart handoff owner-bind and retroactive-synthesis-refusal spec (non-SSOT; owner NONE; restart false; retroactive synthesis refused) |
