...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 SEND_PERMITTED | Navigation-only current §11.14 bound send_permitted seam (`permit_productive_send_v1` implemented; bind-level send_permitted remains false; NO_SEND adapter unchanged; no GET; no POST; no inner.send; `CURRENT_CANONICAL_BOUNDARY=INNER_SEND_NOT_INVOKED_IN_THIS_IMPLEMENTATION`; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_SEND_PERMITTED_V1.md`](../ops/specs/SECTION_11_14_SEND_PERMITTED_V1.md) | Derived §11.14 bound send_permitted spec (non-SSOT; adapter.send_permitted seam only; no inner.send; no GET; no POST) |
