...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 INNER_SEND_INVOCATION_SEAM | Navigation-only current §11.14 fake inner.send invocation seam (`post` → `LiveCanaryHttpRequestV1` → `RecordingFakeProductiveSendInnerV1.send`; productive transport send refused; NO_SEND adapter unchanged; no GET; no HTTP POST; `CURRENT_CANONICAL_BOUNDARY=PRODUCTIVE_INNER_SEND_EXECUTION_NOT_AUTHORIZED`; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_INNER_SEND_INVOCATION_SEAM_V1.md`](../ops/specs/SECTION_11_14_INNER_SEND_INVOCATION_SEAM_V1.md) | Derived §11.14 fake inner.send invocation-seam spec (non-SSOT; fake send only; no productive inner.send; no GET; no POST) |
