...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 LIVE_HANDOFF_POS_PRODUCER_SEMANTICS_AND_CONTRACT | Navigation-only current §11.14 Live handoff pos producer semantics and contract (`POS_SEMANTICS=PROVEN`; unique meaning/unit/sign Owner-bound; `NEW_PRODUCER_CONTRACT_DEFINED=true`; `NEW_PRODUCER_IMPLEMENTED=false`; `COMPLETE_CAPTURE_SEAM=UNPROVEN`; `COMPLETE_CAPTURE_SEAM_CAN_NOW_BE_ADJUDICATED=true`; `SECTION_11_14_LIVE_HANDOFF_OWNER_CURRENT=NONE`; no productive writer/reader/storage mint; `IMPLEMENTATION_AUTHORIZED=false`; no GET; no POST; no restart execution; `OWNER_GO_FOR_LIVE_RESTART_RECONSTRUCTED` remains §11.14 evidence-domain only and is not the productive Live-execution next pointer; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_LIVE_HANDOFF_POS_PRODUCER_SEMANTICS_AND_CONTRACT_V1.md`](../ops/specs/SECTION_11_14_LIVE_HANDOFF_POS_PRODUCER_SEMANTICS_AND_CONTRACT_V1.md) | Derived §11.14 pos producer-semantics and contract spec (non-SSOT; unique meaning proven; producer contract-only; implementation unauthorized) |
