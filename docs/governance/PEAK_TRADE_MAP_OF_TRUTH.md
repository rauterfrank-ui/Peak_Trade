...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.13.5 G12_DELAYED_POSID_ZERO_ROW_FULL_CONJUNCTION_PROOF_CONTRACT | Navigation-only current additive offline G12 delayed-zero conjunction contract (`G12_STATUS=OPEN`; `TARGET_POSITION_ZERO_PROVEN=false`; `LIVE_FLATTEN_PROVABILITY_PROVEN=false`; delayed zero does not imply live flatten; `posId` filter does not prove related completeness; `.ops_local` not canonical; next `PEAK_TRADE_OWNER_GO_G12_CANONICAL_DELAYED_ZERO_PERSIST_AND_PENDING_RELATED_OBSERVATIONS_V1`; MOT not SSOT) |
| [`docs/ops/specs/G12_DELAYED_POSID_ZERO_ROW_FULL_CONJUNCTION_PROOF_CONTRACT_V1.md`](../ops/specs/G12_DELAYED_POSID_ZERO_ROW_FULL_CONJUNCTION_PROOF_CONTRACT_V1.md) | Derived delayed G12 conjunction spec (non-SSOT; no GET; no POST; G12 remains open) |
