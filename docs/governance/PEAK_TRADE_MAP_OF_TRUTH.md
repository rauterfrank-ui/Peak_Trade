...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 RECEIPT_MISSING | Navigation-only current §11.14 attachable pre-send receipt mint (`RECEIPT_MISSING=PROVEN`; `RUNTIME_RECEIPT_MINT_EXECUTED=true`; `FIRST_DENY_AFTER_ATTACHED_RECEIPT=UNSIGNED_PRODUCTIVE_HEADERS`; `CURRENT_CANONICAL_BOUNDARY=UNSIGNED_PRODUCTIVE_HEADERS`; `NEXT_OWNER_AUTHORITY_REQUIRED=HMAC_GENERATION`; no HMAC generation; no GET; no POST; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_RECEIPT_MISSING_V1.md`](../ops/specs/SECTION_11_14_RECEIPT_MISSING_V1.md) | Derived §11.14 RECEIPT_MISSING spec (non-SSOT; attachable mint; one-shot attach; unsigned headers deny; no POST) |
