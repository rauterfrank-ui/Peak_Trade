...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 FLATTEN_PRE_SEND_RECEIPT_AUTHORITY_AND_BINDING | Navigation-only current §11.14 flatten pre-send receipt authority adjudication (`RECEIPT_AUTHORITY_SCHEMA_STATUS=DEFINED`; producer/consumer match; `RECEIPT_MINTED=false`; `RECEIPT_WORK_BLOCKED_BY_OPEN_GATE_ORDER=true`; `OPEN_GATE_ORDER_BLOCKER=FRESH_PRE_SUBMIT_GET`; `CURRENT_CANONICAL_BOUNDARY=RECEIPT_MISSING`; no GET; no HTTP POST; no HMAC; no attachable mint; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_FLATTEN_PRE_SEND_RECEIPT_AUTHORITY_AND_BINDING_V1.md`](../ops/specs/SECTION_11_14_FLATTEN_PRE_SEND_RECEIPT_AUTHORITY_AND_BINDING_V1.md) | Derived §11.14 flatten pre-send receipt authority spec (non-SSOT; schema defined; mint blocked by open gate order; no GET; no POST) |
