...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.2.1.U FULL_CORE_RUNNING_EQUITY_AUTHORITY_ARCHITECTURE_RATIFICATION | Navigation-only pointer to Full-Core running-equity future authority-architecture ratification persist (`ARCHITECTURE_RATIFIED=true`; future owner/source/producer classes; `ACCOUNT_EQUITY_AUTHORITY_OWNER=UNRESOLVED`; `SOURCE_SELECTED=false`; `MAPPING_PROVEN=false`; `EARLIEST_UNRESOLVED_FULL_CORE_DEPENDENCY=NO_CANONICALLY_VALID_ACCOUNT_EQUITY_SOURCE_MAPPING`; Cap 11.1 construction remains forbidden; no POST; MOT not SSOT) |
