...
| [`docs/ops/specs/MF_SELECTION_AND_ANTI_CHURN_SEMANTICS_CONTRACT_V1.md`](../ops/specs/MF_SELECTION_AND_ANTI_CHURN_SEMANTICS_CONTRACT_V1.md) | Isolated MF selection and anti-churn mechanism semantics (`TIE_BREAK` origin-only plus unresolved MF-own; hysteresis/min-holding concepts not rules; Cap-2.3 `REPLACEMENT_PENDING` not imported and not rotation; `OPEN_DECISION_01`–`07` unclosed; fail-closed boundaries in §1.1–§1.8; neither `EXACTLY_N` nor `AT_MOST_N` is default; `N` not decidable while OD02 unclosed; `AUTHORITY_EFFECT=NONE`; numerics unratified; navigation only) |
