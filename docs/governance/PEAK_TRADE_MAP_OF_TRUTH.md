...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 CURRENT_ORIGIN_MAIN_8605BF_GET_ONLY_TRADE_FEE_AND_STALE_PRETRADE_PREDICATE_REFRESH | Navigation-only current §11.14 GET-only SUI trade-fee and stale pretrade refresh (`GET_PERFORMED=true`; `TRADE_FEE_GET_PERFORMED=true`; `TRADE_FEE_OKX_CODE=0`; `CURRENT_ACCOUNT_FEE_RATE_OBSERVED=true`; `BOUNDED_FEE_ENVELOPE_CURRENT_NUMERIC_PROVEN=true`; `EXACT_OKX_FEE_FORMULA_UNPROVEN=true`; `TECHNICAL_PRE_EXECUTION_READINESS=true`; `OWNER_EXECUTION_AUTHORIZED=false`; GET-only; no POST; no submit; no restart; no crash test; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_CURRENT_ORIGIN_MAIN_8605BF_GET_ONLY_TRADE_FEE_AND_STALE_PRETRADE_PREDICATE_REFRESH_V1.md`](../ops/specs/SECTION_11_14_CURRENT_ORIGIN_MAIN_8605BF_GET_ONLY_TRADE_FEE_AND_STALE_PRETRADE_PREDICATE_REFRESH_V1.md) | Derived §11.14 current-origin/main GET-only trade-fee and stale pretrade refresh spec (non-SSOT; GET-only; current SUI family rates observed; OEM formula unproven; technical pre-execution true; Owner execution false; no POST) |
