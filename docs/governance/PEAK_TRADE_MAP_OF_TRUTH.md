...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 LIVE_HANDOFF_CONTEMPORANEOUS_PRE_RESTART_CAPTURE_OBSERVATION_AND_NON_EXECUTION_PROOF | Navigation-only current §11.14 contemporaneous PRE-RESTART capture observation and non-execution proof (`CONTEMPORANEOUS_PRE_RESTART_CAPTURE_OBSERVATION=NOT_EXECUTED`; `CONTEMPORANEOUS_CAPTURE_CAN_BE_ISOLATED_FROM_LIVE_EXECUTION=false`; `AUTHORIZATION_MATRIX_STATUS=COMPLETE`; `NON_CAPTURE_SIDE_EFFECTS_AUTHORIZED=false`; `CONTEMPORANEOUS_PRODUCTIVE_CAPTURE_EXECUTED=false`; `BACKFILL_USED=false`; `RETROACTIVE_SYNTHESIS_USED=false`; `OWNER_GO_IS_NOT_GATE_BYPASS=true`; `CURRENT_RUNTIME_EXECUTION_AUTHORIZED=false`; `AUTHORIZED_RUNTIME_SURFACE=NONE`; `HOST_CRASH_DURABILITY=UNPROVEN`; `LIVE_RESTART_RECONSTRUCTED=false`; `IMPLEMENTATION_AUTHORIZED=true`; no GET; no POST; no productive capture executed; no restart execution; `OWNER_GO_FOR_LIVE_RESTART_RECONSTRUCTED` remains §11.14 evidence-domain only and is not the productive Live-execution next pointer; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_LIVE_HANDOFF_CONTEMPORANEOUS_PRE_RESTART_CAPTURE_OBSERVATION_AND_NON_EXECUTION_PROOF_V1.md`](../ops/specs/SECTION_11_14_LIVE_HANDOFF_CONTEMPORANEOUS_PRE_RESTART_CAPTURE_OBSERVATION_AND_NON_EXECUTION_PROOF_V1.md) | Derived §11.14 contemporaneous PRE-RESTART capture observation and non-execution proof spec (non-SSOT; observation not executed; isolation from live execution false; no productive capture executed; host-crash unproven; restart false) |
