...
| [`docs/runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md`](../runbooks/canonical/PEAK_TRADE_MASTER_RUNBOOK.md) §11.14 LIVE_HANDOFF_REQUIRED_FIELD_CAPTURE_SEAM_POS_AND_OWNER_VACANCY_CONTRACT | Navigation-only current §11.14 required-field capture-seam and owner-vacancy contract (`POS_SEMANTICS=UNPROVEN`; `COMPLETE_CAPTURE_SEAM=UNPROVEN`; `SECTION_11_14_LIVE_HANDOFF_OWNER_CURRENT=NONE`; vacancy contract bound not minted; no productive writer/reader join; no GET; no POST; no restart execution; `OWNER_GO_FOR_LIVE_RESTART_RECONSTRUCTED` remains §11.14 evidence-domain only and is not the productive Live-execution next pointer; MOT not SSOT) |
| [`docs/ops/specs/SECTION_11_14_LIVE_HANDOFF_REQUIRED_FIELD_CAPTURE_SEAM_POS_AND_OWNER_VACANCY_CONTRACT_V1.md`](../ops/specs/SECTION_11_14_LIVE_HANDOFF_REQUIRED_FIELD_CAPTURE_SEAM_POS_AND_OWNER_VACANCY_CONTRACT_V1.md) | Derived §11.14 required-field capture-seam and owner-vacancy spec (non-SSOT; pos unproven; capture seam unproven; vacancy bound; owner NONE) |
