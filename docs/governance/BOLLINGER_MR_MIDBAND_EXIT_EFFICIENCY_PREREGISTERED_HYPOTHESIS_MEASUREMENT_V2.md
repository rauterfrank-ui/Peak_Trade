# Bollinger/MR midband exit-efficiency — preregistered hypothesis and measurement v2

---
docs_token: DOCS_TOKEN_BOLLINGER_MR_MIDBAND_EXIT_EFFICIENCY_PREREGISTERED_HYPOTHESIS_MEASUREMENT_V2
...
> hypothesis with identical measurement definition to V1. V2 is **not** a rerun of V1.
> V1 remains terminal `INCONCLUSIVE_INFRASTRUCTURE_FAILURE` with run count 1.
...
- Contract: `bollinger_mr_midband_exit_efficiency_preregistered_economic_hypothesis_measurement_contract.v2`
...
- Historical process-death cause: `UNKNOWN`
- No V1 result, partial measurement, or checkpoint is transferred into V2
- V2 is a new independently preregistered measurement after the generic runner
  lifecycle&#47;observability repair

## Research question (exact; identical to V1)
...
## Exit mechanism (ex ante, deterministic; identical to V1)
...
## Pass criteria (frozen before any run; identical to V1)
...
## Observability binding (mandatory)

Future evaluation must bind `EVALUATION_RUNNER_LIFECYCLE_OBSERVABILITY_V1` with durable
diagnostics for phase, last confirmed member, heartbeat&#47;progress, exit code, signal,
exception class + truncated traceback, and atomic lifecycle checkpoint. Incomplete
execution ends fail-closed as `INCONCLUSIVE_INFRASTRUCTURE_FAILURE`. No auto-resume
and no auto-rerun.
...
Review and merge this definition-only V2 PR. Any development evaluation requires a
