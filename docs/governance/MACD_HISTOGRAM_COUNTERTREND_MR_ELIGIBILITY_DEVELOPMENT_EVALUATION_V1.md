# MACD histogram-countertrend MR eligibility — DEVELOPMENT evaluation v1
...
- Contract: `config/research/macd_histogram_countertrend_mr_eligibility_preregistered_economic_hypothesis_measurement_contract_v1.json`
- Preregistration module: `src/research/macd_histogram_countertrend_mr_eligibility_hypothesis_preregistration_v1.py`
...
- Treatment: research-only `ENTRY_EFFECTIVE_PRE_ENTRY_ELIGIBILITY_FILTER` — frozen
  MACD(12,26,9) histogram-sign countertrend admission via
  `src&#47;strategies&#47;macd.py` (`_calculate_macd`)
- Filter ID: `canonical_macd_histogram_countertrend_entry_eligibility_v1`
...
Long entry-candidate (mapped signal `+1`): `ELIGIBLE` iff `histogram < 0`.
Short entry-candidate (mapped signal `-1`): `ELIGIBLE` iff `histogram > 0`.
First `warmup_bars=35` bars are always `STAND_ASIDE`. Mapped `0` (flat/exit)
...
- Does not reuse or retune the three-feature absolute-threshold classifier,
  ATR-percentile mid-band filter, RSI exhaustion filter, ADX(14) level
  range-admission filter, or SMA(50) with-trend admission filter.
...
`docs/evidence/evaluate_macd_histogram_countertrend_mr_eligibility_development_v1/` (populated by the runner)
