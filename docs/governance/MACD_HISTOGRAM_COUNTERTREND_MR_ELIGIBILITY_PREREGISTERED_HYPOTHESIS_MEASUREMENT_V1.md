# MACD histogram-countertrend MR eligibility — preregistered hypothesis and measurement v1
...
- Contract: `macd_histogram_countertrend_mr_eligibility_preregistered_economic_hypothesis_measurement_contract.v1`
...
Canonical MACD(12,26,9) histogram-sign countertrend admission from repo SSOT defaults in
`config/config.toml` `[strategies.macd.defaults]`, computed via
...
MACD histogram. For a long entry-candidate (mapped position intent +1):
ELIGIBLE iff histogram < 0. For a short entry-candidate (mapped position
intent -1): ELIGIBLE iff histogram > 0. Otherwise STAND_ASIDE. First 35 bars
...
This is a single-feature MACD histogram-sign countertrend admission mechanism,
orthogonal to five prior FAILs:

1. Regime-gated absolute-threshold multi-feature classifier
2. ATR(14) rolling-percentile mid-band
3. RSI(14) exhaustion
4. ADX(14) level range admission
5. SMA(50) side-aware with-trend admission
