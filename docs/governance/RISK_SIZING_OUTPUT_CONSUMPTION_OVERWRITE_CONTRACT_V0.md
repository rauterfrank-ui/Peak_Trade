...
Consumption &#47; overwrite topology is **frozen** but **not resolved** (`OUTPUT_CONSUMPTION_OVERWRITE_RESOLVED=false`). Federation, authority assignment, rewire, decommission, and percent &#47; leverage normalization remain Operator-GO gated (`NEXT_SLICE_REQUIRES_OPERATOR_GO=true`).
