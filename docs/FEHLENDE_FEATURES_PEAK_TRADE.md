...
- **Inzwischen umgesetzt (Research-Track):** Sortino/Calmar in Sweeps, Drawdown-Heatmap, Monte-Carlo-Robustness, Auto-Portfolio-Builder, Risk-Parity v1 (inverse-vol). Siehe Abschnitt 5.1 „Bereits umgesetzt“.
...
| **ECM-Fenster / ECM-Features** | ❌ Fehlt | In `src/features/__init__.py` als „später“ genannt, nicht implementiert. | <!-- pt:ref-target-ignore -->
...
| **16** | Advanced Risk & Portfolio | VaR/CVaR, Stress-Testing, Portfolio-Optimization (Markowitz, Black-Litterman), Risk-Parity (v1 inverse-vol vorhanden; erweiterte Varianten offen), Factor-Models, Attribution |
...
**Bereits umgesetzt (Stand 2026-02-10):**

- ✅ Sortino-Ratio in Sweep-Ergebnissen (`src/backtest/stats.py`, Sweep-Reports)
- ✅ Calmar-Ratio in Backtest und Sweep-Ergebnissen
- ✅ Drawdown-Heatmap (Parameter vs. Max-Drawdown) – `src/reporting/sweep_visualization.py`, `create_drawdown_heatmap`
- ✅ Monte-Carlo-Robustness – `research_cli.py` / `run_research_golden_path.py` mit `--run-montecarlo`
- ✅ Auto-Portfolio-Builder – `scripts/build_auto_portfolios.py`, `src/analytics/portfolio_builder.py`
- ✅ Risk-Parity-Integration im Portfolio – Allocation-Methode `risk_parity` (inverse-vol v1) in Backtest-Engine und CLI
...
- Vol-Regime-Filter als Wrapper für alle Strategien (Strategie existiert; universeller Wrapper fehlt)
...
- Zusätzliche Metriken: Ulcer-Index, Recovery-Factor
- Regime-adaptive Strategien (Parameter-Switching nach Regime)
- Nightly-Sweep-Automation (Cron/Scheduler + Alerts für Strategy-Sweeps)
- Interaktive Dashboards (Plotly/Web-UI)
- Feature-Importance-Analyse (SHAP/Permutation-Importance in Code)
...
| **Research-Track** | Sweeps, Metriken, Heatmaps (inkl. Drawdown) ✅, Vol-Regime-Wrapper, Regime-adaptive Strategien, Auto-Portfolio ✅, Risk-Parity v1 ✅, Nightly-Sweeps, Feature-Importance. |
